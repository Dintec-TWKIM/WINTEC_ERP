...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SO_MNGH_WINTEC_GI_S]    Script Date: 2019-11-14 오후 3:11:28 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SO_MNGH_WINTEC_GI_S]
(          
...
JOIN (SELECT SL.CD_COMPANY, SL.NO_SO,
      ...
      AND EXISTS (SELECT 1
	              ...
	              WHERE OH.CD_COMPANY = SL.CD_COMPANY
                  AND OH.DT_IO BETWEEN @P_DT_FROM AND @P_DT_TO
                  AND OL.FG_PS = '2'
                  AND OL.CD_QTIOTP NOT IN ('500')
	              AND OL.NO_PSO_MGMT = SL.NO_SO
                  AND OL.NO_PSOLINE_MGMT = SL.SEQ_SO)
