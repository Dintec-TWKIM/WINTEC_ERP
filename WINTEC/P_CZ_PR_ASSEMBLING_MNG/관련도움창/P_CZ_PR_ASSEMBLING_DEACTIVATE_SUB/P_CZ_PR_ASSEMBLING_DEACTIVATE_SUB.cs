...
namespace cz
{
    public partial class P_CZ_PR_ASSEMBLING_DEACTIVATE_SUB : Duzon.Common.Forms.CommonDialog
    {
        P_CZ_PR_ASSEMBLING_DEACTIVATE_SUB_BIZ _biz = new P_CZ_PR_ASSEMBLING_DEACTIVATE_SUB_BIZ();

        public P_CZ_PR_ASSEMBLING_DEACTIVATE_SUB()
        {
            InitializeComponent();
        ...
        private void btn폐기처리_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                query3 = @"UPDATE CZ_PR_WO_INSP
                           ...
                           AND NO_LINE = '{1}'
                           AND SEQ_WO = '{2}'";
                query4 = @"UPDATE CZ_PR_ASSEMBLING_DEACTIVATE
                           SET YN_DEACTIVATE = 'Y'
                           WHERE NO_ID = '{0}'";
                foreach (DataRow dr in dataRowArray)
                {
