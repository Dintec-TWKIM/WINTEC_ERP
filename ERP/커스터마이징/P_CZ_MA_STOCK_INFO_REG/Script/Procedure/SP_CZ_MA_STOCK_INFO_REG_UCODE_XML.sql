...
/****** Object:  StoredProcedure [NEOE].[PX_CZ_MA_SOURCING_REG_UCODE]    Script Date: 2019-05-03 오후 4:35:45 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_MA_STOCK_INFO_REG_UCODE_XML]
(
	  @P_XML	XML
)
AS
...
IF EXISTS (SELECT * FROM #UPDATE) BEGIN
	...
	FROM CZ_MA_UCODE_HGS A
	JOIN #UPDATE B ON B.CD_COMPANY = A.CD_COMPANY AND B.UCODE = A.UCODE

	UPDATE A
	SET A.WEIGHT = (CASE WHEN ISNULL(B.WEIGHT_LOG, 0) <> 0 THEN B.WEIGHT_LOG ELSE A.WEIGHT END)
	FROM MA_PITEM A
	JOIN #UPDATE B ON B.CD_COMPANY = A.CD_COMPANY AND B.UCODE = A.STND_DETAIL_ITEM
END
