...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_GI_H_DU_S]    Script Date: 2015-10-20 오후 4:21:13 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_GIH_DU_S]
(
...
DECLARE @V_QT_GUBUN     NVARCHAR(3),
        @V_BAD_BUGUN    NVARCHAR(3),   -- 2011-11-16, 최승애 추가
        @V_INV_REQ      NVARCHAR(3),    --2012.04.18
        
        @V_YN_MFG_AUTH  NVARCHAR(1),
...
SELECT	'N' S, A.NO_GIR, A.CD_PLANT, ML.NM_PLANT, A.CD_PARTNER, MP.LN_PARTNER, A.DT_GIR, A.DC_RMK,
	    '' AS NO_IO, '' AS FG_TRANS, '' AS DT_IO, '' AS CD_DEPT,  '' AS NO_EMP,  '' AS YN_RETURN, A.NO_EMP AS NO_GI_EMP, ME.NM_KOR,
	    ISNULL(C.ST_STAT, '0') AS ST_STAT, A.DC_RMK1, MP.SN_PARTNER
FROM	SA_GIRL GL
        ...
        LEFT OUTER JOIN MA_PLANT ML ON A.CD_COMPANY = ML.CD_COMPANY AND A.CD_PLANT = ML.CD_PLANT
        LEFT OUTER JOIN MA_PARTNER MP ON A.CD_COMPANY = MP.CD_COMPANY AND A.CD_PARTNER = MP.CD_PARTNER
        LEFT OUTER JOIN MA_EMP ME ON A.CD_COMPANY = ME.CD_COMPANY AND A.NO_EMP = ME.NO_EMP
        LEFT OUTER JOIN SA_GIRH_EXC C ON A.CD_COMPANY = C.CD_COMPANY AND A.NO_GIR = C.NO_GIR
        LEFT OUTER JOIN MA_PITEM P ON P.CD_COMPANY = @P_CD_COMPANY AND P.CD_PLANT = @P_CD_PLANT AND P.CD_ITEM = GL.CD_ITEM
        LEFT OUTER JOIN MA_PARTNER_SUB MS ON MS.CD_COMPANY = GL.CD_COMPANY AND MS.CD_PARTNER = GL.GI_PARTNER
WHERE A.CD_COMPANY = @P_CD_COMPANY
	AND A.STA_GIR = 'R'
    --AND		(A.NO_GIR = @P_NO_GIR OR @P_NO_GIR = '' OR @P_NO_GIR IS NULL)
    ...
    AND     GL.CD_COMPANY = @P_CD_COMPANY
    AND		GL.DT_DUEDATE BETWEEN @P_DT_GIR_FROM AND @P_DT_GIR_TO
    AND		(GL.GI_PARTNER = @P_GI_PARTNER OR @P_GI_PARTNER = '' OR @P_GI_PARTNER IS NULL)
    AND		(GL.TP_GI = @P_TP_GI OR @P_TP_GI = '' OR @P_TP_GI IS NULL)
    --AND		GL.QT_GIR - GL.QT_GI > 0  --의뢰수량 - 출고수량이 0보다  큰거, 2011-11-16, 최승애 주석처리함.
    AND		GL.RET = @P_RET
    ...
    AND (GL.CD_SL = @P_CD_SL_GIR OR @P_CD_SL_GIR = '' OR @P_CD_SL_GIR IS NULL)
...
GROUP BY A.NO_GIR, A.CD_PLANT, ML.NM_PLANT, A.CD_PARTNER, MP.LN_PARTNER, A.DT_GIR, A.DC_RMK,
	    A.NO_EMP, ME.NM_KOR, C.ST_STAT, A.DC_RMK1, MP.SN_PARTNER
