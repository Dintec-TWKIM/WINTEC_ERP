...
namespace cz
{
    public partial class P_CZ_SA_GI : Duzon.Common.Forms.PageBase
    {
        #region ♣ 생성자 & 변수 선언
        private P_CZ_SA_GI_BIZ _biz = new P_CZ_SA_GI_BIZ();
        ...
        public P_CZ_SA_GI(string 협조전번호)
        {
            try
            {
                StartUp.Certify(this);
        ...
        private void InitGrid()
        {
            ...
            _flexH.SetCol("CD_PARTNER", "매출처", 80);
            _flexH.SetCol("LN_PARTNER", "매출처명", 120);
            _flexH.SetCol("DT_GIR", "의뢰일자", 80, false, typeof(string), FormatTpType.YEAR_MONTH_DAY);
            _flexH.SetCol("NO_GI_EMP", "출고의뢰자", 80);
            _flexH.SetCol("NM_KOR", "출고의뢰자명", 100);
            _flexH.SetCol("DC_RMK", "비고", 200);
            _flexH.SetCol("DC_RMK1", "비고1", 200);
            _flexH.SetCol("SN_PARTNER", "매출처명(약칭)", false);
            ...
            _flexL.SetCol("NM_QTIOTP", "출고형태", 100);
            ...
            _flexL.SetCol("QT_UNIT_MM", "출고수량", 90, false, typeof(decimal), FormatTpType.QUANTITY);
            _flexL.SetCol("QT_IO", "출고관리수량", 90, false, typeof(decimal), FormatTpType.QUANTITY);
            ...
            _flexL.SetCol("CD_ITEM_PARTNER", "매출처품번", 150, false);
            _flexL.SetCol("NM_ITEM_PARTNER", "매출처품명", 150, false);
            ...
            string str단가금액통제 = Duzon.ERPU.MF.ComFunc.전용코드("출고등록-단가금액통제");
            if (str단가금액통제 == "N")
            {
            ...
            {
                _flexL.SetCol("CD_PARTNER_GRP", "매출처그룹", 150, false);
                _flexL.SetCol("CD_PARTNER_GRP_2", "매출처그룹2", 150, false);
            ...
            _flexL.SetCol("NO_PO_PARTNER", "매출처발주번호", 120);
            _flexL.SetCol("NO_POLINE_PARTNER", "매출처발주항번", 120);
        ...
        protected override void InitPaint()
        {
            ...
            // 공장 콤보
            cbo_Plant.DataSource = g_dsCombo.Tables[0].Copy();
            ...
            dr["CODE"] = "GI";
            //dr["NAME"] = "의뢰일자";
            dr["NAME"] = this.DD("의뢰일자");
            dtCombo.Rows.Add(dr);

            dr = dtCombo.NewRow();
            dr["CODE"] = "DU";
            //dr["NAME"] = "납기일자";
            dr["NAME"] = this.DD("납기일자");
            dtCombo.Rows.Add(dr);

            dr = dtCombo.NewRow();
            dr["CODE"] = "RQ";
            //dr["NAME"] = "출고예정일";
            dr["NAME"] = this.DD("출고예정일");
            dtCombo.Rows.Add(dr);
            ...
            DataTable dtFG_SERNO = this.GetComboDataCombine("N;MA_B000015");
            DataRow[] drs = dtFG_SERNO.Select("CODE = '001'", "", DataViewRowState.CurrentRows);
            if (drs.Length == 1)
                drs[0]["NAME"] = "";
            _flexL.SetDataMap("FG_SERNO", dtFG_SERNO, "CODE", "NAME");

            _flexL.SetDataMap("CLS_ITEM", this.GetComboDataCombine("N;MA_B000010"), "CODE", "NAME");

            // 조회조건 초기화
            pp일자.StartDateToString = Global.MainFrame.GetDateTimeToday().AddMonths(-1).ToString("yyyyMMdd");
            //dt_Gir_To.Text = Global.MainFrame.GetStringToday;
            ctx매출처.CodeValue = string.Empty;
            ctx매출처.CodeName = string.Empty;
        ...
        void 그리드통제적용()
        {
            //재고단위EDIT여부(2중단위관리여부) 에 따라서 출고관리수량을 Edit 가능 여부를 체크한다.
            if (two_Unit_Mng != "N")
        ...
        public override void OnToolBarSearchButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                object[] obj = new object[] {
                    ...
                    D.GetString(cbo매출처그룹.SelectedValue), 
                    D.GetString(cbo매출처그룹2.SelectedValue), 
        ...
        public override void OnToolBarSaveButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                DataRow[] line_check_Rows = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "'", "", DataViewRowState.CurrentRows);
                ...
                foreach (DataRow drl in line_check_Rows)
                {
                    ...
                    {
                        ShowMessage(공통메세지._은_보다커야합니다, this.DD("출고수량"), "0");
        ...
        protected override bool SaveData()
        {
            try
            {
                ...
                //출고수량(QT_UNIT_MM) * 환산수량(UNIT_SO_FACT) 이 관리수량(QT_IO) 과 불일치할 경우 체크로직을 걸어준다.
                DataTable dt_TwoEx = _flexL.DataTable.Clone();
                DataRow[] cnt_Rows = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "' AND ISNULL(QT_IO, 0) <> ISNULL(QT_UNIT_MM, 0) * ISNULL(UNIT_SO_FACT, 1)", "", DataViewRowState.CurrentRows);
                ...
                #region 여신체크
                DataTable temp_dt = new DataTable(); // 매출처로 Group 해서 총 사용할 금액을 가지고 있을 DataTable을 생성한다.
                //매출처 : 금액 컬럼 스키마 추가
                ...
                DataRow[] cnt_Line = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) +"' AND ST_STAT <> '1'");
                //선택된 매출처로 묶어서 여신을 체크한다음 ERROR 인 경우와 경고를 무시하지 않는 경우에는 선택버튼을 풀어준다.
                ...
                warning_Dt.Columns.Add("CD_PARTNER", typeof(string));           //매출처
                warning_Dt.Columns.Add("CD_PARTNER_NAME", typeof(string));      //매출처명
                ...
                warning_Dt.Columns.Add("AM_SUM", typeof(decimal));              //출고금액
                ...
                error_Dt.Columns.Add("CD_PARTNER", typeof(string));             //매출처
                error_Dt.Columns.Add("CD_PARTNER_NAME", typeof(string));        //매출처명
                ...
                error_Dt.Columns.Add("AM_SUM", typeof(decimal));                //출고금액
                ...
                foreach (DataRow dr in temp_dt.Rows)
                {
                    ...
                    /*
                     * 여기에서 처리될 내역은 매출처로 여신을 체크해서 ROW 를 받아온다. 
                     ...
                     * WARNING 에서 체크되지 않은 매출처와 ERROR 에 걸린 매출처만 리턴하여
                     * 본 등록의 상/하단 매출처의 처리건에 대한 체크를 해제한다. 
                     */
                    Decimal am_sum = 0;
                ...
                #region 헤더에 데이터 채우기
                NO_IO = D.GetString(this.GetSeq(LoginInfo.CompanyCode, "SA", "07")).Substring(0, 2) + D.GetString(this._flexH["NO_GIR"]).Substring(2); //출고번호
                ...
                line_Row = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "'", "", DataViewRowState.CurrentRows);
                ...
                //원래 출고의뢰를 조회해서 가져올 사항에 데이터는 없지만 조회쿼리에 쓰레기 값을 가져오는 이유는 스키마 생성해서 멀티저장 바로 가능하게 하기위함
                //멀티저장시에 다른 것들은 코딩상에서 스키마를 추가해서 보여줬으나 쿼리에 쓰레기값 가져와서 스키마 만들어주는 방법도 괜찮은듯...
                this._flexH["NO_IO"] = NO_IO;
                this._flexH["FG_TRANS"] = line_Row[0]["FG_TRANS"].ToString();


                //출고등록-출고담당자와 출고의뢰자 동기화 전용코드에 따라 
                //출고등록담당자를 담당자/의뢰담당자로 처리하도록 수정(2011/01/20, BY SMJUNG)
                if (BASIC.GetMAEXC("출고등록-출고담당자와 출고의뢰자 동기화") == "001")
                    this._flexH["NO_EMP"] = this._flexH["NO_GI_EMP"];
                else
                    this._flexH["NO_EMP"] = bp_Emp.CodeValue;


                this._flexH["CD_DEPT"] = CD_DEPT;
                this._flexH["DT_IO"] = dt_gi.Text;
                this._flexH["YN_RETURN"] = "N";

                if (chk_YN.Checked != true)
                    this._flexH["DC_RMK"] = txt_DcRmk.Text;

                dt_H.ImportRow(this._flexH.GetDataRow(this._flexH.Row));

                if (NO_IO == string.Empty)
                {
                    ShowMessage("출고번호가 존재하지 않습니다.");
                    return false;
                }
                #endregion
                ...
                if (bStorageLocation)
                {
                    ...
                    {
                        dtSave = this._flexL.DataTable.Clone();
                        drs = this._flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "'", "", DataViewRowState.CurrentRows);
                ...
                DataRow[] dr_grid = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "' AND NO_LOT = 'YES'", "", DataViewRowState.CurrentRows);//수불 라인에 저장되는 LOT 품목 추출
        ...
        #region ♣ 양품적용
        private void 양품적용(DataRow[] dataRows)
        {
            try
            {
        ...
        private void btn_apply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataRow[] ldrchk = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "'", "", DataViewRowState.CurrentRows);
        ...
        private void rdo_Yes_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (rdo_Yes.Checked == true)
                {
                    ...
                    args[0] = dt_gi.Text;                         //출고일자
                    args[1] = cbo_Plant.SelectedValue == null ? string.Empty : cbo_Plant.SelectedValue.ToString(); //공장
                    args[2] = ctx매출처.CodeValue;               //매출처코드
                    args[3] = ctx매출처.CodeName;                //매출처명
                    args[4] = bp_Emp.CodeValue;                   //사번
                    args[5] = bp_Emp.CodeName;                    //이름

                    //Main 이 살아 있는지 확인한후 살아 있으면 저장을 실행하고 죽어 있으면 그냥 리턴시켜버린다.
                    if (this.MainFrameInterface.IsExistPage("P_CZ_SA_GIM_REG", false))
                        this.UnLoadPage("P_CZ_SA_GIM_REG", false);   //특정 페이지 닫기

                    bool isComplete = this.LoadPageFrom("P_CZ_SA_GIM_REG", "출고관리(딘텍)", this.Grant, args);
            ...
            try
            {
                ...
                switch (flex.Name)
                {
                    case "_flexH":
                        ...
                        _flexL.SetDummyColumn("CD_SL", "NM_SL", "QT_UNIT_MM", "QT_IO");    //왜 했지?
        ...
        private void _flexL_ValidateEdit(object sender, ValidateEditEventArgs e)
        {
            try
            {
                    ...
                    case "QT_UNIT_MM":  //출고수량
                        ...
                        else if (qtso_AddAllowYN)
                        {
                            if (qtso_AddAllow < D.GetDecimal(newValue))
                            {
                                ...
                                ShowMessage("출고수량이 의뢰수량을 초과하였습니다.");
                            }
                        }
                ...
                DataRow[] idx_Row = _flexL.DataTable.Select("NO_GIR = '" + D.GetString(this._flexH["NO_GIR"]) + "'");
        ...
        private void _flex_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (D.GetString(ComFunc.전용코드("출고등록-재고출고반려도움창(제다전용)")) != "001")
                    return;
        ...
        public void Menu_Click(object sender, EventArgs e)
        {
            switch (((ToolStripMenuItem)sender).Name)
            {
                case "현재고조회":
                    ...
                    P_PU_STOCK_SUB m_dlg = new P_PU_STOCK_SUB(cbo_Plant.SelectedValue.ToString(), cd_item_multi);
        ...
        {
            DataTable dt = DBHelper.GetDataTable(@"SELECT FG_LANG 
