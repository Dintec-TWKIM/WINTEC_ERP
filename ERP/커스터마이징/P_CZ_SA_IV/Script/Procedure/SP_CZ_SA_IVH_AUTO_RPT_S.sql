...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_IVH_AUTO_RPT_S]    Script Date: 2015-06-26 오전 9:15:42 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_IVH_AUTO_RPT_S]        
(        
...
JOIN (SELECT IL.CD_COMPANY, IL.NO_IV,
	  ...
	  FROM (SELECT IL.CD_COMPANY, IL.NO_IV, IL.NO_SO,
		    ...
		    FROM SA_IVL IL
			WHERE (ISNULL(@P_YN_PARTIAL, '') = 'N' OR EXISTS (SELECT 1 
															  FROM B 
															  WHERE B.CD_COMPANY = IL.CD_COMPANY 
															  AND B.NO_SO = IL.NO_SO))
