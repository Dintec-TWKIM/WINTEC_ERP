...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_LOG_PLAN_MNG_GIR_DELIVERY_S]    Script Date: 2016-02-12 오후 1:40:06 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_LOG_PLAN_MNG_GIR_DELIVERY_S]  
(
    @P_DTS_CONFIRM	    NVARCHAR(14),
...
(
    ...
    AND ((ISNULL(@P_NO_GIR, '') <> '' AND GH.NO_GIR = @P_NO_GIR) OR 
         (ISNULL(@P_NO_IMO, '') <> '' AND WD.NO_IMO = @P_NO_IMO AND (CASE WHEN GH.DTS_UPDATE > WD.DTS_CONFIRM THEN GH.DTS_UPDATE ELSE WD.DTS_CONFIRM END) >= @P_DTS_CONFIRM) OR
         (ISNULL(@P_NO_GIR, '') = '' AND ISNULL(@P_NO_IMO, '') = '' AND (CASE WHEN GH.DTS_UPDATE > WD.DTS_CONFIRM THEN GH.DTS_UPDATE ELSE WD.DTS_CONFIRM END) >= @P_DTS_CONFIRM))
    UNION ALL
    SELECT GH.CD_COMPANY,
           MC1.NM_SYSDEF AS NM_SUB_CATEGORY,
           MC2.NM_SYSDEF AS NM_RMK,
    ...
    LEFT JOIN MA_CODEDTL MC2 ON MC2.CD_COMPANY = GH.CD_COMPANY AND MC2.CD_FIELD = 'CZ_SA00032' AND MC2.CD_SYSDEF = WD.CD_RMK
    WHERE GH.CD_COMPANY IN ('K100', 'K200')
    AND ((WD.CD_MAIN_CATEGORY = '002' AND WD.CD_SUB_CATEGORY = 'DIR') OR
         (WD.CD_MAIN_CATEGORY = '003' AND WD.CD_SUB_CATEGORY = '009')) 
    AND ((ISNULL(@P_NO_GIR, '') <> '' AND GH.NO_GIR = @P_NO_GIR) OR 
         (ISNULL(@P_NO_IMO, '') <> '' AND WD.NO_IMO = @P_NO_IMO AND (CASE WHEN GH.DTS_UPDATE > WD.DTS_CONFIRM THEN GH.DTS_UPDATE ELSE WD.DTS_CONFIRM END) >= @P_DTS_CONFIRM) OR
         (ISNULL(@P_NO_GIR, '') = '' AND ISNULL(@P_NO_IMO, '') = '' AND (CASE WHEN GH.DTS_UPDATE > WD.DTS_CONFIRM THEN GH.DTS_UPDATE ELSE WD.DTS_CONFIRM END) >= @P_DTS_CONFIRM))
)
