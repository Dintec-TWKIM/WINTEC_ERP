...
namespace cz
{
	public partial class P_CZ_HR_OVERTIME_GW : PageBase
	{
		// ============================== 초기화 ============================== //
		...
		{
			dtp작성일.StartDate = Convert.ToDateTime("2016-01-01");
            dtp작성일.EndDate =  Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd"));
		...
		private void InitGrid()
		{
			...
			flexH.SettingVersion = "1.12.26.03";
		...
		//{
		//    btn전자결재.Left = spcM.SplitterDistance + 865;
		//}
  ...
  <tr>
	...
	<th>사유</th>
			...
			for (int i = flexL.Rows.Fixed; i < flexL.Rows.Count; i++)
			{
				html += @"
  <tr>
	<td class = 'col1'>" + ((DataTable)cbo구분.DataSource).Select("CODE = '" + flexL[i, "CD_OCODE"] + "'")[0]["NAME"] + @"</td>
	<td class = 'col2'>" + flexL[i, "DT_START"].ToString() + "\r\n~" + flexL[i, "DT_END"].ToString() + @"</td>
	<td class = 'col3'>" + flexL[i, "DC_CONTENT"] + @"</td>
	<td class = 'col4'>" + flexL[i, "DC_REASON"] + @"</td>
  </tr>";
			}
...
<div style='padding: 20px; text-align:center'>위의 사유로 추가연장근로 신청서를 제출 합니다.</div>";
...
WHERE 1 = 1
	...
	AND NO_EMP = '" + NO_EMP + "'";
		...
		{
			if (!flexH["ST_STAT"].ToString().Equals("0") || !flexH["ST_STAT"].ToString().Equals("1"))
			{
				flexL.Rows.Add();
                ...
                dtp시작일자.Value = Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd HH:mm"));
                dtp종료일자.Value = Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd HH:mm"));
		...
		{
			DateTime changeDt = new DateTime(dtp시작일자.Value.Year, dtp시작일자.Value.Month, dtp시작일자.Value.Day, dtp종료일자.Value.Hour, dtp종료일자.Value.Minute, 0);

			dtp종료일자.Value = changeDt;
		...
		{
			DateTime changeDt = new DateTime(dtp종료일자.Value.Year, dtp종료일자.Value.Month, dtp종료일자.Value.Day, dtp시작일자.Value.Hour, dtp시작일자.Value.Minute, 0);

			dtp시작일자.Value = changeDt;
		...
		public void timeCal()
		{
			...
			if (dtp시작일자.Value.Day != dtp종료일자.Value.Day)
			{
			    ShowMessage("동일한 날짜만 가능합니다.");
			}
		}
		#endregion 컨트롤 이벤트
		...
		private void flexH_AfterRowChange(object sender, RangeEventArgs e)
		{
			...
			if (flexH.DetailQueryNeed)
                dtL = DBMgr.GetDataTable("SP_CZ_HR_OVERTIME_GW_CLICK", new object[] { CD_COMPANY, NO_EMP, SEQ });
		...
		{
			string dateS = dtp작성일.StartDate.ToString("yyyyMMdd");
			string dateE = dtp작성일.EndDate.ToString("yyyyMMdd");


			DataTable dt = DBHelper.GetDataTable("SP_CZ_HR_OVERTIME_GW_S", new object[] { CD_COMPANY, NO_EMP, dateS, dateE });
		...
		{ 
            ...
            if (dtp시작일자.Value.Year != dtp종료일자.Value.Year) { ShowMessage("날짜를 확인해주세요."); return false; }
			...
			bnow = DateTime.Now.AddDays(-14);


			DateTime anow;
			anow = DateTime.Now;

			anow = DateTime.Now.AddDays(14);


			if (bnow > dtp시작일자.Value) { ShowMessage("2주 이상 차이 납니다."); return false; }
			if (anow < dtp시작일자.Value) { ShowMessage("2주 이상 차이 납니다."); return false; }
		...
		public override void OnToolBarDeleteButtonClicked(object sender, EventArgs e)
		{
			...
			if (!Util.GetTO_String(flexH["ST_STAT"]).Equals("0"))
			{
                if (!Util.GetTO_String(flexH["ST_STAT"]).Equals("1"))
