...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_HR_WBARCODE_S1]    Script Date: 2020-03-12 오전 11:07:56 ******/
SET ANSI_NULLS ON
GO
...
/*******************************************
*********************************************/
ALTER PROCEDURE [NEOE].[SP_CZ_HR_WBARCODE_S1]
(
...
BEGIN
	...
	SELECT 'N' AS S,
		   ...
		   (CASE WHEN EXISTS (SELECT 1 
							  ...
							  AND BC.NO_CARD = PV.NO_CARD
							  AND BC.DT_WORK = PV.DT_WORK) THEN 'Y' ELSE 'N' END) AS YN_CONFIRM
	FROM (SELECT ME.NO_EMP,
		  ...
		  AND (ISNULL(ME.CD_DUTY_RANK, '') > '009' OR ME.NO_EMP LIKE 'A-%')
		  ...
		  AND (ISNULL(@P_MULTI_EMP, '') = '' OR ME.NO_EMP IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_MULTI_EMP)))
		  ) A
