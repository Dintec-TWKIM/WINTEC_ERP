...
WHERE 1 = 1
	...
	AND USE_YN = 'Y'";
		...
		public static string Query(string keyCode)
		{
			...
			if (keyCode == "GRP_ITEM")
			{
				string col = (Global.MainFrame.LoginInfo.Language == "KR") ? "NM_ITEMGRP" : "EN_ITEMGRP";
				query = @"
SELECT
	  CD_ITEMGRP AS CODE
	, " + col + @" AS NAME
...
WHERE 1 = 1
	...
	AND USE_YN = 'Y'";
			...
			{
				string col = (Global.MainFrame.LoginInfo.Language == "KR") ? "NM_SYSDEF" : "ISNULL(NM_SYSDEF_E, NM_SYSDEF)";
				query = @"
SELECT
	  CD_SYSDEF AS CODE
	, " + col + @" AS NAME
	...
	, CD_FLAG2
	, CD_FLAG3
FROM MA_CODEDTL AS A
WHERE 1 = 1
	AND CD_COMPANY = '" + Global.MainFrame.LoginInfo.CompanyCode + @"'
	AND CD_FIELD = '" + keyCode + @"'
	AND USE_YN = 'Y'
	AND NOT EXISTS (SELECT 1 FROM CZ_MA_CODEDTL AS X WHERE A.CD_COMPANY = X.CD_COMPANY AND A.CD_FIELD = X.CD_FIELD)
...
ORDER BY NM_PTR";
		...
		{
			DataTable dt = DBMgr.GetDataTable("PS_CZ_MA_EXCHANGE", Global.MainFrame.LoginInfo.CompanyCode, yymmdd, 2, currency);

			if (mode == "P") mode = "RATE_PURCHASE";
			else if (mode == "S") mode = "RATE_SALES";
			else if (mode == "B") mode = "RATE_BASE";

			if (dt.Rows.Count == 1)
				return GetTo.Decimal(dt.Rows[0][mode]);
			else
				return 1;
		}

		public static DataSet ExchangeRates(object yymmdd, string mode)
		{
			DataSet dt = DBMgr.GetDataSet("PS_CZ_MA_EXCHANGE_MULTI", Global.MainFrame.LoginInfo.CompanyCode, yymmdd, 2, mode);
			return dt;
		}
...
WHERE 1 = 1
	...
	ORDER BY CONVERT(INT, CD_FLAG4)";
		...
		public static DataTable DHLUnitCodeGet()
		{
			string query = @"
SELECT 
	CD_SYSDEF AS CODE, CD_FLAG3
FROM CZ_MA_CODEDTL
WHERE 1 = 1
	--AND CD_COMPANY = '" + Global.MainFrame.LoginInfo.CompanyCode + @"'
	AND CD_FIELD = 'CZ_MA00050'
	AND YN_USE = 'Y'";
