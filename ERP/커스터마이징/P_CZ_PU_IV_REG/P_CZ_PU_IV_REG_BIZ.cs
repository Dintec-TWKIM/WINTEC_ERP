...
namespace cz
{
	public class P_CZ_PU_IV_REG_BIZ
	{
		private IMainFrame _mf;

		public P_CZ_PU_IV_REG_BIZ()
		{
		}

		public P_CZ_PU_IV_REG_BIZ(IMainFrame mf)
		{
			_mf = mf;
		}
		...
		public DataTable GetLineTable()
		{
            ...
            dt.Columns.Add("AM_ADPAY", typeof(decimal));
		...
		public object Save(DataTable dt, DataTable dt1, DataTable ds, string CD_CC, string FG_TRANS, string 매입일자)
		{
            ...
            si1.SpNameInsert = "SP_CZ_PU_IVL_I";			//Insert 프로시저명
		...
		{ 
			...
			SelectQuery = @"SELECT CD_DOCU 
						    FROM MA_AISPOSTH WITH(NOLOCK)
						    WHERE CD_COMPANY = '" + _mf.LoginInfo.CompanyCode + "'" + Environment.NewLine +
						   "AND CD_TP = '" + p_FG_TPPURCHASE + "'" + Environment.NewLine +
						   "AND FG_TP = '200'";
		...
		public DataTable getCd_Pc(string strCd_Bizarea)
		{
			...
			SelectQuery = @"SELECT M.CD_PC,
                                   P.NM_PC,
                                   B.CD_BIZAREA,
                                   B.NM_BIZAREA
						    FROM MA_BIZAREA M WITH(NOLOCK)
						    JOIN MA_PC P WITH(NOLOCK) ON M.CD_PC = P.CD_PC AND M.CD_COMPANY = P.CD_COMPANY
						    LEFT JOIN MA_BIZAREA B WITH(NOLOCK) ON M.VAT_BIZAREA = B.CD_BIZAREA AND M.CD_COMPANY = B.CD_COMPANY
						    WHERE M.CD_COMPANY = '" + _mf.LoginInfo.CompanyCode + "'" + Environment.NewLine +
		...
		internal DataTable Search_Line1(string key, string 발주일fr, string 발주일to, string 사업장, string 과세구분, string 거래구분, string 매입처, string 프로젝트, string gs_FGTAX, string 발주번호, string 담당자, string 구매그룹, string 요청그룹, string bp수불형태, string 창고)
		{
			SpInfo si = new SpInfo();
			si.SpNameSelect = "UP_PU_GIIV_SUB_SELECT_L";
			si.CompanyID = Global.MainFrame.LoginInfo.CompanyCode;
			si.SpParamsSelect = new string[] { key, Global.MainFrame.LoginInfo.CompanyCode, 발주일fr, 발주일to, 사업장, 과세구분, 거래구분, 매입처, 프로젝트, gs_FGTAX, 발주번호, 담당자, 구매그룹, 요청그룹, bp수불형태, 창고 };
