...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_FI_BANK_SEND_SUBH_LIMITE_I]    Script Date: 2018-03-15 오전 11:32:18 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_FI_BANK_SEND_SUBH_LIMITE_I]
(
	@P_CD_COMPANY  		VARCHAR(10),	--회사								
	@P_NODE_CODE  		VARCHAR(10),	--회계단위							
	@P_TRANS_DATE  		CHAR(8),		--파일작성일						
	@P_TRANS_SEQ  		VARCHAR(16),	--파일순번							
	@P_BANK_CODE  		VARCHAR(10),	--출금은행							
	@P_ACCT_NO  		VARCHAR(200),	--출금계좌							
	@P_SEQ				VARCHAR(16),	--순번								
	@P_TRANS_BANK_CODE	VARCHAR(10),	--지금은행							
	@P_TRANS_ACCT_NO  	VARCHAR(200),	--지급계좌							
	@P_TRANS_NAME  		VARCHAR(50),	--지급계좌명
...
-- 환경설정값 (은행코드, 사업자번호)  
SELECT  @P_BANK = BANK_CODE1
FROM BANK_SYS   
WHERE C_CODE = @P_CD_COMPANY AND USE_YN = '1'   

IF(ISNULL(@P_BANK,'') = '06' OR ISNULL(@P_BANK,'') = '006' OR ISNULL(@P_BANK,'') = '04' OR ISNULL(@P_BANK,'') = '004' OR ISNULL(@P_BANK,'') = '019' OR ISNULL(@P_BANK,'') = '029' OR ISNULL(@P_BANK,'') = '078' OR ISNULL(@P_BANK,'') = '079')  
BEGIN
