...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_FI_BANK_SEND_PARTNER_S]    Script Date: 2016-12-26 오전 10:07:25 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_FI_BANK_SEND_BAN_PARTNER_S]
( 
...
BEGIN 
	SELECT DISTINCT 'N' S, 
					...
					BH.AM_EX,
					BH.AM_EXBAN,
					ISNULL(SD.TRANS_AMT, 0) AS TRANS_AMT_A, -- 이체금액
					ISNULL(SD.TRANS_AMT_EX, 0) AS TRANS_AMT_EX_A,
					(CASE WHEN (BH.AMT_BAN > 0 OR BH.AM_EXBAN > 0) THEN (BH.AMT - BH.AMT_BAN) ELSE (BH.AMT - ISNULL(SD.TRANS_AMT, 0)) END) AS TRANS_AMT, -- 이체가능금액
					(CASE WHEN (BH.AMT_BAN > 0 OR BH.AM_EXBAN > 0) THEN (BH.AM_EX - BH.AM_EXBAN) ELSE (BH.AM_EX - ISNULL(SD.TRANS_AMT_EX, 0)) END) AS TRANS_AMT_EX,
					BH.DT_END, 
	...
	LEFT JOIN (SELECT C_CODE,
			   ...
			   WHERE C_CODE = @P_CD_COMPANY 
			   AND NODE_CODE IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_PC)) 
			   GROUP BY C_CODE, NO_DOCU, LINE_NO) SD
	ON BH.CD_COMPANY = SD.C_CODE AND BH.NO_DOCU = SD.NO_DOCU AND BH.NO_DOLINE = SD.LINE_NO
	...
	LEFT JOIN (SELECT PD.CD_COMPANY,
			   ...
			   WHERE PD.USE_YN = 'Y' 
			   GROUP BY PD.CD_COMPANY, PD.CD_PARTNER) PD 
	ON BH.CD_COMPANY = PD.CD_COMPANY AND BH.TR_CODE = PD.CD_PARTNER 
	...
	AND ((@P_YN_TRAN = '' OR @P_YN_TRAN IS NULL)
						  OR (@P_YN_TRAN = 'N' AND (BH.AMT - ISNULL(SD.TRANS_AMT, 0)) <> 0)
						  OR (@P_YN_TRAN = 'Y' AND (BH.AMT - ISNULL(SD.TRANS_AMT, 0)) = 0)))
...
BEGIN 
	...
	LEFT JOIN (SELECT C_CODE,
					  ...
					  SUM(TRANS_AMT) AS TRANS_AMT
			   FROM	BANK_SENDD
			   WHERE NODE_CODE IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_PC))
			   GROUP BY C_CODE, NO_DOCU, LINE_NO) SD
	ON BH.CD_COMPANY = SD.C_CODE AND BH.NO_DOCU = SD.NO_DOCU AND BH.NO_DOLINE = SD.LINE_NO
	LEFT JOIN (SELECT PD.CD_COMPANY,
					  PD.CD_PARTNER,
