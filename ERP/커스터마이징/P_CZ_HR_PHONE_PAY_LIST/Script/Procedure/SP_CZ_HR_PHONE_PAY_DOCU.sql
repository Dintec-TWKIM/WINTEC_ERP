...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_HR_PHONE_PAY_DOCU]    Script Date: 2017-04-13 오전 11:17:46 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_HR_PHONE_PAY_DOCU]                                                      
(                                                      
...
SET @V_NO_MDOCU = ('PHONE_' + @P_YYYYMM)
...
SELECT '1' AS TP_DRCR,
	   '55200' AS CD_ACCT,
	   HP.NM_CC + ' ' + RIGHT(@P_YYYYMM, 2) + '월 개인 휴대폰 요금 (' + HP.CNT + '명)' AS NM_NOTE,
	   ISNULL(HP.AM, 0) AS AM_DR,
	   0 AS AM_CR,
	   '0' AS TP_ACAREA,
	   AC.CD_RELATION,
	   HP.CD_BIZAREA,
	   ...
	   NULL AS CD_FUND,
	   0.0 AS AM_SUPPLY
FROM (SELECT HP.CD_COMPANY, ME.CD_CC,
			 MAX(MC.NM_CC) AS NM_CC,
			 MAX(MC.CD_BIZAREA) AS CD_BIZAREA,
			 SUM(HP.AM) AS AM,
			 CONVERT(NVARCHAR, COUNT(1)) AS CNT
	  FROM CZ_HR_PHONE_PAY HP
	  LEFT JOIN MA_EMP ME ON ME.CD_COMPANY = HP.CD_COMPANY AND ME.NO_EMP = HP.NO_EMP
	  LEFT JOIN MA_CC MC ON MC.CD_COMPANY = ME.CD_COMPANY AND MC.CD_CC = ME.CD_CC
	  WHERE HP.CD_COMPANY = @P_CD_COMPANY
	  AND HP.YM = @P_YYYYMM
	  AND HP.YN_PAY = 'Y'
	  GROUP BY HP.CD_COMPANY, ME.CD_CC) HP
LEFT JOIN MA_BIZAREA MB ON MB.CD_COMPANY = HP.CD_COMPANY AND MB.CD_BIZAREA = HP.CD_BIZAREA
LEFT JOIN FI_ACCTCODE AC ON AC.CD_COMPANY = HP.CD_COMPANY AND AC.CD_ACCT = '55200'
UNION ALL
/* 대변 : 미지급 비용 */    
SELECT '2' AS TP_DRCR,
	   '31200' AS CD_ACCT,
	   RIGHT(@P_YYYYMM, 2) + '월 ' + HP.NM_KOR + ' 휴대폰 요금' AS NM_NOTE,
	   0 AS AM_DR,
	   HP.AM AS AM_CR,
	   '4' AS TP_ACAREA,
	   AC.CD_RELATION,
...
FROM (SELECT HP.CD_COMPANY, 
	  ...
	  AND HP.YM = @P_YYYYMM
	  AND HP.YN_PAY = 'Y') HP
LEFT JOIN FI_ACCTCODE AC ON AC.CD_COMPANY = HP.CD_COMPANY AND AC.CD_ACCT = '31200'
LEFT JOIN MA_USER MU ON MU.CD_COMPANY = HP.CD_COMPANY AND MU.ID_USER = @P_ID_INSERT
...
SET @V_NM_PUMM = RIGHT(@P_YYYYMM, 2) + '월 개인 휴대폰 요금'
