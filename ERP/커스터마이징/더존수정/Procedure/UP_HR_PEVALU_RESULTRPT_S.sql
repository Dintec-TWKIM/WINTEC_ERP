	...
	*********************************************/
	...
	ALTER PROCEDURE [NEOE].[UP_HR_PEVALU_RESULTRPT_S]
	(
		@P_CD_COMPANY NVARCHAR(7),
		@P_CD_EVALU  NVARCHAR(8),
		@P_M_BIZAREA NVARCHAR(4000),
		@P_M_DEPT  NVARCHAR(4000),
		@P_M_NO_EMPAN NVARCHAR(4000),
		@P_M_CD_INCOM NVARCHAR(4000),
		@P_CD_EMP  NVARCHAR(3),
		@P_M_TP_EMP  NVARCHAR(4000),
	...
	BEGIN
	 ...
	 BEGIN
	  ...
	  SET  @V_ID = @V_ID + 1
	 END
	 SET @V_SELECT1 = 'SELECT MAN.CD_COMPANY, MAN.CD_EVALU, MAN.NO_EMPM, MAN.NO_EMPAN, MAN.NM_KOR, MAN.CD_EVNUMBER, PCE.NM_HCODE,
	        RST.PT_SCORE, RST.PT_ADJUST, RST.PT_FINAL, RST.GRD_FINAL, MAN.NM_BIZAREA, MAN.NM_DEPT'
	 SET @V_FROM1 = ' FROM ( SELECT PS.CD_COMPANY, PS.CD_EVALU, PS.CD_EVNUMBER, PS.NO_EMPM, PS.NO_EMPAN, EP.NM_KOR, EP.CD_INCOM, EP.CD_EMP, EP.TP_EMP, BA.CD_BIZAREA, BA.NM_BIZAREA, DT.CD_DEPT, DT.NM_DEPT
							 ...
							 AND  PS.NO_EMPAN = EP.NO_EMP
							 LEFT OUTER JOIN MA_BIZAREA BA
							 ON   HN.CD_BIZAREA = BA.CD_COMPANY
							 AND  HN.CD_BIZAREA = BA.CD_BIZAREA
							 LEFT OUTER JOIN MA_DEPT DT
							 ON   HN.CD_BIZAREA = DT.CD_COMPANY
							 AND  HN.CD_BIZAREA = DT.CD_BIZAREA
							 AND  EP.CD_DEPT  = DT.CD_DEPT
							 WHERE PS.CD_COMPANY = ''' + @P_CD_COMPANY + '''
							 AND  PS.CD_EVALU = ''' + @P_CD_EVALU + '''
							 AND  PS.NO_EMPM  = ''' + @P_NO_EMPM + '''
							 GROUP BY PS.CD_COMPANY, PS.CD_EVALU, PS.CD_EVNUMBER, PS.NO_EMPM, PS.NO_EMPAN, EP.NM_KOR, EP.CD_INCOM, EP.CD_EMP, EP.TP_EMP, BA.CD_BIZAREA, BA.NM_BIZAREA, DT.CD_DEPT, DT.NM_DEPT) MAN
	 ...
	 SET @V_WHERE = ' WHERE (''' + @P_M_BIZAREA + ''' IS NULL OR ''' + @P_M_BIZAREA + ''' = '''' OR MAN.CD_BIZAREA IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(''' + @P_M_BIZAREA + ''')))
	        AND (''' + @P_M_DEPT + ''' IS NULL OR ''' + @P_M_DEPT + ''' = '''' OR MAN.CD_DEPT IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(''' + @P_M_DEPT + ''')))
	        AND (''' + @P_M_NO_EMPAN + ''' IS NULL OR ''' + @P_M_NO_EMPAN + ''' = '''' OR MAN.NO_EMPAN IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(''' + @P_M_NO_EMPAN + ''')))
	        AND (''' + @P_M_CD_INCOM + ''' IS NULL OR ''' + @P_M_CD_INCOM + ''' = '''' OR MAN.CD_INCOM IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(''' + @P_M_CD_INCOM + ''')))
	        AND (''' + @P_CD_EMP + ''' IS NULL OR ''' + @P_CD_EMP + ''' = '''' OR MAN.CD_EMP IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(''' + @P_CD_EMP + ''')))
	        AND (''' + @P_M_TP_EMP + ''' IS NULL OR ''' + @P_M_TP_EMP + ''' = '''' OR MAN.TP_EMP IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(''' + @P_M_TP_EMP + ''')))'
	 
	 DROP TABLE #TEMP1

	 --SELECT @V_SELECT1, @V_SELECT2, @V_FROM1, @V_FROM2, @V_FROM3 + @V_FROM4 + @V_FROM5 + @V_FROM6 + @V_FROM7 + @V_FROM8 + @V_FROM9
	 EXEC (@V_SELECT1 + @V_SELECT2 + @V_FROM1 + @V_FROM2 + @V_FROM3 + @V_FROM4 + @V_FROM5 + @V_FROM6 + @V_FROM7 + @V_FROM8 + @V_FROM9 + @V_WHERE)
