...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_DEFERRED_DELIVERY_S3]    Script Date: 2016-04-21 오후 2:16:59 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_DEFERRED_DELIVERYH_S3] 
(
...
JOIN (SELECT SL.CD_COMPANY, SL.NO_SO, SL.SEQ_SO,
	  ...
	  JOIN (SELECT PL.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE,
				   ...
				   PL.AM AS AM_PO,
				   1 AS QT_PO,
				   (CASE WHEN ISNULL(PL.QT_PO, 0) = ISNULL(IL.QT_IN, 0) THEN 1 ELSE 0 END) AS QT_IN,
				   PL.DT_EXPECT 
			FROM PU_POL PL
	  ...
	  ON GL.CD_COMPANY = SL.CD_COMPANY AND GL.NO_SO = SL.NO_SO AND GL.SEQ_SO = SL.SEQ_SO) SL
...
LEFT JOIN CZ_SA_QTNL QL ON QL.CD_COMPANY = SL.CD_COMPANY AND QL.NO_FILE = SL.NO_SO AND QL.NO_LINE = SL.NO_LINE
...
ORDER BY SL.NO_SO, SL.NO_PO, SL.NO_LINE
