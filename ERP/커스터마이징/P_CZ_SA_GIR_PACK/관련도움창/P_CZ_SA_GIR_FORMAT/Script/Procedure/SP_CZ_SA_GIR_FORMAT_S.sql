...
SELECT MAX(GH.DT_GIR) AS DT_GIR,
	   ...
	   MH.NO_HULL,
	   MH.NM_VESSEL,	
	   DM.CD_PARTNER_NEW AS CD_DELIVERY_TO,
	   ...
	   TH.REMARK2,
	   TH.REMARK3,
	   (CASE WHEN ISNULL(PD.CD_COLLECT_FROM, '') <> '' AND DM.CD_PARTNER_NEW IS NULL THEN 'Y' ELSE 'N' END) AS YN_ERROR,
	   MAX(ISNULL(MP.DC_ADDRESS, '') + ' ' + ISNULL(MP.DC_ADDRESS1, '') + ' ' + ISNULL(MP.NM_PIC, '') + ' ' + ISNULL(MP.NO_TEL, '')) AS DC_DELIVERY_ADDR
...
LEFT JOIN CZ_MA_DELIVERY MP ON MP.CD_COMPANY = DM.CD_COMPANY AND MP.CD_PARTNER = DM.CD_PARTNER_NEW
...
GROUP BY GH.CD_PARTNER, MP1.LN_PARTNER, PD.NO_IMO, MH.NO_HULL, MH.NM_VESSEL,
		 PD.CD_COLLECT_FROM, DM.CD_PARTNER_NEW, MP.LN_PARTNER, PD.CD_PACK_CATEGORY, PD.CD_SUB_CATEGORY, PD.CD_RMK, MC.NM_SYSDEF, MC1.NM_SYSDEF, MC2.NM_SYSDEF,
...
SELECT MAX(GH.DT_GIR) AS DT_GIR,
	   ...
	   TH.REMARK2,
	   TH.REMARK3,
	   (CASE WHEN ISNULL(WD.CD_DELIVERY_TO, '') <> '' AND DM.CD_PARTNER_NEW IS NULL THEN 'Y' ELSE 'N' END) AS YN_ERROR,
	   MAX(ISNULL(MP.DC_ADDRESS, '') + ' ' + ISNULL(MP.DC_ADDRESS1, '') + ' ' + ISNULL(MP.NM_PIC, '') + ' ' + ISNULL(MP.NO_TEL, '')) AS DC_DELIVERY_ADDR
FROM SA_GIRH GH
...
GROUP BY GH.CD_PARTNER, MP1.LN_PARTNER, WD.NO_IMO, MH.NO_HULL, MH.NM_VESSEL,
		 WD.CD_DELIVERY_TO, DM.CD_PARTNER_NEW, MP.LN_PARTNER, WD.CD_MAIN_CATEGORY, WD.CD_SUB_CATEGORY, WD.CD_RMK, MC.NM_SYSDEF, MC1.NM_SYSDEF, MC2.NM_SYSDEF,
		 TH.CD_CONSIGNEE, TH.NM_CONSIGNEE, TH.ADDR1_CONSIGNEE, TH.ADDR2_CONSIGNEE, TH.CD_PRODUCT, TH.ARRIVER_COUNTRY, MC3.NM_SYSDEF_E, TH.PORT_ARRIVER, TH.REMARK1, TH.REMARK2, TH.REMARK3
