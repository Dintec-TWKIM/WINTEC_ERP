...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_IV_BASE_IOL_S]    Script Date: 2018-06-26 오후 2:13:34 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_IV_BASE_IOL_S]  
(
...
(
	...
	JOIN (SELECT IH.CD_COMPANY, IH.NO_IO, IH.DT_IO,
	      ...
	      ON IL.CD_COMPANY = IH.CD_COMPANY AND IL.NO_IO = IH.NO_IO) IL 
...
(
    ...
    FROM (SELECT SH.CD_COMPANY, SH.NO_SO, SL.SEQ_SO,
	      ...
	      JOIN SA_SOL SL ON SL.CD_COMPANY = SH.CD_COMPANY AND SL.NO_SO = SH.NO_SO
		  WHERE SH.CD_COMPANY = @P_CD_COMPANY
		  AND SL.NO_PROJECT = @P_NO_PROJECT
...
(
	...
	FROM (SELECT SH.CD_COMPANY, SH.NO_SO, SL.SEQ_SO,
		  ...
		  LEFT JOIN CZ_SA_CLAIMH CH ON CH.CD_COMPANY = SH.CD_COMPANY AND CH.NO_CLAIM = SH.NO_PROJECT
	  	  ...
	  	  WHERE SH.CD_COMPANY = @P_CD_COMPANY
		  AND ISNULL(CH.NO_SO, SH.NO_PROJECT) = @P_NO_PROJECT
		  AND EXISTS (SELECT 1 
					  FROM CZ_SA_QTNH QH
					  WHERE QH.CD_COMPANY = SH.CD_COMPANY 
					  AND QH.NO_FILE = SH.NO_SO)
...
SELECT *, 'A' AS TP_TYPE FROM A
UNION ALL
SELECT *, 'B' AS TP_TYPE FROM B 
UNION ALL
SELECT *, 'C' AS TP_TYPE FROM C
ORDER BY DT_IO ASC
