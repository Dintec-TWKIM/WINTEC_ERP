...
namespace DX
{
	public static class FILE
	{
		...
		public static string 고유파일이름(string fileName)
		{
			//고유파일이름
			FileInfo file = new FileInfo(fileName);
			string newName = file.Name.Replace(file.Extension, "");
			string path = 경로_워크플로우(fileName);
			int index = 0;
		...
		public static string 고유파일이름(string url, string fileName)
		{
			//고유파일이름
			...
			while (파일유무(url, newName + file.Extension))
			{
				index++;
		...
		{
			return Download(경로_워크플로우(fileNumber.ToString()) + "/" + fileName, run);
		}
		...
		public static string Download(object path, bool run)
		{
			...
			// 웹서버경로와 로컬경로
			string webPath = 경로(path);
			string locPath = 경로_임시();

			// 다운로드할 파일과 로컬에 저장할 파일
			string fileName = 파일이름(path);
			string uniqName = 고유파일이름(locPath + @"\" + fileName);
		...
		public static string DownloadWF(string companyCode, string fileNumber, string fileName, bool run)
		{
			...
			// 다운로드 파일 이름 결정
			string fileNameNew = 고유파일이름(localPath + @"\" + fileName);
		...
		public static string UploadWF(string companyCode, string fileNumber, string fileName, bool overwrite)
		{
			...
			string newFileName = overwrite ? Path.GetFileName(tempFileName) : 고유파일이름(uploadDir, tempFileName);
		...
		/// </summary>		
		public static string 경로(object 파일이름)
		{
			string fileName = 파일이름.ToString();
		...
		/// 확장자 가져오기
		/// </summary>
		public static string 확장자_점제외(object 파일이름)
		{
			return Path.GetExtension(파일이름.ToString()).Replace(".", "");
		}

		/// <summary>
		/// 확장자 가져오기
		/// </summary>
		public static string 확장자(object 파일이름)
		{
			return Path.GetExtension(파일이름.ToString());
		}
