...
namespace cz
{
    public partial class P_CZ_SA_BILL_SUB : Duzon.Common.Forms.CommonDialog, IHelpWindow
    {
        private object[] _Return = new object[1];
        private string _MultiNoTax = string.Empty;
        ...
        public P_CZ_SA_BILL_SUB()
        {
            this.InitializeComponent();
        }

        public P_CZ_SA_BILL_SUB(string MultiNoTax, string 수금일자)
        {
            this._MultiNoTax = MultiNoTax;
            this.dt매출반제 = this.매출반제();
            this.수금일자 = 수금일자;
            this.InitializeComponent();
        }

        public P_CZ_SA_BILL_SUB(string MultiNoTax, DataTable 매출반제, string 수금일자)
        {
            this._MultiNoTax = MultiNoTax;
            this.dt매출반제 = 매출반제;
            this.수금일자 = 수금일자;
        ...
        private void InitGrid()
        {
            ...
            this._flex.SetCol("AM_K", "공급가액(원화)", 115, false, typeof(decimal), FormatTpType.MONEY);
            this._flex.SetCol("AM_VAT", "부가세", 100, false, typeof(decimal), FormatTpType.MONEY);
            this._flex.SetCol("AM_TOT", "합계금액", 100, false, typeof(decimal), FormatTpType.MONEY);
            ...
            this._flex.SetCol("NO_PROJECT_PJTCREDIT", "특별여신프로젝트", 200);
            this._flex.SetCol("DC_REMARK", "비고", 100);
            ...
            this._flex.LoadUserCache("P_CZ_SA_BILL_SUB_flex");
        }
        ...
        private void btn조회_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this._flex.Binding = DBHelper.GetDataTable("SP_CZ_SA_BILL_SUB_S", new object[] { Global.MainFrame.LoginInfo.CompanyCode,
                ...
                {
                    foreach (DataRow dataRow in this._flex.DataTable.Select(" NO_IV = '" + this.dt매출반제.Rows[index]["NO_IV"].ToString() + "'", "", DataViewRowState.CurrentRows))
                    {
        ...
        public void btn적용_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.반제액체크())
                {
                    Global.MainFrame.ShowMessage("SA_M000126");
                }
                else
                {
                    if (this.반제액체크2() && Global.MainFrame.ShowMessage("외화반제 적용할 반제액이 총수금액과 일치하지않습니다. 그래도 적용하시겠습니까?", "QY2") == DialogResult.No)
                        return;
        ...
        {
            this._flex.SaveUserCache("P_CZ_SA_BILL_SUB_flex");
        }
