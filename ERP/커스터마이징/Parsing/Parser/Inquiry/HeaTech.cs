...
namespace Parsing
{
    class HeaTech
    {
        ...
        public HeaTech(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("BB")) return;

                    if (firstColStr.StartsWith("REF No"))
                        reference = dt.Rows[i][1].ToString().Trim();
                    else if (firstColStr.StartsWith("SUBJECT"))
                    {
                        string[] subjSpl = dt.Rows[i][1].ToString().Split('-');

                        if (subjSpl.Length == 2)
                        {
                            vessel = subjSpl[0].ToString().Trim();
                            subjStr = subjSpl[1].ToString().Trim();
                            ...
                            subjStr = subjStr.Replace("S/P for", "").Trim();
                        }
                    }
                    else if (firstColStr.Equals("No"))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("D E S C R I P T I O N")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Equals("P/No")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().Equals("Q'TY")) _itemQt = c;
                        }
                    }
                    else if (firstColStr.Equals("A"))
                    {
