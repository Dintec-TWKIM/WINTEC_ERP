            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("Bid Number"))
                                reference = dt.Rows[i + 3][c].ToString().Trim();
                        }
                    }

                    if (GetTo.IsInt(firstColStr) && firstColStr.Length >= 2 && firstColStr.EndsWith("0"))
                    {
                        ...
                        if (!_itemDesc.Equals(-1))
                        {
                            ...
                            while (!GetTo.IsInt(dt.Rows[_i][0].ToString()))
                            {
                                ...
                                {
                                    if (dt.Rows[_i][c].ToString().StartsWith("https://"))
                                        break;
                                    else if (dt.Rows[_i][c].ToString().StartsWith("Note:"))
                                        break;
                                    else
                        ...
                        dtItem.Rows[dtItem.Rows.Count - 1]["DESC"] = iTemDESC.Replace(";","\r\n");
