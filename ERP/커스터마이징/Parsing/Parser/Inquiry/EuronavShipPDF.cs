...
namespace Parsing
{
    class EuronavShipPDF
    {
        ...
        public EuronavShipPDF(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
            ...
            //dtItem.DefaultView.Sort = "NO ASC";
            this.fileName = fileName;
            this.uc = new UnitConverter();
        }

        public static DataTable OrderByDT(DataTable dt)
		{
            DataTable sortDt = dt.Clone();

            sortDt.Columns["NO"].DataType = Type.GetType("System.Int32");
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
					...
					if (string.IsNullOrEmpty(reference))
					{
						for (int c = 0; c < dt.Columns.Count; c++)
						{
                            ...
                            else if (dt.Rows[i][c].ToString().ToLower().Contains("quote: "))
							{
                                reference = dt.Rows[i][c].ToString().ToLower().Replace("quote:","").Trim();
							}
						}
					}
                    ...
                    {
                        imoNumber = firstColStr.Replace("imo no", "").Replace(".", "").Replace(":", "").Trim() + dt.Rows[i][_noStr + 1].ToString().Replace("imo no", "").Replace(".", "").Replace(":", "").Trim();
                    }
                    ...
                    else if (firstColStr.StartsWith("pos."))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().ToLower().Contains("quantity")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().ToLower().Contains("description")) _itemDesc = c;
                        }

                    }
                    else if (firstColStr.StartsWith("parent component no.:") || firstColStr.StartsWith("component no.:"))
                    {
                        if (!subjStr.Contains(dt.Rows[i][_noStr].ToString()))
                        {
                            subjStr = dt.Rows[i][_noStr].ToString();

                            int _i = i + 1;

                            while (!dt.Rows[_i][_noStr].ToString().Contains("Pos."))
                            {
                                subjStr = subjStr + Environment.NewLine + dt.Rows[_i][_noStr].ToString().Trim();
                    ...
                    else if (GetTo.IsInt(firstColStr) && firstColStr.Length < 4)
                    {
                        ...
                        if (_itemDesc != -1)
                        {
                            ...
                            if (idx_s != -1)
                            {
                                ...
                                int strInt = idx_s + iTemDESC.Length;

                                xmlTemp = xmlTemp.Substring(strInt, xmlTemp.Length - strInt);
                            ...
                            if (iTemDESC.Contains("Remarks: For Supplier:"))
                            {

                                string removeStr1 = "Remarks: For Supplier: Please provide the Equipment Maker’s Part No. for this item.This is necessary for placing the Order.When quoting through Connect Web, please add it in";

                                iTemDESC = iTemDESC.ToLower().Replace("remarks: for supplier: please provide the equipment maker’s", "");
                                iTemDESC = iTemDESC.ToLower().Replace("part no. for this item.", "");
                                iTemDESC = iTemDESC.ToLower().Replace("this is necessary for placing the order.", "");
                                iTemDESC = iTemDESC.ToLower().Replace("when quoting through connect web, please add it in", "");
                                iTemDESC = iTemDESC.ToLower().Replace("\"supplier item no\" field.", "");
                                iTemDESC = iTemDESC.ToLower().Replace("thank you.", "");
					...
					else if (firstColStr.ToLower().Contains("remarks to supplier") || zeroColStr.ToLower().Contains("remarks to supplier"))
					{
                        ...
                        string subjAllRmk = string.Empty;

                        while (!dt.Rows[_i][_noStr].ToString().Contains("Requirements for Quotation"))
						{
                            subjAllRmk = dt.Rows[_i][_noStr].ToString();

                            _i += 1;

                            if (dt.Rows[_i][_noStr].ToString().Contains("Please see attached file."))
                                break;
						...
						{
							dtItem.Rows[_c]["SUBJ"] = dtItem.Rows[_c]["SUBJ"] + Environment.NewLine + subjAllRmk;
						}
