                    ...
                    else
                    {
                        ...
                        else if (GetTo.IsInt(firstColString))
                        {
                            if (Convert.ToInt16(dt.TableName.ToString().Replace("Table", "")) > 1)
                            {
                                ...
                                itemDESC = itemDESC.Replace("[EA]", "").Replace("[PC]", "").Trim();
                                itemCODE = itemCODE.Replace("\n", "").Trim();

                                int idx_s = itemDESC.IndexOf("Part number:");
                                int idx_e = itemDESC.IndexOf("(");
                                
                                if (!idx_s.Equals(-1))
                                {
                                    itemUNIQ = itemCODE;
                                    itemCODE = itemDESC.Substring(idx_s, idx_e - idx_s).Replace("Part number", "").Replace(":", "").Replace("N/A","").Trim();

                                    idx_e = itemDESC.IndexOf("Comments");

                                    if (string.IsNullOrEmpty(itemSUBJ))
                                        itemSUBJ = itemDESC.Substring(idx_e, itemDESC.Length - idx_e).Replace("Comments:","").Trim();

                                    itemDESC = itemDESC.Substring(0, idx_e).Replace("Comments", "").Trim();
                                }
