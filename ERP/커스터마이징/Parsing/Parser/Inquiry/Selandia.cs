...
namespace Parsing
{
    class Selandia
    {
        ...
        public Selandia(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (secondColStr.StartsWith("Vessel Name"))
                        vessel = dt.Rows[i][2].ToString().Trim();
                    else if (secondColStr.StartsWith("Order No "))
                        reference = dt.Rows[i][2].ToString().Trim();
                    else if (secondColStr.StartsWith("Name Of Equipment"))
                    {
                    ...
                    else if (GetTo.IsInt(secondColStr))
                    {
                        ...
                        if (rowValueSpl[5] != null && rowValueSpl[6] == null)
                        {
                            ...
                            _itemDwg = Convert.ToInt16(rowValueSpl[5].ToString());
                        }
                        else if (rowValueSpl[4] != null && rowValueSpl[5] == null)
                        {
                            if (!GetTo.IsInt(dt.Rows[i][Convert.ToInt16(rowValueSpl[1].ToString())].ToString()))
                            {
