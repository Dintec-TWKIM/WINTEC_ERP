...
namespace Parsing
{
    class CyprusSea
    {
        ...
        public CyprusSea(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    //        vessel = dt.Rows[i][_vesselColInt + 1].ToString().Trim();
                    //}
                    ...
                    if (firstColStr.Equals("No."))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("QTY")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("ITEM CODE")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("PRODUCT DESCRIPTION")) _itemDesc = c;
                        }
                    }
                    else if (secondColStr.StartsWith("System"))
                    {
                        systemStr = secondColStr.Replace("System","").Replace(":","").Trim();
                    }
                    else if (secondColStr.StartsWith("Particulars"))
                    {
                        particularsStr = secondColStr.Replace("Particulars","").Replace(":","").Trim();
                    }
                    else if (secondColStr.StartsWith("Maker"))
                    {
                        makerStr = secondColStr.Replace("Maker","").Replace(":","").Replace("N/A","").Replace("Serial No","").Replace("/","").Trim();
                    }
                    ...
                    {
                        dwgStr = secondColStr.Replace("DWG No", "").Replace(":","").Trim();
                    }
                    ...
                    {
                        makerStr = firstColStr.Replace("Maker", "").Replace(":", "").Replace("N/A", "").Replace("Serial No", "").Replace("/", "").Trim();
                    }
                    else if (firstColStr.StartsWith("Subsystem"))
                    {
                        subsystemStr = firstColStr.Replace("Subsystem", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.StartsWith("DWG No"))
                    {
                        dwgStr = firstColStr.Replace("DWG No", "").Replace(":", "").Trim();
                    }
                    ...
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!string.IsNullOrEmpty(dwgStr))
                            iTemSUBJ = iTemSUBJ.Trim() + Environment.NewLine + "DWG NO: " + dwgStr;
