...
namespace Parsing
{
    class TBmarine_pdf
	{
        ...
        public TBmarine_pdf(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().ToLower().StartsWith("requisition no"))
                            {
                    ...
                    if (string.IsNullOrEmpty(imoNumber))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().ToLower().StartsWith("imo:"))
                            {
                                for(int c2 = c+1; c2 < dt.Columns.Count; c2++)
                                {
                                    if (string.IsNullOrEmpty(imoNumber))
                                        imoNumber = dt.Rows[i][c2].ToString().Trim();
                                }
                            }
                    ...
                    if (firstColStr.StartsWith("no."))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().ToLower().Contains("umo")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().ToLower().Contains("description")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().ToLower().Contains("part no.")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().ToLower().Contains("drawing no.")) _itemDwg = c;
                        }
                    }
