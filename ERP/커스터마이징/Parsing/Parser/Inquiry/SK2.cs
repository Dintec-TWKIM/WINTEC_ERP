            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(vessel))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("Vessel"))
                            {
                                ...
                                if (!vesselRowCount.Equals(-1) && string.IsNullOrEmpty(vessel))
                                {
                                    for (int c1 = c; c1 < dt.Columns.Count; c++)
                                    {
                                        if (dt.Rows[i][c1].ToString().Equals("Quotation No"))
                                            reference = dt.Rows[i][c1 + 1].ToString().Trim();
                                    }
                                }
                            }
                        }
                    ...
                    if (firstColStr.Equals("No"))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("IMPA")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().Equals("Description & Spec")) _itemDesc = c;
