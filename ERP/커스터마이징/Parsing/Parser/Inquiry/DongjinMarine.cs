...
namespace Parsing
{
    class DongjinMarine
    {
        ...
        public DongjinMarine(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if (firstColStr.StartsWith("NO."))
                    {
                        ...
                        iTemSUBJ = dt.Rows[i + 1][1].ToString().Trim();
                    }
                    else if (!string.IsNullOrEmpty(firstColStr) && itemStart && GetTo.IsInt(firstColStr.Substring(0, 1)))
                    {
                        if (!_itemDesc.Equals(-1))
                        {
                            string[] descSpl = dt.Rows[i][_itemDesc].ToString().Split('.');

                            if (descSpl.Length == 2)
                            {
                                iTemNo = descSpl[0].ToString().Trim();
                                iTemDESC = descSpl[1].ToString().Trim();
                            }
                        }
