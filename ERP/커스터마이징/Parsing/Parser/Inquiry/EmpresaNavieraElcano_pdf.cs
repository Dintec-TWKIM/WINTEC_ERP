...
namespace Parsing
{
    class EmpresaNavieraElcano_pdf
    {
        string vessel;
        string reference;
        DataTable dtItem;
        ...
        public EmpresaNavieraElcano_pdf(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstStr.ToUpper().Contains("VESSEL"))
                        vessel = dt.Rows[i - 1][1].ToString().Trim();
                    else if (firstStr.ToUpper().Contains("REQUEST"))
                        reference = dt.Rows[i - 1][1].ToString().Trim();
                    else if (firstStr.ToUpper().Contains("EQUIPMENT"))
                        subjStr = dt.Rows[i - 1][1].ToString().Trim();
                    else if (firstStr.ToUpper().Contains("MANUFACTURER"))
                        makerStr = dt.Rows[i - 1][1].ToString().Trim();
                    else if (secondStr.ToUpper().Contains("DESCRIPTION"))
                        _itemDesc = 1;
                    else if (GetTo.IsInt(firstStr))
                    {
                        ...
                        if (_itemQt != -1)
                        {
                            ...
                            if (iTemQt.Contains(","))
                            {
                                int stx_id = iTemQt.IndexOf(",");

                                if (stx_id != -1)
                                {
                                    iTemQt = iTemQt.Substring(0, stx_id).Trim();
                                }
                            }
                        }
                        ...
                        if (!string.IsNullOrEmpty(typeStr))
                            iTemSUBJ = iTemSUBJ.Trim() + Environment.NewLine + "TYPE / SERIAL: " + typeStr.Trim();
