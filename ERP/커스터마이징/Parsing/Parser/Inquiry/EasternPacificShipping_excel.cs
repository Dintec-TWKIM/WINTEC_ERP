...
namespace Parsing
{
    class EasternPacificShipping_excel
    {
        string contact;
        string reference;
        string vessel;
        ...
        public EasternPacificShipping_excel(string fileName)
        {
            reference = "";
            vessel = "";
        ...
        public void Parse()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (firstColStr.ToUpper().StartsWith("CATALOG"))
                {
                    for (int c = 1; c < dt.Columns.Count; c++)
                    {
                        if (dt.Rows[i][c].ToString().Contains("ITEM REFERENCE")) _itemCode = c;
                        ...
                        else if (dt.Rows[i][c].ToString().Contains("SUB CATALOG")) _itemSubj = c;
                    }
                }
                else if (_itemQt != -1 && GetTo.IsInt(dt.Rows[i][_itemQt].ToString()))
                {
                    if (!_itemQt.Equals(-1))
                    ...
                    if (!_itemSubj.Equals(-1))
                        iTemSUBJ = dt.Rows[i][0].ToString().Trim() + Environment.NewLine + dt.Rows[i][_itemSubj].ToString().Trim();
