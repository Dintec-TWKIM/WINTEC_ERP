...
namespace Parsing
{
    class Dynagas
    {
        ...
        public Dynagas(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("Awaiting your Quotation.")) break;
                    ...
                    else if (firstColStr.StartsWith("OUR REF"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                    ...
                    {
                        if (!dt.Rows[i + 1][0].ToString().Equals("No"))
                        {
                    ...
                    else if (firstColStr.StartsWith("No"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("Plate/Drg No")) _itemDrw = c;
                    ...
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!_itemDesc.Equals(-1))
                        {
                            ...
                            {
                                iTemDESC = iTemDESC.Trim() + Environment.NewLine + dt.Rows[i][_itemDesc].ToString().Trim();
                                _i += 1;
                            }
                        }
