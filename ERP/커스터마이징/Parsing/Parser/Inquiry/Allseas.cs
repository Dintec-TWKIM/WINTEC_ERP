...
namespace Parsing
{
    class Allseas
    {
        ...
        public Allseas(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if ((secondColStr.Contains(")") && GetTo.IsInt(secondColStr.Substring(0, 1))) || (firstColStr.Contains(")") && GetTo.IsInt(firstColStr.Substring(0, 1))))
                    {
                        ...
                        if (!_itemPos.Equals(-1))
                            iTemPos = dt.Rows[i][_itemPos].ToString().Trim();

                        if (!string.IsNullOrEmpty(iTemPos))
                            iTemDESC = iTemDESC.Trim() + Environment.NewLine + "POS NO: " + iTemPos.Trim();
