...
namespace Parsing
{
    class Seadrill
    {
        ...
        public Seadrill(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("Line"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Description")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Mfr Name")) _itemMaker = c;
                            else if (dt.Rows[i + 1][c].ToString().StartsWith("Quantity")) _itemQt = c;
                    ...
                    else if (GetTo.IsInt(firstColStr) && firstColStr.Length < 4)
                    {
                        ...
                        if (_itemMaker != -1)
                        {
                            ...
                            while (string.IsNullOrEmpty(dt.Rows[_i][0].ToString()))
                            {
                                iTemMaker = iTemMaker.Trim() + " " + dt.Rows[_i][_itemMaker].ToString().Trim();
