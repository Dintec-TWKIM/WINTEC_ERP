...
namespace Parsing
{
    public class ShipServ
    {
        ...
        public ShipServ(string fileName)
        {
            contact = "";
            reference = "";
            vessel = "";
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(contact))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Buyer Contact:"))
                                contact = dt.Rows[i + 1][c].ToString().Trim();
                        }

                        if (!string.IsNullOrEmpty(contact))
                        {
                            string[] contactSpl = contact.Split('\n');

                            if(contactSpl.Length > 1)
                            {
                                string _contact = contactSpl[0].ToString();
                                contact = _contact.Replace(".", "").Replace("Contact", "").Replace(":", "").Trim();
                    ...
                    if (!addible)
                    {
                        ...
                        else if (firstColString.StartsWith("RFQ Deta"))
                        {
                            ...
                            if (reference.Contains("RFQ Date"))
                            {
                                idx_s = reference.IndexOf("RFQ Date");

                                reference = reference.Substring(0,idx_s).Trim();
                            }
                            //{
                            //    int indexOfstr = reference.IndexOf("RFQ");
                        ...
                        else if (GetTo.IsInt(firstColString) || GetTo.IsInt(secondColStr))
                        {
                            if (Convert.ToInt16(dt.TableName.ToString().Replace("Table", "")) > 1)
                            {
                                ...
                                if (!itemDESC.Contains("Freight and Packing & Handling"))
                                {
                                    ...
                                    {
                                        int idx_s = itemDESC.IndexOf("[Buyer Item");

                                        itemUNIQ = itemDESC.Substring(idx_s, itemDESC.Length - idx_s).Trim();

                                        itemUNIQ = itemUNIQ.Replace(":","").Replace("#", "").Replace("Buyer Item", "").Replace("[", "").Replace("]", "").Replace("Comments:", "").Trim();
