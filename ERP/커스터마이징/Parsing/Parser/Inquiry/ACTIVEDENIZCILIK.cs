...
namespace Parsing
{
	internal class ACTIVEDENIZCILIK
	{
		...
		public ACTIVEDENIZCILIK(string fileName)
		{
			reference = "";
			vessel = "";
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				else if (firstColStr.StartsWith("Item No"))
				{
					for (int c = 1; c < dt.Columns.Count; c++)
					{
						...
						else if (dt.Rows[i][c].ToString().Contains("Part Name")) _itemDesc = c;
						else if (dt.Rows[i][c].ToString().Contains("Requested Qty")) _itemQt = c;
