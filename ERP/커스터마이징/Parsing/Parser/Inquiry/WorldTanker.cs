...
namespace Parsing
{
    class WorldTanker
    {
        ...
        public WorldTanker(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            imoNumber = "";
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if (firstColStr.StartsWith("pos"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("Item No") || dt.Rows[i][c].ToString().StartsWith("Pos.") || dt.Rows[i][c].ToString().StartsWith("Part No.") || dt.Rows[i][c].ToString().StartsWith("Code No") || dt.Rows[i][c].ToString().StartsWith("Plate No")) _itemCode = c;
                    ...
                    else if (GetTo.IsInt(firstColStr) && firstColStr.Length < 3)
                    {
                        if (!_itemQt.Equals(-1))
                        {
                            ...
                            if (string.IsNullOrEmpty(iTemUnit) && _itemQt + 2 < dt.Columns.Count)
                            {
                        ...
                        if (!_itemCode.Equals(-1))
                        {
							...
							{
                                descPos = descPos + " " + dt.Rows[_i][0].ToString().Trim();
