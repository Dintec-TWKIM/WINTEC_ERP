...
namespace Parsing
{
    class Seateam_pdf
    {
        ...
        public Seateam_pdf(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("Our Reference"))
                    {
                    ...
                    else if (firstColStr.StartsWith("Supplier Reference"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                    ...
                    else if (secondColStr.Equals("Line#"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Part")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().Contains("Full Description")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Equals("Unit Type")) _itemUnit = c;
