...
namespace Parsing
{
    class SOCATRA_pdf
    {
        ...
        public SOCATRA_pdf(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.ToLower().StartsWith("for component"))
                    {
                        subjStr = firstColStr.Trim() + dt.Rows[i][1].ToString().Trim() + dt.Rows[i][2].ToString().Trim();
                        subjStr = subjStr.Replace("For component", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.ToLower().StartsWith("maker"))
                    {
                        makerStr = firstColStr.Trim() + dt.Rows[i][1].ToString().Trim() + dt.Rows[i][2].ToString().Trim();  
                        makerStr = makerStr.Replace("Maker","").Replace(":","").Trim();
                    }
                    else if (firstColStr.ToLower().StartsWith("model:"))
                    {
                        modelStr = firstColStr.Trim() + dt.Rows[i][1].ToString().Trim() + dt.Rows[i][2].ToString().Trim();
                        modelStr = modelStr.Replace("Model", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.ToLower().StartsWith("serial"))
                    {
                        serialStr = firstColStr.Trim() + dt.Rows[i][1].ToString().Trim() + dt.Rows[i][2].ToString().Trim();
                        serialStr = serialStr.Replace("Serial", "").Replace("No.", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.ToLower().StartsWith("type"))
                    {
                        typeStr = firstColStr.Trim() + dt.Rows[i][1].ToString().Trim() + dt.Rows[i][2].ToString().Trim();
                        typeStr = typeStr.Replace("Type", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.Equals("Item"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().Equals("Description") || dt.Rows[i][c].ToString().Contains("Name/Maker's")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Equals("Qty. Measure") || dt.Rows[i][c].ToString().Contains("Quantity")) _itemQt = c;
