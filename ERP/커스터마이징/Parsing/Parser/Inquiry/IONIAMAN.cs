...
namespace Parsing
{
    ...
    class IONIAMAN
    {
        ...
        public IONIAMAN(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            {
                                reference = dt.Rows[i][c].ToString().Replace("Our Ref :", "").Trim();
                            }
                        }
                    }
                    ...
                    else if (firstColStr.Equals("#"))
                    {
                        for(int c = 0; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("Pos No")) _itemPos = c;
                    ...
                    {
                        if (firstColStr.Replace(")", "").Equals("1"))
                            iTemSUBJ = dt.Rows[i - 1][0].ToString().Trim();
                        ...
                        if (_itemPos != -1)
                        {
                            if(!string.IsNullOrEmpty(dt.Rows[i][_itemPos].ToString()))
                                iTemCode = dt.Rows[i][_itemPos].ToString().Trim();
                        }
