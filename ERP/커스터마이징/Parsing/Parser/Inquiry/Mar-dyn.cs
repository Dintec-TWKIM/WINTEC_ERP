...
namespace Parsing
{
    class Mar_dyn
    {
        ...
        public Mar_dyn(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("OUR") && firstColStr.Contains("EF"))
                    {
                        reference = firstColStr.Trim() + dt.Rows[i][1].ToString() + dt.Rows[i][2].ToString().Trim();
                        vessel = dt.Rows[i+1][0].ToString().Trim() + " " + dt.Rows[i+1][1].ToString().Trim();
                    }
                    else if (firstColStr.StartsWith("PLEASE"))
                    {
                        ...
                        while (GetTo.IsInt(dt.Rows[_i][0].ToString()))
                        {
                            ...
                            if (dt.Rows[_i][0].ToString().StartsWith("IT/NO"))
                                break;
                        ...
                        subjStr = subjStr.Replace("TYPE OF MACHINERY", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.StartsWith("MANUFACTURER"))
                    {
                        ...
                        makerStr = makerStr.Replace("MANUFACTURER", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.StartsWith("DWG NO"))
                    {
                        ...
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            dwgStr = dwgStr.Trim() + " " + dt.Rows[i][c].ToString().Trim();
                        }

                        dwgStr = dwgStr.Replace("DWG NO", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.StartsWith("IT/NO"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
