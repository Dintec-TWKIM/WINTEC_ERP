        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(vessel) && i == 0)
                    {
                    ...
                    if (firstColStr.StartsWith("Req ID"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                    ...
                    else if (firstColStr.StartsWith("Line"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("Units")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Description")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Mfg. Part")) _itemCode = c;
                        }
                    }
                    else if (secondColStr.StartsWith("Drawing Number"))
                    {
                        dwgStr = secondColStr.Replace("Drawing Number", "").Replace(":", "").Trim();
                    }
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!_itemDesc.Equals(-1))
                        {
                            ...
                            while (string.IsNullOrEmpty(dt.Rows[_i][0].ToString()))
                            {
                                ...
                                if (_i >= dt.Rows.Count || dt.Rows[_i][_itemDesc].ToString().Contains("***"))
                                    break;
                            }
