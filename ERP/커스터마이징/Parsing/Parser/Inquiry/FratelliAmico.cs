...
namespace Parsing
{
    class FratelliAmico
    {
        ...
        public FratelliAmico(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("Ship's Name:") || secondColStr.StartsWith("Ship's Name:"))
                    {
                        vessel = firstColStr.Replace("Ship's Name:", "").Trim();

                        if(string.IsNullOrEmpty(vessel))
                            vessel = dt.Rows[i][1].ToString().Replace("Ship's Name:", "").Trim() + dt.Rows[i][2].ToString().Replace("Ship's Name:", "").Trim();
                    }
                    else if (firstColStr.StartsWith("Query No."))
                    {
                        reference = firstColStr.Replace("Query No.", "").Replace(":", "").Trim();
                    ...
                    else if (firstColStr.StartsWith("Item"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().Equals("Name/Maker's Reference")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Equals("Unit")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().Equals("Qty")) _itemQt = c;
                        }
                    }
                    else if (firstColStr.StartsWith("For component") || secondColStr.StartsWith("For component"))
                    {
                        ...
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (string.IsNullOrEmpty(subjStr))
                                subjStr = dt.Rows[i][c].ToString().Replace("For component","").Replace(":","").Trim();
                        }
                    }
                    ...
                    else if (firstColStr.StartsWith("Serial No") || secondColStr.StartsWith("Serial No"))
                    {
                        ...
                        serialStr = serialStr.Replace("Serial No", "").Replace(":", "").Replace(".","").Trim();
                    }
