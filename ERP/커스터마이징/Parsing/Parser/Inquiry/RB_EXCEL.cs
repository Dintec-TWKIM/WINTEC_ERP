...
namespace Parsing
{
    class RB_EXCEL
    {
        ...
        public RB_EXCEL(string fileName)
        {
            reference = "";
            vessel = "";
        ...
        public void Parse()
        {
            ...
            {
                string firstColStr = dt.Rows[i][2].ToString();
                ...
                {
                    vessel = dt.Rows[i][3].ToString().Trim() + dt.Rows[i][4].ToString().Trim();

                    vessel = vessel.Replace("M.V.", "").Trim();
                }
                else if (firstColStr.StartsWith("Equipment"))
                {
                    subjStr = dt.Rows[i][3].ToString().Trim() + dt.Rows[i][4].ToString().Trim();

                    if (dt.Rows[i][10].ToString().StartsWith("Maker"))
                    {
                        makerStr = dt.Rows[i][11].ToString().Trim() + dt.Rows[i][12].ToString().Trim() + dt.Rows[i][13].ToString().Trim();
                    }
                }
                else if (firstColStr.StartsWith("Type"))
                {
                    typeStr = dt.Rows[i][3].ToString().Trim() + dt.Rows[i][4].ToString().Trim();

                    if (dt.Rows[i][10].ToString().StartsWith("Serial Num"))
                    {
                        SerialStr = dt.Rows[i][11].ToString().Trim() + dt.Rows[i][12].ToString().Trim() + dt.Rows[i][13].ToString().Trim();
                    }
                }
