...
namespace Parsing
{
    class Gestioni2
    {
        ...
        public Gestioni2(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(vessel))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("Ship"))
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("Req. No."))
                    ...
                    else if (firstColStr.StartsWith("DRAWING/"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("Q.TY")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().Equals("U.")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().Equals("Code Number") || dt.Rows[i][c].ToString().Equals("CODE NUMBER")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().Equals("DESCRIPTION")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("DRAWING/")) _itemDrw = c;
                        }
