...
namespace Parsing
{
    class GreenPearl
    {
        ...
        public GreenPearl(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("NO."))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("CODE")) _itemCdoe = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("DESCRIPTION")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Q'TY UNIT")) _itemQt = c;
                        }
                        ...
                        {
                            subjStr = subjStr + Environment.NewLine + dt.Rows[i][1].ToString().Trim();
