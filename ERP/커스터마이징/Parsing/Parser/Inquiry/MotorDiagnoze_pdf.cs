...
namespace Parsing
{
    class MotorDiagnoze_pdf
    {
        ...
        public MotorDiagnoze_pdf(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if (firstColStr.StartsWith("For spare parts supply"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                    ...
                    else if (firstColStr.Equals("Item"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Contains("Part Nr.")) _itemCode = c;
