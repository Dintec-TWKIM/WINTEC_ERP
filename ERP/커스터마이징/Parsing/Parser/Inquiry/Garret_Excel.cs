...
namespace Parsing
{
    class Garret_Excel
    {
        ...
        public Garret_Excel(string fileName)
        {
            reference = "";
            vessel = "";
        ...
        public void Parse()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (string.IsNullOrEmpty(reference))
                {
                    for (int c = 1; c < dt.Columns.Count; c++)
                    {
                        if (dt.Rows[i][c].ToString().Contains("Department") && dt.Rows[i][c].ToString().Contains("Vessel"))
                            reference = dt.Rows[i][c + 1].ToString().Trim() + dt.Rows[i][c + 2].ToString().Trim();
                    }
                }


                if (firstColStr.StartsWith("Order") && firstColStr.EndsWith("Line"))
                {
                    for (int c = 1; c < dt.Columns.Count; c++)
                    {
                        if (dt.Rows[i][c].ToString().Contains("Impa")) _itemCode = c;
                        ...
                        else if (dt.Rows[i][c].ToString().Contains("Item") && dt.Rows[i][c].ToString().Contains("No:")) _itemUniq = c;
                        else if (dt.Rows[i][c].ToString().Equals("SHIPS REMARKS")) _itemRmk = c;
                    }
                }
