...
namespace Parsing
{
    class SEADRILL_excel
    {
        ...
        public SEADRILL_excel(string fileName)
        {
            reference = "";
            vessel = "";
        ...
        public void Parse()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                string secColStr = dt.Rows[i][1].ToString().ToUpper();



                if (secColStr.Equals("REQUESTOR NAME"))
				{
                    
                    for (int c = 1; c < dt.Columns.Count; c++)
                    {
                        if (dt.Rows[i][c].ToString().Equals("MPN")) _itemCode = c;
                        ...
                        else if (dt.Rows[i][c].ToString().Equals("MFR")) _itemMaker = c;
                    }
                }
                else if (GetTo.IsInt(firstColStr.Replace(".", "")))
                {
                    ...
                    if (_itemMaker != -1)
                        subjStr = "MAKER: " + dt.Rows[i][_itemMaker].ToString().Trim(); 
