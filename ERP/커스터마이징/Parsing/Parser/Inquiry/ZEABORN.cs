...
namespace Parsing
{
    class ZEABORN
    {
        ...
        public ZEABORN(string fileName)
        {
            vessel = "";                        // 선명
            reference = "";                     // 문의번호
            partner = "";                       // 매입처 담당자
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.ToLower().Contains("requisition no"))
                    {
                        ...
                        for (int c = 2; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().ToLower().Contains("vessel"))
                            {
                                ...
                                vessel = vessel.Replace("E.R.","").Trim();
                            }
                        }
                    }
                    else if (firstColStr.ToLower().StartsWith("equipment"))
                    {
                        subjStr = firstColStr.Trim();
                    }
                    else if (firstColStr.ToLower().StartsWith("particulars"))
                    {
                        partStr = firstColStr.Trim();
                    }
                    else if (firstColStr.ToLower().StartsWith("assembly"))
                    {
                        assemStr = firstColStr.Trim();
                    }
                    ...
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (_itemDesc != -1)
                        {
                            ...
                            if (string.IsNullOrEmpty(iTemDESC))
                            {
                                ...
                                while (!GetTo.IsInt(dt.Rows[_i + 1][0].ToString()) && string.IsNullOrEmpty(dt.Rows[_i][0].ToString()))
                                {
                                    iTemDESC = iTemDESC.Trim() + " " + dt.Rows[_i][_itemDesc].ToString().Trim();

                                    _i += 1;

                                    if (_i + 1 >= dt.Rows.Count)
                                        break;
                                }
                            }
                            ...
                            if (_itemDwg != -1)
                            {
                                ...
                                if(!string.IsNullOrEmpty(iTemDwg))
                                    iTemDESC = iTemDESC.Trim() + Environment.NewLine + "DWG: " + iTemDwg; 
                            }
                            ...
                            if (!string.IsNullOrEmpty(partStr))
                                iTemSUBJ = iTemSUBJ + Environment.NewLine + partStr.Trim();

                            if (!string.IsNullOrEmpty(assemStr))
                                iTemSUBJ = iTemSUBJ + Environment.NewLine + assemStr.Trim();
