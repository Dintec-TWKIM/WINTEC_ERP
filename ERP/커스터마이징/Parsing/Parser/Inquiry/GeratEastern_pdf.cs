...
namespace Parsing
{
	class GeratEastern_pdf
	{
		...
		public GeratEastern_pdf(string fileName)
		{
			vessel = "";                        // 선명
			reference = "";                     // 문의번호
			...
			foreach (DataTable dt in ds.Tables)
			{
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					if (string.IsNullOrEmpty(vessel))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
							if (dt.Rows[i][c].ToString().StartsWith("Vessel"))
								vessel = dt.Rows[i][c+1].ToString().Replace("Vessel", "").Replace("Name","").Replace(":", "").Trim() + dt.Rows[i][c + 2].ToString().Replace("Vessel", "").Replace("Name", "").Replace(":", "").Trim();
						}
					}
					...
					if (string.IsNullOrEmpty(reference))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
							if (dt.Rows[i][c].ToString().StartsWith("Inquiry"))
								reference = dt.Rows[i][c + 1].ToString().Replace("Inquiry", "").Replace("No.", "").Replace(":", "").Trim() + dt.Rows[i][c + 2].ToString().Replace("Inquiry", "").Replace("No.", "").Replace(":", "").Trim();
						}
					}



					if (firstStr.Equals("Item"))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
					...
					{
						if(dt.Rows[i-1][0].ToString().StartsWith("****") || dt.Rows[i-1][1].ToString().StartsWith("****"))
						{
							iTemSUBJ = dt.Rows[i - 1][0].ToString().Trim() + dt.Rows[i - 1][1].ToString().Trim();
						}
						...
						iTemSUBJ = iTemSUBJ.Replace("****", "").Trim();
