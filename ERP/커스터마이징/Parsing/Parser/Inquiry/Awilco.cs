            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(vessel))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("Reqisition No:"))
                                reference = dt.Rows[i + 1][c].ToString().Trim();
                            else if (dt.Rows[i][c].ToString().Equals("Vessel name:"))
                                vessel = dt.Rows[i + 1][c].ToString().Trim();
                        }
                    }

                    if (firstColStr.Equals("L.no"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("Qty")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().Equals("Desctiption")) _itemDesc = c;
                        }
                    }
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        {
                            if (dt.Rows[_i][0].ToString().StartsWith("Description"))
                                break;
                        ...
                        iTemSUBJ = subjStr.Replace("Maker", "\r\nMaker").Replace("Part No", "\r\nPart No").Replace("Specification", "\r\nSpecification").Trim();
