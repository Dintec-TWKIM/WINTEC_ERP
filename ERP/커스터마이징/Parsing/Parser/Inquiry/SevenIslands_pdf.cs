...
namespace Parsing
{
	class SevenIslands_pdf
	{
		...
		public SevenIslands_pdf(string fileName)
		{
			vessel = "";                        // 선명
			reference = "";                     // 문의번호
			partner = "";                       // 매입처 담당자
				...
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					if (firstColStr.Equals("no."))
					{
						for (int c = 0; c < dt.Columns.Count; c++)
						{
							...
							else if (dt.Rows[i][c].ToString().Equals("Part Code")) _itemCode = c;
					...
					else if (firstColStr.Contains("maker"))
					{
						...
						{
							if (xmlSpl[r].ToString().ToLower().Contains(firstColStr))
							{
								string xmlRow_1 = xmlSpl[r].ToString();

								string[] xmlSplSUBJ = xmlRow_1.Split(new string[] { "<cell>" }, StringSplitOptions.None);

								iTemSUBJ = xmlRow_1.Replace("</cell>", "").Replace("\r\n", "").Replace("<cell />", "").Replace("</row>", "").Replace("PLATE", "").Replace("<cell colspan=\"6\">", "").Trim();
								xmlSpl[r] = "";
								break;
							}
						}
