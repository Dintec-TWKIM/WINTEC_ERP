...
namespace Parsing
{
    class Goodwood_excel
    {
        ...
        public Goodwood_excel(string fileName)
        {
            reference = "";
            vessel = "";
        ...
        public void Parse()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (firstColStr.ToUpper().StartsWith("NO"))
                {
                    for (int c = 1; c < dt.Columns.Count; c++)
                    {
                        if (dt.Rows[i][c].ToString().Contains("PARTICULARS")) _itemDesc = c;
                        else if (dt.Rows[i][c].ToString().Equals("Unit")) _itemUnit = c;
                        else if (dt.Rows[i][c].ToString().Contains("Quantity ordered")) _itemQt = c;
                    }
                }
