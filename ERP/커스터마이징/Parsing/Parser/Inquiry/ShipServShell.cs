...
namespace Parsing
{
    class ShipServShell
    {
        ...
        public ShipServShell(string fileName)
        {
            contact = "";
            reference = "";
            vessel = "";
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (!addible)
                    {
                        ...
                        else if (firstColString.IndexOf("RFQ Ref:") == 0 || firstColString.StartsWith("PO Ref"))
                        {
                            ...
                            vessel = vessel.Replace("MV", "").Replace("M/T", "").Replace("MT","").Trim();
                    ...
                    else
                    {
                        ...
                        else if ((!GetTo.IsInt(firstColString) && firstColString.Length > 10) )
                        {
                            ...
                            if(subject.Contains("[Buyer Item"))
                            {
                                int idx_s = subject.IndexOf("[Buyer Item");

                                itemUNIQ = subject.Substring(idx_s, subject.Length - idx_s).Trim();

                                itemUNIQ = itemUNIQ.Replace(":", "").Replace("#", "").Replace("Buyer Item", "").Replace("[", "").Replace("]", "").Trim();
                        ...
                        else if (GetTo.IsInt(firstColString) || GetTo.IsInt(secondColStr))
                        {
                            if (Convert.ToInt16(dt.TableName.ToString().Replace("Table", "")) > 1)
                            {
                                ...
                                if (string.IsNullOrEmpty(itemDESC))
                                {
                                    ...
                                    int _i = i + 1;
                                    while (string.IsNullOrEmpty(dt.Rows[_i][firstColumNum].ToString()) && _i < dt.Rows.Count - 1)
                                    {
                                        if (_i == i)
                                        {
                                            for (int c = _itemUnitInt + 1; c <= _itemDescInt; c++)
                                ...
                                if (!itemDESC.Contains("Freight and Packing & Handling"))
                                {
                                    ...
                                    string allStr = itemDESC;


                                    int idx_s = -1;
                                    int idx_e = -1;

                                    idx_s = allStr.IndexOf("Item text");
                                    idx_e = allStr.IndexOf("Material PO text");


                                    // ITEM SUBJECT
                                    if (idx_e != -1 || idx_e != 0)
                                    {
                                        itemSUBJ = allStr.Substring(idx_s, idx_e - idx_s);

                                        itemSUBJ = itemSUBJ.Replace("Item text", "").Trim();
                                    }


                                    idx_s = -1; idx_e = -1;

                                    idx_s = allStr.IndexOf("Material PO text");
                                    idx_e = allStr.IndexOf("Manu Ref");

                                    if (idx_e != -1 || idx_e != 0)
                                    {
                                        itemDESC = allStr.Substring(idx_s, idx_e - idx_s);

                                        itemDESC = itemDESC.Replace("Material PO text", "").Trim();
                                    }


                                    idx_s = -1; idx_e = -1;

                                    idx_s = allStr.IndexOf("Manu Ref");

                                    if (idx_s != -1 || idx_e != 0)
                                    {
                                        itemCODE = allStr.Substring(idx_s, allStr.Length - idx_s).Trim();

                                        itemCODE = itemCODE.Replace("Manu Ref", "").Replace(":", "").Replace("N/A", "").Trim();
                                    }
