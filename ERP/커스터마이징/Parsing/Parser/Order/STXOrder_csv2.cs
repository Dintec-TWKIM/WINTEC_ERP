...
namespace Parsing.Parser.Order
{
	class STXOrder_csv2
	{
		...
		public STXOrder_csv2(string fileName)
		{
		...
		public void Parse()
		{
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				if (firstColStr.StartsWith("변경구분"))
				{
					for (int c = 0; c < dt.Columns.Count; c++)
					{
						if (dt.Rows[i][c].ToString().Contains("발주번호")) _계약번호 = c;
						else if (dt.Rows[i][c].ToString().Equals("PJT_NO")) _NO_SO = c;
						else if (dt.Rows[i][c].ToString().Equals("품목코드")) _도면번호 = c;
						else if (dt.Rows[i][c].ToString().Contains("PJT_NM")) _TXT_USERDEF6 = c;
						else if (dt.Rows[i][c].ToString().Equals("변경수량")) _QT_SO = c;
						else if (dt.Rows[i][c].ToString().Contains("통화")) _화폐 = c;
						else if (dt.Rows[i][c].ToString().Contains("변경납기")) _수정계약납기 = c;
						else if (dt.Rows[i][c].ToString().Contains("발주일")) _계약일 = c;
						else if (dt.Rows[i][c].ToString().Contains("변경단가")) _UM_SO = c;
						else if (dt.Rows[i][c].ToString().Contains("변경선급")) _CD_USERDEF1 = c;
						else if (dt.Rows[i][c].ToString().Contains("LINENO")) _LINE_NO = c;
						else if (dt.Rows[i][c].ToString().Contains("차수")) _차수 = c;
					}
				}
				...
				if (i != 0)
				{
					...
					if(_차수 != -1)
						차수 = dt.Rows[i][_차수].ToString().Trim();
					...
					dtItem.Rows[dtItem.Rows.Count - 1]["NO_HST"] = 차수;
					dtItem.Rows[dtItem.Rows.Count - 1]["YN_FLAG"] = "Y";
