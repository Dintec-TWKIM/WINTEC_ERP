        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("1. Delivery"))
                    {
                        lt = dt.Rows[i][1].ToString().Replace("발주일로부터", "").Replace("납", "").Replace(":", "").Replace("기", "").Replace("일", "").Replace("발주 후 ", "").Trim() + dt.Rows[i][2].ToString().Replace("발주일로부터", "").Replace("납", "").Replace(":", "").Replace("기", "").Replace("일", "").Replace("발주 후 ", "").Trim();

                        if (string.IsNullOrEmpty(lt))
                        {
                            lt = dt.Rows[i][3].ToString().Replace("발주일로부터", "").Replace("납", "").Replace(":", "").Replace("기", "").Replace("일", "").Replace("발주 후 ", "").Trim();
                        }
