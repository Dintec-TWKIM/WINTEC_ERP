...
namespace Parsing
{
    class GEAKOREA
    {
        ...
        public GEAKOREA(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("Quote No."))
                    ...
                    if (firstColStr.Equals("Item"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Part Number")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Material Designation")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Qty")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Unit")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Net")) _itemPrice = c;
