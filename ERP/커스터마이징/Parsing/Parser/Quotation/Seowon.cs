                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.Equals("NO."))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i-1][c].ToString().StartsWith("Q'TY")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Q'TY")) _itemQt = c;
                            //else if (dt.Rows[i][c].ToString().Equals("UNIT")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().ToUpper().Equals("U/PRICE")) _itemPrice = c;
                    ...
                    else if (GetTo.IsInt(firstColStr.Replace(".","")))
                    {
                        ...
                        if (string.IsNullOrEmpty(lt))
                        {
                            ...
                            {
                                if (dt.Rows[r][0].ToString().Contains("DELIVERY") && dt.Rows[r][0].ToString().Contains("*") && !dt.Rows[r][0].ToString().Contains("SCHEDULE"))
                                {
                                    for (int c = 0; c < dt.Columns.Count; c++)
                                    {
                                        lt = lt.Trim() + dt.Rows[r][c].ToString().Trim();
                                    }

                                    lt = lt.Replace(":", "").Replace("발주 후", "").Replace("일", "").Replace("이내", "").Replace("*", "").Replace("DELIVERY", "").Replace("발주후", "").Replace("(주)서원텍", "").Trim();

                                    if (lt.Contains("주"))
                                    {
                                        int _lt = Convert.ToInt16(lt.Replace("주", "")) * 7;

                                        lt = Convert.ToString(_lt);
                                    }
                                    else if (lt.Contains("WEEK"))
									{
                                        int _lt = Convert.ToInt16(lt.Replace("WEEK", "")) * 7;
                        ...
                        if (!_itemValue.Equals(-1))
                        {
                            ...
                            if (string.IsNullOrEmpty(iTemAm))
                            {
                                ...
                                if (string.IsNullOrEmpty(iTemAm))
                                    iTemAm = dt.Rows[i][_itemValue + 2].ToString().Trim();
                            }

                        }

                        if (!_itemDesc.Equals(-1))
                            iTemDesc = dt.Rows[i][_itemDesc].ToString().Replace("`","").Trim();
                        ...
                        dtIteml.Rows[dtIteml.Rows.Count - 1]["UM"] = iTemUm.Replace(",","").Trim();
                        dtIteml.Rows[dtIteml.Rows.Count - 1]["AM"] = iTemAm.Replace(",", "").Trim();
