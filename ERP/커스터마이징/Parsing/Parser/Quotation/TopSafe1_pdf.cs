...
namespace Parsing
{
	class TopSafe1_pdf
	{
        ...
        public TopSafe1_pdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().ToUpper().Contains("REQNO"))
                            {
                    ...
                    if(string.IsNullOrEmpty(lt))
					{
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().ToUpper().Contains("납기조건"))
                            {
                                lt = dt.Rows[i][c].ToString() + dt.Rows[i][c+1].ToString().Trim();
                                lt = lt.Replace("납기조건", "").Replace(":", "").Replace("발주후", "").Replace("(", "").Replace(")", "").Replace("일", "").Replace("주", "").Trim();
                            }
                        }
                    }

                    
                    if (firstColStr.StartsWith("項目"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("品")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Equals("PART NO")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("數")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("單")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("金")) _itemValue = c;
                        }
                    }
