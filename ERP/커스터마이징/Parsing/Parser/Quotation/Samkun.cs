		...
		public void Parse()
		{
			...
			foreach (DataTable dt in ds.Tables)
			{
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					if (secondColStr.StartsWith("Quote No"))
					{
						reference = dt.Rows[i][2].ToString().Trim();
					}
					else if (secondColStr.StartsWith("Terms of"))
					{
						lt = dt.Rows[i][2].ToString().Replace("EXW:", "").Replace("Ex Works", "").Replace("/", "").Replace("days", "").Replace("DAYS", "").Replace("EXW:", "").Trim();
					}
					else if (firstColStr.Equals("No."))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
							...
							else if (dt.Rows[i][c].ToString().StartsWith("Unit Price") || dt.Rows[i - 1][c].ToString().StartsWith("Unit Price")) _itemPrice = c;
							else if (dt.Rows[i][c].ToString().StartsWith("Total Amount") || dt.Rows[i - 1][c].ToString().StartsWith("Total Amount")) _itemValue = c;
						}
					}
					else if (GetTo.IsInt(firstColStr) && !dt.Rows[i][1].ToString().StartsWith("OF"))
					{
						if (!_itemQt.Equals(-1))
						{
					...
					else if (secondColStr.StartsWith("Remark"))
					{
