        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if (firstColStr.StartsWith("1. Delivery"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                                lt = lt.Trim() + dt.Rows[i][c].ToString().Trim();
                        }

                        lt = lt.Replace(":", "").Replace("발주 후", "").Replace("일", "").Replace("이내","").Trim();
                    }
                    else if (firstColStr.Equals("NO."))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().ToUpper().Equals("UNIT PRICE")) _itemPrice = c;
                    ...
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!_itemPrice.Equals(-1))
                        {
                            ...
                            if (string.IsNullOrEmpty(iTemUm))
                            {
                                ...
                                if (string.IsNullOrEmpty(iTemUm))
                                    iTemUm = dt.Rows[i][_itemPrice + 2].ToString().Trim();
                            }
                        }
