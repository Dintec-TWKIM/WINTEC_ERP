...
namespace Parsing
{
	class 서주해양_pdf
	{
        ...
        public 서주해양_pdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if(string.IsNullOrEmpty(reference))
					{
						...
						{
                            if(dt.Rows[i][c].ToString().Contains("DATE CODE"))
							{
                                reference = dt.Rows[i][c].ToString().Replace("DATE", "").Replace("CODE", "").Replace(":", "").Trim();

                                if(string.IsNullOrEmpty(reference))
                                    reference = dt.Rows[i][c+1].ToString().Replace("DATE", "").Replace("CODE", "").Replace(":", "").Trim();
                            }
						}
					}


                   if (firstColStr.Contains("납기"))
                    {
                        lt = dt.Rows[i][0].ToString().Trim() + dt.Rows[i][1].ToString().Trim();

                        lt = lt.Replace("납기", "").Replace("after", "").Replace("ordering", "").Replace(":", "").Replace(".","").Trim();
