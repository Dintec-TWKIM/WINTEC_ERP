...
namespace Parsing
{
	class SMSSME_pdf
	{
		...
		public SMSSME_pdf(string fileName)
		{
			lt = string.Empty;
			rmk = string.Empty;
			...
			foreach (DataTable dt in ds.Tables)
			{
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					if (string.IsNullOrEmpty(lt))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
							if (dt.Rows[i][c].ToString().ToLower().StartsWith("quotation validity"))
					...
					if (firstColStr.Contains("NO."))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
							if (dt.Rows[i][c].ToString().Contains("SPEC")) _itemCode = c;
							...
							else if (dt.Rows[i][c].ToString().StartsWith("FINAL PRICE")) _itemValue = c;
						}
					}
