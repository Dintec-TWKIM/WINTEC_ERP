            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("Quote"))
                                reference = dt.Rows[i + 1][c].ToString().Trim();
                        }
                    }


                    if (firstColStr.Equals("ID"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("Ext. Price")) _itemValue = c;
                        }
                    }
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!_itemLt.Equals(-1))
                        {
                            iTemLt = dt.Rows[i][_itemLt].ToString().Replace("working days", "").Trim();

                            if (iTemLt.Contains("-"))
                            {
                                string[] ltSpl = iTemLt.Split('-');

                                if (ltSpl.Length >= 2)
                                {
                                    iTemLt = ltSpl[1].ToString().Trim();
                                }
                            }
                        }
