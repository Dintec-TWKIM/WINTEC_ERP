...
namespace Parsing
{
    class Wooyang
    {
        ...
        public Wooyang(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("OUR REF NO"))
                                reference = dt.Rows[i][c].ToString().Replace("OUR REF NO", "").Replace(":", "").Trim() + dt.Rows[i][c + 1].ToString().Replace("OUR REF NO", "").Replace(":", "").Trim() + dt.Rows[i][c+2].ToString().Replace("OUR REF NO", "").Replace(":", "").Trim();
                        }
                    }

                    
                    if (firstColStr.Equals("REMARK") || secondColStr.Equals("REMARK"))
                    {
                    ...
                    else if (firstColStr.Equals("No") || secondColStr.Equals("No"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i+1][c].ToString().StartsWith("PRICE")) _itemPrice = c;
                    ...
                    else if (GetTo.IsInt(firstColStr) || GetTo.IsInt(secondColStr))
                    {
                        ...
                        {
                            for (int _i = i; _i < dt.Rows.Count; _i++)
                            {
                                if (dt.Rows[_i][0].ToString().Contains("유 효 기 간") || dt.Rows[_i][0].ToString().Contains("유 효 기 간"))
                                {
                                    for (int c = 1; c < dt.Columns.Count; c++)
                                    {
                                        if (dt.Rows[_i][c].ToString().StartsWith("납"))
                                            lt = dt.Rows[_i][c + 1].ToString().Replace(":", "").Replace("일", "").Trim() + dt.Rows[_i][c + 2].ToString().Replace(":", "").Replace("일", "").Trim();
                                    }
                                }
                            }
                        }
                        ...
                        if (!_itemQt.Equals(-1))
                        {
                            ...
                            else if (qtunitSpl.Length == 1)
                            {
                                iTemQt = qtunitSpl[0].ToString().ToUpper().Replace("EA", "").Replace("PCS", "").Replace("PC", "").Replace("SET","").Trim();
                                iTemUnit = "EA";
                            }
                            else
                            {
