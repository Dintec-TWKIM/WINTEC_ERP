		...
		public void Parse()
		{
			...
			foreach (DataTable dt in ds.Tables)
			{
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					if (firstColStr.Contains("납품") && firstColStr.Contains("기한"))
					{
						for (int c = 1; c < dt.Columns.Count; c++)
						{
							if (string.IsNullOrEmpty(lt))
								lt = dt.Rows[i][c].ToString().Replace("발주후", "").Replace("일", "").Replace("주", "").Trim();
							else
								break;
						}
						...
						//	int idx_e = -1;
						//	idx_e = lt.IndexOf("Days");

						//	if (idx_e != -1)
						//	{
						//		lt = lt.Substring(0, lt.Length - idx_e).Trim();
						//	}
						//}
					}

					if (firstColStr.Contains("名"))
					{
						for (int c = 0; c < dt.Columns.Count; c++)
						{
							if (dt.Rows[i][c].ToString().Contains("格")) _itemCode = c;
							else if (dt.Rows[i][c].ToString().Contains("名")) _itemDesc = c;
							else if (dt.Rows[i][c].ToString().Contains("量")) _itemQt = c;
							else if (dt.Rows[i][c].ToString().Contains("位")) _itemUnit = c;
							else if (dt.Rows[i][c].ToString().Contains("價")) _itemPrice = c;
							else if (dt.Rows[i][c].ToString().Contains("額")) _itemValue = c;
						}
					}

					if (_itemQt != -1)
					{
						if (GetTo.IsInt(dt.Rows[i][_itemQt].ToString().Trim()))
						{
							count += 1;
							...
							if (_itemQt != -1)
								iTemQt = dt.Rows[i][_itemQt].ToString().ToUpper().Replace("PCS", "").Replace("EA", "").Replace("PCE", "").Trim();
							...
							//ITEM ADD START
							dtIteml.Rows.Add();
							dtIteml.Rows[dtIteml.Rows.Count - 1]["NO"] = Convert.ToString(count);
