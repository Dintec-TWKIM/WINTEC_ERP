            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Our ref. No."))
                                reference = dt.Rows[i][c + 1].ToString().Replace("Our ref. No.", "").Replace(":", "").Trim() + dt.Rows[i][c + 2].ToString().Replace("Our ref. No.", "").Replace(":", "").Trim();
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(lt))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Delivery Date") || dt.Rows[i][c].ToString().StartsWith("Delivery Date"))
                                leadTimeStr = dt.Rows[i][c+1].ToString().Trim() +dt.Rows[i][c+2].ToString().Trim();
                        }
                        lt = leadTimeStr.Replace("발주후", "").Replace("일","").Trim();
                    }
                    ...
                    if (firstColStr.Equals("No"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i + 1][c].ToString().StartsWith("UNIT")) _itemUnit = c;
                            else if (dt.Rows[i+1][c].ToString().StartsWith("U/Price")) _itemPrice = c;
                            else if (dt.Rows[i+1][c].ToString().StartsWith("Amount")) _itemValue = c;
                        }
                    }
