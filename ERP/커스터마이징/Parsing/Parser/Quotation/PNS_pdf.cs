...
namespace Parsing
{
    class PNS_pdf
	{
        ...
        public PNS_pdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().ToLower().StartsWith("ref no."))
                            {
                                for (int cc = c; cc < dt.Columns.Count; cc++)
                                {
                                    if(string.IsNullOrEmpty(reference))
                                        reference = dt.Rows[i][cc].ToString().Trim();
                                }
                            }
                        }

                        reference = reference.ToLower().Replace("Ref No", "").Replace(".", "").Trim();
                    }
                    ...
                    if (firstColStr.Equals("No."))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("DESC")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Q'TY")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().Equals("UNIT")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().ToUpper().Contains("PR") && dt.Rows[i][c].ToString().ToUpper().Contains("ICE")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().ToUpper().StartsWith("AMOUNT")) _itemValue = c;
                        }
                    }
                    else if (firstColStr.StartsWith("1)"))
                    {
                        lt = dt.Rows[i][1].ToString().Replace("기","").Replace("발주", "").Replace("후", "").Replace("일", "").Replace(":", "").Trim();
                    }
