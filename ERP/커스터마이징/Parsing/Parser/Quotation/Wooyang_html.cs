...
namespace Parsing
{
    class Wooyang_html
    {
        ...
        public Wooyang_html(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
        ...
        public void Parse()
        {
            ...
            int idx_lte = mailBodyStr.IndexOf(">");

            string url = @"" + mailBodyStr.Substring(idx_lts, idx_lte - idx_lts).Replace("<","").Replace("\r\n","").Trim();
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if (firstColStr.Contains("유 효 기 간") || secondColStr.Contains("유 효 기 간"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("납 기"))
                                lt = dt.Rows[i][c + 1].ToString().Replace(":","").Replace("일","").Trim();
                        }
                    }
