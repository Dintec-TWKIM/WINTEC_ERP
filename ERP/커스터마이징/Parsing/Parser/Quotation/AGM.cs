            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("No."))
                                reference = dt.Rows[i][c].ToString().Replace("No.", "").Trim();
                        }
                    }

                    if (secondColStr.StartsWith("· 납"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("기"))
                               lt  = dt.Rows[i][c].ToString().Replace("기 : 발주 후", "").Replace("일", "").Trim();
                        }
                    }
                    ...
                    if (firstColStr.Equals("No."))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("품 번")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("품 명")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("수량")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().ToUpper().StartsWith("단가(원)")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().ToUpper().StartsWith("금액(원)")) _itemValue = c;
                        }
                    }
