...
namespace Parsing
{
    class donghwa
    {
        ...
        public donghwa(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("견적번호"))
                                reference = dt.Rows[i][c + 1].ToString().Replace(":", "").Trim() + dt.Rows[i][c + 2].ToString().Replace(":", "").Trim();
                        }
                    }
                    if (firstColStr.StartsWith("납품기일"))
                    {
                        lt = dt.Rows[i][3].ToString().Replace(":","").Replace("발주후","").Replace("일","").Trim();
                    }
                    else if (firstColStr.Equals("No"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("번")) _itemCode = c - 1;
                            else if (dt.Rows[i][c].ToString().StartsWith("명")) _itemDesc = c - 1;
                            else if (dt.Rows[i][c].ToString().StartsWith("수량")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("단위")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("판매단가")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("합계")) _itemValue = c;
                        }
                    }
