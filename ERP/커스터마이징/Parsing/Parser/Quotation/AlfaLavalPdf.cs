...
namespace Parsing
{
    class AlfaLavalPdf
    {
        ...
        public AlfaLavalPdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Quotation No."))
                                reference = dt.Rows[i][c].ToString().Replace("Quotation No.", "").Replace(":", "").Replace(".", "").Trim();
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(lt))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("Delivery Date"))
                                lt = dt.Rows[i][c].ToString().Replace("Delivery Date", "").Replace(":", "").Trim();


                            if (lt.ToUpper().Contains("WEEK"))
                            {
                                lt = lt.Replace("WEEK", "").Trim();
                    ...
                    if (firstColStr.Equals("NO"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i-1][c].ToString().ToUpper().StartsWith("UNIT PRICE")) _itemPrice = c;
                            else if (dt.Rows[i-1][c].ToString().ToUpper().StartsWith("TOTAL PRICE")) _itemValue = c;
                        }
                    }
