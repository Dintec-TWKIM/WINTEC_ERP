...
namespace Parsing
{
    class PANASIA_pdf
    {
        ...
        public PANASIA_pdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (dt.Rows[i][0].ToString().Contains("견적번호"))
                    {
                        reference = firstColStr.Replace("견적번호", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.Contains("납") && firstColStr.Contains("품") && firstColStr.Contains("기"))
					{
                        lt = firstColStr.Replace("납", "").Replace("품", "").Replace("기", "").Replace("일", "").Replace(":", "").Replace("발주", "").Replace("후", "").Replace("3.","").Trim();
                    }
                    ...
                    if (firstColStr.Equals("NO"))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Equals("Part Number")) _itemCode = c;
