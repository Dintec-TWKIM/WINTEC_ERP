...
namespace Parsing
{
	class DESCO_pdf
	{
        ...
        public DESCO_pdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("remark")) itemEnd = true;
                    ...
                    if (!itemEnd)
                    {
                        if (string.IsNullOrEmpty(reference))
                        {
                            for (int c = 0; c < dt.Columns.Count; c++)
                            {
                                ...
                                {
                                    reference = dt.Rows[i][c].ToString().Replace("Quotation", "").Replace("No.", "").Replace(":", "").Trim();

                                    if (string.IsNullOrEmpty(reference))
                                        reference = dt.Rows[i][c + 1].ToString().Replace("Quotation", "").Replace("No.", "").Replace(":", "").Trim();
                                }
                            }
                        }
                        ...
                        if (firstColStr.Equals("description"))
                        {
                            for (int c = 0; c < dt.Columns.Count; c++)
                            {
                                ...
                                else if (dt.Rows[i][c].ToString().Equals("TOTAL PRICE")) _itemValue = c;
                            }
                        }
                        else if (!string.IsNullOrEmpty(firstColStr.Trim()) && firstColStr.Length > 3 && GetTo.IsInt(firstColStr.Substring(0,1).Replace(".", "").Trim()) && !firstColStr.StartsWith("964"))
                        {
                            ...
                            else if (rowValueSpl[3] != null && rowValueSpl[4] == null)
                            {
                                ...
                                _itemPrice = Convert.ToInt16(rowValueSpl[2].ToString());
                                _itemValue = Convert.ToInt16(rowValueSpl[3].ToString());
                            }
                            ...
                            else
                            {
                                ...
                                if (!_itemPrice.Equals(-1))
                                    iTemUm = dt.Rows[i][_itemPrice].ToString().Replace(",", "").Replace("-","").Trim();
					...
					else
					{
                        ...
                        if (firstColStr.Contains("2. delivery"))
                        {
                            lt = dt.Rows[i][0].ToString().Trim() + dt.Rows[i][1].ToString().Trim();

                            lt = lt.Replace("2", "").Replace("AFTER", "").Replace("YOUR", "").Replace("ORDER","").Replace("DELIVERY","").Replace(":", "").Replace(".", "").Trim();

                            if (lt.Contains("WEEKS"))
                            {
                                lt = lt.Replace("WEEKS", "").Trim();
                            ...
                            else if (lt.Contains("DAYS"))
                            {
                                lt = lt.Replace("DAYS", "").Trim();
