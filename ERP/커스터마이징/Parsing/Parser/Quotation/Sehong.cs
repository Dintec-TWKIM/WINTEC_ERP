            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("SH Ref"))
                                reference = dt.Rows[i][c].ToString().Replace("SH Ref","").Replace("No","").Replace(".","").Trim();
                        }
                    }

                    if (firstColStr.Equals("No.") || secondColStr.Equals("No."))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("중분류")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Equals("자재번호")) _itemCode = c;
                    ...
                    else if (firstColStr.StartsWith("Time of Delivery"))
                    {
                        if (firstColStr.ToLower().Contains("week"))
                        {
                            lt = firstColStr.ToLower().Replace("time of delivery", "").Replace(":", "").Replace("within", "").Replace("weeks after receipt of order.", "").Trim();

                            int _lt = Convert.ToInt32(lt.Trim()) * 7;
                    ...
                    else if (GetTo.IsInt(firstColStr) || GetTo.IsInt(secondColStr))
                    {
                        ...
                        if (rowValueSpl[8] != null && rowValueSpl[9] == null)
                        {
                            ...
                            _itemPrice = Convert.ToInt16(rowValueSpl[6].ToString());
                            _itemValue = Convert.ToInt16(rowValueSpl[7].ToString());
                        }
