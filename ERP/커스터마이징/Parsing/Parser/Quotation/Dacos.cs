        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("DATE"))
                        reference = dt.Rows[i][1].ToString().Trim() + "(E)";

                    else if (firstColStr.StartsWith("2. 납"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("2."))
                                lt = dt.Rows[i][c].ToString().Replace("2. 납 기 :", "").Replace("일", "").Trim();
                        }

                        if (lt.Contains("재고 보유"))
                            lt = "3";
                    }
                    else if (firstColStr.Equals("No"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("Model &")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Q'ty")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().ToUpper().StartsWith("UNIT PRICE")) _itemPrice = c;
