            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().Contains("견적번호"))
                                reference = dt.Rows[i][c + 1].ToString().Replace("견적번호", "").Replace(":", "").Replace("(", "").Replace(")", "").Trim() + dt.Rows[i][c + 2].ToString().Replace("견적번호", "").Replace(":", "").Replace("(", "").Replace(")", "").Trim();
                        }
                    }

                    if (dt.Rows[i][4].ToString().StartsWith("발주"))
                    {
                        lt = dt.Rows[i][5].ToString().Trim() + dt.Rows[i][6].ToString().Trim();

                        if (lt.Contains("주"))
                        {
                            lt = lt.Replace("주", "").Trim();

                            int ltInt = Convert.ToInt16(lt) * 7;

                            lt = Convert.ToString(ltInt);
                        }
                    ...
                    else if (firstColStr.Equals("No"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().Equals("Product")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Q'ty")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Unit")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("Price")) _itemValue = c;
                        }
                    }
