...
namespace Parsing
{
    class MRC_pdf
    {
        ...
        public MRC_pdf(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(reference))
                    {
                        ...
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("견적 번호"))
                    ...
                    if (string.IsNullOrEmpty(lt))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("납 품 기 간"))
                                lt = dt.Rows[i][c + 1].ToString().Trim() + dt.Rows[i][c + 2].ToString().Trim();
                        }

                        if (!string.IsNullOrEmpty(lt) && lt.Contains("weeks"))
                        {
                            lt = lt.Replace("weeks", "").Replace("after", "").Replace("confirmation", "").Replace("order", "").Trim();
                    ...
                    if (firstColStr.Equals("NO"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Contains("품목")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().Contains("수량")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().Contains("단가")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().Contains("공급가")) _itemValue = c;
                        }
                    }
