...
namespace Parsing
{
    class Consillium
    {
        ...
        public Consillium(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    else if (firstColStr.Contains("㫴⺼G㦤ᶨ"))
                    {
                        lt = dt.Rows[i][1].ToString().Replace("Net","").Replace("days","").Trim();

                        if(string.IsNullOrEmpty(lt))
                            lt = dt.Rows[i][2].ToString().Replace("Net", "").Replace("days", "").Trim();
                    }
                    else if (firstColStr.Equals("POS ⶼ䝬"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().Equals("㤵㟈")) _itemDesc = c;
                            //else if (dt.Rows[i][c].ToString().Equals("")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().Equals("㍌⣽")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().Equals("␜㠸")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().Equals("␜ᴴ")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().Equals("Ἴ㚕")) _itemValue = c;
                        }
                    }
