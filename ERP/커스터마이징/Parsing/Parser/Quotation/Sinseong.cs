...
namespace Parsing
{
    class Sinseong
    {
        ...
        public Sinseong(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("OUR"))
                    {
                        reference = dt.Rows[i][1].ToString().Replace("REF NO", "").Replace(":", "").Trim();
                    }
                    else if (firstColStr.Equals("No."))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("재")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("수")) _itemQt = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("량")) _itemUnit = c;
                    ...
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!_itemPrice.Equals(-1))
                        {
                            ...
                            {
                                if(GetTo.IsInt(dt.Rows[i][c].ToString().Replace(",","")))
                        ...
                        {
                            for (int c = _itemValue; c < dt.Columns.Count; c++)
                        ...
                        {
                            for (int c = _itemCode; c < _itemQt; c++)
                                iTemCode = iTemCode.Trim() + dt.Rows[i][c].ToString().Trim();
                        }

                        if (iTemCode.Equals("BLANK"))
                            break;

                        if (string.IsNullOrEmpty(iTemUm) || iTemUm.Equals("-"))
                            iTemUm = "0";


                        if (string.IsNullOrEmpty(iTemAm) || iTemAm.Equals("-"))
                            iTemAm = "0";

                        if (iTemUm != "0" && iTemAm != "0" && !string.IsNullOrEmpty(iTemQt))
                        {
                            //ITEM ADD START
                            dtIteml.Rows.Add();
                        ...
                        if (string.IsNullOrEmpty(lt))
                        {
                            ...
                            {
                                if (dt.Rows[dt.Rows.Count - 2][c].ToString().Contains("발주후"))
                                {
                                    lt = dt.Rows[dt.Rows.Count - 2][c].ToString().Replace("발주후", "").Replace(":", "").Replace("일", "").Trim();
                                }
                            }
                        }
                    }
