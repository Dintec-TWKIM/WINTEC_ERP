...
namespace Parsing
{
    class Kukdong
    {
        ...
        public Kukdong(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
        ...
        public void Parse()
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.StartsWith("견적번호"))
                    {
                        reference = firstColStr.Replace("견적번호", "").Replace(":", "").Replace("(0)", "").Trim() + dt.Rows[i][1].ToString().Replace("견적번호", "").Replace(":", "").Replace("(0)", "").Trim();
                    }
                    ...
                    {
                        lt = firstColStr.Replace("발주일로부터", "").Replace("납", "").Replace(":", "").Replace("기", "").Replace("일", "").Trim();


                        if (string.IsNullOrEmpty(lt))
                        {
                            lt = dt.Rows[i-1][1].ToString().Replace("발주일로부터", "").Replace("납", "").Replace(":", "").Replace("기", "").Replace("일", "").Trim();
                        }

                        if(string.IsNullOrEmpty(lt))
						{
                            lt = dt.Rows[i][1].ToString().Replace("발주일로부터", "").Replace("납", "").Replace(":", "").Replace("기", "").Replace("일", "").Trim();
                        }
                    }
                    else if (firstColStr.Equals("순번"))
                    {
                        for (int c = 0; c < dt.Columns.Count; c++)
                        {
                            if (dt.Rows[i][c].ToString().StartsWith("품목명")) _itemDesc = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("품목코드")) _itemCode = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("수량")) _itemQt = c;
                            //else if (dt.Rows[i][c].ToString().StartsWith("")) _itemUnit = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("단가")) _itemPrice = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("공급")) _itemValue = c;
                            else if (dt.Rows[i][c].ToString().StartsWith("비고")) _itemRmk = c;
                        }
                    }
