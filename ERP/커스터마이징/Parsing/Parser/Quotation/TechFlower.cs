...
namespace Parsing
{
    class TechFlower
    {
        ...
        public TechFlower(string fileName)
        {
            lt = string.Empty;
            rmk = string.Empty;
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (firstColStr.Equals("NO"))
                    {
                        for (int c = 1; c < dt.Columns.Count; c++)
                        {
                            ...
                            else if (dt.Rows[i][c].ToString().StartsWith("DELIVERY")) _itemLt = c;
                        }
                    }
                    else if (GetTo.IsInt(firstColStr))
                    {
                        ...
                        if (!_itemQt.Equals(-1))
                        {
                            string qtTemp = dt.Rows[i][_itemQt].ToString().Trim();

                            if (qtTemp.Contains("EA"))
                            {
                                iTemQt = qtTemp.Replace("EA", "").Trim();
                                iTemUnit = qtTemp.Replace(iTemQt, "").Trim();
                            }
                            else if (qtTemp.Contains("PCS"))
                            {
                                iTemQt = qtTemp.Replace("PCS", "").Trim();
                                iTemUnit = qtTemp.Replace(iTemQt, "").Trim();
                            }
                        }
                        ...
                        if (!_itemLt.Equals(-1))
                            iTemLt = dt.Rows[i][_itemLt].ToString().Replace("발주 후","").Replace("일","").Trim();
