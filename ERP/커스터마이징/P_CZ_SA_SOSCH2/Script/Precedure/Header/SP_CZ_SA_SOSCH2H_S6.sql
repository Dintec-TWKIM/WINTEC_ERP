...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2H_S6]    Script Date: 2018-07-23 오후 1:03:46 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2H_S6] 
(
...
(
	...
	AND (@P_CD_SUPPLIER = '' OR EXISTS (SELECT 1 
										...
										AND PH.CD_PARTNER IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_SUPPLIER))) OR (SB.CD_COMPANY IS NOT NULL AND EXISTS (SELECT 1 
																																							   ...
																																							   WHERE CD_STR = 'STOCK')))
	GROUP BY SH.CD_COMPANY, SH.NO_SO, SL.CD_ITEM
)
SELECT SL.CD_ITEM, 
	   MI.NM_ITEM, 
	   MI.STND_ITEM,
...
GROUP BY SL.CD_ITEM, MI.NM_ITEM, MI.STND_ITEM
ORDER BY SL.CD_ITEM
