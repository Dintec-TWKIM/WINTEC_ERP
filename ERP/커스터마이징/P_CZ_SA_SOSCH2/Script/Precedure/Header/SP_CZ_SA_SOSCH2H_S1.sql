...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2H_S1]    Script Date: 2018-07-23 오후 1:03:13 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2H_S1] 
(
	...
	SELECT SH.CD_COMPANY,
	  ...
	  AND (@P_CD_SUPPLIER = '' OR EXISTS (SELECT 1 
										  ...
										  AND PH.CD_PARTNER IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_SUPPLIER))) OR (SB.CD_COMPANY IS NOT NULL AND EXISTS (SELECT 1 
																																								 ...
																																								 WHERE CD_STR = 'STOCK')))
	  GROUP BY SH.CD_COMPANY, SH.NO_SO, PH.NO_PO, PH.CD_PARTNER
)

SELECT SL.NO_PO,
	   SL.CD_PARTNER AS CD_SUPPLIER,
...
GROUP BY SL.NO_PO, SL.CD_PARTNER, MP.LN_PARTNER
ORDER BY SL.NO_PO DESC
