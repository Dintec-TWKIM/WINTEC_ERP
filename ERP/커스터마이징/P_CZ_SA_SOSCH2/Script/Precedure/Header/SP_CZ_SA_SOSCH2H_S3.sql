...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2H_S3]    Script Date: 2018-07-23 오후 1:03:26 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2H_S3] 
(
...
(
	...
	AND (@P_CD_SUPPLIER = '' OR EXISTS (SELECT 1 
										...
										AND PH.CD_PARTNER IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_SUPPLIER))) OR (SB.CD_COMPANY IS NOT NULL AND EXISTS (SELECT 1 
																																							   ...
																																							   WHERE CD_STR = 'STOCK')))
	GROUP BY SH.CD_COMPANY, SH.NO_SO, (CASE WHEN SL.LT IN (997, 998, 999) THEN '' ELSE SL.DT_DUEDATE END)
)
SELECT SL.DT_DUEDATE,
