...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2H_S0_1]    Script Date: 2018-08-01 오후 5:48:29 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2H_S0_1] 
(
...
CREATE TABLE #SA_RCPH
(
	CD_COMPANY		NVARCHAR(7),
	NO_SO			NVARCHAR(20),
	DT_RCP			NVARCHAR(8),
	AM_RCP_A		NUMERIC(17, 4)
)

CREATE NONCLUSTERED INDEX SA_RCPH ON #SA_RCPH (CD_COMPANY, NO_SO)
...
INSERT INTO #SA_RCPH
SELECT RH.CD_COMPANY, RH.NO_PROJECT,
	   MAX(RH.DT_RCP) AS DT_RCP,
	   SUM(RH.AM_RCP_A) AS AM_RCP_A
FROM SA_RCPH RH
GROUP BY RH.CD_COMPANY, RH.NO_PROJECT
HAVING EXISTS (SELECT 1 
			   FROM #SA_SOH
			   WHERE CD_COMPANY = RH.CD_COMPANY
			   AND NO_SO = RH.NO_PROJECT)
...
LEFT JOIN #SA_RCPH RH ON RH.CD_COMPANY = SH.CD_COMPANY AND RH.NO_SO = SH.NO_SO
