...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2H_S4]    Script Date: 2018-07-23 오후 1:03:33 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2H_S4] 
(
...
(
	SELECT SH.CD_COMPANY,
		   SH.NO_SO,
		   PH.CD_PARTNER,
		   AVG(CONVERT(NUMERIC, DATEDIFF(DAY, (CASE WHEN SL.LT IN (997, 998, 999) THEN '' ELSE SL.DT_DUEDATE END), QO.DT_IO))) AS DT_OUT_DIFF,
	...
	LEFT JOIN (SELECT PL.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE, PH.CD_PARTNER
	  		   ...
	  		   GROUP BY PL.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE, PH.CD_PARTNER
	  		   UNION ALL
	...
	AND (@P_CD_SUPPLIER = '' OR EXISTS (SELECT 1 
										...
										AND PH.CD_PARTNER IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_SUPPLIER))) OR (SB.CD_COMPANY IS NOT NULL AND EXISTS (SELECT 1 
																																							   ...
																																							   WHERE CD_STR = 'STOCK')))
	GROUP BY SH.CD_COMPANY, SH.NO_SO, PH.CD_PARTNER
)

SELECT SL.CD_PARTNER AS CD_SUPPLIER,
	   ...
	   ROUND(AVG(SL.DT_OUT_DIFF), 1) AS DT_OUT_DIFF
FROM SA_SOL1 SL
LEFT JOIN MA_PARTNER MP ON MP.CD_COMPANY = SL.CD_COMPANY AND MP.CD_PARTNER = SL.CD_PARTNER
WHERE 1=1
...
GROUP BY SL.CD_PARTNER, MP.LN_PARTNER
ORDER BY MP.LN_PARTNER DESC
