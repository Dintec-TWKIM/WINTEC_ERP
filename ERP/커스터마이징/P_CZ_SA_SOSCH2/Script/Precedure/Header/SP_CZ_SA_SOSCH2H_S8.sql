...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2H_S8]    Script Date: 2018-07-23 오후 1:03:59 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2H_S8] 
(
...
SELECT SH.CD_SALEGRP, SG.NM_SALEGRP,
	   ISNULL(COUNT(DISTINCT SH.NO_SO), 0) AS CNT_SO,
	   ISNULL(SUM(SL.AM_SO), 0) AS AM_SO,
	   ISNULL(SUM(SL.QT_SO), 0) AS QT_SO
FROM SA_SOH SH
JOIN SA_SOL1 SL ON SL.CD_COMPANY = SH.CD_COMPANY AND SL.NO_SO = SH.NO_SO
...
GROUP BY SH.CD_SALEGRP, SG.NM_SALEGRP
