...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2M_S2]    Script Date: 2018-07-23 오후 1:05:59 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2M_S2] 
(
...
(
	...
	LEFT JOIN (SELECT PL.CD_COMPANY, PL.NO_SO, IL.DT_IO 
		       FROM PU_POL PL
		       ...
		       GROUP BY PL.CD_COMPANY, PL.NO_SO, IL.DT_IO) IL
    ON IL.CD_COMPANY = SH.CD_COMPANY AND IL.NO_SO = SH.NO_SO 
	WHERE SH.CD_COMPANY = @P_CD_COMPANY
	AND (IL.DT_IO = @P_KEY OR @P_KEY = '')
...
(
	SELECT SH.CD_COMPANY,
	  ...
	  WHERE (IL.DT_IO = @P_KEY OR (@P_KEY = '' AND IL.DT_IO IS NULL)) 
