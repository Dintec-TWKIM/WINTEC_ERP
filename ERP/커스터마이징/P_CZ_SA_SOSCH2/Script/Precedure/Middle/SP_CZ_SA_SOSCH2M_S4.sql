...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2M_S4]    Script Date: 2018-07-23 오후 1:06:11 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2M_S4] 
(
...
(
	...
	LEFT JOIN (SELECT CD_COMPANY, CD_PJT, CD_PARTNER 
		       FROM PU_POH
		       GROUP BY CD_COMPANY, CD_PJT, CD_PARTNER
		       UNION ALL
		       SELECT CD_COMPANY, NO_FILE, 'STOCK' 
		       FROM CZ_SA_STOCK_BOOK
		       GROUP BY CD_COMPANY, NO_FILE) PH 
    ON PH.CD_COMPANY = SH.CD_COMPANY AND PH.CD_PJT = SH.NO_SO
	WHERE SH.CD_COMPANY = @P_CD_COMPANY
	AND (PH.CD_PARTNER = @P_KEY OR @P_KEY = '')
...
(
	SELECT SH.CD_COMPANY,
	  ...
	  WHERE (PH.CD_PARTNER = @P_KEY OR (@P_KEY = '' AND PH.CD_PARTNER IS NULL))
