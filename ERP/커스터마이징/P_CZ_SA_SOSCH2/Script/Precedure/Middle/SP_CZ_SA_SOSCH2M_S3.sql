...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2M_S3]    Script Date: 2018-07-23 오후 1:06:05 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2M_S3] 
(
...
(
	...
	LEFT JOIN (SELECT CD_COMPANY, NO_SO, 
					  (CASE WHEN LT IN (997, 998, 999) THEN '' ELSE ISNULL(DT_DUEDATE, '') END) AS DT_DUEDATE 
		       FROM SA_SOL
		       GROUP BY CD_COMPANY, NO_SO, (CASE WHEN LT IN (997, 998, 999) THEN '' ELSE ISNULL(DT_DUEDATE, '') END)) SL
	ON SL.CD_COMPANY = SH.CD_COMPANY AND SL.NO_SO = SH.NO_SO
	WHERE SH.CD_COMPANY = @P_CD_COMPANY
	AND SL.DT_DUEDATE = @P_KEY
...
(
	SELECT SH.CD_COMPANY,
	  ...
	  WHERE (CASE WHEN SL.LT IN (997, 998, 999) THEN '' ELSE ISNULL(SL.DT_DUEDATE, '') END) = @P_KEY
