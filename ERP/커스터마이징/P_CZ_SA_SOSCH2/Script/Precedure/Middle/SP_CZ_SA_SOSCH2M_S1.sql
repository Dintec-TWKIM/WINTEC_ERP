...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2M_S1]    Script Date: 2018-07-23 오후 1:05:52 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2M_S1] 
(
...
(
	...
	LEFT JOIN (SELECT CD_COMPANY, NO_PO, CD_PJT 
			   FROM PU_POH
			   UNION ALL
			   SELECT CD_COMPANY, 'STOCK', NO_FILE 
...
(
	SELECT SH.CD_COMPANY,
	  ...
	  LEFT JOIN (SELECT PL.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE, PH.NO_PO
	  		     FROM PU_POH PH
	  		     JOIN PU_POL PL ON PL.CD_COMPANY = PH.CD_COMPANY AND PL.NO_PO = PH.NO_PO
	  		     GROUP BY PL.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE, PH.NO_PO
	  		     UNION ALL
	  ...
	  WHERE (PH.NO_PO = @P_KEY OR (@P_KEY = '' AND PH.NO_PO IS NULL)) 
