...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_SOSCH2L_S0]    Script Date: 2018-07-23 오후 1:04:46 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_SOSCH2L_S0] 
(
...
LEFT JOIN (SELECT PL.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE,
		   	      ...
		   	      PL.DT_EXPECT,
		   	      PL.DT_EXDATE,
		   	      ROW_NUMBER() OVER (PARTITION BY PH.CD_COMPANY, PL.NO_SO, PL.NO_SOLINE ORDER BY PL.DT_EXDATE, PL.DT_EXPECT) AS IDX
		   FROM PU_POH PH
		   JOIN PU_POL PL ON PL.CD_COMPANY = PH.CD_COMPANY AND PL.NO_PO = PH.NO_PO
		   WHERE PH.CD_COMPANY = 'K100'
		   AND PH.CD_PARTNER IN ('11823', '00799')) PL
ON PL.CD_COMPANY = SL.CD_COMPANY AND PL.NO_SO = SL.NO_SO AND PL.NO_SOLINE = SL.SEQ_SO AND PL.IDX = 1
