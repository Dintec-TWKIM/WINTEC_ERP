...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_MA_HULL_S1]    Script Date: 2015-06-22 오후 1:29:29 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_MA_HULL_S1]
(
...
(
	...
	DTS_ETRYPT  NVARCHAR(30)
)

CREATE NONCLUSTERED INDEX 국내기항 ON #국내기항 (CALL_SIGN)
...
SELECT A.CALL_SIGN,
	   A.DTS_ETRYPT
FROM (SELECT A.CALL_SIGN,
	 	     CONVERT(CHAR(19), CONVERT(DATETIME, LEFT(A.DTS_ETRYPT, 8) + ' ' + SUBSTRING(A.DTS_ETRYPT, 9, 2) + ':' + SUBSTRING(A.DTS_ETRYPT, 11, 2) + ':' + SUBSTRING(A.DTS_ETRYPT, 13, 2)), 20) + ' (' + A.NM_PRTAG + ')' AS DTS_ETRYPT,
	 	     ROW_NUMBER() OVER(PARTITION BY A.CALL_SIGN ORDER BY A.DTS_ETRYPT DESC) AS IDX
	  FROM CZ_SA_VSSL_ETRYNDH A 
	  WHERE EXISTS (SELECT 1 
	 			    FROM CZ_MA_HULL MH
	 			    WHERE MH.CALL_SIGN = A.CALL_SIGN)) A
WHERE A.IDX = 1
...
SELECT MH.NO_IMO,
	   ...
	   (SELECT A.DTS_ETRYPT 
	    FROM #국내기항 A 
		WHERE A.CALL_SIGN = MH.CALL_SIGN) AS DTS_ETRYPT,
	   SS.NO_SISTER,
       SS.CD_SISTER,
	   MH.DC_SHIPBUILDER,
