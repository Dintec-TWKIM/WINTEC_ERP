...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_AC_RECEIVABLE_RPT_S]        
(        
...
SELECT 'N' AS S,
	   ...
	   RH.DT_RCP,
	   DATEDIFF(DAY, IH.DT_PROCESS, RH.DT_RCP) AS DT_RCP_DAY,
	   IH.DT_IV_MONTH,
	   ...
	   (CASE WHEN IH.DT_IV_MONTH > 2 AND IH.DT_IV_MONTH <= 3 THEN (ISNULL(IL.AM_EX_CLS, 0) - ISNULL(RH.AM_RCP_EX, 0)) ELSE 0 END) AS AM_EX_2MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 3 AND IH.DT_IV_MONTH <= 6 THEN (ISNULL(IL.AM_EX_CLS, 0) - ISNULL(RH.AM_RCP_EX, 0)) ELSE 0 END) AS AM_EX_3MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 6 AND IH.DT_IV_MONTH <= 12 THEN (ISNULL(IL.AM_EX_CLS, 0) - ISNULL(RH.AM_RCP_EX, 0)) ELSE 0 END) AS AM_EX_6MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 12 THEN (ISNULL(IL.AM_EX_CLS, 0) - ISNULL(RH.AM_RCP_EX, 0)) ELSE 0 END) AS AM_EX_12MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 2 AND IH.DT_IV_MONTH <= 3 THEN (ISNULL(IL.AM_CLS, 0) - ISNULL(RH.AM_RCP, 0)) ELSE 0 END) AS AM_2MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 3 AND IH.DT_IV_MONTH <= 6 THEN (ISNULL(IL.AM_CLS, 0) - ISNULL(RH.AM_RCP, 0)) ELSE 0 END) AS AM_3MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 6 AND IH.DT_IV_MONTH <= 12 THEN (ISNULL(IL.AM_CLS, 0) - ISNULL(RH.AM_RCP, 0)) ELSE 0 END) AS AM_6MONTH,
	   (CASE WHEN IH.DT_IV_MONTH > 12 THEN (ISNULL(IL.AM_CLS, 0) - ISNULL(RH.AM_RCP, 0)) ELSE 0 END) AS AM_12MONTH,
	   STUFF((SELECT DISTINCT ',' + IL.NO_SO
			  FROM SA_IVL IL
			  WHERE IL.CD_COMPANY = IH.CD_COMPANY
			  AND IL.NO_IV = IH.NO_IV
			  FOR XML PATH('')),1,1,'') AS NO_SO,
	   STUFF((SELECT DISTINCT ',' + IL.CD_PJT
			  FROM SA_IVL IL
			  WHERE IL.CD_COMPANY = IH.CD_COMPANY
			  AND IL.NO_IV = IH.NO_IV
	   ...
	   STUFF((SELECT DISTINCT ',' + SH.NO_IMO
			  FROM SA_IVL IL
			  LEFT JOIN SA_SOH SH ON SH.CD_COMPANY = IL.CD_COMPANY AND SH.NO_SO = IL.NO_SO
			  WHERE IL.CD_COMPANY = IH.CD_COMPANY
			  AND IL.NO_IV = IH.NO_IV
			  FOR XML PATH('')),1,1,'') AS NO_IMO,
	   STUFF((SELECT DISTINCT ',' + SH.DC_RMK1
			  FROM SA_IVL IL
			  LEFT JOIN SA_SOH SH ON SH.CD_COMPANY = IL.CD_COMPANY AND SH.NO_SO = IL.NO_SO
			  WHERE IL.CD_COMPANY = IH.CD_COMPANY
			  AND IL.NO_IV = IH.NO_IV
			  FOR XML PATH('')),1,1,'') AS DC_RMK1,
	   IH.DC_RMK
