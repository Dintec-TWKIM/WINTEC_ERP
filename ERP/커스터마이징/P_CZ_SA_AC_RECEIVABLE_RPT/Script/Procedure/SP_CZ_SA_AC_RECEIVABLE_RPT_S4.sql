...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_AC_RECEIVABLE_RPT_S4]        
(        
...
FROM (SELECT IH.CD_COMPANY,
	  ...
	  FROM (SELECT IH.CD_COMPANY, IH.NO_IV,
			...
			AND (ISNULL(@P_NO_IV, '') = '' OR IH.NO_IV = @P_NO_IV)) IH
	  JOIN (SELECT IL.CD_COMPANY, IL.NO_IV, IL.CD_PJT,
				   ...
				   SUM(CASE WHEN IL.CD_ITEM LIKE 'SD%' THEN (CASE WHEN IL.YN_RETURN = 'Y' THEN -(ISNULL(IL.AM_EX_CLS, 0) + ROUND(IL.VAT / IL.RT_EXCH, 2)) 
																						  ELSE (ISNULL(IL.AM_EX_CLS, 0) + ROUND(IL.VAT / IL.RT_EXCH, 2)) END) 
													   ELSE 0 END) AS AM_EX_CHARGE,
				   SUM(CASE WHEN IL.CD_ITEM LIKE 'SD%' THEN (CASE WHEN IL.YN_RETURN = 'Y' THEN -(ISNULL(IL.AM_CLS, 0) + ISNULL(IL.VAT, 0))
																						  ELSE (ISNULL(IL.AM_CLS, 0) + ISNULL(IL.VAT, 0)) END) 
...
LEFT JOIN MA_EMP ME ON ME.CD_COMPANY = SH.CD_COMPANY AND ME.NO_EMP = (CASE WHEN AR.NO_EMP IS NULL THEN SH.NO_EMP ELSE AR.NO_EMP END)
