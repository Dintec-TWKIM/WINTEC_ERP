USE [NEOE]
GO

/****** Object:  StoredProcedure [NEOE].[SP_CZ_PU_STS_BATCH_REGL_S]    Script Date: 2017-01-24 오전 9:23:56 ******/
SET ANSI_NULLS ON
GO

SET QUOTED_IDENTIFIER ON
GO



ALTER PROCEDURE [NEOE].[SP_CZ_PU_STS_BATCH_REGL1_S]
(
	@P_CD_COMPANY			NVARCHAR(MAX),
	@P_NO_GIREQ				NVARCHAR(20),
	@P_TP_DATE				NVARCHAR(3),
	@P_DT_FROM				NVARCHAR(8),
	@P_DT_TO				NVARCHAR(8),
	@P_NO_GIR				NVARCHAR(20),
	@P_NO_SO				NVARCHAR(20),
	@P_CD_ITEM				NVARCHAR(20),
    @P_TP_DONE				NVARCHAR(1),
	@P_CLS_ITEM				NVARCHAR(3),
	@P_GRP_ITEM				NVARCHAR(MAX),
	@P_YN_GIR				NVARCHAR(1),
	@P_UM_STOCK				NUMERIC(15, 4)
)
AS

SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED

SELECT (CASE WHEN QL.QT_GIREQ = QL.QT_GI THEN '완료' ELSE '미완료' END) AS TP_STATE,
	   MC.NM_COMPANY,
	   QH.NO_GIREQ,
	   QH.NO_PR_PACK AS NO_GIR,
	   GH.DT_GIR,
	   GH.DT_START,
	   QH.DT_GIREQ,
	   MS.NM_SL,
	   MS1.NM_SL AS NM_GRSL,
	   MP.LN_PARTNER AS NM_PARTNER,
	   GH.NM_DELIVERY_TO,
	   GH.NM_VESSEL,
	   ME.NM_KOR AS NM_EMP_GIREQ,
	   GH.NM_STA_GIR,
	   GH.NM_TP_GIR,
	   GH.NM_MAIN_CATEGORY,
	   GH.NM_SUB_CATEGORY,
	   ISNULL(QH.CNT_PRINT, 0) AS CNT_PRINT,
	   (SELECT TOP 1 NM_USER 
	    FROM MA_USER
		WHERE ID_USER = QH.LAST_PRINT_ID) AS NM_PIRNT,
	   GH.YN_DELETE,
	   QH.DC50_PO,

	   QL.NO_SO,
	   SL.NO_DSP,
	   QL.NO_LINE,
	   QL.CD_ITEM,
	   MI.NM_ITEM,
	   CD.NM_SYSDEF AS NM_CLS_ITEM,
	   MG.NM_ITEMGRP,
	   MI.STND_DETAIL_ITEM,
	   MI.STND_ITEM,
	   SL.NM_SUBJECT,
	   SL.CD_ITEM_PARTNER,
	   SL.NM_ITEM_PARTNER,
	   MI.CD_ZONE,
	   QL.QT_GIREQ,
	   ISNULL(QL.QT_GI, 0) AS QT_GI,
	   ISNULL(ML.QT_INV, 0) AS QT_INV,
	   (ISNULL(ML.QT_INV, 0) - ISNULL(QL.QT_GIREQ, 0)) AS QT_REMAIN,
	   OL.NO_IO,
	   OL.DT_IO,
	   (SELECT NM_KOR 
		FROM MA_EMP
		WHERE CD_COMPANY = OL.CD_COMPANY
		AND NO_EMP = OL.NO_EMP) AS NM_EMP_IO,
	   QL.DC_RMK,
	   MI.STAND_PRC
FROM MM_GIREQH QH
JOIN MM_GIREQL QL ON QL.CD_COMPANY = QH.CD_COMPANY AND QL.NO_GIREQ = QH.NO_GIREQ
LEFT JOIN CZ_SA_QTNL SL ON SL.CD_COMPANY = QL.CD_COMPANY AND SL.NO_FILE = QL.NO_SO AND SL.NO_LINE = QL.SEQ_SO
LEFT JOIN MA_PITEM MI ON MI.CD_COMPANY = QL.CD_COMPANY AND MI.CD_PLANT = QL.CD_PLANT AND MI.CD_ITEM = QL.CD_ITEM
LEFT JOIN (SELECT OH.CD_COMPANY,
		   		  OL.NO_ISURCV,
				  OL.NO_ISURCVLINE,
		   		  MAX(OH.NO_IO) AS NO_IO,
				  MAX(OH.DT_IO) AS DT_IO,
		   		  MAX(OH.NO_EMP) AS NO_EMP
		   FROM MM_QTIOH OH
		   LEFT JOIN MM_QTIO OL ON OL.CD_COMPANY = OH.CD_COMPANY AND OL.NO_IO = OH.NO_IO
		   GROUP BY OH.CD_COMPANY, OL.NO_ISURCV, OL.NO_ISURCVLINE) OL
ON OL.CD_COMPANY = QL.CD_COMPANY AND OL.NO_ISURCV = QL.NO_GIREQ AND OL.NO_ISURCVLINE = QL.NO_LINE
LEFT JOIN (SELECT MY.CD_COMPANY, MY.CD_SL, MY.CD_ITEM, 
				  (SUM(MY.QT_GOOD_OPEN + MY.QT_REJECT_OPEN + MY.QT_INSP_OPEN + MY.QT_TRANS_OPEN) 
			       + SUM(MY.QT_GOOD_GR + MY.QT_REJECT_GR + MY.QT_INSP_GR + MY.QT_TRANS_GR) 
			       - SUM(MY.QT_GOOD_GI + MY.QT_REJECT_GI + MY.QT_INSP_GI + MY.QT_TRANS_GI)) AS QT_INV
		   FROM MM_PINVN MY
		   JOIN MA_PITEM MI ON MI.CD_COMPANY = MY.CD_COMPANY AND MI.CD_PLANT = MY.CD_PLANT AND MI.CD_ITEM = MY.CD_ITEM
		   WHERE MY.P_YR = SUBSTRING(CONVERT(NVARCHAR(8), GETDATE(), 112), 1, 4)
		   GROUP BY MY.CD_COMPANY, MY.CD_SL, MY.CD_ITEM) ML
ON ML.CD_COMPANY = QL.CD_COMPANY AND ML.CD_SL = QL.CD_SL AND ML.CD_ITEM = QL.CD_ITEM
LEFT JOIN (SELECT GH.CD_COMPANY,
		   	      GH.NO_GIR,
		   	      CD1.NM_SYSDEF AS NM_TP_GIR,
		   	      'N' AS YN_DELETE,
		   	      CD.NM_SYSDEF AS NM_STA_GIR,
		   	      GH.DT_GIR,
		   	      GD.DT_START,
		   	      MH.NM_VESSEL,
		   	      CD2.NM_SYSDEF AS NM_MAIN_CATEGORY,
				  CD3.NM_SYSDEF AS NM_SUB_CATEGORY,
		   	      MP.LN_PARTNER AS NM_DELIVERY_TO
		   FROM SA_GIRH GH
		   JOIN CZ_SA_GIRH_WORK_DETAIL GD ON GD.CD_COMPANY = GH.CD_COMPANY AND GD.NO_GIR = GH.NO_GIR
		   LEFT JOIN CZ_MA_DELIVERY MP ON MP.CD_COMPANY = GH.CD_COMPANY AND MP.CD_PARTNER = GD.CD_DELIVERY_TO
		   LEFT JOIN CZ_MA_HULL MH ON MH.NO_IMO = GD.NO_IMO
		   LEFT JOIN MA_CODEDTL CD ON CD.CD_COMPANY = GH.CD_COMPANY AND CD.CD_FIELD = 'CZ_SA00030' AND CD.CD_SYSDEF = GH.STA_GIR
		   LEFT JOIN MA_CODEDTL CD1 ON CD1.CD_COMPANY = GH.CD_COMPANY AND CD1.CD_FIELD = 'CZ_SA00021' AND CD1.CD_SYSDEF = '001'
		   LEFT JOIN MA_CODEDTL CD2 ON CD2.CD_COMPANY = GD.CD_COMPANY AND CD2.CD_FIELD = 'CZ_SA00006' AND CD2.CD_SYSDEF = GD.CD_MAIN_CATEGORY
		   LEFT JOIN MA_CODEDTL CD3 ON CD3.CD_COMPANY = GD.CD_COMPANY AND CD3.CD_FIELD = CD2.CD_FLAG1 AND CD3.CD_SYSDEF = GD.CD_SUB_CATEGORY
		   UNION ALL
		   SELECT GH.CD_COMPANY,
		   	      GH.NO_GIR,
		   	      CD1.NM_SYSDEF AS NM_TP_GIR,
		   	      'N' AS YN_DELETE,
		   	      CD.NM_SYSDEF AS NM_STA_GIR,
		   	      GH.DT_GIR,
		   	      GD.DT_START,
		   	      MH.NM_VESSEL,
		   	      CD2.NM_SYSDEF AS NM_MAIN_CATEGORY,
				  CD3.NM_SYSDEF AS NM_SUB_CATEGORY,
		   	      MP.LN_PARTNER AS NM_DELIVERY_TO
		   FROM CZ_SA_GIRH_PACK GH
		   JOIN CZ_SA_GIRH_PACK_DETAIL GD ON GD.CD_COMPANY = GH.CD_COMPANY AND GD.NO_GIR = GH.NO_GIR
		   LEFT JOIN CZ_MA_DELIVERY MP ON MP.CD_COMPANY = GH.CD_COMPANY AND MP.CD_PARTNER = GD.CD_COLLECT_FROM
		   LEFT JOIN CZ_MA_HULL MH ON MH.NO_IMO = GD.NO_IMO
		   LEFT JOIN MA_CODEDTL CD ON CD.CD_COMPANY = GH.CD_COMPANY AND CD.CD_FIELD = 'CZ_SA00030' AND CD.CD_SYSDEF = GH.STA_GIR
		   LEFT JOIN MA_CODEDTL CD1 ON CD1.CD_COMPANY = GH.CD_COMPANY AND CD1.CD_FIELD = 'CZ_SA00021' AND CD1.CD_SYSDEF = '002'
		   LEFT JOIN MA_CODEDTL CD2 ON CD2.CD_COMPANY = GD.CD_COMPANY AND CD2.CD_FIELD = 'CZ_SA00020' AND CD2.CD_SYSDEF = GD.CD_PACK_CATEGORY
		   LEFT JOIN MA_CODEDTL CD3 ON CD3.CD_COMPANY = GD.CD_COMPANY AND CD3.CD_FIELD = CD2.CD_FLAG1 AND CD3.CD_SYSDEF = GD.CD_SUB_CATEGORY
		   UNION ALL
		   SELECT GH.CD_COMPANY,
		          GH.NO_GIR,
		          CD1.NM_SYSDEF AS NM_TP_GIR,
				  'Y' AS YN_DELETE,
				  CD.NM_SYSDEF AS NM_STA_GIR,
		          GH.DT_GIR,
		          GD.DT_START,
		          MH.NM_VESSEL,
		          CD2.NM_SYSDEF AS NM_MAIN_CATEGORY,
				  CD3.NM_SYSDEF AS NM_SUB_CATEGORY,
		          MP.LN_PARTNER AS NM_DELIVERY_TO
		   FROM (SELECT *,
						ROW_NUMBER() OVER (PARTITION BY CD_COMPANY, NO_GIR, TP_GIR ORDER BY NO_HST DESC) IDX 
				 FROM CZ_SA_GIRH_LOG) GH
		   JOIN CZ_SA_GIRH_DETAIL_LOG GD ON GD.CD_COMPANY = GH.CD_COMPANY AND GD.NO_GIR = GH.NO_GIR AND GD.TP_GIR = GH.TP_GIR AND GD.NO_HST = GH.NO_HST
		   LEFT JOIN CZ_MA_DELIVERY MP ON MP.CD_COMPANY = GH.CD_COMPANY AND MP.CD_PARTNER = GD.CD_DELIVERY_TO
		   LEFT JOIN CZ_MA_HULL MH ON MH.NO_IMO = GD.NO_IMO
		   LEFT JOIN MA_CODEDTL CD ON CD.CD_COMPANY = GH.CD_COMPANY AND CD.CD_FIELD = 'CZ_SA00030' AND CD.CD_SYSDEF = GH.STA_GIR
		   LEFT JOIN MA_CODEDTL CD1 ON CD1.CD_COMPANY = GH.CD_COMPANY AND CD1.CD_FIELD = 'CZ_SA00021' AND CD1.CD_SYSDEF = GH.TP_GIR
		   LEFT JOIN MA_CODEDTL CD2 ON CD2.CD_COMPANY = GH.CD_COMPANY AND CD2.CD_FIELD = (CASE WHEN GH.TP_GIR = '001' THEN 'CZ_SA00006' ELSE 'CZ_SA00020' END) AND CD2.CD_SYSDEF = GD.CD_MAIN_CATEGORY
		   LEFT JOIN MA_CODEDTL CD3 ON CD3.CD_COMPANY = GD.CD_COMPANY AND CD3.CD_FIELD = CD2.CD_FLAG1 AND CD3.CD_SYSDEF = GD.CD_SUB_CATEGORY
		   WHERE GH.IDX = 1
		   AND NOT EXISTS (SELECT 1 
						   FROM CZ_SA_GIRH_WORK_DETAIL WD
						   WHERE WD.CD_COMPANY = GH.CD_COMPANY
						   AND WD.NO_GIR = GH.NO_GIR)
		   AND NOT EXISTS (SELECT 1 
						   FROM CZ_SA_GIRH_PACK_DETAIL PD
						   WHERE PD.CD_COMPANY = GH.CD_COMPANY
						   AND PD.NO_GIR = GH.NO_GIR)) GH
ON GH.CD_COMPANY = QH.CD_COMPANY AND GH.NO_GIR = QH.NO_PR_PACK 
LEFT JOIN MA_ITEMGRP MG ON MG.CD_COMPANY = MI.CD_COMPANY AND MG.CD_ITEMGRP = MI.GRP_ITEM
LEFT JOIN MA_COMPANY MC ON MC.CD_COMPANY = QH.CD_COMPANY
LEFT JOIN MA_PARTNER MP ON MP.CD_COMPANY = QH.CD_COMPANY AND MP.CD_PARTNER = QH.CD_PARTNER
LEFT JOIN MA_SL MS ON MS.CD_COMPANY = QL.CD_COMPANY AND MS.CD_SL = QL.CD_SL
LEFT JOIN MA_SL MS1 ON MS1.CD_COMPANY = QL.CD_COMPANY AND MS1.CD_SL = QL.CD_GRSL
LEFT JOIN MA_EMP ME ON ME.CD_COMPANY = QH.CD_COMPANY AND ME.NO_EMP = QH.NO_EMP
LEFT JOIN MA_CODEDTL CD ON CD.CD_COMPANY = QL.CD_COMPANY AND CD.CD_FIELD = 'MA_B000010' AND CD.CD_SYSDEF = MI.CLS_ITEM
WHERE ((@P_TP_DATE = '001' AND QH.DT_GIREQ BETWEEN @P_DT_FROM AND @P_DT_TO) OR
       (@P_TP_DATE = '002' AND GH.DT_START BETWEEN @P_DT_FROM AND @P_DT_TO) OR
	   (@P_TP_DATE = '003' AND OL.DT_IO BETWEEN @P_DT_FROM AND @P_DT_TO))
AND (@P_YN_GIR = 'N' OR (@P_YN_GIR = 'Y' AND QL.CD_SL = 'SL02' AND QL.CD_GRSL = 'SL03'))
AND (@P_UM_STOCK = 0 OR MI.STAND_PRC >= @P_UM_STOCK)
AND (ISNULL(@P_CD_COMPANY, '') = '' OR QH.CD_COMPANY IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_COMPANY)))
AND (ISNULL(@P_NO_GIREQ, '') = '' OR QH.NO_GIREQ = @P_NO_GIREQ)
AND (ISNULL(@P_NO_GIR, '') = '' OR GH.NO_GIR = @P_NO_GIR)
AND (ISNULL(@P_CD_ITEM, '') = '' OR QL.CD_ITEM = @P_CD_ITEM)
AND (ISNULL(@P_NO_SO, '') = '' OR QL.NO_SO = @P_NO_SO)
AND (ISNULL(@P_CLS_ITEM, '') = '' OR MI.CLS_ITEM = @P_CLS_ITEM)
AND (ISNULL(@P_GRP_ITEM, '') = '' OR MI.GRP_ITEM IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_GRP_ITEM)))
AND (ISNULL(@P_TP_DONE, '') = '' OR 
	 (@P_TP_DONE = '1' AND QL.QT_GIREQ > QL.QT_GI) OR
	 (@P_TP_DONE = '2' AND QL.QT_GIREQ = QL.QT_GI))

GO