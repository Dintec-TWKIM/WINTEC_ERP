USE [NEOE]
GO
/****** Object:  StoredProcedure [NEOE].[CZ_MAILBAGLIST_MSG_SEND2]    Script Date: 2023-05-03 오후 4:45:50 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [NEOE].[CZ_MAILBAGLIST_MSG_SEND2]
		@P_NO_EMP			NVARCHAR(10)
AS

DECLARE	@V_CONTENTS		NVARCHAR(MAX)
DECLARE	@V_CONTENTS2	NVARCHAR(MAX)

 SET @V_CONTENTS2 = Convert(varchar(10),Getdate(),111)+' 양산에서 본사창고로 발송'+char(10)+char(10)+
 (SELECT DISTINCT  
	...
	AND A.SEND_COMPANY = '02'
	FOR XML PATH('')),1,1,'')  
FROM CZ_MAILBAGLIST_L AS A)


SET @V_CONTENTS = ISNULL(@V_CONTENTS2,'금일 양산에서 본사로 발송할 물건이 없습니다')
