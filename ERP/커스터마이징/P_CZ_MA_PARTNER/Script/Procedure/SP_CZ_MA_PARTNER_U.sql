USE [NEOE]
GO
/****** Object:  StoredProcedure [NEOE].[SP_CZ_MA_PARTNER_U]    Script Date: 2023-04-04 오후 5:42:07 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [NEOE].[SP_CZ_MA_PARTNER_U]
(
	@P_CD_PARTNER 				NVARCHAR(20),
	@P_CD_COMPANY 				NVARCHAR(7),
	@P_LN_PARTNER   			NVARCHAR(50),
	@P_DT_OPEN        			NVARCHAR(8),
	@P_AM_CAP         			NUMERIC(17,4),
	@P_MANU_EMP  				NUMERIC(7,0),
	@P_MANA_EMP       			NUMERIC(7,0),
	@P_SD_PARTNER  				NVARCHAR(8),
	...
	@P_OLD_NO_COMPANY			NVARCHAR(20),
	@P_ID_COMPANY				NVARCHAR(20),
	@P_PW_COMPANY				NVARCHAR(20),
	@P_CD_PARTNER_GRP			NVARCHAR(20),	--2007.11.15 주홍열
	@P_CD_EMP_SALE				NVARCHAR(14),	--2007.11.15 주홍열
	@P_CD_EMP_PARTNER			NVARCHAR(14),	--2007.11.15 주홍열
	@P_NO_HPEMP_PARTNER 		NVARCHAR(15),	--2007.11.15 주홍열
	@P_CD_AREA					NVARCHAR(3),	--2007.11.15 주홍열
	@P_DT_RCP_PREARRANGED		NUMERIC(3,0),	--2007.12.07 제갈경미
	@P_NM_DEPOSIT				NVARCHAR(100),	--2007.12.07 제갈경미
	@P_CD_PARTNER_GRP_2 		NVARCHAR(20),	--2008.03.26 허성철
	@P_FG_BILL					NVARCHAR(3),	--2009.02.17 이대성
	@P_FG_PAYBILL				NVARCHAR(3),	--2009.12.01 박태호
	@P_DT_PAY_PREARRANGED		NUMERIC(5, 0),	--2009.12.01 박태호
	@P_CD_CON          			NVARCHAR(3),	--2010.01.04 정남진	
	@P_ID_UPDATE      			NVARCHAR(15),
	@P_DT_CLOSE					NVARCHAR(8) = NULL,	--2010.03.24 이대성:폐업일자
	@P_YN_BIZTAX				NVARCHAR(1),	-- 2010.11.16 이대성 추가 : 사업자단위과세 여부
	@P_NO_BIZTAX				NVARCHAR(4),	-- 2010.11.16 이대성 추가 : 종사업장번호
	@P_YN_JEONJA				NVARCHAR(1),	-- 2010.11.16 이대성 추가 : 전자세금계산서발행여부
	@P_TP_DEFER					NVARCHAR(1),	-- 2010.11.16 이대성 추가 : 지급보류여부
	@P_TP_RCP_DD				NVARCHAR(2),	-- 2010.11.16 이대성 추가 : 자금수금예정월
	@P_DT_RCP_DD				NVARCHAR(2),	-- 2010.11.16 이대성 추가 : 자금수금예정일
	@P_TP_PAY_DD				NVARCHAR(2),	-- 2010.11.16 이대성 추가 : 자금지급에정월
	@P_DT_PAY_DD				NVARCHAR(2),	-- 2010.11.16 이대성 추가 : 자금지급예정일
	@P_FG_CORP					NVARCHAR(1)	= NULL,	-- 2011.01.20 이대성 추가 : 연결법인구분
...
-- ================================================ 
-- CHANGE HISTORY
-- v1.1 : 2010.11.16 이대성 수정 - 신규 컬럼 추가
-- v1.2 : 2011.01.20 이대성 수정 - 부가정보 탭에 연결법인 구분 추가
-- v1.3 : 2011.09.15 김병훈 수정 - 기타정보 비고 
-- ================================================
SET NOCOUNT ON


DECLARE @P_DC_ADS   NVARCHAR(100)
SET @P_DC_ADS = ISNULL(@P_DC_ADS1_H,'') + ISNULL(@P_DC_ADS1_D,'')
...
SET	LN_PARTNER = @P_LN_PARTNER,	
	...
	MANU_EMP = @P_MANU_EMP,
	MANA_EMP = @P_MANA_EMP, 		
	SD_PARTNER	= @P_SD_PARTNER,
	LT_TRANS = @P_LT_TRANS, 		
	SN_PARTNER	= @P_SN_PARTNER,
	EN_PARTNER = @P_EN_PARTNER, 	
	TP_PARTNER = @P_TP_PARTNER,
	...
	CLS_PARTNER	= @P_CLS_PARTNER,
	CD_NATION = @P_CD_NATION, 	
	STA_CREDIT	= @P_STA_CREDIT,
	E_MAIL = @P_E_MAIL, 		
	NO_TEL = @P_NO_TEL,
	NO_FAX = @P_NO_FAX, 		
	NO_POST1 = @P_NO_POST1,
	...
	NO_FAX1	= @P_NO_FAX1,
	NO_POST2 = @P_NO_POST2, 		
	DC_ADS2_H = @P_DC_ADS2_H,
	DC_ADS2_D = @P_DC_ADS2_D,		
	NO_TEL2	= @P_NO_TEL2,
	NO_FAX2	= @P_NO_FAX2, 		
	NO_POST3 = @P_NO_POST3,
	DC_ADS3_H = @P_DC_ADS3_H, 	
	DC_ADS3_D = @P_DC_ADS3_D,
	NO_TEL3	= @P_NO_TEL3, 		
	NO_FAX3	= @P_NO_FAX3,
	NM_PTR = @P_NM_PTR, 		
	USE_YN = @P_USE_YN,
	FG_CREDIT = @P_FG_CREDIT, 	
	TP_TAX = @P_TP_TAX,
	NO_MERCHANT	= @P_NO_MERCHANT, 	
	NO_DEPOSIT = @P_NO_DEPOSIT,
	CD_BANK	= @P_CD_BANK,
	ID_COMPANY = @P_ID_COMPANY,	
	PW_COMPANY = @P_PW_COMPANY,
	CD_PARTNER_GRP = @P_CD_PARTNER_GRP,	 
	CD_EMP_SALE = @P_CD_EMP_SALE ,
 	CD_EMP_PARTNER = @P_CD_EMP_PARTNER,	
 	NO_HPEMP_PARTNER = @P_NO_HPEMP_PARTNER ,
	CD_AREA = @P_CD_AREA,
	DT_RCP_PREARRANGED = @P_DT_RCP_PREARRANGED, 
	NM_DEPOSIT = @P_NM_DEPOSIT,					-- 2007.12.07 제갈경미
	CD_PARTNER_GRP_2 = @P_CD_PARTNER_GRP_2,		-- 2008.03.26 허성철
	FG_BILL	= @P_FG_BILL,						-- 2009.02.17 이대성
	FG_PAYBILL = @P_FG_PAYBILL,
	DT_PAY_PREARRANGED = @P_DT_PAY_PREARRANGED,
	CD_CON = @P_CD_CON,
	DT_CLOSE = @P_DT_CLOSE,						-- 2010.03.24 이대성:폐업일자
	ID_UPDATE = @P_ID_UPDATE,
	DTS_UPDATE = NEOE.SF_SYSDATE(GETDATE()),
	YN_BIZTAX = @P_YN_BIZTAX,					-- 2010.11.16 이대성 추가 : 사업자단위과세 여부
	NO_BIZTAX = @P_NO_BIZTAX,					-- 2010.11.16 이대성 추가 : 종사업장번호
	YN_JEONJA = @P_YN_JEONJA,					-- 2010.11.16 이대성 추가 : 전자세금계산서발행여부
	TP_DEFER = @P_TP_DEFER,						-- 2010.11.16 이대성 추가 : 지급보류여부
	TP_RCP_DD = @P_TP_RCP_DD,					-- 2010.11.16 이대성 추가 : 자금수금예정월
	DT_RCP_DD = @P_DT_RCP_DD,					-- 2010.11.16 이대성 추가 : 자금수금예정일
	TP_PAY_DD = @P_TP_PAY_DD,					-- 2010.11.16 이대성 추가 : 자금지급에정월
	DT_PAY_DD = @P_DT_PAY_DD,					-- 2010.11.16 이대성 추가 : 자금지급예정일
	FG_CORP = @P_FG_CORP,						-- 2011.01.20 이대성 추가 : 연결법인구분
	NM_CURE_AGENCY = @P_NM_CURE_AGENCY,
	NO_CUER_AGENCY = @P_NO_CUER_AGENCY,
	NM_TEXT = @P_NM_TEXT,						-- kyila 추가 기타정보 비고 
	TP_NATION = @P_TP_NATION,
	NM_DCDEPOSIT = @P_NM_DCDEPOSIT,
	FG_CORPCODE = @P_FG_CORPCODE,
	DT_RCP_PRETOLERANCE = @P_DT_RCP_PRETOLERANCE,
	NO_COR = @P_NO_COR
...
--BEGIN
--	UPDATE CZ_MA_PARTNER
--	SET FG_PAYMENT = @P_FG_PAYMENT,
--		ID_UPDATE = @P_ID_UPDATE,
--		DTS_UPDATE = NEOE.SF_SYSDATE(GETDATE())
--	WHERE CD_COMPANY = 'K200'
--	AND CD_PARTNER = @P_CD_PARTNER
--END
...
--담당자추가
--EXEC UP_FI_Z_DINTEC_PARTNERPTR_I  @P_CD_COMPANY, @P_CD_PARTNER, @P_CD_EMP_PARTNER, @P_E_MAIL, @P_NO_HPEMP_PARTNER, @P_NO_TEL, @P_NO_FAX, @P_ID_UPDATE 
