...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_WEEKLY_RPT_S4]    Script Date: 2018-11-26 오후 3:09:09 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_WEEKLY_RPT_S4]
(
...
WHERE IH.NO_EMP = @P_NO_EMP
AND (ISNULL(IL.AM_CLS, 0) - ISNULL(RH.AM_RCP, 0)) <> 0
AND NOT EXISTS (SELECT 1 
                FROM CZ_SA_WEEKLY_RPT_L RL
                WHERE RL.CD_COMPANY = IH.CD_COMPANY
                AND RL.DT_YEAR = @P_DT_YEAR
                AND RL.NO_EMP = @P_NO_EMP
                AND RL.QT_WEEK = @P_QT_WEEK
                AND RL.TP_GUBUN = '005'
                AND RL.NO_KEY = IH.NO_IV)
