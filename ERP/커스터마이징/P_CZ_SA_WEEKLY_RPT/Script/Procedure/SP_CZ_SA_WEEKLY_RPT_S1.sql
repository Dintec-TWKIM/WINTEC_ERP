...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_WEEKLY_RPT_S1]    Script Date: 2018-11-26 오후 3:09:09 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_WEEKLY_RPT_S1]
(
...
AND (@P_AM_QTN = 0 OR ISNULL(QL.AM_QTN_S, 0) >= @P_AM_QTN)
AND NOT EXISTS (SELECT 1 
                FROM CZ_SA_WEEKLY_RPT_L RL
                WHERE RL.CD_COMPANY = QH.CD_COMPANY
                AND RL.NO_EMP = @P_NO_EMP
                AND RL.DT_YEAR = @P_DT_YEAR
                AND RL.QT_WEEK = @P_QT_WEEK
                AND RL.TP_GUBUN = '001'
                AND RL.NO_KEY = QH.NO_FILE)
