...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_CRM_PARTNER_VESSEL_S]
(
...
SELECT MH.NO_IMO,
	   ...
	   MH.DC_RMK
FROM CZ_MA_HULL MH
JOIN (SELECT QH.NO_IMO,
		     MAX(QH.CD_EXCH) AS CD_EXCH,
	  	     SUM(1.0) AS QT_QTN,
	  	     SUM(CASE WHEN SH.CD_COMPANY IS NULL THEN 0 ELSE 1.0 END) AS QT_SO,
	  	     SUM(QL.AM_SO_QTN_EX) AS AM_SO_QTN_EX,
			 SUM(QL.AM_SO_QTN) AS AM_SO_QTN,
			 SUM(QL.AM_PO_QTN_EX) AS AM_PO_QTN_EX,
	  	     SUM(QL.AM_PO_QTN) AS AM_PO_QTN,
			 SUM(QL.AM_SO_EX) AS AM_SO_EX,
	  	     SUM(QL.AM_SO) AS AM_SO,
			 SUM(QL.AM_PO_EX) AS AM_PO_EX,
	  ...
	  AND ((@P_TP_DATE = '001' AND QH.DT_QTN BETWEEN @P_DT_START AND @P_DT_END) OR 
		   (@P_TP_DATE = '002' AND SH.DT_SO BETWEEN @P_DT_START AND @P_DT_END))
...
LEFT JOIN MA_CODEDTL MC1 ON MC1.CD_COMPANY = @P_CD_COMPANY AND MC1.CD_FIELD = 'MA_B000005' AND MC1.CD_SYSDEF = QH.CD_EXCH
