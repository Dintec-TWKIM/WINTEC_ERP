...
/****** Object:  Trigger [NEOE].[TD_CZ_TR_INVL]    Script Date: 2015-10-22 오후 5:32:38 ******/
SET ANSI_NULLS ON
GO
...
ALTER TRIGGER [NEOE].[TD_CZ_TR_INVL] ON [NEOE].[CZ_TR_INVL] AFTER DELETE  
AS  
DECLARE @ERRNO               INT,
		@ERRMSG              VARCHAR(255),
...
DECLARE CUR_TD_CZ_TR_INVL CURSOR FOR
  
SELECT CD_COMPANY, NO_INV, NO_LINE, NO_PO, NO_LINE_PO, NO_QTIO, NO_LINE_QTIO, QT_SO, AM_EXSO  
FROM DELETED D 
 
OPEN CUR_TD_CZ_TR_INVL

FETCH NEXT FROM CUR_TD_CZ_TR_INVL
...
BEGIN
	...
	BEGIN
		...
		--선적등록된거 있으면 삭제가 안됨
		SELECT TOP 1 @V_NO_BL = NO_BL   --2012.07.11 최창종 수정 : 송장이 여러개 조회될수 에러 발생
		FROM CZ_TR_INVL  
		WHERE CD_COMPANY = @V_CD_COMPANY  
		AND NO_INV = @V_NO_INV  
		ORDER BY NO_LINE;      
		...
		IF (ISNULL(@V_NO_BL, '') <> '')  
		BEGIN  
			--선적자료가 존재하므로 삭제할 수 없습니다  
			SELECT @ERRNO = 100000,
				   @ERRMSG = '선적자료가 존재하므로 삭제할 수 없습니다.'
			GOTO ERROR 
		END 
...
CLOSE CUR_TD_CZ_TR_INVL
DEALLOCATE CUR_TD_CZ_TR_INVL
