...
namespace cz
{
    public partial class P_CZ_SA_RCP : PageBase
    {
        #region 생성자 & 전역변수
        private P_CZ_SA_RCP_BIZ _biz;
        ...
        public P_CZ_SA_RCP(string 수금번호)
        {
            try
            {
                StartUp.Certify(this);
                ...
                if (string.IsNullOrEmpty(수금번호)) return;
        ...
        protected override void InitLoad()
        {
            ...
            this._biz = new P_CZ_SA_RCP_BIZ(this.MainFrameInterface);
            this.InitGrid();
            this.InitEvent();
        }
        ...
        private void InitGrid()
        {
            ...
            this._flexL.SetCol("AM_IV", "대상액(원화)", 100, false, typeof(decimal), FormatTpType.FOREIGN_MONEY);
            this._flexL.SetCol("AM_RCP_TX_EX", "반제액", 100, false, typeof(decimal), FormatTpType.FOREIGN_MONEY);
            this._flexL.SetCol("AM_RCP_TX", "반제액(원화)", 100, true, typeof(decimal), FormatTpType.FOREIGN_MONEY);
            ...
            this._flexL.SetCol("AM_PL", "환차손익", 100, true, typeof(decimal), FormatTpType.FOREIGN_MONEY);
            this._flexL.SetCol("NO_DOCU", "전표번호", 100);
            this._flexL.SetCol("NO_DOLINE", "전표라인", 100);
        ...
        private void InitEvent()
        {
			...
			this.ctx매출처.QueryAfter += Ctx매출처_QueryAfter;
...
AND MC.CD_SYSDEF = '{1}'";
				...
				{
                    this.ShowMessage(dt.Rows[0]["CD_FLAG1"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void _header_ControlValueChanged(object sender, FreeBindingArgs e)
        {
            try
            {
                ...
                if (this.dtp수금일.Text != string.Empty && this.dtp수금일.IsValidated &&
                    ...
                    (string.IsNullOrEmpty(this.ctx수금처.CodeValue) == false) && 
                    (string.IsNullOrEmpty(this.ctx수금담당자.CodeValue) == false))
                {
            ...
            try
            {
                ...
                switch (((C1FlexGridBase)sender).Cols[e.Col].Name)
                {
                    ...
                    case "AM_RCP":
                        ...
                        {
                            this.ShowMessage("좌측 정상수금란을 먼저 입력하십시요");
                        ...
                        if (Math.Abs(Math.Abs(Convert.ToDecimal(this._flexH["AM_RCP"])) - Math.Abs(Convert.ToDecimal(this._flexH["AM_RCP_EX"]) * Convert.ToDecimal(this._flexH["RT_EXCH"]))) > new Decimal(100))
                        {
                    ...
                    case "AM_RCP_A":
                        ...
                        {
                            this.ShowMessage("좌측 선수금란을 먼저 입력하십시요");
                        ...
                        if (Math.Abs(Math.Abs(Convert.ToDecimal(this._flexH["AM_RCP_A"])) - Math.Abs(Convert.ToDecimal(this._flexH["AM_RCP_A_EX"]) * Convert.ToDecimal(this._flexH["RT_EXCH"]))) > new Decimal(100))
                        {
                            this.ShowMessage("원화 조정금액은 100원이내입니다.");
        ...
        private void ctx계산서번호검색_QueryAfter(object sender, BpQueryArgs e)
        {
            try
            {
                ...
                else if (this.cbo거래구분.SelectedValue.ToString() == "005")
                    this.cbo수금형태.SelectedValue = "001";

                this._header.CurrentRow["CD_TP"] = this.cbo수금형태.SelectedValue;
        ...
        private void btn회계전표처리_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    query = @"SELECT CD_COMPANY, NO_DOCU, CD_PC 
                ...
                else
                {
                    ...
                    for (int @fixed = this._flexL.Rows.Fixed; @fixed < this._flexL.Rows.Count; ++@fixed)
                    {
                        if (this._flexL.RowState(@fixed) == DataRowState.Added)
                        {
                            ...
                            row["NO_DOCU"] = D.GetString(this._flexL[@fixed, "NO_DOCU"]);
                            row["NO_DOLINE"] = D.GetString(this._flexL[@fixed, "NO_DOLINE"]);
                            row["AM_BAN_EX"] = D.GetDecimal(this._flexL[@fixed, "AM_RCP_TX_EX"]);
                            row["AM_BAN"] = (D.GetDecimal(this._flexL[@fixed, "AM_RCP_TX"]) + D.GetDecimal(this._flexL[@fixed, "AM_PL"]));

                            매출반제.Rows.Add(row);
                        }
                    }

                    P_CZ_SA_RCP_SUB dialog = new P_CZ_SA_RCP_SUB(매출반제, this.dtp수금일.Text);

                    dialog.거래구분코드 = (this.cbo거래구분.SelectedValue == null ? string.Empty : this.cbo거래구분.SelectedValue.ToString());
                    dialog.거래구분명 = this.cbo거래구분.Text;
                    dialog.매출처코드 = this.ctx매출처.CodeValue;
                    dialog.매출처명 = this.ctx매출처.CodeName;
                    dialog.수금처코드 = this.ctx수금처.CodeValue;
                    dialog.수금처명 = this.ctx수금처.CodeName;
                    DataRow currentRow = this._header.CurrentRow;
                    dialog.총수금원화 = D.GetDecimal(currentRow["AM_RCP"]);
                    dialog.반제액원화 = D.GetDecimal(currentRow["AM_RCP_TX"]);
                    dialog.총수금 = D.GetDecimal(currentRow["AM_RCP_EX"]);
                    dialog.반제액 = D.GetDecimal(currentRow["AM_RCP_EX_TX"]);
                    dialog.통화코드 = this.cbo통화명.SelectedValue.ToString();
                    dialog.통화명 = this.cbo통화명.Text;
                    dialog.환율 = D.GetDecimal(this._flexH["RT_EXCH"]);
                    dialog.Set매출구분 = "0";

                    if (dialog.ShowDialog() != DialogResult.OK) return;

                    DataTable getDataTable = dialog.GetDataTable;
                    ...
                    foreach (DataRow dataRow in getDataTable.Rows)
                    {
                        ...
                        row["NO_DOCU"] = dataRow["NO_DOCU"];
                        row["NO_DOLINE"] = dataRow["NO_DOLINE"];
        ...
        private void Control_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (name == this.cbo통화명.Name)
                    this.화폐단위셋팅();
                else if (name == this.cbo거래구분.Name)
                    this.거래구분셋팅();
            }
            catch (Exception ex)
            {
