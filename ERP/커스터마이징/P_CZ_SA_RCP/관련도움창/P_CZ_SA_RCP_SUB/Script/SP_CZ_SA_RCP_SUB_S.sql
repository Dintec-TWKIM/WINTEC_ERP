...
*********************************************
*********************************************/
ALTER PROCEDURE [NEOE].[SP_CZ_SA_RCP_SUB_S]
(
...
SELECT 'N' AS S,
	   ...
	   ISNULL((CASE WHEN FD1.CD_EXCH = '000' THEN BH.AM_DOCU ELSE BH.AM_EX END), 0) AS AM_EX,
	   ISNULL(BH.AM_DOCU, 0) AS AM_DOCU,
	   ISNULL((CASE WHEN FD1.CD_EXCH = '000' THEN BH.AM_BAN ELSE BH.AM_EXBAN END), 0) AS AM_BAN_EX,
	   ISNULL(BH.AM_BAN, 0) AS AM_BAN,
	   0.00 AS AM_RCP_EX,
	   0.00 AS AM_RCP,
	   0.00 AS AM_PL_LOSS,
	   0.00 AS AM_PL_PROFIT,
	   (CASE WHEN FD1.CD_EXCH = '000' THEN (ISNULL(BH.AM_DOCU, 0) - ISNULL(BH.AM_BAN, 0)) ELSE (ISNULL(BH.AM_EX, 0) - ISNULL(BH.AM_EXBAN, 0)) END) AS AM_RCP_JAN_EX,
	   ...
	   IL.TP_IV,
	   IH.DC_REMARK,
	   IH.AM_EX AS AM_EX_ALL
FROM SA_IVH IH
JOIN (SELECT CD_COMPANY, NO_MDOCU, NO_DOCU 
	  FROM FI_DOCU
	  WHERE ST_DOCU = '2' 
...
JOIN FI_DOCU FD1 ON FD1.CD_COMPANY = FD.CD_COMPANY AND FD1.NO_DOCU = FD.NO_DOCU AND FD1.CD_ACCT = AL.CD_ACCT
JOIN FI_BANH BH ON BH.CD_COMPANY = FD1.CD_COMPANY AND BH.NO_DOCU = FD1.NO_DOCU AND BH.NO_DOLINE = FD1.NO_DOLINE
...
LEFT JOIN MA_CODEDTL MC ON MC.CD_COMPANY = FD1.CD_COMPANY AND MC.CD_FIELD = 'MA_B000005' AND MC.CD_SYSDEF = FD1.CD_EXCH
