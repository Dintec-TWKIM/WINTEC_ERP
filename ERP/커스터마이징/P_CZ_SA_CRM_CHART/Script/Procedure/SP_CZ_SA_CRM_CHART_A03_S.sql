...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_CRM_CHART_A03_S]    Script Date: 2017-07-13 오후 3:13:12 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_CRM_CHART_A03_S]
(
...
SELECT MP.LN_PARTNER, SH.CD_PARTNER,
	   ISNULL(SUM(SL.AM_SO), 0) AS AM_SO,
	   SUM(ISNULL(SL.AM_PO, 0) + ISNULL(SL.AM_STOCK, 0)) AS AM_PO,
	   SUM(ISNULL(SL.AM_SO, 0) - (ISNULL(SL.AM_PO, 0) + ISNULL(SL.AM_STOCK, 0))) AS AM_PROFIT,
	   (CASE WHEN ISNULL(SUM(SL.AM_SO), 0) = 0 THEN 0
										       ELSE ROUND((ISNULL(SUM(SL.AM_SO), 0) - SUM(ISNULL(SL.AM_PO, 0) + ISNULL(SL.AM_STOCK, 0))) / ISNULL(SUM(SL.AM_SO), 0) * 100 ,2) END) AS RT_PROFIT
FROM SA_SOH SH
JOIN (SELECT SL.CD_COMPANY, SL.NO_SO,
			 SUM(ISNULL(SL.AM_KR_S, 0)) AS AM_SO,
			 SUM(ISNULL(SB.QT_STOCK, 0) * ISNULL(SB.UM_KR, 0)) AS AM_STOCK,
			 SUM(ISNULL(PL.AM, 0)) AS AM_PO 
	  FROM SA_SOL SL
...
JOIN MA_PARTNER MP ON MP.CD_COMPANY = SH.CD_COMPANY AND MP.CD_PARTNER = SH.CD_PARTNER
...
ORDER BY SUM(ISNULL(SL.AM_SO, 0) - (ISNULL(SL.AM_PO, 0) + ISNULL(SL.AM_STOCK, 0))) DESC
