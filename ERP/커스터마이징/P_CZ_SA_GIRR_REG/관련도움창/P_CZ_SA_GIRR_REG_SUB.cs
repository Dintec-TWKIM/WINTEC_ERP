...
namespace cz
{
	...
	{
		#region ★ 멤버필드
		P_SA_GIRR_REG_SUB_BIZ _biz = new P_SA_GIRR_REG_SUB_BIZ();
		DataTable _dt = new DataTable();
		#endregion

		#region ★ 초기화
		public P_SA_GIRR_REG_SUB(string 수주번호)
		{
            InitializeComponent();
		...
		private void InitGrid()
		{
            ...
            this._flexH.SetCol("DT_IO", "수불일자", 80, false, typeof(string), FormatTpType.YEAR_MONTH_DAY);
            ...
            this._flexL.SetCol("QT_IO", "재고수량", 65, false, typeof(decimal), FormatTpType.QUANTITY);
            this._flexL.SetCol("NM_SL", "창고", 100);
            ...
            this._flexL.SetCol("LN_PARTNER", "납품처명", 120);
		...
		protected override void InitPaint()
		{
			try
			{
                ...
                this.dtp출고일자.StartDateToString = Global.MainFrame.GetDateTimeToday().AddYears(-1).ToString("yyyyMMdd");
                this.dtp출고일자.EndDateToString = Global.MainFrame.GetStringToday;
			}
			catch (Exception ex)
			{
				Global.MainFrame.MsgEnd(ex);
