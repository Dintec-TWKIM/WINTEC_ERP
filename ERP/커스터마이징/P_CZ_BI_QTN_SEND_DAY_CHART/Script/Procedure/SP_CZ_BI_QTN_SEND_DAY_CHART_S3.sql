...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_BI_QTN_SEND_DAY_CHART_S3]    Script Date: 2018-09-03 오후 8:02:37 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_BI_QTN_SEND_DAY_CHART_S3]
(
...
(
	...
	LEFT JOIN MA_EMP ME ON ME.CD_COMPANY = QH.CD_COMPANY AND ME.NO_EMP = (CASE WHEN @P_TP_QTN = '000' THEN QH.NO_EMP
																			   WHEN @P_TP_QTN = '001' THEN QH.NO_EMP_STK
																									  ELSE QH.NO_EMP_QTN END)
...
WH3 AS
(
	SELECT WH.NM_SALEORG, MC.NM_CC, WH.NM_KOR,
	       SUM(1.0) AS QT_INQ,
    ...
    GROUP BY WH.NM_SALEORG, MC.NM_CC, WH.NM_KOR
)
SELECT WH.NM_SALEORG, WH.NM_CC, WH.NM_KOR,
       WH.QT_INQ,
	   WH.QT_DONE,
	   (CASE WHEN WH.QT_INQ >= 10 THEN WH.QT_DONE1 ELSE NULL END) AS QT_DONE1,
	   WH.QT_ITEM,
	   WH.AM_QTN,
	   (CASE WHEN WH.QT_INQ >= 10 THEN WH.DT_QTN ELSE NULL END) AS DT_QTN,
	   WH.QT_2DAY,
	   WH.QT_SO
FROM WH3 WH
ORDER BY WH.NM_SALEORG, WH.NM_CC, WH.NM_KOR
