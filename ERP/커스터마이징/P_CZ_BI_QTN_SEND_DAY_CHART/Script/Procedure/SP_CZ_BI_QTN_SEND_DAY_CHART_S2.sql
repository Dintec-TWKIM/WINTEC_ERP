...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_BI_QTN_SEND_DAY_CHART_S2]    Script Date: 2018-09-03 오후 7:55:09 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_BI_QTN_SEND_DAY_CHART_S2]
(
...
(
	SELECT WH.CD_COMPANY,
		   WH.NO_KEY,
		   WH.NO_EMP,
		   (CASE WHEN SG.CD_SALEORG = '010301' AND QL.YN_ENGINE = 'Y' THEN '영업부(엔진)' 
				 WHEN SG.CD_SALEORG = '010301' AND QL.YN_ENGINE = 'N' THEN '영업부(엔진외)'
				 ELSE SO.NM_SALEORG END) AS NM_SALEORG,
		   SG.CD_CC,
	...
	LEFT JOIN (SELECT QL.CD_COMPANY, QL.NO_FILE,
			   ...
			   FROM CZ_SA_QTNL QL
			   GROUP BY QL.CD_COMPANY, QL.NO_FILE, (CASE WHEN QL.GRP_ITEM IN ('AE', 'HE', 'ME', 'PP', 'TC') THEN 'Y' ELSE 'N' END)) QL
...
WH3 AS
(
	SELECT WH.NM_SALEORG, MC.NM_CC, WH.NO_EMP,
	       SUM(1.0) AS QT_INQ,
    ...
    GROUP BY WH.NM_SALEORG, MC.NM_CC, WH.NO_EMP
)
SELECT WH.NM_SALEORG, WH.NM_CC,
	   SUM(WH.QT_INQ) AS QT_INQ,
	   SUM(WH.QT_DONE) AS QT_DONE,
	   SUM(CASE WHEN WH.QT_INQ >= 10 THEN WH.QT_DONE1 ELSE NULL END) AS QT_DONE1,
	   SUM(WH.QT_ITEM) AS QT_ITEM,
	   SUM(WH.AM_QTN) AS AM_QTN,
	   SUM(CASE WHEN WH.QT_INQ >= 10 THEN WH.DT_QTN ELSE NULL END) AS DT_QTN,
	   SUM(WH.QT_2DAY) AS QT_2DAY,
	   SUM(WH.QT_SO) AS QT_SO 
FROM WH3 WH
GROUP BY WH.NM_SALEORG, WH.NM_CC
ORDER BY WH.NM_SALEORG, WH.NM_CC
