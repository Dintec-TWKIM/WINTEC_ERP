...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_IVMNGL_S]    Script Date: 2015-06-22 오후 7:27:55 ******/
SET ANSI_NULLS ON
GO
...
-- HISTORY --------------------------------
-- 2011.03.04 SOURCESAFE 관리시작
-- 2011.03.09 프로시져 줄 맞춤	BY SJH
-- 2011.03.09 출고일자(DT_IO)	BY SJH 
-------------------------------------------
ALTER PROCEDURE [NEOE].[SP_CZ_SA_IVMNGL_S]
(
...
JOIN (SELECT CD_COMPANY, NO_IV, COUNT(DISTINCT NO_SO) AS CNT_SO
	  FROM SA_IVL 
	  GROUP BY CD_COMPANY, NO_IV) IL1
ON IL1.CD_COMPANY = IH.CD_COMPANY AND IL1.NO_IV = IH.NO_IV
...
AND (ISNULL(@P_CD_PLANT, '') = '' OR IL.CD_PLANT = @P_CD_PLANT)
AND (ISNULL(@P_NO_PO_PARTNER, '') = '' OR SL.NO_PO_PARTNER = @P_NO_PO_PARTNER)
AND (ISNULL(@P_NO_IMO, '') = '' OR MH.NO_IMO = @P_NO_IMO)
AND (ISNULL(@P_MULTI_SALEORG, '') = '' OR OG.CD_SALEORG IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_MULTI_SALEORG)))
AND (ISNULL(@P_MULTI_SALEGRP, '') = '' OR SG.CD_SALEGRP IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_MULTI_SALEGRP)))
