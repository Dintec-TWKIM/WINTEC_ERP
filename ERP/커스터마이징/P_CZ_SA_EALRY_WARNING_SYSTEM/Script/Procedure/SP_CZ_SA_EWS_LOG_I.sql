...
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_EWS_LOG_I]    Script Date: 2019-02-26 오후 3:00:55 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [NEOE].[SP_CZ_SA_EWS_LOG_I]
(
...
IF EXISTS (SELECT 1 
		   ...
		   AND DT_EWS = CONVERT(NVARCHAR(8), GETDATE(), 112))
BEGIN
	RETURN
END
...
FROM (SELECT IH.CD_COMPANY, IH.CD_PARTNER 
	  FROM #SA_IVH IH
	  WHERE IH.DT_PROCESS BETWEEN CONVERT(NVARCHAR(8), DATEADD(MONTH, -6, GETDATE()), 112) AND CONVERT(NVARCHAR(8), GETDATE(), 112)
...
(
	SELECT MP.CD_COMPANY,
		   MP.CD_PARTNER,
		   C1.RESULT AS CD_RESULT1,
		   C2.RESULT AS CD_RESULT2,
		   C3.RESULT AS CD_RESULT3,
		   C4.RESULT AS CD_RESULT4,
		   C5.RESULT AS CD_RESULT5,
		   C6.RESULT AS CD_RESULT6,
		   C7.RESULT AS CD_RESULT7
	FROM #MA_PARTNER MP
...
(
	SELECT RE.CD_COMPANY,
		   ...
		   RE.CD_RESULT6,
		   RE.CD_RESULT7,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT1) AS NM_RESULT1,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT2) AS NM_RESULT2,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT3) AS NM_RESULT3,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT4) AS NM_RESULT4,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT5) AS NM_RESULT5,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT6) AS NM_RESULT6,
		   (SELECT DC_MESSAGE FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT7) AS NM_RESULT7,
		   (SELECT CONVERT(NVARCHAR, ISNULL(MAX(A.WN_LEVEL), 0)) 
			FROM (SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT1
			      UNION ALL
			      SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT2
			      UNION ALL
			      SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT3
			      UNION ALL
			      SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT4
			      UNION ALL
			      SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT5
				  UNION ALL
			      SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT6
				  UNION ALL
			      SELECT WN_LEVEL FROM #WARNING_MESSAGE WHERE CD_EWS = RE.CD_RESULT7) A) AS CD_LEVEL
	FROM RE
	LEFT JOIN CZ_MA_PARTNER MP ON MP.CD_COMPANY = RE.CD_COMPANY AND MP.CD_PARTNER = RE.CD_PARTNER
)
INSERT INTO CZ_SA_EWS_LOG
(
	CD_COMPANY,
	CD_PARTNER,
	DT_EWS,
...
SELECT RE1.CD_COMPANY,
	   RE1.CD_PARTNER,
	   CONVERT(NVARCHAR(8), GETDATE(), 112) AS DT_EWS,
	   RE1.CD_RESULT1,
	   RE1.CD_RESULT2,
	   RE1.CD_RESULT3,
	   RE1.CD_RESULT4,
