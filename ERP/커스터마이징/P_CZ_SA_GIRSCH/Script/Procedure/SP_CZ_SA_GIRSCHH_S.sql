USE [NEOE]
GO
/****** Object:  StoredProcedure [NEOE].[SP_CZ_SA_GIRSCHH_S]    Script Date: 2017-07-03 오후 3:42:46 ******/
SET ANSI_NULLS ON
GO
...
--EXEC SP_CZ_SA_GIRSCHH_S 'TEST', 'KR', '1', '1', '20150601', '20150618', '1', '1', '1', 'Y', '1', '1', '0', '1', '1', '1', '1', 'Y', 'Y', '20150601', '20150618', '1', '1', '1', '1', '1', 'S-391', '1', '1'
ALTER PROCEDURE [NEOE].[SP_CZ_SA_GIRSCHH_S]  
(
...
AND (@P_TP_DATE = '' OR ((CASE @P_TP_DATE WHEN '001' THEN GD.DT_START
...
AND (@P_YN_URGENT <> 'Y' OR 
	 (SUBSTRING(GD.DTS_SUBMIT, 1, 8) = SUBSTRING(GD.DT_COMPLETE, 1, 8)) OR
...
AND (@P_CD_PARTNER_GRP = '' OR EXISTS (SELECT 1 
									   FROM MA_PARTNER MP
									   WHERE MP.CD_COMPANY = GH.CD_COMPANY
									   AND MP.CD_PARTNER = GH.CD_PARTNER
									   AND MP.CD_PARTNER_GRP IN (SELECT CD_STR FROM GETTABLEFROMSPLIT(@P_CD_PARTNER_GRP))))
...
LEFT JOIN MA_USER MU ON MU.CD_COMPANY = GD.CD_COMPANY AND MU.ID_USER = GD.ID_CONFIRM
...
ELSE IF @P_STATE = '1'
BEGIN
SELECT ISNULL(LEFT(GH.DTS_SUBMIT, 8), '') AS DT_SUBMIT
FROM CZ_SA_GIRH_PACK_DETAIL GH
WHERE EXISTS (SELECT 1 
			  FROM @CZ_SA_GIRL_PACK
			  WHERE CD_COMPANY = GH.CD_COMPANY
			  AND NO_GIR = GH.NO_GIR)
...
ELSE IF @P_STATE = '2'
BEGIN
SELECT ISNULL(GH.CD_PARTNER, '') AS CD_PARTNER,
	   MP.LN_PARTNER,
	   ME.NM_KOR AS NM_EMP_SALE
FROM CZ_SA_GIRH_PACK GH
...
ELSE IF @P_STATE = '3'
BEGIN
SELECT '002' AS CD_TYPE,
	   ISNULL(GD.CD_PACK_CATEGORY, '') AS CD_TYPE2,
	   ISNULL(GD.CD_SUB_CATEGORY, '') AS CD_TYPE3,
	   MC.NM_SYSDEF AS NM_TYPE1,
	   MC1.NM_SYSDEF AS NM_TYPE2,
	   MC2.NM_SYSDEF AS NM_TYPE3
FROM CZ_SA_GIRH_PACK_DETAIL GD
LEFT JOIN MA_CODEDTL MC ON MC.CD_COMPANY = GD.CD_COMPANY AND MC.CD_FIELD = 'CZ_SA00021' AND MC.CD_SYSDEF = '002'
LEFT JOIN MA_CODEDTL MC1 ON MC1.CD_COMPANY = GD.CD_COMPANY AND MC1.CD_FIELD = 'CZ_SA00020' AND MC1.CD_SYSDEF = GD.CD_PACK_CATEGORY
...
WHERE EXISTS (SELECT 1 
			  FROM @CZ_SA_GIRL_PACK
			  WHERE CD_COMPANY = GD.CD_COMPANY
			  AND NO_GIR = GD.NO_GIR)
UNION
SELECT '001' AS CD_TYPE,
	   ISNULL(GD.CD_MAIN_CATEGORY, '') AS CD_TYPE2,
       ISNULL(GD.CD_SUB_CATEGORY, '') AS CD_TYPE3,
       MC.NM_SYSDEF AS NM_TYPE1,
       MC1.NM_SYSDEF AS NM_TYPE2,
       MC2.NM_SYSDEF AS NM_TYPE3
FROM CZ_SA_GIRH_WORK_DETAIL GD
LEFT JOIN MA_CODEDTL MC ON MC.CD_COMPANY = GD.CD_COMPANY AND MC.CD_FIELD = 'CZ_SA00021' AND MC.CD_SYSDEF = '001'
LEFT JOIN MA_CODEDTL MC1 ON MC1.CD_COMPANY = GD.CD_COMPANY AND MC1.CD_FIELD = 'CZ_SA00006' AND MC1.CD_SYSDEF = GD.CD_MAIN_CATEGORY
LEFT JOIN MA_CODEDTL MC2 ON MC2.CD_COMPANY = GD.CD_COMPANY AND MC2.CD_FIELD = MC1.CD_FLAG1 AND MC2.CD_SYSDEF = GD.CD_SUB_CATEGORY
WHERE EXISTS (SELECT 1 
			  FROM @SA_GIRL
			  WHERE CD_COMPANY = GD.CD_COMPANY
...
ELSE IF @P_STATE = '4'
BEGIN
SELECT ISNULL(GD.DT_COMPLETE, '') AS DT_COMPLETE
FROM CZ_SA_GIRH_PACK_DETAIL GD
WHERE EXISTS (SELECT 1 
			  FROM @CZ_SA_GIRL_PACK
			  WHERE CD_COMPANY = GD.CD_COMPANY
...
ELSE IF @P_STATE = '5'
BEGIN
SELECT ISNULL(GD.DT_START, '') AS DT_START
FROM CZ_SA_GIRH_PACK_DETAIL GD
WHERE EXISTS (SELECT 1 
			  FROM @CZ_SA_GIRL_PACK
			  WHERE CD_COMPANY = GD.CD_COMPANY
