...
namespace sale
{
    public partial class P_SA_ESTMT_REG : PageBase
    {
        #region ♣ 생성자 & 변수선언

        P_SA_ESTMT_REG_BIZ _biz = new P_SA_ESTMT_REG_BIZ();
        public P_SA_ESTMT_REG()
        {
            InitializeComponent();
        ...
        private void btn파일업로드_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (D.GetString(_flexH["STA_EST"]) == "1")
                {
                    ShowMessage("확정된 내용은 파일업로드하실 수 없습니다.");
                    return;
                }
        ...
        bool Chk미등록품목
        {
            get
            {
                if (서버키 == "PNE" || 서버키 == "FORTIS" || 서버키 == "LGDISP" || 서버키 == "WJIS")
                {
