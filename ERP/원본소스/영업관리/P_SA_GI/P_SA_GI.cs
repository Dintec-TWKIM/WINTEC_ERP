...
namespace sale
{
    ...
    // **************************************
    public partial class P_SA_GI : Duzon.Common.Forms.PageBase
    {
        #region ♣ 생성자 & 변수 선언
        ...
        private void InitGrid()
        {
            ...
            if (Config.MA_ENV.YN_UNIT == "Y")
            {
                _flexL.SetExceptSumCol("SEQ_PROJECT");
            }
            #endregion
        }
        ...
        protected override bool SaveData()
        {
            try
            {
                ...
                if (Global.MainFrame.ServerKeyCommon == "DZSQL" || Global.MainFrame.ServerKeyCommon.ToUpper() == "ANJUN")
                {
                    DataRow[] drarr = dt_L.Select("FG_TRANS IN ('004', 005) AND PITEM_CD_USERDEF1 = 'Y' AND CD_USERDEF1 = 'Y'", "", DataViewRowState.CurrentRows);
                    if (drarr.Length > 0)
                    {
            ...
            try
            {
                ...
                if (ldrchk != null && ldrchk.Length > 0)
                {
                    foreach (DataRow row in ldrchk)
                    {
                        ...
                        //row["AM_EX"] = D.GetDecimal(row["UM_EX"]) * D.GetDecimal(row["QT_IO"]);
                        //row["AM"] = Decimal.Truncate(D.GetDecimal(row["AM_EX"]) * D.GetDecimal(row["RT_EXCH"]));

                        //rowUnit합계 += D.GetDecimal(row["QT_GOOD_INV"]);
                        //rowIO합계 += D.GetDecimal(row["QT_IO"]);
            ...
            try
            {
                ...
                switch (flex.Name)
                {
                    case "_flexH":  //상단 그리드 Header Click 이벤트
                        ...
                        DataTable dtL = _flexL.DataTable;
                        ...
                        if (_flexH.GetCellCheck(_flexH.Row, _flexH.Cols["S"].Index) == CheckEnum.Unchecked)
                        {
                            ...
                            {
                                if (_flexL.GetCellCheck(i, _flexL.Cols["S"].Index) == CheckEnum.Checked)
                                {
                            ...
                            int rowh = _flexH.Row;
                            int rowch = _flexH.Rows.Count - _flexH.Rows.Fixed;
                            if (rowh == rowch)
                            {
                                rowch = -1;
                            }
                        ...
                        else
                        {
                            ...
                            ArrayList al그리드헤더라인 = new ArrayList();

                            for (int i = _flexH.Rows.Fixed; i < _flexH.Rows.Count; i++)
                            {
                                string Filter = "NO_GIR = '" + D.GetString(_flexH[i, "NO_GIR"]) + "'";
                                DataRow[] dr = _flexL.DataTable.Select(Filter);
                                if (dr == null || dr.Length == 0)
                                {
                                    ...
                                    {
                                        al그리드헤더라인.Add(i);
                                        의뢰번호 += D.GetString(_flexH[i, "NO_GIR"]) + "|";
                                    }
                                }
                            }

                            string FilterL = "NO_GIR = '" + D.GetString(_flexH["NO_GIR"]) + "'";
                            DataTable dt = null;
                            if (의뢰번호 != string.Empty)
                            {
                                object[] obj = new object[] {
                                    LoginInfo.CompanyCode, 
                                    "", 
                                ...
                                dt = _biz.SearchCheckHeader(의뢰번호, obj);
                            ...
                            _flexL.BindingAdd(dt, FilterL);
                            foreach (int i in al그리드헤더라인)
                            {
                                _flexH.SetDetailQueryNeedByRow(i, false);
                            }

                            for (int i = _flexL.Rows.Fixed; i < dtL.Rows.Count; i++)
                            {
                                dtL.Rows[i]["S"] = "Y";
                            }
                            ...
                            for (int i = _flexL.Rows.Fixed; i < _flexL.Rows.Count; i++)
                            {
                                if (_flexL.GetCellCheck(i, _flexL.Cols["S"].Index) == CheckEnum.Unchecked)
                                {
