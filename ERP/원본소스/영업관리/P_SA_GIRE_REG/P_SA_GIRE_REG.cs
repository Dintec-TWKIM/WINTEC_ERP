...
namespace sale
{
    ...
    // 서브시스템명 : 출하관리
    // 페 이 지  명 : 출하반품등록
    // 프로젝트  명 : P_SA_GIRE_REG => 다시 맹그럿음~캬캬
    // **************************************
    public partial class P_SA_GIRE_REG : Duzon.Common.Forms.PageBase
    {
        ...
        protected override void InitPaint()
        {
            ...
            // 조회조건 초기화
            dp시작일.Text = Global.MainFrame.GetStringFirstDayInMonth;
                    ...
                    foreach (DataRow dr in dr_grid)
                    {
                        foreach (DataRow dr_lot in temp_Lot.Rows)
                        {
                            if (dr["NO_IO"].ToString() == dr_lot["NO_IO"].ToString() && dr["NO_IOLINE"].ToString() == dr_lot["NO_IOLINE"].ToString())
                            {
                                ...
                                if (dr["CD_ITEM"].ToString() == dr_lot["CD_ITEM"].ToString() && _flexL.CDecimal(dr["QT_UNIT_MM"]) != _flexL.CDecimal(dr_lot["QT_IO"]))
                                    cnt_Lot++;
                            }
                        }
                    }
