...
namespace sale
{
    public partial class P_SA_SO_NEW : PageBase
    {
        #region ▶ 멤버필드        
        ...
        string _배송여부 = string.Empty;    //수주유형 선택시 배송여부 디폴트값을 넣어준다
        string _자동승인여부 = string.Empty;    //수주유형 선택시 자동승인여부 디폴트값을 넣어준다
        bool _헤더만복사 = false;    //수주조회 도움창에서 복사버튼 클릭 시 헤더만 복사 할지 라인데이터도 복사 할지 결정.
        ...
        public P_SA_SO_NEW(string noSo) : this() { _수주번호 = noSo; }
        ...
        #region ▷ InitGrid 업체전용           

        #region ▷ SANGBO

        private void InitGrid_SANGBO()
        {
            ...
            _flex.SetCol("CD_EXCH", "화폐단위", false);
            _flex.SetCol("UM_BASE", "기준가", 100, 15, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("NUM_USERDEF1", "F/DC", 100, 15, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("RT_DSCNT", "DC", 100, 15, true, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("NUM_USERDEF5", "OLD_DC", false);
            _flex.SetCol("NUM_USERDEF2", "단가", 100, 15, true, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("NUM_USERDEF3", "추가할인B(%)", 100, 15, true, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("NUM_USERDEF4", @"추가할인A(\)", 100, 15, true, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("NUM_USERDEF6", "중간단가", 100, 15, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("UM_SO", "최종단가", 100, 15, true, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST);
            _flex.SetCol("AM_SO", "최종금액", 100, 17, true, typeof(decimal), FormatTpType.FOREIGN_MONEY);
            ...
            _flex.SetCol("TXT_USERDEF1", "TEXT사용자정의1", 150, true);
            ...
            if (서버키 == "CNP")
            {
                _flex.SetCol("QT_EXP", "출고예정수량", 100, false, typeof(decimal), FormatTpType.QUANTITY);
                _flex.SetCol("QT_USEINV", "가용재고", 100, false, typeof(decimal), FormatTpType.QUANTITY);
                _flex.SetCol("QT_AVA", "출하가능수량", 100, false, typeof(decimal), FormatTpType.QUANTITY);
                _flex.SetCol("NO_LOT", "로트번호", 100, false);
            }
            else if (Use가용재고 || 서버키 == "KOREAF" || 서버키 == "ACADEMY" || PageID == "P_SA_Z_KORFNT_SO" || 서버키== "DNCOMPANY")
                _flex.SetCol("QT_USEINV", "가용재고", 100, false, typeof(decimal), FormatTpType.QUANTITY);
            else
            ...
            if (서버키 == "SIMMONS")
                _flex.SetCol("TP_IV", "매출형태", 120, true);
            else
            {
                _flex.SetCol("TP_IV", "매출형태", 120, false);
            ...
            else if (_biz.Get업체별프로세스 == "009")
            {
                _flex.SetCol("CD_ITEM_REF", "프로모션대상품목", 150, false);
                _flex.SetCol("NM_ITEM_REF", "프로모션대상품명", 150, false);
                _flex.SetCol("STND_ITEM_REF", "프로모션대상품목규격", 150, false);
            }

            if (BASIC.GetMAEXC("배차사용유무") == "Y")
                _flex.SetCol("YN_PICKING", "배송여부", 80, true, CheckTypeEnum.Y_N);

            _flex.SetCol("FG_USE2", "수주용도2", 100, true);
            ...
            if (Config.MA_ENV.YN_UNIT == "Y")
                ...
                _flex.SetExceptEditCol("UM_SO", "AM_SO", "UM_BASE", "NUM_USERDEF1", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "FG_MODEL", "NM_SL", "LN_PARTNER", "RT_VAT", "NM_CC", "NM_MANAGER1", "NO_IO_MGMT", "NO_IOLINE_MGMT", "NM_PROJECT", "NO_SO_ORIGINAL", "SEQ_SO_ORIGINAL");
            ...
            if (Use가용재고 || PageID == "P_SA_Z_KORFNT_SO") listNotNullCol.Add("CD_SL");
            ...
            if (PageID != "P_SA_Z_KORFNT_SO") list.Add("NUM_USERDEF2");
            if (disCount_YN == "Y")
            {
                list.Add("UM_BASE");
                list.Add("RT_DSCNT");
        ...
        private void InitGrid()
        {
            ...
            else if (_biz.Get업체별프로세스 == "003")    //아카데미과학(PIMS:D20111115050)
            {
                ...
                _flex.SetCol("PITEM_NUM_USERDEF4", "품목숫자사용자정의4", 100, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST); _flex.Cols["PITEM_NUM_USERDEF4"].Visible = false;
                _flex.SetCol("PITEM_NUM_USERDEF5", "품목숫자사용자정의5", 100, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST); _flex.Cols["PITEM_NUM_USERDEF5"].Visible = false;
                _flex.SetCol("PITEM_NUM_USERDEF6", "품목숫자사용자정의6", 100, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST); _flex.Cols["PITEM_NUM_USERDEF6"].Visible = false;
                _flex.SetCol("PITEM_NUM_USERDEF7", "품목숫자사용자정의7", 100, false, typeof(decimal), FormatTpType.FOREIGN_UNIT_COST); _flex.Cols["PITEM_NUM_USERDEF7"].Visible = false;
                _flex.SetCol("PITEM_CD_USERDEF2", "품목코드사용자정의2", 100, false); _flex.Cols["PITEM_CD_USERDEF2"].Visible = false;
                _flex.SetCol("PITEM_CD_USERDEF3", "품목코드사용자정의3", 100, false); _flex.Cols["PITEM_CD_USERDEF3"].Visible = false;
            }
            ...
            else if (Use가용재고 || 서버키 == "KOREAF" || 서버키 == "ACADEMY" || PageID == "P_SA_Z_KORFNT_SO")
                _flex.SetCol("QT_USEINV", "가용재고", 100, false, typeof(decimal), FormatTpType.QUANTITY);
            else
                _flex.SetCol("QT_USEINV", "가용재고", 0, false, typeof(decimal), FormatTpType.QUANTITY);
            ...
            if (서버키 == "SAMWON")
            {
                _flex.Cols["NUM_USERDEF4"].Visible = true;
            }
            ...
            if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
                _flex.SetCol("TXT_USERDEF1", "TEXT사용자정의1", 80, true, CheckTypeEnum.Y_N);
            else
                _flex.SetCol("TXT_USERDEF1", "TEXT사용자정의1", 150, true);
            _flex.Cols["TXT_USERDEF1"].Visible = false;
            _flex.SetCol("TXT_USERDEF2", "TEXT사용자정의2", 150, true); _flex.Cols["TXT_USERDEF2"].Visible = false;
            ...
            _flex.SetCol("CD_USERDEF1", "사용자정의코드1", 100, true); _flex.Cols["CD_USERDEF1"].Visible = false;
            ...
            if (Config.MA_ENV.YN_UNIT == "Y")
                listNotNullCol.Add("SEQ_PROJECT");
            if (서버키 == "CNP") listNotNullCol.Add("FG_USE");
            listNotNullCol.Add("TP_IV");

            _flex.VerifyNotNull = listNotNullCol.ToArray();

            //미샤 업체일 경우 수주수량이 0이여도 저장 가능하게처리
            if (Global.MainFrame.ServerKey == "NU1211-Z01-N36773")
            {
                _flex.VerifyCompare(_flex.Cols["QT_SO"], 0, OperatorEnum.GreaterOrEqual);
                _flex.VerifyCompare(_flex.Cols["QT_IM"], 0, OperatorEnum.GreaterOrEqual);
            }
            else
            {
                _flex.VerifyCompare(_flex.Cols["QT_SO"], 0, OperatorEnum.Greater);
        ...
        protected override void InitPaint()
        {
            ...
            str.SetCombobox(cbo계정처리유형, GetComboData("N;MA_AISPOSTH;100").Tables[0]);
           ...
           //CNP 140312
            if (서버키 == "CNP")
            {
                _flex.SetDataMap("FG_USE", MA.GetCode("SA_B000057", false), "CODE", "NAME"); //수주용도 
                btn견적적용.Visible = false;
                btn_PRJ_SUB.Visible = false;
                btn단가적용.Visible = false;
                btn_ATP.Visible = false;
                btn_Spitem_Help.Visible = false;
            }
            else
            {
                _flex.SetDataMap("FG_USE", MA.GetCode("SA_B000057", true), "CODE", "NAME"); //수주용도 
            }
            _flex.SetDataMap("FG_USE2", MA.GetCode("SA_B000063", true), "CODE", "NAME"); //수주용도2
            _flex.SetDataMap("CD_EXCH", MA.GetCode("MA_B000005", true), "CODE", "NAME");
            _flex.SetDataMap("TP_IV", GetComboData("N;MA_AISPOSTH;100").Tables[0], "CODE", "NAME");
            _flex.SetDataMap("CLS_ITEM", MA.GetCode("MA_B000010"), "CODE", "NAME");

            if (_biz.Get업체별프로세스 == "003")
            {
                _flex.SetDataMap("PITEM_CD_USERDEF2", MA.GetCode("MA_B000082", true), "CODE", "NAME");
                _flex.SetDataMap("PITEM_CD_USERDEF3", MA.GetCode("MA_B000083", true), "CODE", "NAME");
            }

            _flex.SetDataMap("CD_USERDEF1", MA.GetCode("SA_B000125", true), "CODE", "NAME"); // 수주라인사용자정의CODE1
            ...
            if (서버키 == "DAEYONG") // 대용산업
             _flexUser.Binding = _biz.Search_DAEYONG("#%#%");
        ...
        void InitEvent()
        {
            ...
            btn계정처리유형변경.Click += new EventHandler(btn계정처리유형변경_Click);
            cur공급가액.Validated +=new EventHandler(cur공급가액_Validated);
            cur부가세액.Validated += new EventHandler(cur부가세액_Validated);

            btn담당자정보적용.Click += new EventHandler(btn담당자정보적용_Click);
            btn메일전송.Click += new EventHandler(btn메일전송_Click);
        }
        ...
        public override void OnToolBarAddButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                m_tabSo.Enabled = m_pnlLine.Enabled = pnl소요자재.Enabled = btn전자결재.Enabled = btn업체전용2.Enabled = btn견적적용.Enabled = btn_PRJ_SUB.Enabled = true;
        ...
        public override void OnToolBarDeleteButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (서버키 == "DAEYONG")
                        _biz.DeleteDaeyongPacking(txt수주번호.Text); //대용산업패킹정보삭제
        ...
        protected override bool BeforeSave()
        {
            ...
            if (서버키 == "YWD")
            {
                if (_매출자동여부 == "Y" && _자동승인여부 == "Y")
                {
                    if (!Chk매출일자 || !Chk수금예정일자) return false;
                }
            }
            else if (_biz.Get과세변경유무 == "N" && _매출자동여부 == "Y")
            {
                if (!Chk매출일자 || !Chk수금예정일자) return false;
            }
            ...
            if ((서버키 == "NEOPH" || 서버키 == "HKCOS") && _반품여부 == "Y")
            {
                if (!Chk반품사유) return false;
            }

            if (서버키 == "MHIK") // 엠에이치파워시스템즈코리아 전용
            {
                if (D.GetString(_header.CurrentRow["TXT_USERDEF4"]) == string.Empty)
                {
                    ShowMessage("수주번호를 입력하십시오.");
                    return false;
                }

                if (!_biz.Chk전용수주번호(D.GetString(_header.CurrentRow["TXT_USERDEF4"])))
                {
                    ShowMessage("해당 수주번호는 이미 등록된 건입니다.");
                    return false;
                }
            }

            if (서버키 == "DNCOMPANY") //(주)디엔컴퍼니
            {
                bool is품목 = true;
                ...
                foreach (DataRow dr in _flex.DataTable.Rows)
                {
                    ...
                    if (D.GetDecimal(dr["QT_SO"]) > (D.GetDecimal(dr["QT_USEINV"])))
                    {
                        str품목.AppendLine(D.GetString(dr["CD_ITEM"]));
                        is품목 = false;
                    }
                }
                ...
                if (!is품목)
                {
                    ...
                    Global.MainFrame.ShowDetailMessage("가용재고를 초과한 품목이 있습니다." + Environment.NewLine + "[︾] 버튼을 눌러 내역을 확인하세요.", sb.ToString());
                    return false;
                }
            }
        ...
        protected override bool SaveData()
        {
            ...
            if (dtL != null)
            {
                ...
                if (_수주상태 == "R")
                {
                    if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
                    {
                        bool isPass = Chk_SDBIO_UM(_flex.DataTable, true);
                        if (!isPass) _수주상태 = "O";
                    }

                    if (_거래구분 == "001" || 서버키 == "CHAMP") //여신체크(거래구분이 국내인것만)
                    {
                        decimal 금액 = 0;
                        string 여신구분 = BASIC.GetMAEXC("여신한도");

                        if (MA.ServerKey(false, "YWD") && 여신구분 == "200" && _출하여부 == "Y" && _자동승인여부 == "Y")
                        {
                            ...
                            obj[2] = D.GetString(_flex.DataTable.Compute("MAX(NO_PROJECT)", "ISNULL(NO_PROJECT,'') <> ''"));
                            obj[3] = Unit.원화금액(DataDictionaryTypes.SA, 금액);
                            obj[4] = dtp수주일자.Text;
                            bool isPass = _biz.GetPrjCreditChk(obj);
            ...
            if (_출하여부 == "Y" && _자동승인여부 == "Y")
            {
                ...
                if (Config.MA_ENV.LOT관리 && drs != null && drs.Length != 0)
                {
                    ...
                    if (_반품여부 == "N")
                    {
                        ...
                        {
                            DataTable dtSlMngChk = new DataTable();
                            dtSlMngChk.Columns.Add("MNG_SL", typeof(string));
                            DataTable dtGroupSl = dtL.DefaultView.ToTable(true, new string[] {"CD_PLANT", "CD_SL" });
                            foreach (DataRow row in dtGroupSl.Rows)
                            {
                                DataRow NewRow;
                                NewRow = dtSlMngChk.NewRow();
                                DataRow row창고 = BASIC.GetSL(D.GetString(row["CD_PLANT"]), D.GetString(row["CD_SL"]));
                                if (row창고 == null) continue;
                                NewRow["MNG_SL"] = row창고["MNG_SL"];
                                dtSlMngChk.Rows.Add(NewRow);
                            }
                            DataRow[] ChkSl = dtSlMngChk.Select("ISNULL(MNG_SL,'') = '004'");
                            if (ChkSl.Length > 0)
                            {
            ...
            if (서버키 == "DAEYONG")
            {
                decimal SUM_QT_SO = D.GetDecimal(_flex.DataTable.Compute("SUM(QT_SO)", ""));
                decimal SUM_QT_PACKING = D.GetDecimal(_flexUser.DataTable.Compute("SUM(QT_PACKING)", ""));
                if (SUM_QT_SO != SUM_QT_PACKING)
                {
                    DialogResult dlresult = ShowMessage("수주수량합과 패킹수량합이 다릅니다. 계속 진행하시겠습니까?", "IY2");
                    if (dlresult == DialogResult.No) return false;
                }
            } 
            #endregion

            #region -> 상보기업 할인율 변경체크

            bool 할인율변경 = false;
            DataTable 상보dt = new DataTable();
            ...
            if (서버키 == "SANGBO")
            {
                ...
                상보dt = new DataTable();
                상보dt.Columns.Add("CD_PARTNER");
                상보dt.Columns.Add("CD_ITEM");
                상보dt.Columns.Add("CD_PLANT");
                상보dt.Columns.Add("UM_ITEM", typeof(decimal));
                상보dt.Columns.Add("SDT_UM");
                상보dt.Columns.Add("NUM_USERDEF1", typeof(decimal));
                상보dt.Columns.Add("CLS_DT");
                상보dt.AcceptChanges();
                ...
                foreach (DataRow row in dtL.Rows)
                {
                    ...
                    if (D.GetDecimal(row["RT_DSCNT"]) != D.GetDecimal(row["NUM_USERDEF5"]))
                    {
                        if (D.GetDecimal(row["RT_DSCNT"]) < 0)
                        {
                            row["RT_DSCNT"] = 0;
                            continue;
                        }
                        할인율변경 = true;
                        DataRow newRow = 상보dt.NewRow();
                        newRow["CD_PARTNER"] = ctx거래처.CodeValue;
                        newRow["CD_ITEM"] = D.GetString(row["CD_ITEM"]);
                        newRow["CD_PLANT"] = D.GetString(row["CD_PLANT"]);
                        newRow["UM_ITEM"] = 0;
                        newRow["SDT_UM"] = dtp수주일자.Text;
                        newRow["NUM_USERDEF1"] = D.GetDecimal(row["RT_DSCNT"]);
                        newRow["CLS_DT"] = DateTime.ParseExact(dtp수주일자.Text, "yyyyMMdd", null).AddDays(-1).ToShortDateString().Replace("-", "");
                        상보dt.Rows.Add(newRow);
                    }

                    row["DC1"] = string.Format("할인 {0:N0}", (D.GetDecimal(row["NUM_USERDEF2"]) * D.GetDecimal(row["QT_SO"])) - (D.GetDecimal(row["UM_SO"]) * D.GetDecimal(row["QT_SO"])));
                }

                foreach (DataRow chkRow in dtL.Rows)
                {
                    if (chkRow.RowState == DataRowState.Deleted) continue;

                    if (D.GetDecimal(chkRow["AM_SO"]) == 0)
                    {
                        ShowMessage("수주금액이 없습니다");
                        return false;
                    }
                }
            }
            ...
            string[] strArr = new string[] { NO_SO, _수주상태, _거래구분, _출하형태, _매출형태, _의뢰여부, _출하여부, _매출여부, _수출여부, _구분, _반품여부, _매출자동여부, _자동승인여부};
            ...
            if (서버키 == "SANGBO")
            {
                if (할인율변경)
                {
                    if (ShowMessage("할인율을 적용하시겠습니까?", "QY2") == DialogResult.Yes)
                    {
                        _biz.상보할인율Save(상보dt);
                    }
                }
            ...
            #region 저장 후 setting 값 적용

            _biz.Set설정(DefaultSettings.계산서처리, rdo계산서처리일괄.Checked ? "001" : "002");
        ...
        public override void OnToolBarPrintButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (D.GetString(cbo원산지.SelectedValue) != "")
                {
                    DataTable 원산지dt = DBHelper.GetDataTable(string.Format("SELECT NM_SYSDEF_E FROM MA_CODEDTL WHERE CD_COMPANY = '{0}' AND CD_FIELD = 'MA_B000020' AND CD_SYSDEF = '{1}'", MA.Login.회사코드, D.GetString(cbo원산지.SelectedValue)));
                    if(원산지dt != null && 원산지dt.Rows.Count > 0) row원산지Info = 원산지dt.Rows[0];
                }

                if (ctx거래처.CodeValue != "")
                {
                    DataTable 거래처dt = DBHelper.GetDataTable(string.Format("SELECT NO_TEL1, NO_FAX1, CD_EMP_PARTNER, E_MAIL FROM MA_PARTNER WHERE CD_COMPANY = '{0}' AND CD_PARTNER = '{1}'", MA.Login.회사코드, ctx거래처.CodeValue));
                    if (거래처dt != null && 거래처dt.Rows.Count > 0) row거래처Info = 거래처dt.Rows[0];
                }
                if (ctx착하통지처.CodeValue != "")
                    row착하통지처Info = Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(MasterSearch.MA_PARTNER_DETAIL, new string[] { MA.Login.회사코드, ctx착하통지처.CodeValue });
                if (ctx수하인.CodeValue != "")
                    row수하인Info = Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(MasterSearch.MA_PARTNER_DETAIL, new string[] { MA.Login.회사코드, ctx수하인.CodeValue });
                if (ctx은행.CodeValue != "")
                    row은행Info = Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(MasterSearch.MA_PARTNER_DETAIL, new string[] { MA.Login.회사코드, ctx은행.CodeValue });
                if (ctx수출자.CodeValue != "")
                    row수출자Info = Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(MasterSearch.MA_PARTNER_DETAIL, new string[] { MA.Login.회사코드, ctx수출자.CodeValue });
                ...
                rptHelper.SetData("DC_RMK_TEXT2", txt멀티비고2.Text);
                ...
                if (row은행Info != null)
                {
                    rptHelper.SetData("CD_BANK_SO", ctx은행.CodeValue);   //거래은행
                    rptHelper.SetData("NM_BANK_SO", ctx은행.CodeName);   //거래은행명
                ...
                DataTable dt = _biz.Search_Print(txt수주번호.Text);
        ...
        void btn견적적용_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (서버키 == "SLFIRE")// || 서버키 == "DZSQL" || 서버키 == "SQL_")
                {
                    if (_flex.HasNormalRow && string.IsNullOrEmpty(txt통합견적번호.Text))
                    {
                        ShowMessage("견적적용이 아닌 데이터가 존재합니다.");
                        return;
                    }

                    P_SA_Z_SLFIRE_ESTMT_SUB dlg = new P_SA_Z_SLFIRE_ESTMT_SUB();
                    ...
                    if (!string.IsNullOrEmpty(txt통합견적번호.Text))
                        dlg.Set견적번호 = txt통합견적번호.Text;
        ...
        private void btn_PRJ_SUB_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _header.CurrentRow["NM_PROJECT"] = D.GetString(rowReturn["NM_PROJECT"]);
                _header.CurrentRow["CD_PARTNER"] = D.GetString(rowReturn["CD_PARTNER"]);
                _header.CurrentRow["LN_PARTNER"] = D.GetString(rowReturn["LN_PARTNER"]);
                _header.CurrentRow["CD_SALEGRP"] = D.GetString(rowReturn["CD_SALEGRP"]);
                _header.CurrentRow["NM_SALEGRP"] = D.GetString(rowReturn["NM_SALEGRP"]);
                ...
                if (서버키 == "KORAVL" || 서버키 == "CARGOTEC" || 서버키 == "UNIPOINT") //2012.04.19 한국AVL(PIMS:D20120409075)
                {
                ...
                foreach (DataRow dr in dsReturn.Tables[1].Rows)
                {
                    ...
                    if (Use부가세포함)
                    {
                        ...
                        if (Global.MainFrame.LoginInfo.CompanyLanguage == Language.KR)
                            ...
                            row["AM_WONAMT"] = D.GetDecimal(UDecimal.GetConvertFormatData(DataDictionaryTypes.SA, FormatTpType.MONEY, FormatFgType.INSERT, (D.GetDecimal(row["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(row["RT_VAT"]))))));
                    ...
                    row["TP_IV"] = _매출형태;
                    row["MAT_ITEM"] = D.GetString(dr["MAT_ITEM"]);

                    if (BASIC.GetMAEXC("배차사용유무") == "Y")
                        row["YN_PICKING"] = _배송여부;

                    row["CLS_ITEM"] = D.GetString(dr["CLS_ITEM"]);
            ...
            switch (서버키)
            {
                ...
                default:
                    if (_biz.Get특수단가적용 == 특수단가적용.NONE)
                    {
                        ...
                        UM_BASE = BASIC.GetUM(D.GetString(dr["CD_PLANT"]), D.GetString(dr["CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA");
            ...
            if (bChangeUM)
            {
                if (Use부가세포함)
                {
                    ...
                    if (Global.MainFrame.LoginInfo.CompanyLanguage == Language.KR)
                        ...
                        dr["AM_WONAMT"] = D.GetDecimal(UDecimal.GetConvertFormatData(DataDictionaryTypes.SA, FormatTpType.MONEY, FormatFgType.INSERT, (D.GetDecimal(dr["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(dr["RT_VAT"]))))));
        ...
        private void 출하적용_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(서버키 == "SANGBO") dtQtInv = _biz.현재고가져오기(multiItem, Global.MainFrame.GetStringToday);
                else dtQtInv = BASIC.GetQtInvMulti(multiItem, dtp수주일자.Text);
                ...
                foreach (DataRow dr in dlg.출하테이블.Rows)
                {
                    ...
                    if (BASIC.GetMAEXC("SET품 사용유무") == "Y")
                    {
                        row["CD_ITEM_REF"] = dr["CD_ITEM_REF"];
                        row["NM_ITEM_REF"] = dr["NM_SET_ITEM"];
                        row["STND_ITEM_REF"] = dr["STND_ITEM_SET"];
                    }
        ...
        private void btn_AddData_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string GI_PARTNER = string.Empty;

                if (서버키 == "SIMMONS")
                {
                    GI_PARTNER = D.GetString(_flex[_flex.Rows.Fixed, "GI_PARTNER"]);
                    if (GI_PARTNER == string.Empty)
                    {
                        ShowMessage("첫번째 라인에 납품처가 없습니다.");
                        return;
                    }
                }
                ...
                if (서버키 == "SIMMONS")
                    dlg.Str사용자정의코드1 = D.GetString(dt_partinfo.Rows[0]["CD_AREA"]);
                    ...
                    foreach (DataRow dx in dlg.ReturnTable.Rows)
                    {
                        ...
                        for (int i = 0; i < _flex.DataTable.Rows.Count; i++)
                        {
                            if (D.GetString(dx["SEQ_SO"]) == D.GetString(_flex[i + 2, "SEQ_SO"]))
                            {
                                ...
                                _flex[i + 2, "NO_ORDER"] = dx["NO_ORDER"];
                                _flex[i + 2, "NM_CUST"] = dx["NM_CUST"];
                                _flex[i + 2, "NO_TEL1"] = dx["NO_TEL1"];
                                _flex[i + 2, "NO_TEL2"] = dx["NO_TEL2"];
        ...
        private void 엑셀업로드_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable 공장품목dt = _biz.공장품목(multiItem, string.Empty);
                공장품목dt.PrimaryKey = new DataColumn[] { 공장품목dt.Columns["CD_PLANT"], 공장품목dt.Columns["CD_ITEM"] };
                ...
                if (_dt_EXCEL.Columns.Contains("NO_PROJECT"))
                {
                    dt프로젝트 = _biz.Get프로젝트();
                    dt프로젝트.PrimaryKey = new DataColumn[] { dt프로젝트.Columns["NO_PROJECT"] };
                }
                ...
                foreach (DataRow row in 엑셀dt.Rows)
                {
                    ...
                    cdItemExcel = D.GetString(row["CD_ITEM"]).ToUpper();
                    if (cdItemExcel == string.Empty) { continue; }

                    DataRow rowFindItem = 공장품목dt.Rows.Find(new object[] { D.GetString(row["CD_PLANT"]), cdItemExcel });
                    ...
                    if (품목적합)
                    {
                        bool 금액포함여부 = _dt_EXCEL.Columns.Contains("AM_SO"); //업로드양식이 금액 포함일 경우
                        ...
                        else
                        {
                            ...
                            DataRow rowSl = BASIC.GetSL(D.GetString(row["CD_PLANT"]), D.GetString(row["CD_SL"]));
                            if (rowSl == null)
                            {
                        ...
                        if (_biz.Get업체별프로세스 == "003")   //아카데미과학(PIMS:D20120716070)
                        {
                            ...
                            NewRow["PITEM_CD_USERDEF2"] = rowFindItem["CD_USERDEF2"];
                            NewRow["PITEM_CD_USERDEF3"] = rowFindItem["CD_USERDEF3"];
                        }
                        ...
                        else if (_biz.Get업체별프로세스 == "010") //대용산업
                        {
                            NewRow["PITEM_NUM_USERDEF1"] = rowFindItem["NUM_USERDEF1"];
                            NewRow["PITEM_NUM_USERDEF2"] = rowFindItem["NUM_USERDEF2"];
                            NewRow["PITEM_NUM_USERDEF3"] = rowFindItem["NUM_USERDEF3"];
                            NewRow["URL_ITEM"] = rowFindItem["URL_ITEM"];
                        }
                        ...
                        // 현재고 세팅(2011.09.19)
                        DataRow rowQtInv = dtQtInv.Rows.Find(new object[] { D.GetString(row["CD_PLANT"]), D.GetString(NewRow["CD_SL"]), cdItemExcel });
                        ...
                        {
                            한국화장품 hkcos = new 한국화장품();
                            NewRow["QT_USEINV"] = hkcos.Search가용재고(D.GetString(row["CD_PLANT"]), D.GetString(NewRow["CD_SL"]), cdItemExcel + "|");
                        }
                        ...
                        if (_biz.Get특수단가적용 == 특수단가적용.NONE)
                        {
                            if (disCount_YN == "Y")
                            {
                                ...
                                {
                                    NewRow["UM_BASE"] = Unit.외화단가(DataDictionaryTypes.SA, BASIC.GetUM(D.GetString(NewRow["CD_PLANT"]), D.GetString(NewRow["CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA"));
                            ...
                            {
                                if (서버키 == "CNP" || 서버키 == "GNSD" || PageID == "P_SA_Z_KORFNT_SO")
                                {
                                    if ((_단가적용형태 == "002" || _단가적용형태 == "003"))
                                    {
                                        decimal 적용단가 = BASIC.GetUM(D.GetString(NewRow["CD_PLANT"]), D.GetString(NewRow["CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA");

                                        if (서버키 == "CNP")
                                        {
                                            DataTable dt = MA.GetCode("SA_B000057");
                                            string filter = string.Format("CODE = '{0}'", D.GetString(row["FG_USE"]));
                                            DataRow[] rows = dt.Select(filter);
                                            string CD_FLAG1 = "";
                                            if (rows != null && rows.Length > 0) CD_FLAG1 = D.GetString(rows[0]["CD_FLAG1"]);
                                            if (CD_FLAG1 == "Y") 적용단가 = 0;
                                        }

                                        if (Use부가세포함)    //부가세포함단가 사용 시 할인율은 일단 배제하였음(2011.11.11)
                                            NewRow["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, 적용단가);
                                        else
                                            NewRow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, 적용단가);

                                        if (BASIC.GetMAEXC_Menu("P_SA_SO", "SA_A00000001") == "001")
                                            NewRow["NUM_USERDEF1"] = D.GetDecimal(NewRow["UM_SO"]);
                                    }
                                    else //엑셀 업로드시 엑셀 업로드 양식이 1.단가미포함 2.단가포함 이 있는데 단가가 있고 단가적용이 사용안하는 품목의 단가를 셋팅한다.
                        ...
                        else if (_biz.Get특수단가적용 == 특수단가적용.중량단가)
                        {
                            if ((_단가적용형태 == "002" || _단가적용형태 == "003"))
                            {
                                ...
                                {
                                    NewRow["UM_OPT"] = Unit.외화단가(DataDictionaryTypes.SA, BASIC.GetUM(D.GetString(NewRow["CD_PLANT"]), D.GetString(NewRow["CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA"));
                                }
                            }
                        ...
                        else if (_biz.Get특수단가적용 == 특수단가적용.거래처별고정단가)   //2011.12.28 SJH(PIMS:D20111226030) : 제원요청(할인율, 단가통제 사용안함)
                        {
                            ...
                            if (rowUmFixed == null)
                                NewRow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, BASIC.GetUM(D.GetString(NewRow["CD_PLANT"]), D.GetString(NewRow["CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA"));
                            else
                                NewRow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(rowUmFixed["UM_FIXED"]));
                        }

                        if ((Use부가세포함 && 서버키 == "CNP") || (Use부가세포함 && PageID == "P_SA_Z_KORFNT_SO"))
                        {
                            NewRow["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(NewRow["QT_SO"]) * D.GetDecimal(NewRow["UMVAT_SO"]));
                            NewRow["AM_WONAMT"] = Decimal.Round(D.GetDecimal(NewRow["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(NewRow["RT_VAT"]))), MidpointRounding.AwayFromZero);
                            NewRow["AM_VAT"] = D.GetDecimal(NewRow["AMVAT_SO"]) - D.GetDecimal(NewRow["AM_WONAMT"]);
                            NewRow["AM_SO"] = cur환율.DecimalValue == 0m ? 0m : Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(NewRow["AM_WONAMT"]) / cur환율.DecimalValue);
                            NewRow["UM_SO"] = D.GetDecimal(NewRow["QT_SO"]) == 0m ? 0m : Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(NewRow["AM_SO"]) / D.GetDecimal(NewRow["QT_SO"]));
                        }
                        else if (금액포함여부)
                        {

                            decimal AM_SO = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_SO"]));
                            decimal QT_SO = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(NewRow["QT_SO"]));
                            decimal UM_SO = QT_SO == 0 ? 0 : AM_SO / QT_SO;

                            NewRow["AM_SO"] = AM_SO;
                            NewRow["UM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, UM_SO);
                            NewRow["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, AM_SO * cur환율.DecimalValue);
                        ...
                        if (PageID == "P_SA_Z_KORFNT_SO")   //한국가구
                        {
                            NewRow["NUM_USERDEF3"] = NewRow["UMVAT_SO"];
                            NewRow["NUM_USERDEF4"] = NewRow["AMVAT_SO"];
                        }
                        ...
                        NewRow["TP_IV"] = _매출형태;
                        NewRow["MAT_ITEM"] = rowFindItem["MAT_ITEM"];
                        NewRow["CLS_ITEM"] = rowFindItem["CLS_ITEM"];

                        if (BASIC.GetMAEXC("배차사용유무") == "Y")
                            NewRow["YN_PICKING"] = _배송여부;

                        //배송지정보 추가
                        if (엑셀dt.Columns.Contains("NM_CUST_DLV")) NewRow["NM_CUST_DLV"] = row["NM_CUST_DLV"];
                        if (엑셀dt.Columns.Contains("CD_ZIP")) NewRow["CD_ZIP"] = row["CD_ZIP"];
                        if (엑셀dt.Columns.Contains("ADDR1")) NewRow["ADDR1"] = row["ADDR1"];
                        if (엑셀dt.Columns.Contains("ADDR2")) NewRow["ADDR2"] = row["ADDR2"];
                        if (엑셀dt.Columns.Contains("NO_TEL_D1")) NewRow["NO_TEL_D1"] = row["NO_TEL_D1"];
                        if (엑셀dt.Columns.Contains("NO_TEL_D2")) NewRow["NO_TEL_D2"] = row["NO_TEL_D2"];
                        if (엑셀dt.Columns.Contains("TP_DLV")) NewRow["TP_DLV"] = row["TP_DLV"];
                        if (엑셀dt.Columns.Contains("DC_REQ")) NewRow["DC_REQ"] = row["DC_REQ"];
                        if (엑셀dt.Columns.Contains("NO_ORDER")) NewRow["NO_ORDER"] = row["NO_ORDER"];
                        if (엑셀dt.Columns.Contains("NM_CUST")) NewRow["NM_CUST"] = row["NM_CUST"];
                        if (엑셀dt.Columns.Contains("NO_TEL1")) NewRow["NO_TEL1"] = row["NO_TEL1"];
                        if (엑셀dt.Columns.Contains("NO_TEL2")) NewRow["NO_TEL2"] = row["NO_TEL2"];
                        if (엑셀dt.Columns.Contains("DLV_TXT_USERDEF1")) NewRow["DLV_TXT_USERDEF1"] = row["DLV_TXT_USERDEF1"];
                        if (엑셀dt.Columns.Contains("NO_LINK")) NewRow["NO_LINK"] = row["NO_LINK"];
                        if (엑셀dt.Columns.Contains("NO_LINE_LINK")) NewRow["NO_LINE_LINK"] = row["NO_LINE_LINK"];

                        if (서버키 == "NEOPH")
                        {
                            string 수주용도2 = D.GetString(rowFindItem["CD_USERDEF1"]);
                            decimal 판매단가 = D.GetDecimal(NewRow["UM_SO"]);
                            if (수주용도2 == "100" && 판매단가 == 0) 수주용도2 = "101";

                            NewRow["FG_USE2"] = 수주용도2;
                        }

                        if (_dt_EXCEL.Columns.Contains("NO_PROJECT"))
                        {
                            DataRow row프로젝트 = dt프로젝트.Rows.Find(D.GetString(row["NO_PROJECT"]));
                            if (row프로젝트 != null)
                            {
                                NewRow["NO_PROJECT"] = row["NO_PROJECT"];
                                NewRow["NM_PROJECT"] = row프로젝트["NM_PROJECT"];
                            }
                        }
                        
                        if (서버키 == "DNCOMPANY")
                        {
                            DataRow rowQtMiGi = _biz.Get_SearchMiGi(D.GetString(NewRow["CD_PLANT"]), D.GetString(NewRow["CD_ITEM"]));
                            NewRow["QT_USEINV"] = rowQtMiGi == null ? D.GetDecimal(NewRow["SL_QT_INV"]) : D.GetDecimal(NewRow["SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_GI_REMAIN"]);
                        }
        ...
        private void m_btnAppend_Click(object sender, EventArgs e)
        {
            try
            {
                if (서버키 != "HKCOS" && 서버키 != "TOKIMEC" && 서버키 != "DAEYONG")
                {
                    if (!Chk확정여부()) return;
                }
                ...
                if (Global.MainFrame.ServerKeyCommon == "CNP")
                {
                    _flex["FG_USE"] = "001";
                }
        ...
        private void m_btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (서버키 == "DAEYONG")
                {
                    DataRow[] dr = _flexUser.DataTable.Select("SEQ_SO = '" + _flex["SEQ_SO"].ToString() + "'", "", DataViewRowState.CurrentRows);
        ...
        private void btn_apply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt = null;
                if (서버키 == "SANGBO") dt = _biz.현재고가져오기(cdPlant, cdSl, multiItem, Global.MainFrame.GetStringToday);
                else dt = BASIC.GetQtInvMulti(cdPlant, cdSl, multiItem, dtp수주일자.Text);

                dt.PrimaryKey = new DataColumn[] { dt.Columns["CD_PLANT"], dt.Columns["CD_SL"], dt.Columns["CD_ITEM"] };

                DataTable dtQtMiGi = null;
                if (서버키 == "KOREAF" || 서버키 == "ACADEMY" || 서버키 == "CNP" || PageID == "P_SA_Z_KORFNT_SO")    //가용재고(현재고 - 미출하수량) 2012.02.24(PIMS:D20120220022)
                {
                ...
                foreach (DataRow row in ldrchk)
                {
                    ...
                    if (서버키 == "KOREAF" || 서버키 == "ACADEMY" || PageID == "P_SA_Z_KORFNT_SO")    //가용재고(현재고 - 미출하수량) 2012.02.24(PIMS:D20120220022)
                    {
                    ...
                    else if (서버키 == "CNP")
                    {
                        ...
                        {
                            row["QT_EXP"] = D.GetDecimal(rowQtMiGi["QT_MI_GI"]);
                            row["QT_USEINV"] = D.GetDecimal(row["SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_MI_GI"]);
                        }
                        row["QT_AVA"] = D.GetDecimal(row["QT_USEINV"]) - D.GetDecimal(row["QT_SO"]);
                        row["NO_LOT"] = _biz.SearchNoLotCNP(cdSl, D.GetString(row["CD_ITEM"]));
                    }
                    else if (서버키 == "DNCOMPANY")
                    {
                        DataRow rowQtMiGi = _biz.Get_SearchMiGi(D.GetString(row["CD_PLANT"]), D.GetString(row["CD_ITEM"]));
                        row["QT_USEINV"] = rowQtMiGi == null ? D.GetDecimal(row["SL_QT_INV"]) : D.GetDecimal(row["SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_GI_REMAIN"]);
                    }
                }
        ...
        /// 대용산업
        /// </summary>
        void btnPackingInfo_DAEYONG_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Chk거래처 || !Chk확정여부()) return;
                ...
                string multi_cd_item = string.Empty;
                ...
                foreach (DataRow dr in _flex.DataTable.Rows)
                {
                    ...
                    if (D.GetDecimal(dr["QT_SO"]) == 0)
                    {
                        ShowMessage("수량이 0인 품목이 있습니다.");
                        return;
                    }

                    multi_cd_item += D.GetString(dr["CD_ITEM"]) + "|";

                    dt.ImportRow(dr);
                }

                P_SA_Z_DAEYONG_PACKING_SUB dlg = new P_SA_Z_DAEYONG_PACKING_SUB(dt, _flexUser.DataTable, PageID, ctx거래처.CodeValue, multi_cd_item);
        ...
        /// 지니틱스
        /// </summary>
        void btn계획적용_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Chk거래처 || !Chk영업그룹 || !Chk수주유형 || !Chk화폐단위 || !Chk과세구분) return;
                ...
                P_SA_Z_NICEGR_SA_PLAN_SUB dlg = new P_SA_Z_NICEGR_SA_PLAN_SUB(D.GetString(ctx거래처.CodeValue), D.GetString(ctx거래처.CodeName), D.GetString(cbo화폐단위.SelectedValue), D.GetString(cbo화폐단위.Text));

                if (dlg.ShowDialog() != DialogResult.OK) return;

                판매계획품목셋팅(dlg.GetData);
            }
            catch (Exception ex)
            {
                MsgEnd(ex);
        ...
        /// 에스디바이오센서(주)
        /// </summary>
        void btn프로모션전개_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Chk거래처 || !Chk영업그룹 || !Chk수주유형 || !Chk화폐단위 || !Chk과세구분) return;
                ...
                P_SA_Z_SDBIO_PROMOTION_SUB m_dlg = new P_SA_Z_SDBIO_PROMOTION_SUB(D.GetString(cbo공장.SelectedValue), ctx거래처.CodeValue, ctx거래처.CodeName,
                ...
                프로모션품목셋팅(m_dlg.GetRowsH, false);
                프로모션품목셋팅(m_dlg.GetRowsD, true);
            }
            catch (Exception ex)
            {
                MsgEnd(ex);
        ...
        /// 차앤박
        /// </summary>
        void btn엑셀내려받기_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string localPath = dlg.SelectedPath + "\\" + "수주등록_엑셀업로드 양식.xls";
                string serverPath = Global.MainFrame.HostURL + "/shared/SA/" + "수주등록_엑셀업로드 양식.xls";

                System.Net.WebClient client = new System.Net.WebClient();
                client.DownloadFile(serverPath, localPath);
                ShowMessage(공통메세지._작업을완료하였습니다, DD("다운로드"));
            }
            catch (Exception ex)
            {
                MsgEnd(ex);
        ...
        /// 상보기업
        /// </summary>
        void btn현재고도움창_Click(object sender, EventArgs e)
        {
            try
            {
                if (!_flex.HasNormalRow) return;
                
                P_PU_STOCK_SUB m_dlg = new P_PU_STOCK_SUB(D.GetString(_flex["CD_PLANT"]), D.GetString(_flex["CD_ITEM"]));
        ...
        /// 신진에스엠
        /// </summary>
        void btn본사발주적용_Click(object sender, EventArgs e)
        {
            try
            {
                if (견적적용건())
                ...
                H_SA_Z_SINJINSM_PM_ORDER_SUB dlg = new H_SA_Z_SINJINSM_PM_ORDER_SUB(ctx거래처.CodeValue, ctx거래처.CodeName);
                if (dlg.ShowDialog() != DialogResult.OK) return;

                DataRow[] returnRows = dlg.ReturnDataRows;
                cbo화폐단위.SelectedValue = D.GetString(returnRows[0]["CD_EXCH"]);
                cur환율.DecimalValue = D.GetDecimal(returnRows[0]["RT_EXCH"]);
                cbo부가세구분.SelectedValue = D.GetString(returnRows[0]["TP_VAT"]);
                cur부가세율.DecimalValue = D.GetDecimal(returnRows[0]["RT_VAT"]);
                txt비고.Text = D.GetString(returnRows[0]["REMARK_H"]);
                //ctx거래처.CodeValue = D.GetString(returnRows[0]["BRANCH"]);
                //ctx거래처.CodeName = D.GetString(returnRows[0]["LN_PARTNER"]);
                _header.CurrentRow["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                _header.CurrentRow["RT_EXCH"] = cur환율.DecimalValue;
                _header.CurrentRow["TP_VAT"] = D.GetString(cbo부가세구분.SelectedValue);
                _header.CurrentRow["RT_VAT"] = cur부가세율.DecimalValue;
                _header.CurrentRow["DC_RMK"] = txt비고.Text;
                //_header.CurrentRow["CD_PARTNER"] = ctx거래처.CodeValue;
                ...
                foreach (DataRow row in returnRows)
                {
                    ...
                    CC조회(D.GetString(row["MTCODE"]), out cdCc, out nmCc);

                    newRow["SEQ_SO"] = idx;
                    newRow["CD_PLANT"] = Global.MainFrame.LoginInfo.CdPlant;
                    newRow["CD_ITEM"] = row["CD_ITEM"];
                    newRow["NM_ITEM"] = row["NM_ITEM"];
                    newRow["STND_ITEM"] = row["STND_ITEM"];
                    newRow["UNIT_SO"] = row["UNIT_SO"];
                    newRow["EN_ITEM"] = row["EN_ITEM"];
                    newRow["STND_DETAIL_ITEM"] = row["STND_DETAIL_ITEM"];
                    newRow["TP_ITEM"] = row["ITEMTYPE"];
                    newRow["LT_GI"] = D.GetDecimal(row["LT_GI"]);
                    newRow["WEIGHT"] = row["WEIGHT"];
                    newRow["UNIT_WEIGHT"] = row["UNIT_WEIGHT"];
                    newRow["FG_SERNO"] = row["FG_SERNO"];
                    newRow["YN_ATP"] = row["YN_ATP"];
                    newRow["CUR_ATP_DAY"] = row["CUR_ATP_DAY"];
                    newRow["FG_MODEL"] = row["FG_MODEL"];
                    newRow["DT_DUEDATE"] = row["DUEDATE"];
                    newRow["DT_REQGI"] = _CommFun.DateAdd(D.GetString(row["DUEDATE"]), "D", D.GetInt(row["LT_GI"]) * -1);
                    newRow["DC1"] = D.GetString(row["REMARK_L"]);
                    newRow["CD_CC"] = cdCc;
                    newRow["NM_CC"] = nmCc;
                    newRow["TP_VAT"] = D.GetString(cbo부가세구분.SelectedValue);
                    newRow["RT_VAT"] = cur부가세율.DecimalValue;
                    newRow["GI_PARTNER"] = ctx납품처.CodeValue;
                    newRow["LN_PARTNER"] = ctx납품처.CodeName;
                    newRow["QT_SO"] = D.GetString(row["QTY"]);
                    newRow["UNIT_SO_FACT"] = D.GetDecimal(row["UNIT_SO_FACT"]) == 0m ? 1m : row["UNIT_SO_FACT"];
                    newRow["UNIT_GI_FACT"] = D.GetDecimal(row["UNIT_GI_FACT"]) == 0m ? 1m : row["UNIT_GI_FACT"];
                    newRow["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(newRow["QT_SO"]) * D.GetDecimal(newRow["UNIT_SO_FACT"]));
                    newRow["UNIT_IM"] = D.GetString(row["UNIT_IM"]);
                    newRow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row["UNITPRICE"]));
                    newRow["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AMOUNT"]));
                    newRow["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]));
                    newRow["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_VAT"]));
                    newRow["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]) + D.GetDecimal(row["AM_VAT"]));
                    newRow["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(newRow["AMVAT_SO"]) / D.GetDecimal(newRow["QT_SO"]));
                    newRow["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                    newRow["TP_IV"] = _매출형태;
                    newRow["MAT_ITEM"] = D.GetString(row["MAT_ITEM"]);
                    if (_구분 != "복사") newRow["STA_SO1"] = _수주상태;
                    newRow["NO_RELATION"] = row["PONO"];
                    newRow["SEQ_RELATION"] = row["POSUBNO"];
                    newRow["CLS_ITEM"] = row["CLS_ITEM"];
        ...
        void btn견적적용신진_Click(object sender, EventArgs e)
        {
            try
            {
                if (견적적용건())
                ...
                H_SA_Z_SINJINSM_EM_ORDER_SUB dlg = new H_SA_Z_SINJINSM_EM_ORDER_SUB(ctx거래처.CodeValue, ctx거래처.CodeName);
                if (dlg.ShowDialog() != DialogResult.OK) return;
                ...
                foreach (DataRow row in returnRows)
                {
                    ...
                    newRow["MAT_ITEM"] = D.GetString(row["MAT_ITEM"]);
                    if (_구분 != "복사") newRow["STA_SO1"] = _수주상태;
                    newRow["NO_RELATION"] = row["EMNO"];
                    newRow["SEQ_RELATION"] = row["EMSUBNO"];
                    newRow["CLS_ITEM"] = row["CLS_ITEM"];
                    dt.Rows.Add(newRow.ItemArray);
                    idx++;
                }
        ...
        /// 상보기업
        /// </summary>
        void btn수주이력_Click(object sender, EventArgs e)
        {
            try
            {
                object[] args = new Object[] { ctx거래처.CodeValue, ctx거래처.CodeName };
                CallOtherPageMethod("P_SA_SOSCH", "수주리스트", Grant, args);
            }
            catch (Exception ex)
            {
                MsgEnd(ex);
        ...
        /// (주)케이피아이씨코포레이션
        /// </summary>
        void btn계약적용_Click(object sender, EventArgs e)
        {
            try
            {
                H_SA_Z_KPCI_CONTRACT_SUB dlg = new H_SA_Z_KPCI_CONTRACT_SUB(D.GetString(cbo화폐단위.SelectedValue));
                if (dlg.ShowDialog() != DialogResult.OK) return;

                DataRow[] returnRows = dlg.ReturnDataRows;

                ctx거래처.CodeValue = D.GetString(returnRows[0]["CD_PARTNER"]);
                ctx거래처.CodeName = D.GetString(returnRows[0]["LN_PARTNER"]);
                ctx영업그룹.CodeValue = D.GetString(returnRows[0]["CD_SALEGRP"]);
                ctx영업그룹.CodeName = D.GetString(returnRows[0]["NM_SALEGRP"]);
                ctx수주형태.CodeValue = D.GetString(returnRows[0]["SETTLE_KIND"]);
                ctx수주형태.CodeName = D.GetString(returnRows[0]["NM_SO"]);
                수주형태변경시셋팅(D.GetString(returnRows[0]["SETTLE_KIND"]));
                txt비고.Text = D.GetString(returnRows[0]["NO_ORDER"]);
                ...
                {
                    txt매출비고.Text = D.GetString(returnRows[0]["NO_ORDER"]);
                    _header.CurrentRow["DC_RMK_IVH"] = txt매출비고.Text;
                }
                ...
                foreach (DataRow row in returnRows)
                {
                    ...
                    newRow["GI_PARTNER"] = ctx납품처.CodeValue;
                    newRow["LN_PARTNER"] = ctx납품처.CodeName;
                    newRow["QT_SO"] = D.GetString(row["QT_CON"]);
                    newRow["UNIT_SO_FACT"] = D.GetDecimal(row["UNIT_SO_FACT"]) == 0m ? 1m : row["UNIT_SO_FACT"];
                    newRow["UNIT_GI_FACT"] = D.GetDecimal(row["UNIT_GI_FACT"]) == 0m ? 1m : row["UNIT_GI_FACT"];
                    newRow["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(newRow["QT_SO"]) * D.GetDecimal(newRow["UNIT_SO_FACT"]));
                    newRow["UNIT_IM"] = D.GetString(row["UNIT_IM"]);
                    newRow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row["UM_UNIT"]));
                    newRow["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_AMOUNT_EX"]));
                    newRow["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_AMOUNT_EX"]) * cur환율.DecimalValue);
                    newRow["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newRow["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100m));
                    newRow["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newRow["AM_WONAMT"]) + D.GetDecimal(newRow["AM_VAT"]));
                    newRow["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(newRow["AMVAT_SO"]) / D.GetDecimal(newRow["QT_SO"]));
                    newRow["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                    newRow["TP_IV"] = _매출형태;
                    newRow["MAT_ITEM"] = D.GetString(row["MAT_ITEM"]);
                    if (_구분 != "복사") newRow["STA_SO1"] = _수주상태;
                    newRow["NO_RELATION"] = row["NO_CONTRACT"];
                    newRow["SEQ_RELATION"] = row["NO_LINE"];
                    newRow["CLS_ITEM"] = row["CLS_ITEM"];
                    newRow["CD_SL"] = row["CD_GISL"];
                    newRow["NM_SL"] = row["NM_GISL"];
                    newRow["DC1"] = row["NO_CONTRACT"];
                    dt.Rows.Add(newRow.ItemArray);
                    idx++;
                }
        ...
        /// 신진에스엠
        /// </summary>
        void btn품목자동생성_Click(object sender, EventArgs e)
        {
            try
            {
                if (!FieldCheck("")) return;

                H_SA_Z_SINJINSM_PITEM_AUTO_CREATE_SUB dlg = new H_SA_Z_SINJINSM_PITEM_AUTO_CREATE_SUB();
                if (dlg.ShowDialog() != DialogResult.OK) return;
                ...
                foreach (DataRow row in returnRows)
                {
                    ...
                    if (_구분 != "복사") newRow["STA_SO1"] = _수주상태;
                    newRow["CLS_ITEM"] = row["CLS_ITEM"];
                    newRow["GRP_ITEM"] = row["GRP_ITEM"];
                    newRow["CLS_L"] = row["CLS_L"];
                    newRow["CLS_S"] = row["CLS_S"];
                    newRow["NUM_STND_ITEM_1"] = row["NUM_STND_ITEM_1"];
                    newRow["NUM_STND_ITEM_2"] = row["NUM_STND_ITEM_2"];
                    newRow["NUM_STND_ITEM_3"] = row["NUM_STND_ITEM_3"];
                    newRow["NUM_STND_ITEM_4"] = row["NUM_STND_ITEM_4"];
                    newRow["NUM_STND_ITEM_5"] = row["NUM_STND_ITEM_5"];
                    dt.Rows.Add(newRow.ItemArray);
                    idx++;
                }
        ...
        /// 신진에스엠
        /// </summary>
        void btn수주가계산_Click(object sender, EventArgs e)
        {
            try
            {
                if (!_flex.HasNormalRow) return;
                ...
                MsgControl.ShowMsg("수주가계산 중.......");

                H_SA_Z_SINJINSM_UM_GUBUN_SUB dlg = new H_SA_Z_SINJINSM_UM_GUBUN_SUB();
                if (dlg.ShowDialog() != DialogResult.OK) return;

                DataRow dr = null;
                DataSet ds = null;
                List<object> list = null;
                string CD_PARTNER = string.Empty;

                foreach (DataRow row in _flex.DataTable.Rows)
                {
                    if (row.RowState == DataRowState.Deleted) continue;

                    if (D.GetString(row["CD_PLANT"]) == "1000")
                        CD_PARTNER = "00042"; //장수
                    else if (D.GetString(row["CD_PLANT"]) == "2000")
                        CD_PARTNER = "00623"; //정남
                    else if (D.GetString(row["CD_PLANT"]) == "3000")
                        CD_PARTNER = "05132"; //동탄
                    else if (D.GetString(row["CD_PLANT"]) == "4000")
                        CD_PARTNER = "05135"; //발안
                    else if (D.GetString(row["CD_PLANT"]) == "5000")
                        CD_PARTNER = "05558"; //천안

                    if (D.GetString(row["CLS_L"]) == "100")
                    {
                        list = new List<object>();
                        list.Add(MA.Login.회사코드);                //회사
                        list.Add(CD_PARTNER);                      //대리점코드
                        list.Add(string.Empty);                    //고객코드
                        list.Add(row["CLS_L"]);                    //수주종류
                        list.Add(row["CLS_S"]);                    //제품분류
                        list.Add(row["GRP_ITEM"]);                 //품목군
                        list.Add(row["NUM_STND_ITEM_1"]);          //두께
                        list.Add(row["NUM_STND_ITEM_2"]);          //폭
                        list.Add(row["NUM_STND_ITEM_3"]);          //길이
                        list.Add(row["QT_SO"]);                    //주문수량
                        list.Add(_header.CurrentRow["TP_VAT"]);
                        list.Add(_header.CurrentRow["CD_EXCH"]);
                        list.Add(_header.CurrentRow["RT_EXCH"]);
                        list.Add(string.Empty);
                        list.Add(dlg.ReturnStr);                   // D판매단가, B공급단가
                    }
                    else if (D.GetString(row["CLS_L"]) == "200")
                    {
                        list = new List<object>();
                        list.Add(MA.Login.회사코드);                //회사
                        list.Add(CD_PARTNER);                      //대리점코드
                        ...
                        list.Add(row["NUM_STND_ITEM_2"]);          //폭
                        list.Add(row["NUM_STND_ITEM_3"]);          //길이
                        list.Add(row["NUM_STND_ITEM_4"]);          //
                        list.Add(row["NUM_STND_ITEM_5"]);          //
                        list.Add(row["QT_SO"]);                    //주문수량
                        list.Add(_header.CurrentRow["TP_VAT"]);
                        list.Add(_header.CurrentRow["CD_EXCH"]);
                        list.Add(_header.CurrentRow["RT_EXCH"]);
                    ...
                    else if (D.GetString(row["CLS_L"]) == "300" || D.GetString(row["CLS_L"]) == "400")
                    {
                        list = new List<object>();
                        list.Add(MA.Login.회사코드);                //회사
                        list.Add(CD_PARTNER);                      //대리점코드
                        list.Add(dtp수주일자.Text);
                        list.Add(row["CLS_L"]);                    //수주종류
                        list.Add(row["CD_PLANT"]);                 //공장코드
                        list.Add(row["CD_ITEM"]);                  //품목코드
                        list.Add(row["QT_SO"]);                    //주문수량
                        list.Add(_header.CurrentRow["TP_VAT"]);
                        list.Add(_header.CurrentRow["CD_EXCH"]);
                        list.Add(_header.CurrentRow["RT_EXCH"]);
                    ...
                    if (!(D.GetString(row["CLS_L"]) == "100" || D.GetString(row["CLS_L"]) == "200" || D.GetString(row["CLS_L"]) == "300" || D.GetString(row["CLS_L"]) == "400")) continue;

                    ds = _biz.수주가계산(list.ToArray(), D.GetString(row["CLS_L"]));
                    dr = ds.Tables[0].Rows[0];

                    if (D.GetString(row["CLS_L"]) == "100")
                        row["UM_SO"] = dlg.ReturnStr == "D" ? dr["BRANCH_PRICE"] : dr["UNIT_PRICE"];
                    else if (D.GetString(row["CLS_L"]) == "200")
                        row["UM_SO"] = dlg.ReturnStr == "D" ? dr["UNITPRICE"] : dr["UNIT_PRICE"];
                    else if (D.GetString(row["CLS_L"]) == "300" || D.GetString(row["CLS_L"]) == "400")
                        row["UM_SO"] = dr["UNIT_PRICE"];

                    row["AM_SO"] = dr["AMOUNT"];
                    row["AM_WONAMT"] = dr["AM_WONAMT"];
                    row["AM_VAT"] = dr["AM_VAT"];
                    row["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]) + D.GetDecimal(row["AM_VAT"]));
                    row["UMVAT_SO"] = D.GetDecimal(row["QT_SO"]) == 0 ? 0m : Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(row["AMVAT_SO"]) / D.GetDecimal(row["QT_SO"]));
        ...
        /// 신진에스엠
        /// </summary>
        void btn발주적용_Click(object sender, EventArgs e)
        {
            try
            {
                if (견적적용건())
                ...
                if (!Chk영업그룹 || !Chk수주유형 || !Chk화폐단위 || !Chk과세구분) return;

                H_SA_Z_SINJINSM_PO_APPPLY_SUB dlg = new H_SA_Z_SINJINSM_PO_APPPLY_SUB(D.GetString(cbo화폐단위.SelectedValue));
                if (dlg.ShowDialog() != DialogResult.OK) return;

                ctx거래처.CodeValue = D.GetString(dlg.ReturnData[0]["CD_PARTNER"]);
                ctx거래처.CodeName = D.GetString(dlg.ReturnData[0]["LN_PARTNER"]);
                _header.CurrentRow["CD_PARTNER"] = ctx거래처.CodeValue;
                ...
                foreach (DataRow row in dlg.ReturnData)
                {
                    if (row.RowState == DataRowState.Deleted) continue;
                    ...
                    newRow["GI_PARTNER"] = ctx납품처.CodeValue;
                    newRow["LN_PARTNER"] = ctx납품처.CodeName;
                    newRow["QT_SO"] = D.GetString(row["QT_PO"]);
                    newRow["UNIT_SO_FACT"] = D.GetDecimal(row["UNIT_SO_FACT"]) == 0m ? 1m : row["UNIT_SO_FACT"];
                    newRow["UNIT_GI_FACT"] = D.GetDecimal(row["UNIT_GI_FACT"]) == 0m ? 1m : row["UNIT_GI_FACT"];
                    newRow["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(newRow["QT_SO"]) * D.GetDecimal(newRow["UNIT_SO_FACT"]));
                    newRow["UNIT_IM"] = D.GetString(row["UNIT_IM"]);
                    newRow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row["UM_EX_PO"]));
                    newRow["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_EX"]));
                    newRow["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_EX"]) * cur환율.DecimalValue);
                    newRow["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newRow["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100));
                    newRow["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newRow["AM_WONAMT"]) + D.GetDecimal(newRow["AM_VAT"]));
                    newRow["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(newRow["AMVAT_SO"]) / D.GetDecimal(newRow["QT_SO"]));
                    newRow["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                    newRow["TP_IV"] = _매출형태;
        ...
        void btn첨부파일_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (서버키 == "CIS") // 씨아이에스 영업폴더 밑에 첨부파일 저장 
                    {
        ...
        void btn전자결재_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
                {
                    bool isPass = Chk_SDBIO_UM(_flex.DataTable, false);
                    if (!isPass) return;
        ...
        void btn할인율적용_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (서버키 != "WINPLUS" && cur할인율.DecimalValue == decimal.Zero) return;
                ...
                foreach (DataRow row in drs)
                {
                    ...
                    {
                        row["RT_DSCNT"] = cur할인율.DecimalValue;

                        if (D.GetDecimal(row["UM_BASE"]) != 0)
                        {
                            if (서버키 == "KFL") // (한국화스너전용 : 할인율 KEY-IN시 단가 올림). 2011.06.13 SJH
                                row["UM_SO"] = decimal.Ceiling(D.GetDecimal(row["UM_BASE"]) - (D.GetDecimal(row["UM_BASE"]) * D.GetDecimal(row["RT_DSCNT"])) / 100);
                            else
                                row["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row["UM_BASE"]) - (D.GetDecimal(row["UM_BASE"]) * D.GetDecimal(row["RT_DSCNT"])) / 100);
                        }
                        else
                        ...
                        row["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row["QT_SO"]) * D.GetDecimal(row["UM_SO"]));
                        row["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_SO"]) * cur환율.DecimalValue);
                        row["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]) * (D.GetDecimal(row["RT_VAT"]) / 100));
                        row["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]) + D.GetDecimal(row["AM_VAT"]));
                    ...
                    if (_biz.Get업체별프로세스 == "001")      //아사히카세이  
                        row["NUM_USERDEF1"] = D.GetDecimal(row["AREA"]) == 0m ? 0m : Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(row["UM_SO"]) / D.GetDecimal(row["AREA"]));
                    else if (_biz.Get업체별프로세스 == "009") //에스디바이오센서(주)
                    {
                        if (D.GetDecimal(row["UM_SO"]) < D.GetDecimal(row["NUM_USERDEF5"]))
                            row["TXT_USERDEF1"] = "Y";
                        else
                            row["TXT_USERDEF1"] = "N";

                        row["NUM_USERDEF3"] = D.GetDecimal(row["AM_WONAMT"]) - D.GetDecimal(row["NUM_USERDEF2"]);
                        row["NUM_USERDEF4"] = D.GetDecimal(row["AM_WONAMT"]) != 0m ? decimal.Round((D.GetDecimal(row["AM_WONAMT"]) - D.GetDecimal(row["NUM_USERDEF2"])) / D.GetDecimal(row["AM_WONAMT"]) * 100m, 2, MidpointRounding.AwayFromZero) : 0m;
                    }
                }
        ...
        #region ▷ 계정처리유형 적용           
        void btn계정처리유형변경_Click(object sender, EventArgs e)
        {
            try
            {
                if (!_flex.HasNormalRow) return;
                ...
                foreach (DataRow dr in _flex.DataTable.Rows)
                {
                    ...
                    dr["TP_IV"] = D.GetString(cbo계정처리유형.SelectedValue);
                }
            }
            catch (Exception ex)
            {
        ...
        void btn담당자정보적용_Click(object sender, EventArgs e)
        {
            try
            {
                if (ctx거래처.CodeValue == string.Empty)
                {
                    ShowMessage("거래처를 입력해주세요.");
                    return;
                }
                ...
                P_FI_PARTNERPTR_SUB objSub = new P_FI_PARTNERPTR_SUB(ctx거래처.CodeValue, obj);
                if (objSub.ShowDialog() != DialogResult.OK) return;

                object[] rtn = objSub.ReturnValues;
                if (rtn == null) return;

                txt거래처담당자.Text = D.GetString(rtn[0]);
                _header.CurrentRow["NM_PTR"] = D.GetString(rtn[0]);
                txt담당자이메일.Text = D.GetString(rtn[1]);
                _header.CurrentRow["EX_EMIL"] = D.GetString(rtn[1]);
                txt담당자핸드폰.Text = D.GetString(rtn[2]);
                _header.CurrentRow["EX_HP"] = D.GetString(rtn[2]);
            }
            catch (Exception ex)
            {
                MsgEnd(ex.Message);
        ...
        #region  ▷ 메일전송                    
        void btn메일전송_Click(object sender, EventArgs e)
        {
            try
            {
                if (추가모드여부) return;
                ...
                if (IsChanged())
                {
                    ShowMessage("수정된 사항이 있습니다. 저장 후 메일전송버튼을 클릭하세요.");
                    return;
                }
                ...
                dic["NO_SO"] = txt수주번호.Text;
                dic["DT_SO"] = dtp수주일자.Text.Substring(0, 4) + "/" + dtp수주일자.Text.Substring(4, 2) + "/" + dtp수주일자.Text.Substring(6, 2);
                dic["PARTNER"] = ctx거래처.CodeValue;
                dic["CD_PARTNER"] = ctx거래처.CodeName;
                dic["CD_SALEGRP"] = ctx영업그룹.CodeName;
                dic["NO_KOR"] = ctx담당자.CodeName;
                dic["TP_SO"] = ctx수주형태.CodeName;
                dic["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                dic["NM_CD_EXCH"] = cbo화폐단위.Text;
                dic["RT_EXCH"] = cur환율.Text;
                dic["TP_PRICE"] = cbo단가유형.Text;
                dic["NO_PROJECT"] = ctx프로젝트.CodeName;
                dic["TP_VAT"] = cbo부가세구분.Text;
                dic["RT_VAT"] = cur부가세율.Text;
                dic["FG_VAT"] = cbo부가세포함.Text;
                dic["FG_TAXP"] = rdo계산서처리일괄.Checked == true ? rdo계산서처리일괄.Text : rdo계산서처리건별.Text;
                dic["DC_RMK"] = txt비고.Text;
                dic["FG_BILL"] = m_cboFgBill.Text;
                dic["FG_TRANSPORT"] = cbo운송방법.Text;
                dic["NO_CONTRACT"] = txt계약번호.Text;
                dic["NO_PO_PARTNER"] = txt거래처PO.Text;
                dic["NM_CEO"] = D.GetString(rowPartnerInfo["NM_CEO"]);
                dic["NO_TEL"] = D.GetString(rowPartnerInfo["NO_TEL"]);
                dic["NO_FAX"] = D.GetString(rowPartnerInfo["NO_FAX"]);
                dic["NO_COMPANY"] = D.GetString(rowPartnerInfo["NO_COMPANY"]);
                dic["NM_PTR"] = D.GetString(rowPartnerInfo["NM_PTR"]);
                dic["TP_JOB"] = D.GetString(rowPartnerInfo["TP_JOB"]);
                dic["CLS_JOB"] = D.GetString(rowPartnerInfo["CLS_JOB"]);
                dic["DC_ADS1_H"] = D.GetString(rowPartnerInfo["DC_ADS1_H"]);
                dic["DC_ADS1_D"] = D.GetString(rowPartnerInfo["DC_ADS1_D"]);
                dic["DC_ADS1"] = D.GetString(rowPartnerInfo["DC_ADS1_H"]) + " " + D.GetString(rowPartnerInfo["DC_ADS1_D"]);
                dic["AM_WONAMT_SUM"] = D.GetString(_flex[1, "AM_WONAMT"]);
                dic["AM_VAT_SUM"] = D.GetString(_flex[1, "AM_VAT"]);
                dic["AMVAT_SO_SUM"] = D.GetString(_flex[1, "AMVAT_SO"]);
                dic["NM_BUSI"] = D.GetString(Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(MasterSearch.MA_CODEDTL, new object[] { MA.Login.회사코드, "PU_C000016", _거래구분 })["NM_SYSDEF"]);
                dic["AM_SO_SUM"] = D.GetString(_flex[1, "AM_SO"]);
                dic["QT_SO_SUM"] = D.GetString(_flex[1, "QT_SO"]);
                dic["NM_EXPORT"] = ctx수출자.CodeName;
                dic["NM_PRODUCT"] = ctx제조자.CodeName;
                dic["COND_TRANS"] = txt인도조건.Text;
                dic["NM_INSPECT"] = txt검사기관.Text;
                dic["DESTINATION"] = txt목적지.Text;
                dic["NM_COND_PAY"] = cbo결제형태.Text;
                dic["COND_DAYS"] = cur결제일.Text;
                dic["DT_EXPIRY"] = dtp유효일자해외.Text;
                dic["NM_TP_PACKING"] = cbo포장형태.Text;
                dic["NM_TP_TRANSPORT"] = cbo운송형태.Text;
                dic["NM_CD_ORIGIN"] = cbo원산지.Text;
                dic["PORT_LOADING"] = txt선적항.Text;
                dic["PORT_ARRIVER"] = txt도착항.Text;
                dic["NM_COND_PRICE"] = cbo가격조건.Text;
                dic["NM_NOTIFY"] = ctx착하통지처.CodeName;
                dic["NM_CONSIGNEE"] = ctx수하인.CodeName;
                dic["DC_RMK_TEXT"] = txt멀티비고.Text;
                dic["DC_RMK_TEXT2"] = txt멀티비고2.Text;
                dic["DC_RMK1"] = txt비고1.Text;

                dic["NO_BIZAREA"] = D.GetString(rowBizareaInfo["NO_BIZAREA"]);
                dic["NM_BIZAREA"] = D.GetString(rowBizareaInfo["NM_BIZAREA"]);
                dic["NM_MASTER_BIZAREA"] = D.GetString(rowBizareaInfo["NM_MASTER"]);
                dic["ADS_H_BIZAREA"] = D.GetString(rowBizareaInfo["ADS_H"]);
                dic["ADS_D_BIZAREA"] = D.GetString(rowBizareaInfo["ADS_D"]);
                dic["ADS_BIZAREA"] = D.GetString(rowBizareaInfo["ADS_H"]) + " " + D.GetString(rowBizareaInfo["ADS_D"]);
                dic["TP_JOB_BIZAREA"] = D.GetString(rowBizareaInfo["TP_JOB"]);
                dic["CLS_JOB_BIZAREA"] = D.GetString(rowBizareaInfo["CLS_JOB"]);
                dic["NO_TEL_BIZAREA"] = D.GetString(rowBizareaInfo["NO_TEL"]);

                if (row원산지Info != null)
                    dic["NM_SYSDEF_E"] = D.GetString(row원산지Info["NM_SYSDEF_E"]);   //원산지 영문명
                if (row착하통지처Info != null)
                {
                    dic["NOTIFY_DC_ADS1_H"] = D.GetString(row착하통지처Info["DC_ADS1_H"]);   //착하통지처주소
                    dic["NOTIFY_DC_ADS1_D"] = D.GetString(row착하통지처Info["DC_ADS1_D"]);   //착하통지처상세주소
                    dic["NOTIFY_NO_TEL"] = D.GetString(row착하통지처Info["NO_TEL"]);         //착하통지처전화번호
                }
                if (row수하인Info != null)
                {
                    dic["CONSIGNEE_DC_ADS1_H"] = D.GetString(row수하인Info["DC_ADS1_H"]);   //수하인주소
                    dic["CONSIGNEE_DC_ADS1_D"] = D.GetString(row수하인Info["DC_ADS1_D"]);   //수하인상세주소
                }
                if (row은행Info != null)
                {
                    dic["CD_BANK_SO"] = ctx은행.CodeValue;   //거래은행
                    dic["NM_BANK_SO"] = ctx은행.CodeName;   //거래은행명
                    dic["BANK_SO_NM_TEXT"] = D.GetString(row은행Info["NM_TEXT"]);   //거래은행비고
                }
                if (row거래처Info != null)
                {
                    dic["NO_TEL1"] = D.GetString(row거래처Info["NO_TEL1"]);    //본사전화번호
                    dic["NO_FAX1"] = D.GetString(row거래처Info["NO_FAX1"]);    //본사FAX번호
                    dic["CD_EMP_PARTNER"] = D.GetString(row거래처Info["CD_EMP_PARTNER"]);    //전자세금계산서 담당자
                    dic["E_MAIL"] = D.GetString(row거래처Info["E_MAIL"]);    //E_MAIL             
                }
                if (row수출자Info != null)
                {
                    dic["EXPORT_DC_ADS1_H"] = D.GetString(row수출자Info["DC_ADS1_H"]);   //수출자주소
                    dic["EXPORT_DC_ADS1_D"] = D.GetString(row수출자Info["DC_ADS1_D"]);   //수출자상세주소
                }
                ...
                string title = D.GetString(ctx담당자.CodeName) + "/" + D.GetString(txt수주번호.Text) + "/" + D.GetString(ctx거래처.CodeName) + "수주가 등록되었습니다.";
                ...
                foreach (DataRow dr in _flex.DataTable.Rows)
                {
                    ...
                    string msg = "품목코드: " + D.GetString(dr["CD_ITEM"]) + " / 품목명: " + D.GetString(dr["NM_ITEM"]) + " / 규격: " + D.GetString(dr["STND_ITEM"]) + " / 단위: " + D.GetString(dr["UNIT_SO"]) +
                                 " / 수량: " + D.GetDecimal(dr["QT_SO"]).ToString(_flex.Cols["QT_SO"].Format) + " / 단가: " + D.GetDecimal(dr["UM_SO"]).ToString(_flex.Cols["UM_SO"].Format) + "/ 금액: " + D.GetDecimal(dr["AM_SO"]).ToString(_flex.Cols["AM_SO"].Format) + " / 프로젝트코드: " + D.GetString(dr["NO_PROJECT"]) + " / 프로젝트명: " + D.GetString(dr["NM_PROJECT"]);
                    text.AppendLine(msg);
                    text.AppendLine("\n\n");
                }

                string[] str_histext = new string[3];
                str_histext[0] = title;                                   //제목
                str_histext[1] = _biz.Get설정(DefaultSettings.MAIL_ADDR); //받는사람 이메일주소
                str_histext[2] = text.ToString();                        //내용

                P_MF_EMAIL mail = new P_MF_EMAIL(new string[] { txt수주번호.Text }, "R_SA_SO_RPT_001", new ReportHelper[] { rptHelper }, dic, "수주서", str_histext);
                mail.ShowDialog();

                _biz.Set설정(DefaultSettings.MAIL_ADDR, mail._str_rt_data[0]);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "GI_PARTNER":
                        if (서버키 == "SIMMONS")
                        {
                            e.Parameter.P14_CD_PARTNER = string.Empty;
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "CD_SL":
                        ...
                        else if (서버키 == "CNP")
                        {
                            ...
                            {
                                _flex["QT_EXP"] = D.GetDecimal(dtQtMiGi.Rows[0]["QT_MI_GI"]);
                                _flex["QT_USEINV"] = D.GetDecimal(_flex["SL_QT_INV"]) - D.GetDecimal(dtQtMiGi.Rows[0]["QT_MI_GI"]);
                            }
                            _flex["QT_AVA"] = D.GetDecimal(_flex["QT_USEINV"]) - D.GetDecimal(_flex["QT_SO"]);
                            _flex["NO_LOT"] = _biz.SearchNoLotCNP(D.GetString(helpReturn.Rows[0]["CD_SL"]), D.GetString(_flex["CD_ITEM"]));
                        }
                        else if (서버키 == "DNCOMPANY")
                        {
                            DataRow rowQtMiGi = _biz.Get_SearchMiGi(D.GetString(_flex["CD_PLANT"]), D.GetString(_flex["CD_ITEM"]));
                            _flex["QT_USEINV"] = rowQtMiGi == null ? D.GetDecimal(_flex["SL_QT_INV"]) : D.GetDecimal(_flex["SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_GI_REMAIN"]);
                        }
                        break;
                    default:
                        break;
            ...
            try
            {
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "QT_SO":
                        if (서버키 == "KPCI")
                        {
                            if (D.GetString(_flex["NO_RELATION"]) != string.Empty)
                            {
                        ...
                        {
                            if (D.GetDecimal(_flex["WEIGHT"]) != decimal.Zero)
                            {
                                _flex["NUM_USERDEF4"] = Decimal.Round(D.GetDecimal(_flex["QT_SO"]) / D.GetDecimal(_flex["WEIGHT"]));
                            }
                        }
                        ...
                        if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
                            _flex["NUM_USERDEF2"] = D.GetDecimal(_flex["NUM_USERDEF1"]) * D.GetDecimal(newValue);
                        if (Use부가세포함)
                        {
                            ...
                            if (Global.MainFrame.LoginInfo.CompanyLanguage == Language.KR)
                                ...
                                _flex["AM_WONAMT"] = D.GetDecimal(UDecimal.GetConvertFormatData(DataDictionaryTypes.SA, FormatTpType.MONEY, FormatFgType.INSERT, D.GetDecimal(_flex["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(_flex["RT_VAT"])))));
                        ...
                        if (서버키 == "CNP")
                            _flex["QT_AVA"] = D.GetDecimal(_flex["QT_USEINV"]) - (D.GetDecimal(newValue) - D.GetDecimal(oldValue));
                        if (PageID == "P_SA_Z_KORFNT_SO")
                        {
                            if(D.GetDecimal(oldValue) != decimal.Zero)
                                _flex["NUM_USERDEF2"] = (D.GetDecimal(_flex["NUM_USERDEF2"]) / D.GetDecimal(oldValue)) * D.GetDecimal(newValue);
                            _flex["NUM_USERDEF3"] = D.GetDecimal(_flex["UMVAT_SO"]);
                            _flex["NUM_USERDEF4"] = D.GetDecimal(_flex["AMVAT_SO"]);
                        }
                    ...
                    case "UM_SO":
                        ...
                        if (서버키 == "HANILTOYO")
                            _flex["NUM_USERDEF2"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newValue) - D.GetDecimal(_flex["NUM_USERDEF1"])); 
                            IsInvAmCalc();
                        break;
                    ...
                    case "AM_SO":
                        ...
                        if (_biz.Get업체별프로세스 == "001")
                            _flex["NUM_USERDEF1"] = D.GetDecimal(_flex["AREA"]) == 0m ? 0m : Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["UM_SO"]) / D.GetDecimal(_flex["AREA"]));
                        if (서버키 == "HANILTOYO")
                            _flex["NUM_USERDEF2"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["UM_SO"]) - D.GetDecimal(_flex["NUM_USERDEF1"])); 
                            IsInvAmCalc();
                        break;
                    ...
                    case "RT_DSCNT": //할인율
                        if (D.GetDecimal(_flex["UM_BASE"]) != 0)
                        {
                            ...
                            {
                                if (D.GetDecimal(_flex["NUM_USERDEF1"]) < D.GetDecimal(newValue))
                                {
                                    ShowMessage("DC는 F/DC를 초과하여 입력될수 없습니다.");
                                    e.Cancel = true;
                                    return;
                                }
                                _flex["NUM_USERDEF2"] = Math.Truncate(D.GetDecimal(_flex["UM_BASE"]) - (D.GetDecimal(_flex["UM_BASE"]) * D.GetDecimal(newValue)) / 100);

                                _flex["NUM_USERDEF6"] = D.GetDecimal(_flex["NUM_USERDEF2"]) - D.GetDecimal(_flex["NUM_USERDEF4"]);

                                //_flex["UM_SO"] = D.GetDecimal(_flex["NUM_USERDEF2"]) - (D.GetDecimal(_flex["NUM_USERDEF2"]) * D.GetDecimal(_flex["NUM_USERDEF3"]) / 100) - D.GetDecimal(_flex["NUM_USERDEF4"]);
                                _flex["UM_SO"] = D.GetDecimal(_flex["NUM_USERDEF6"]) - (D.GetDecimal(_flex["NUM_USERDEF6"]) * D.GetDecimal(_flex["NUM_USERDEF3"]) / 100);
                            }
                            else
                    ...
                    case "NUM_USERDEF2":
                        switch (서버키)
                        {
                            case "KLW":
                                ...
                                break;
                            case "WINPLUS":
                                _flex["QT_SO"] = decimal.Round(D.GetDecimal(_flex["NUM_USERDEF1"]) * D.GetDecimal(_flex["NUM_USERDEF2"]) / 10000m, 1, MidpointRounding.AwayFromZero);
                                _flex["UNIT_SO_FACT"] = D.GetDecimal(_flex["UNIT_SO_FACT"]) == 0 ? 1m : _flex["UNIT_SO_FACT"];
                                ...
                                break;
                            case "SANGBO":
                                decimal DC = decimal.Ceiling((D.GetDecimal(_flex["UM_BASE"]) - D.GetDecimal(newValue)) / D.GetDecimal(_flex["UM_BASE"]) * 100);

                                if (D.GetDecimal(_flex["NUM_USERDEF1"]) < DC )
                                {
                                    ShowMessage("DC는 F/DC를 초과하여 입력될수 없습니다.");
                                    e.Cancel = true;
                                    return;
                                ...
                                _flex["NUM_USERDEF6"] = D.GetDecimal(newValue) - D.GetDecimal(_flex["NUM_USERDEF4"]);

                                _flex["UM_SO"] = D.GetDecimal(_flex["NUM_USERDEF6"]) - (D.GetDecimal(_flex["NUM_USERDEF6"]) * D.GetDecimal(_flex["NUM_USERDEF3"]) / 100);

                                //_flex["UM_SO"] = D.GetDecimal(newValue) - (D.GetDecimal(newValue) * D.GetDecimal(_flex["NUM_USERDEF3"]) / 100) - D.GetDecimal(_flex["NUM_USERDEF4"]);
                            ...
                            default:
                                ...
                                if (PageID == "P_SA_Z_KORFNT_SO")   //한국가구
                                {
                                    _flex["AMVAT_SO"] = D.GetDecimal(_flex["NUM_USERDEF2"]) - (D.GetDecimal(_flex["NUM_USERDEF2"]) * D.GetDecimal(_flex["NUM_USERDEF1"]) / 100m);
                                    _flex["UMVAT_SO"] = D.GetDecimal(_flex["QT_SO"]) == 0m ? 0m : Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) / D.GetDecimal(_flex["QT_SO"]));
                                    _flex["AM_WONAMT"] = Decimal.Round(D.GetDecimal(_flex["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(_flex["RT_VAT"]))), MidpointRounding.AwayFromZero);
                                    _flex["AM_VAT"] = D.GetDecimal(_flex["AMVAT_SO"]) - D.GetDecimal(_flex["AM_WONAMT"]);
                                    _flex["AM_SO"] = cur환율.DecimalValue == 0m ? 0m : Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) / cur환율.DecimalValue);
                                    _flex["UM_SO"] = D.GetDecimal(_flex["QT_SO"]) == 0m ? 0m : Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_SO"]) / D.GetDecimal(_flex["QT_SO"]));
                    ...
                    case "NUM_USERDEF4":
                        ...
                        {
                            _flex["QT_SO"] = Decimal.Round(D.GetDecimal(_flex["NUM_USERDEF4"]) * D.GetDecimal(_flex["WEIGHT"]));
                    ...
                    case "FG_USE" :
                        ...
                        {
                            DataTable dt = MA.GetCode("SA_B000057");
                            string filter = string.Format("CODE = '{0}'", D.GetString(_flex["FG_USE"]));
                            DataRow[] rows = dt.Select(filter);
                            if (rows == null || rows.Length == 0) return;
                            string CD_FLAG1 = D.GetString(rows[0]["CD_FLAG1"]);
                            if (CD_FLAG1 == "Y")
                            {
                                _flex["UM_SO"] = 0;
                                _flex["UMVAT_SO"] = 0;
                ...
                if (서버키 == "NEOPH" || 서버키 == "DZSQL")
                {
                    decimal AM_SO = D.GetDecimal(_flex["AM_SO"]);
                    if (D.GetDecimal(AM_SO) > 0m && D.GetString(_flex["FG_USE2"]) == "101")
                        _flex["FG_USE2"] = "100";  //본품
                    if (D.GetDecimal(AM_SO) == 0m && D.GetString(_flex["FG_USE2"]) == "100")
                        _flex["FG_USE2"] = "101";  //할증
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                switch (_flex.Cols[_flex.Col].Name)
                {
                    case "UM_SO":
                        if (서버키 == "NIKON")
                        {
                            DataTable dt = _biz.AuthUserMenu(Global.MainFrame.CurrentPageID);
                            if (dt == null || dt.Rows.Count == 0) return;
                            if (D.GetString(dt.Rows[0]["YN_UM"]) == "N") return;
                        }
                        else if (so_Price == "Y") return; //영업그룹에 영업조직을 조회하여 판매단가 통제유무가 "Y" 이면 단가와 금액 컬럼 수정을 할수 없다.
                        if (Use부가세포함) return;
                    ...
                    case "CD_ITEM_REF":
                        ...
                        H_SA_SET_OPEN_SUB dlgSet = new H_SA_SET_OPEN_SUB(D.GetString(cbo공장.SelectedValue), dtp수주일자.Text, D.GetString(cbo화폐단위.SelectedValue));
        ...
        void _flex_StartEdit(object sender, RowColEventArgs e)
        {
            try
            {
                ...
                if (!추가모드여부 && _flex.RowState(e.Row) != DataRowState.Added)
                {
                    if (서버키 == "HKCOS" || 서버키 == "TOKIMEC" || 서버키 == "DAEYONG" || 서버키 == "BROTHER")
                    {
                ...
                switch (ColName)
                {
                    ...
                    case "AM_PRIFT" :
                        ...
                        return;
                    case "UM_SO":
                        if (Use부가세포함 || PageID == "P_SA_Z_KORFNT_SO")
                        {
                            e.Cancel = true;
                            return;
                        }
                        ...
                        if (서버키 == "NIKON")
                        {
                            ...
                            if (D.GetString(dt.Rows[0]["YN_UM"]) == "N")
                            {
                                e.Cancel = true;
                                return;
                            }
                        ...
                        if (so_Price == "Y") //영업그룹에 영업조직을 조회하여 판매단가 통제유무가 "Y" 이면 단가와 금액 컬럼 수정을 할수 없다.
                        {
                    ...
                    case "AM_SO":
                        if (Use부가세포함 || PageID == "P_SA_Z_KORFNT_SO")   //한국가구
                        {
                            e.Cancel = true;
                            return;
                        }
                        ...
                        if (서버키 == "NIKON")
                        {
                            ...
                            if (D.GetString(dt.Rows[0]["YN_AM"]) == "N")
                            {
                                e.Cancel = true;
                                return;
                            }
                        }
                        else if (so_Price == "Y") //영업그룹에 영업조직을 조회하여 판매단가 통제유무가 "Y" 이면 단가와 금액 컬럼 수정을 할수 없다.
                        {
                            ShowMessage("영업단가통제된 영업그룹입니다.");
                            e.Cancel = true;
                        }
                    ...
                    case "CD_ITEM":
                        ...
                        if (서버키 == "NICEGR")
                        {
                            if (D.GetDecimal(_flex["NUM_USERDEF1"]) != 0 || D.GetDecimal(_flex["NUM_USERDEF2"]) != 0)
                            {
                                ShowMessage("주간영업계획등록에서 적용 받은건 이므로 수정하실 수 없습니다.");
                                e.Cancel = true;
                                return;
                            }
                        }
                        ...
                        {
                            if (D.GetString(_flex["CD_ITEM_REF"]) != string.Empty)
                            {
                                ShowMessage("SET품 도움창에서 적용 받은건 이므로 수정하실 수 없습니다.");
                                e.Cancel = true;
                                return;
                            }
                        }
                        else if (서버키 == "KPCI")
                        {
                            if (D.GetString(_flex["NO_RELATION"]) != string.Empty)
                            {
                                ShowMessage("계약등록 도움창에서 적용 받은건 이므로 수정하실 수 없습니다.");
                                e.Cancel = true;
                                return;
                            }
                        }
                    ...
                    case "QT_SO":
                        //수주용도가 할증(020)인 경우 수정불가(D20140121063)
                        if (_biz.Get업체별프로세스 == "009" && D.GetString(_flex["FG_USE"]) == "020")    //에스디바이오센서(주)
                        {
                            e.Cancel = true;
                            return;
                        }
                    ...
                    case "TP_VAT":
                        if (MA.ServerKey(false, "YWD"))
                        {
                            if (_매출자동여부 == "Y")
                            {
                                ShowMessage("매출까지 자동프로세스일 경우 과세구분을 변경 할 수 없습니다.");
                                e.Cancel = true;
                                return;
                            }
                        }
                ...
                if (서버키 == "SANGBO")
                {
                    if (_flex.Cols[e.Col].Name == "NUM_USERDEF2")
                    {
                        if (D.GetDecimal(_flex["UM_BASE"]) == 0)
                        {
                            ShowMessage("도매가가 없습니다.");
                            e.Cancel = true;
                            return;
                        }
                    }
        ...
        private void Control_QueryBefore(object sender, Duzon.Common.BpControls.BpQueryArgs e)
        {
            ...
            switch (e.HelpID)
            {
                ...
                case HelpID.P_SA_TPPTR_SUB:         // 매출거래처
                    if (서버키 == "SIMMONS")
                    {
                        e.HelpParam.P14_CD_PARTNER = string.Empty;
                        e.HelpParam.P61_CODE1 = "001";
                    }
                    else
            ...
            try
            {
                ...
                switch (e.HelpID)
                {
                    ...
                    case Duzon.Common.Forms.Help.HelpID.P_MA_PARTNER_SUB:
                        ...
                        ctx착하통지처.CodeValue = ctx거래처.CodeValue;
                        ctx착하통지처.CodeName = ctx거래처.CodeName;
                        ctx수하인.CodeValue = ctx거래처.CodeValue;
                        ctx수하인.CodeName = ctx거래처.CodeName;
                        _header.CurrentRow["CD_NOTIFY"] = ctx거래처.CodeValue;
                        _header.CurrentRow["CD_CONSIGNEE"] = ctx거래처.CodeValue;
        ...
        void _header_JobModeChanged(object sender, FreeBindingArgs e)
        {
            try
            {
                if (e.JobMode == JobModeEnum.추가후수정)
                {
                    ...
                    pnl해외.Enabled = true;
                    oneGrid매출정보.Enabled = true;
                    txt멀티비고.ReadOnly = false;
                    txt멀티비고2.ReadOnly = false;
                    ...
                    cbo화폐단위.Enabled = true;
                    cur환율.Enabled = true;
                    if (서버키 == "MHIK") //엠에이치파워시스템즈코리아 
                    {
                        Control ctrl = bpPanelControl1.Controls.Find("txt전용수주번호", true)[0] as Control;
                        ctrl.Enabled = true;
                    }
                    if (_구분 == "복사")
                    {
                ...
                else
                {
                    if (!_헤더수정여부)
                    {
                        ...
                        txt멀티비고2.ReadOnly = true;
                        oneGrid매출정보.Enabled = false;
                    }
                    else
                    {
                        m_pnlBaseInfo.Enabled = true;
        ...
        #region ▷ 계산서처리Default셋팅       
        private void 계산서처리Default셋팅()
        {
            string fgTaxp = _biz.Get설정(DefaultSettings.계산서처리) == string.Empty ? "001" : _biz.Get설정(DefaultSettings.계산서처리);
        ...
        private void 수주일자변경()
        {
            _header.CurrentRow["DT_PROCESS"] = dtp수주일자.Text;
            dtp매출일자.Text = dtp수주일자.Text;
        ...
        void 견적품목셋팅(DataRow[] dr견적D)
        {
            ...
            {
                if (row견적.RowState == DataRowState.Deleted) continue;
                ...
                newrow["TP_IV"] = _매출형태;
                newrow["MAT_ITEM"] = row견적["MAT_ITEM"];
        ...
        void 품목추가(int idx, DataRow[] dr품목)
        {
            ...
            if (서버키 == "SANGBO")
            {
                dt상보DC = _biz.상보할인율(공장, ctx거래처.CodeValue, dtp수주일자.Text, dr품목);
            }
            ...
            DataTable dtQtMiGi = null;
            if (서버키 == "KOREAF" || 서버키 == "ACADEMY" || 서버키 == "CNP" || PageID == "P_SA_Z_KORFNT_SO")
            {
                dtQtMiGi = _biz.SearchMiGi(공장, string.Empty, multiItem);
                dtQtMiGi.PrimaryKey = new DataColumn[] { dtQtMiGi.Columns["CD_SL"], dtQtMiGi.Columns["CD_ITEM"] };
            }
            ...
            foreach (DataRow row in dr품목)
            {
                ...
                if (First)
                {
                    ...
                    if (Global.MainFrame.ServerKeyCommon.ToUpper() == "SINJINSM")
                    {
                        _flex[idx, "NUM_STND_ITEM_1"] = row["NUM_STND_ITEM_1"];
                        _flex[idx, "NUM_STND_ITEM_2"] = row["NUM_STND_ITEM_2"];
                        _flex[idx, "NUM_STND_ITEM_3"] = row["NUM_STND_ITEM_3"];
                        _flex[idx, "NUM_STND_ITEM_4"] = row["NUM_STND_ITEM_4"];
                        _flex[idx, "NUM_STND_ITEM_5"] = row["NUM_STND_ITEM_5"];
                    }
                    CC조회(D.GetString(row["GRP_ITEM"]), out cdCc, out nmCc);
                    ...
                    else if (_biz.Get업체별프로세스 == "003")   //아카데미과학(PIMS:D20111115050)
                    {
                        ...
                        _flex[idx, "PITEM_CD_USERDEF2"] = row["CD_USERDEF2"];
                        _flex[idx, "PITEM_CD_USERDEF3"] = row["CD_USERDEF3"];
                    }
                    ...
                    else if (_biz.Get업체별프로세스 == "010") //대용산업
                    {
                        ...
                        _flex[idx, "PITEM_NUM_USERDEF2"] = row["NUM_USERDEF2"];
                        _flex[idx, "PITEM_NUM_USERDEF3"] = row["NUM_USERDEF3"];
                        _flex[idx, "URL_ITEM"] = row["URL_ITEM"];
                    }
                    ...
                    else
                    {
                        switch (서버키)
                        {
                            ...
                            default:
                                ...
                                {
                                    if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
                                        SDBIO단가적용(idx, 기준일자, D.GetString(row["GRP_ITEM"]));
                                    else
                                        일반단가적용(idx, 기준일자);
                                }
                                else if (_biz.Get특수단가적용 == 특수단가적용.중량단가)
                                {
                                    if ((_단가적용형태 == "002" || _단가적용형태 == "003"))
                                        _flex[idx, "UM_OPT"] = Unit.외화단가(DataDictionaryTypes.SA, BASIC.GetUM(D.GetString(_flex[idx, "CD_PLANT"]), D.GetString(_flex[idx, "CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA"));
                    ...
                    else if (서버키 == "KOREAF" || 서버키 == "ACADEMY" || PageID == "P_SA_Z_KORFNT_SO")   //가용재고(현재고 - 미출하수량) 2012.02.24(PIMS:D20120220022)
                    {
                        DataRow rowQtMiGi = dtQtMiGi.Rows.Find(new object[] { D.GetString(_flex[idx, "CD_SL"]), D.GetString(_flex[idx, "CD_ITEM"]) });
                    ...
                    else if (서버키 == "CNP")
                    {
                        ...
                        {
                            _flex[idx, "QT_EXP"] = D.GetDecimal(rowQtMiGi["QT_MI_GI"]);
                            _flex[idx, "QT_USEINV"] = D.GetDecimal(_flex[idx, "SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_MI_GI"]);
                        }
                        _flex[idx, "QT_AVA"] = D.GetDecimal(_flex[idx, "QT_USEINV"]) - D.GetDecimal(_flex[idx, "QT_SO"]);
                        _flex[idx, "NO_LOT"] = _biz.SearchNoLotCNP(D.GetString(_flex[idx, "CD_SL"]), D.GetString(_flex[idx, "CD_ITEM"]));
                    }
                    else if (서버키 == "DNCOMPANY")
                    {
                        DataRow rowQtMiGi = _biz.Get_SearchMiGi(D.GetString(_flex[idx, "CD_PLANT"]), D.GetString(_flex[idx, "CD_ITEM"]));
                        _flex[idx, "QT_USEINV"] = rowQtMiGi == null ? D.GetDecimal(_flex[idx, "SL_QT_INV"]) : D.GetDecimal(_flex[idx, "SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_GI_REMAIN"]); 
                    }
                    ...
                    if (PageID == "P_SA_Z_KORFNT_SO")   //한국가구
                        _flex[idx, "NUM_USERDEF4"] = D.GetDecimal(_flex[idx, "AMVAT_SO"]);
                    ...
                    if (서버키 == "CNP")
                    {
                        _flex[idx, "FG_USE"] = "001";
                    }

                    if (서버키 == "NEOPH")
                    {
                        string 수주용도2 = D.GetString(row["CD_USERDEF1"]);
                        decimal 판매단가 = D.GetDecimal(_flex[idx, "UM_SO"]);
                        if (수주용도2 == "100" && 판매단가 == 0) 수주용도2 = "101";

                        _flex[idx, "FG_USE2"] = 수주용도2;
                    }
                    ...
                    if (서버키 == "HANILTOYO") //한일도요
                    {
                        DataRow 원료비 = _biz.원료비(D.GetString(_flex[idx, "CD_PLANT"]), D.GetString(_flex[idx, "CD_ITEM"]));
                        _flex[idx, "NUM_USERDEF1"] = 원료비 == null ? 0 : D.GetDecimal(원료비["TOT_MATERIAL"]); //원료비
                        _flex[idx, "NUM_USERDEF2"] = D.GetDecimal(_flex[idx, "UM_SO"]) == 0 ? 0m : Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[idx, "UM_SO"]) - D.GetDecimal(_flex[idx, "NUM_USERDEF1"])); //한계이익
                    }

                    else if (서버키 == "SAMWON")
                    {
                        if (D.GetDecimal(row["WEIGHT"]) == decimal.Zero)
                            _flex[idx, "NUM_USERDEF4"] = 1;
                    }
                ...
                else
                {
                    ...
                    if (서버키 == "CNP") _flex["FG_USE"] = "001";
                    else _flex["FG_USE"] = _flex[_flex.Row - 1, "FG_USE"];
                    ...
                    if (Global.MainFrame.ServerKeyCommon.ToUpper() == "SINJINSM")
                    {
                        _flex["NUM_STND_ITEM_1"] = row["NUM_STND_ITEM_1"];
                        _flex["NUM_STND_ITEM_2"] = row["NUM_STND_ITEM_2"];
                        _flex["NUM_STND_ITEM_3"] = row["NUM_STND_ITEM_3"];
                        _flex["NUM_STND_ITEM_4"] = row["NUM_STND_ITEM_4"];
                        _flex["NUM_STND_ITEM_5"] = row["NUM_STND_ITEM_5"];
                    }
                    ...
                    else if (_biz.Get업체별프로세스 == "003")   //아카데미과학(PIMS:D20111115050)
                    {
                        ...
                        _flex["PITEM_CD_USERDEF2"] = row["CD_USERDEF2"];
                        _flex["PITEM_CD_USERDEF3"] = row["CD_USERDEF3"];
                    }
                    else if (_biz.Get업체별프로세스 == "008")
                    {
                    ...
                    else
                    {
                        switch (서버키)
                        {
                            ...
                            default:
                                ...
                                {
                                    if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
                                        SDBIO단가적용(_flex.Row, 기준일자, D.GetString(row["GRP_ITEM"]));
                                    else
                                ...
                                {
                                    if ((_단가적용형태 == "002" || _단가적용형태 == "003"))
                                        _flex["UM_OPT"] = Unit.외화단가(DataDictionaryTypes.SA, BASIC.GetUM(D.GetString(_flex["CD_PLANT"]), D.GetString(_flex["CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA"));
                    ...
                    {
                        DataRow rowQtMiGi = dtQtMiGi.Rows.Find(new object[] { D.GetString(_flex["CD_SL"]), D.GetString(_flex["CD_ITEM"]) });
                    ...
                    else if (서버키 == "CNP")
                    {
                        ...
                        {
                            _flex["QT_EXP"] = D.GetDecimal(rowQtMiGi["QT_MI_GI"]);
                            _flex["QT_USEINV"] = D.GetDecimal(_flex["SL_QT_INV"]) - D.GetDecimal(rowQtMiGi["QT_MI_GI"]);
                        }
                        _flex["QT_AVA"] = D.GetDecimal(_flex["QT_USEINV"]) - D.GetDecimal(_flex["QT_SO"]);
                        _flex["NO_LOT"] = _biz.SearchNoLotCNP(D.GetString(_flex["CD_SL"]), D.GetString(_flex["CD_ITEM"])); ;
                    }
                    else if (서버키 == "DNCOMPANY")
                    {
                        DataRow rowQtMiGi = _biz.Get_SearchMiGi(D.GetString(_flex["CD_PLANT"]), D.GetString(_flex["CD_ITEM"]));
                    ...
                    {
                        string 수주용도2 = D.GetString(row["CD_USERDEF1"]);
                        decimal 판매단가 = D.GetDecimal(_flex["UM_SO"]);
                        if (수주용도2 == "100" && 판매단가 == 0) 수주용도2 = "101";
                    ...
                    if (서버키 == "HANILTOYO") //한일도요
                    {
                        DataRow 원료비 = _biz.원료비(D.GetString(_flex["CD_PLANT"]), D.GetString(_flex["CD_ITEM"]));
                        _flex["NUM_USERDEF1"] = 원료비 == null ? 0 : D.GetDecimal(원료비["TOT_MATERIAL"]); //원료비
                        _flex["NUM_USERDEF2"] = D.GetDecimal(_flex["UM_SO"]) == 0 ? 0m : Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["UM_SO"]) - D.GetDecimal(_flex["NUM_USERDEF1"])); //한계이익
                    }
                    else if (서버키 == "SAMWON")
                    {
                        if (D.GetDecimal(row["WEIGHT"]) == decimal.Zero)
                            _flex["NUM_USERDEF4"] = 1;
                    }
                    #endregion
                }
            }
        ...
        private void 일반단가적용(int idx, string 기준일자)
        {
            ...
            decimal 적용단가 = BASIC.GetUM(D.GetString(_flex[idx, "CD_PLANT"]), D.GetString(_flex[idx, "CD_ITEM"]), ctx거래처.CodeValue, ctx영업그룹.CodeValue, dtp수주일자.Text, D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue), "SA");
            ...
            if (PageID == "P_SA_Z_KORFNT_SO")   //한국가구
                _flex[idx, "NUM_USERDEF3"] = D.GetDecimal(_flex[idx, "UMVAT_SO"]);
        }
        ...
        #region -> 에스디바이오 단가적용
        private void SDBIO단가적용(int idx, string 기준일자, string cdItemGrp)
        {
            if (_단가적용형태 != "003") return; //거래처별단가 사용 안할때

            DataTable dt = _biz.Search_SDBIO_UM(D.GetString(cbo공장.SelectedValue), D.GetString(_flex[idx, "CD_ITEM"]), ctx거래처.CodeValue, dtp수주일자.Text,
                                                D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue));
            if (dt != null && dt.Rows.Count > 0)
            {
                DataTable dtItemGrpUm = _biz.SearchItemGrpUm(D.GetString(cbo공장.SelectedValue), ctx수주형태.CodeValue, dtp수주일자.Text);
                DataRow rowItemGrpUm = dtItemGrpUm.Rows.Find(cdItemGrp);

                _flex[idx, "UM_SO"] = D.GetDecimal(dt.Rows[0]["UM_ITEM"]);

                if (rowItemGrpUm != null)
                {
                    _flex[idx, "NUM_USERDEF1"] = D.GetDecimal(rowItemGrpUm["UM_SALES"]) * cur환율.DecimalValue;
                    _flex[idx, "NUM_USERDEF2"] = D.GetDecimal(_flex[idx, "NUM_USERDEF1"]) * D.GetDecimal(_flex[idx, "QT_SO"]);
                    _flex[idx, "NUM_USERDEF3"] = D.GetDecimal(_flex[idx, "AM_WONAMT"]) - D.GetDecimal(_flex[idx, "NUM_USERDEF2"]);
                    _flex[idx, "NUM_USERDEF4"] = D.GetDecimal(_flex[idx, "AM_WONAMT"]) != 0m ? decimal.Round((D.GetDecimal(_flex[idx, "AM_WONAMT"]) - D.GetDecimal(_flex[idx, "NUM_USERDEF2"])) / D.GetDecimal(_flex[idx, "AM_WONAMT"]) * 100m, 2, MidpointRounding.AwayFromZero) : 0m;
                }
                _flex[idx, "NUM_USERDEF5"] = D.GetDecimal(dt.Rows[0]["UM_ITEM"]);
                _flex[idx, "NUM_USERDEF6"] = D.GetDecimal(dt.Rows[0]["UM_ITEM_LOW"]);
            }
        }
        #endregion
        ...
        {            
            ...
            if (_biz.Get업체별프로세스 == "003")        //아카데미과학(PIMS:D20111205120)
            {
                ...
                ColsSetting("PITEM_CD_USERDEF", "MA_B000092", 2, 3);
            }
            else if (_biz.Get업체별프로세스 == "008")   //(주)아이코닉스엔터테인먼트
        ...
        private void Calc금액변경(int idx)
        {
            ...
            if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
            {
                _flex["NUM_USERDEF3"] = D.GetDecimal(_flex["AM_WONAMT"]) - D.GetDecimal(_flex["NUM_USERDEF2"]);
                _flex["NUM_USERDEF4"] = D.GetDecimal(_flex["AM_WONAMT"]) != 0m ? decimal.Round((D.GetDecimal(_flex["AM_WONAMT"]) - D.GetDecimal(_flex["NUM_USERDEF2"])) / D.GetDecimal(_flex["AM_WONAMT"]) * 100m, 2, MidpointRounding.AwayFromZero) : 0m;
            }
        }
        ...
        private void Calc단가변경(int idx)
        {
            if (_biz.Get업체별프로세스 == "009")    //에스디바이오센서(주)
            {
                if (D.GetDecimal(_flex[idx, "UM_SO"]) < D.GetDecimal(_flex[idx, "NUM_USERDEF5"]))
                    _flex[idx, "TXT_USERDEF1"] = "Y";
                else
                    _flex[idx, "TXT_USERDEF1"] = "N";
            }
        }
        #endregion
        ...
        private void SearchSo(string noSo)
        {
            ...
            if (서버키 == "DAEYONG")
                _flexUser.Binding = _biz.Search_DAEYONG(noSo);
            else if (서버키 == "HANUL" || 서버키 == "LKHOIST" || (서버키 == "HKCOS" && MA.Login.회사코드 == "2000"))
            {
                ds.Tables[0].Rows[0]["GI_PARTNER"] = ds.Tables[0].Rows[0]["CD_PARTNER"];
                ds.Tables[0].Rows[0]["GN_PARTNER"] = ds.Tables[0].Rows[0]["LN_PARTNER"];
                ds.Tables[0].AcceptChanges();
            }
            else if (서버키 == "KORFNT") //한국가구
            {
                ds.Tables[0].Rows[0]["GI_PARTNER"] = ds.Tables[1].Rows[0]["GI_PARTNER"];
                ds.Tables[0].Rows[0]["GN_PARTNER"] = ds.Tables[1].Rows[0]["LN_PARTNER"];
                ds.Tables[0].AcceptChanges();
            }
            ...
            if (D.GetDecimal(ds.Tables[0].Rows[0]["NO_HST"]) > 0)
            {
                ...
                m_tabSo.Enabled = m_pnlLine.Enabled = pnl소요자재.Enabled = btn전자결재.Enabled = btn업체전용2.Enabled = btn견적적용.Enabled = btn_PRJ_SUB.Enabled = false;
        ...
        private void ControlVisibleSetting()
        {
            ...
            switch (서버키)
            {
                ...
                case "DAEYONG": //대용산업
                    btn업체전용1.Visible = true;
                    btn업체전용1.Text = DD("Packing 정보");
                    btn업체전용1.Click += new EventHandler(btnPackingInfo_DAEYONG_Click);
                    break;
                case "NICEGR": //지니틱스
                    btn업체전용1.Visible = true;
                    btn업체전용1.Text = DD("계획적용");
                    btn업체전용1.Click += new EventHandler(btn계획적용_Click);
                    break;
                case "CNP": //차앤박
                    btn업체전용1.Visible = true;
                    btn업체전용1.Text = DD("엑셀내려받기");
                    btn업체전용1.Click += new EventHandler(btn엑셀내려받기_Click);
                    break;
                case "SINJINSM":    //신진에스엠
                    btn업체전용2.Visible = true;
                    btn업체전용2.Text = DD("본사발주적용");
                    btn업체전용2.Click += new EventHandler(btn본사발주적용_Click);
                    //btn업체전용3.Visible = true;
                    //btn업체전용3.Text = DD("품목자동생성");
                    //btn업체전용3.Click += new EventHandler(btn품목자동생성_Click);
                    btn업체전용4.Visible = true;
                    btn업체전용4.Text = DD("수주가계산");
                    btn업체전용4.Click += new EventHandler(btn수주가계산_Click);
                    btn업체전용5.Visible = true;
                    btn업체전용5.Width = 130;
                    btn업체전용5.Text = DD("견적적용(신진SM)");
                    btn업체전용5.Click += new EventHandler(btn견적적용신진_Click);
                    btn업체전용6.Visible = true;
                    btn업체전용6.Text = DD("발주적용");
                    btn업체전용6.Click += new EventHandler(btn발주적용_Click);
                    break;
                case "SANGBO":
                    btn업체전용1.Visible = true;
                    btn업체전용1.Text = DD("현재고도움창");
                    btn업체전용1.Click += new EventHandler(btn현재고도움창_Click);
                    btn업체전용2.Visible = true;
                    btn업체전용2.Text = DD("수주이력");
                    btn업체전용2.Click += new EventHandler(btn수주이력_Click);
                    break;
                case "KPCI": //케이피아이씨코포레이션
                    btn업체전용2.Visible = true;
                    btn업체전용2.Text = DD("계약적용");
                    btn업체전용2.Click += new EventHandler(btn계약적용_Click);
                    break;
                case "UNIPOINT": //유니포인트
                ...
                default:
                    if (_biz.Get업체별프로세스 == "008" || 서버키 == "WINPLUS")    //아이코닉스 OR 윈플러스
                    {
                        txt비고1.Size = new System.Drawing.Size(513, 21);
                        lbl할인율.Visible = cur할인율.Visible = btn할인율적용.Visible = true;
                    }
                    else if (_biz.Get업체별프로세스 == "009")   //에스디바이오센서(주)
                    {
                        btn전자결재.Visible = true;
                        btn품목전개.Visible = false;
                        btn업체전용1.Visible = true;
                        btn업체전용1.Text = DD("프로모션전개");
                        btn업체전용1.Click += new EventHandler(btn프로모션전개_Click);
                    }
                    break;
            }
            ...
            //if (Config.MA_ENV.PJT형여부 == "Y" && Global.MainFrame.ServerKeyCommon != "UCLICK" && Global.MainFrame.ServerKeyCommon != "UNIPOINT")
            //    btn_PRJ_SUB.Visible = false;
        }
        ...
        #region ▷ 공급가액 보정               
        void cur공급가액_Validated(object sender, EventArgs e)
        {
            try
            {
                if (!_flex.HasNormalRow) return;
        ...
        #region ▷ 부가세액 보정               
        void cur부가세액_Validated(object sender, EventArgs e)
        {
            try
            {
                if (!_flex.HasNormalRow) return;
        ...
        #region ▷ 매출정보탭 활성화 여부      
        void IsChkTab5Activate()
        {
            if (MA.ServerKey(false, "YWD"))
            {
                if (_매출자동여부 == "Y" && _자동승인여부 == "Y")
                {
                    if (!m_tabSo.TabPages.ContainsKey("tabPage5"))
                        m_tabSo.TabPages.Add(tabPage5);
                ...
                {
                    m_tabSo.TabPages.Remove(tabPage5);
        ...
        void 원그리드적용하기()
        {
            System.Drawing.Size s1 = m_pnlBaseInfo.Size;

            m_pnlBaseInfo.UseCustomLayout = true;
            m_pnlAppendInfo.UseCustomLayout = true;
            pnl해외.UseCustomLayout = true;
            panelExt30.UseCustomLayout = true;
            oneGrid매출정보.UseCustomLayout = true;

            m_pnlBaseInfo.IsSearchControl = false;   //입력 전용
            m_pnlAppendInfo.IsSearchControl = false;   //입력 전용
            pnl해외.IsSearchControl = false;   //입력 전용
            panelExt30.IsSearchControl = false;   //입력 전용
            oneGrid매출정보.IsSearchControl = false;   //입력 전용
            ...
            bpPanelControl57.IsNecessaryCondition = true;
            bpPanelControl60.IsNecessaryCondition = true;
            bpPanelControl64.IsNecessaryCondition = true;
            bpPanelControl65.IsNecessaryCondition = true;

            m_pnlBaseInfo.InitCustomLayout();
            m_pnlAppendInfo.InitCustomLayout();
            pnl해외.InitCustomLayout();
            panelExt30.InitCustomLayout();
            oneGrid매출정보.InitCustomLayout();

            //tabpage resizing 하기
            //System.Drawing.Size s2 = m_pnlBaseInfo.Size;
            //System.Drawing.Size t = m_tabSo.Size;
            //t.Height = t.Height + (s2.Height - s1.Height);
            //m_tabSo.Size = t;

            //if (m_pnlAppendInfo.Size.Height > t.Height)
            //{
            //    t = m_pnlAppendInfo.Size;
            //    t.Height = m_pnlAppendInfo.Height + (s2.Height - s1.Height);
            //    m_pnlAppendInfo.Size = t;
            //}
            //if (pnl해외.Size.Height > t.Height)
            //{
            //    t = pnl해외.Size;
            //    t.Height = pnl해외.Height + (s2.Height - s1.Height);
            //    pnl해외.Size = t;
            //}

            if (서버키 == "MHIK") // 엠에이치파워시스템즈코리아 전용
            {
                txt수주번호.Visible = false;
                Duzon.Common.Controls.TextBoxExt txt전용수주번호 = new Duzon.Common.Controls.TextBoxExt();
                txt전용수주번호.BackColor = System.Drawing.Color.FromArgb(((int)(((byte)(255)))), ((int)(((byte)(237)))), ((int)(((byte)(242)))));
                txt전용수주번호.BorderColor = System.Drawing.Color.FromArgb(((int)(((byte)(189)))), ((int)(((byte)(199)))), ((int)(((byte)(217)))));
                txt전용수주번호.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                txt전용수주번호.CharacterCasing = System.Windows.Forms.CharacterCasing.Upper;
                txt전용수주번호.Cursor = System.Windows.Forms.Cursors.IBeam;
                txt전용수주번호.Font = new System.Drawing.Font("굴림체", 9F);
                txt전용수주번호.Location = new System.Drawing.Point(81, 1);
                txt전용수주번호.MaxLength = 200;
                txt전용수주번호.Name = "txt전용수주번호";
                txt전용수주번호.SelectedAllEnabled = false;
                txt전용수주번호.Size = new System.Drawing.Size(215, 21);
                txt전용수주번호.TabIndex = 0;
                txt전용수주번호.Tag = "TXT_USERDEF4";
                txt전용수주번호.UseKeyEnter = true;
                txt전용수주번호.UseKeyF3 = false;
                bpPanelControl1.Controls.Add(txt전용수주번호);
                m_lblNoSo.Text = "전용수주번호";
            }
        ...
        void 프로모션품목셋팅(DataRow[] drs프로모션, bool isPromotion)
        {
            //_flex.RemoveViewAll();
            ...
            foreach (DataRow row프로모션 in drs프로모션)
            {
                /*//프로모션 품목인 경우 중복이면 수량만 더해준다.
                if (isPromotion)
                {
                    DataRow[] drs = _flex.DataTable.Select("CD_PLANT = '" + D.GetString(row프로모션["CD_PLANT"]) + "' AND CD_ITEM = '" + D.GetString(row프로모션["CD_ITEM_PRO"]) + "' AND FG_USE = '020'", "", DataViewRowState.CurrentRows);
                    if (drs != null && drs.Length > 0)
                    {
                        drs[0]["QT_SO"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(drs[0]["QT_SO"]) + D.GetDecimal(row프로모션["QT"]));
                        drs[0]["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row프로모션["QT"]) * D.GetDecimal(drs[0]["UM_SO"]));
                        drs[0]["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(drs[0]["AM_SO"]) * cur환율.DecimalValue);
                        drs[0]["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(drs[0]["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100m));
                        drs[0]["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(drs[0]["AM_WONAMT"]) + D.GetDecimal(drs[0]["AM_VAT"]));
                        drs[0]["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(drs[0]["AMVAT_SO"]) / D.GetDecimal(drs[0]["QT_SO"]));
                        drs[0]["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(drs[0]["QT_SO"]) * D.GetDecimal(drs[0]["UNIT_SO_FACT"]));
                        drs[0]["NUM_USERDEF2"] = D.GetDecimal(drs[0]["NUM_USERDEF1"]) * D.GetDecimal(drs[0]["QT_SO"]);
                        drs[0]["NUM_USERDEF3"] = D.GetDecimal(drs[0]["AM_WONAMT"]) - D.GetDecimal(drs[0]["NUM_USERDEF2"]);
                        drs[0]["NUM_USERDEF4"] = D.GetDecimal(drs[0]["AM_WONAMT"]) != 0m ? decimal.Round((D.GetDecimal(drs[0]["AM_WONAMT"]) - D.GetDecimal(drs[0]["NUM_USERDEF2"])) / D.GetDecimal(drs[0]["AM_WONAMT"]) * 100m, 2, MidpointRounding.AwayFromZero) : 0m;
                        continue;
                    }
                }*/

                DataRow rowItemGrpUm = dtItemGrpUm.Rows.Find(row프로모션["CD_ITEMGRP"]);
                DataRow newrow = _flex.DataTable.NewRow();
                ...
                CC조회(D.GetString(row프로모션["CD_ITEMGRP"]), out cdCc, out nmCc);
                newrow["CD_CC"] = cdCc;
                newrow["NM_CC"] = nmCc;
                ...
                newrow["DT_REQGI"] = dtp납기일.Text;
                newrow["STA_SO1"] = _수주상태;
                newrow["CD_PLANT"] = row프로모션["CD_PLANT"];
                newrow["CD_ITEM"] = isPromotion ? row프로모션["CD_ITEM_PRO"] : row프로모션["CD_ITEM"];
                newrow["NM_ITEM"] = row프로모션["NM_ITEM"];
                newrow["STND_ITEM"] = row프로모션["STND_ITEM"];
                newrow["UNIT_SO"] = row프로모션["UNIT_SO"];
                newrow["TP_ITEM"] = row프로모션["TP_ITEM"];
                newrow["TP_VAT"] = D.GetString(cbo부가세구분.SelectedValue);
                newrow["RT_VAT"] = cur부가세율.DecimalValue;
                newrow["QT_SO"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(row프로모션["QT"]));
                newrow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row프로모션["UM_ITEM"]));
                newrow["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row프로모션["QT"]) * D.GetDecimal(row프로모션["UM_ITEM"]));
                newrow["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newrow["AM_SO"]) * cur환율.DecimalValue);
                newrow["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newrow["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100m));
                newrow["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newrow["AM_WONAMT"]) + D.GetDecimal(newrow["AM_VAT"]));
                newrow["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(newrow["AMVAT_SO"]) / D.GetDecimal(newrow["QT_SO"]));
                newrow["UNIT_SO_FACT"] = D.GetDecimal(row프로모션["UNIT_SO_FACT"]) == 0 ? 1 : row프로모션["UNIT_SO_FACT"];
                newrow["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(newrow["QT_SO"]) * D.GetDecimal(newrow["UNIT_SO_FACT"]));
                newrow["UNIT_IM"] = row프로모션["UNIT_IM"];
                newrow["EN_ITEM"] = row프로모션["EN_ITEM"];
                newrow["STND_DETAIL_ITEM"] = row프로모션["STND_DETAIL_ITEM"];
                newrow["GRP_MFG"] = row프로모션["GRP_MFG"];
                newrow["LT_GI"] = D.GetDecimal(row프로모션["LT_GI"]);
                newrow["WEIGHT"] = row프로모션["WEIGHT"];
                newrow["UNIT_WEIGHT"] = row프로모션["UNIT_WEIGHT"];
                newrow["FG_SERNO"] = row프로모션["FG_SERNO"];
                newrow["YN_ATP"] = row프로모션["YN_ATP"];
                newrow["CUR_ATP_DAY"] = row프로모션["CUR_ATP_DAY"];
                newrow["FG_MODEL"] = row프로모션["FG_MODEL"];
                newrow["UNIT_GI_FACT"] = D.GetDecimal(row프로모션["UNIT_GI_FACT"]) == 0m ? 1m : D.GetDecimal(row프로모션["UNIT_GI_FACT"]);
                newrow["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                newrow["CD_MNGD1"] = bp관리내역1.CodeValue;
                newrow["NM_MNGD1"] = bp관리내역1.CodeName;
                newrow["CD_MNGD2"] = bp관리내역2.CodeValue;
                ...
                newrow["CD_MNGD4"] = txt관리내역4.Text;
                newrow["TP_IV"] = _매출형태;
                newrow["MAT_ITEM"] = row프로모션["MAT_ITEM"];
                newrow["FG_USE"] = isPromotion ? "020" : "010"; //010:판매, 020:할증
                newrow["TXT_USERDEF1"] = "N";
                if (rowItemGrpUm != null)
                {
                    newrow["NUM_USERDEF1"] = D.GetDecimal(rowItemGrpUm["UM_SALES"]) * cur환율.DecimalValue;
                    newrow["NUM_USERDEF2"] = D.GetDecimal(newrow["NUM_USERDEF1"]) * D.GetDecimal(newrow["QT_SO"]);
                    newrow["NUM_USERDEF3"] = D.GetDecimal(newrow["AM_WONAMT"]) - D.GetDecimal(newrow["NUM_USERDEF2"]);
                    newrow["NUM_USERDEF4"] = D.GetDecimal(newrow["AM_WONAMT"]) != 0m ? decimal.Round((D.GetDecimal(newrow["AM_WONAMT"]) - D.GetDecimal(newrow["NUM_USERDEF2"])) / D.GetDecimal(newrow["AM_WONAMT"]) * 100m, 2, MidpointRounding.AwayFromZero) : 0m;                    
                }
                newrow["NUM_USERDEF5"] = D.GetDecimal(newrow["UM_SO"]);
                newrow["NUM_USERDEF6"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row프로모션["UM_ITEM_LOW"]));
                newrow["CD_ITEM_REF"] = isPromotion ? row프로모션["CD_ITEM"] : string.Empty;
                newrow["NM_ITEM_REF"] = isPromotion ? row프로모션["NM_ITEM_M"] : string.Empty;
                newrow["STND_ITEM_REF"] = isPromotion ? row프로모션["STND_ITEM_M"] : string.Empty;
        ...
        private bool Chk_SDBIO_UM(DataTable dt, bool isSave)
        {
            string multiItem = Common.MultiString(dt, "CD_ITEM", "|");
            DataTable dtUm = _biz.Search_SDBIO_UM(D.GetString(cbo공장.SelectedValue), multiItem, ctx거래처.CodeValue, dtp수주일자.Text,
                                                  D.GetString(cbo단가유형.SelectedValue), D.GetString(cbo화폐단위.SelectedValue));
            dtUm.PrimaryKey = new DataColumn[] { dtUm.Columns["CD_ITEM"] };

            foreach (DataRow row in dt.Rows)
            {
                if (D.GetString(row["FG_USE"]) != "010") continue;

                DataRow findRow = dtUm.Rows.Find(D.GetString(row["CD_ITEM"]));

                if (D.GetDecimal(row["UM_SO"]) < D.GetDecimal(findRow["UM_ITEM_LOW"]))
                {
                    ShowMessage("단가가 결재단가보다 작습니다. 영업관리팀에 문의하여 주십시오");
                    return false;
                }

                if (isSave)
                {
                    if (D.GetDecimal(findRow["UM_ITEM"]) == decimal.Zero || D.GetDecimal(row["UM_SO"]) < D.GetDecimal(findRow["UM_ITEM"]))
                    {
                        ShowMessage("단가가 기준단가보다 작습니다. 결재상신하여 주십시오");
                        return false;
                    }
                }
            }
        ...
        #region ▷ 지니틱스전용로직            
        private void 판매계획품목셋팅(DataRow[] dr판매계획S)
        {
            try
            {
                string cdCc, nmCc;

                foreach (DataRow row판매계획 in dr판매계획S)
                {
                    DataRow newrow = _flex.DataTable.NewRow();
                    ...
                    CC조회(D.GetString(row판매계획["GRP_ITEM"]), out cdCc, out nmCc);
                    newrow["CD_CC"] = cdCc;
                    newrow["NM_CC"] = nmCc;
                    ...
                    newrow["DT_REQGI"] = dtp납기일.Text;
                    newrow["STA_SO1"] = _수주상태;
                    newrow["CD_PLANT"] = row판매계획["CD_PLANT"];
                    newrow["CD_ITEM"] = row판매계획["CD_ITEM"];
                    newrow["NM_ITEM"] = row판매계획["NM_ITEM"];
                    newrow["STND_ITEM"] = row판매계획["STND_ITEM"];
                    newrow["UNIT_SO"] = row판매계획["UNIT_SO"];
                    newrow["TP_ITEM"] = row판매계획["TP_ITEM"];
                    newrow["TP_VAT"] = D.GetString(cbo부가세구분.SelectedValue);
                    newrow["RT_VAT"] = cur부가세율.DecimalValue;
                    newrow["QT_SO"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(row판매계획["QT_PLAN"]));
                    newrow["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(row판매계획["UM_PLAN"]));
                    newrow["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row판매계획["QT_PLAN"]) * D.GetDecimal(row판매계획["UM_PLAN"]));
                    newrow["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newrow["AM_SO"]) * cur환율.DecimalValue);
                    newrow["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newrow["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100m));
                    newrow["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newrow["AM_WONAMT"]) + D.GetDecimal(newrow["AM_VAT"]));
                    newrow["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(newrow["AMVAT_SO"]) / D.GetDecimal(newrow["QT_SO"]));
                    newrow["UNIT_SO_FACT"] = D.GetDecimal(row판매계획["UNIT_SO_FACT"]) == 0 ? 1 : row판매계획["UNIT_SO_FACT"];
                    newrow["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(newrow["QT_SO"]) * D.GetDecimal(newrow["UNIT_SO_FACT"]));
                    newrow["UNIT_IM"] = row판매계획["UNIT_IM"];
                    newrow["EN_ITEM"] = row판매계획["EN_ITEM"];
                    newrow["STND_DETAIL_ITEM"] = row판매계획["STND_DETAIL_ITEM"];
                    newrow["GRP_MFG"] = row판매계획["GRP_MFG"];
                    newrow["LT_GI"] = D.GetDecimal(row판매계획["LT_GI"]);
                    newrow["WEIGHT"] = row판매계획["WEIGHT"];
                    newrow["UNIT_WEIGHT"] = row판매계획["UNIT_WEIGHT"];
                    newrow["FG_SERNO"] = row판매계획["FG_SERNO"];
                    newrow["YN_ATP"] = row판매계획["YN_ATP"];
                    newrow["CUR_ATP_DAY"] = row판매계획["CUR_ATP_DAY"];
                    newrow["FG_MODEL"] = row판매계획["FG_MODEL"];
                    newrow["UNIT_GI_FACT"] = D.GetDecimal(row판매계획["UNIT_GI_FACT"]) == 0m ? 1m : D.GetDecimal(row판매계획["UNIT_GI_FACT"]);
                    newrow["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                    newrow["CD_MNGD1"] = bp관리내역1.CodeValue;
                    newrow["NM_MNGD1"] = bp관리내역1.CodeName;
                    newrow["CD_MNGD2"] = bp관리내역2.CodeValue;
                    ...
                    newrow["CD_MNGD4"] = txt관리내역4.Text;
                    newrow["TP_IV"] = _매출형태;
                    newrow["MAT_ITEM"] = row판매계획["MAT_ITEM"];
                    newrow["NUM_USERDEF1"] = D.GetString(row판매계획["NUM_USERDEF1"]); //NO_WEEK
                    newrow["NUM_USERDEF1"] = D.GetString(row판매계획["NUM_USERDEF1"]); //SEQ
        ...
        private void Set품목셋팅(int idx, DataRow[] drSet품목S)
        {
            try
            {
                ...
                if (서버키 == "SANGBO")
                {
                    dt상보DC = _biz.상보할인율(공장, ctx거래처.CodeValue, dtp수주일자.Text, drSet품목S);
                }
                ...
                foreach (DataRow rowSet품목 in drSet품목S)
                {
                    if (First)
                    {
                        ...
                        _flex[idx, "DT_DUEDATE"] = dtp납기일.Text;
                        _flex[idx, "DT_REQGI"] = dtp납기일.Text;
                        _flex[idx, "STA_SO1"] = _수주상태;
                        ...
                        _flex[idx, "QT_IM"] = Unit.수량(DataDictionaryTypes.SA, c.관리수량(D.GetDecimal(_flex["QT_SO"]), D.GetDecimal(_flex["UNIT_SO_FACT"])));
                        _flex[idx, "UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(rowSet품목["UM_SET"]));
                        _flex[idx, "AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[idx, "QT_SO"]) * D.GetDecimal(_flex[idx, "UM_SO"]));
                        _flex[idx, "AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_SO"]) * cur환율.DecimalValue);
                        _flex[idx, "AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100m));
                        _flex[idx, "AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) + D.GetDecimal(_flex["AM_VAT"]));
                        _flex[idx, "UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) / D.GetDecimal(_flex["QT_SO"]));
                        ...
                        _flex[idx, "CD_MNGD1"] = bp관리내역1.CodeValue;
                        _flex[idx, "NM_MNGD1"] = bp관리내역1.CodeName;
                        _flex[idx, "CD_MNGD2"] = bp관리내역2.CodeValue;
                        _flex[idx, "NM_MNGD2"] = bp관리내역2.CodeName;
                        _flex[idx, "CD_MNGD3"] = bp관리내역3.CodeValue;
                        _flex[idx, "NM_MNGD3"] = bp관리내역3.CodeName;
                        _flex[idx, "CD_MNGD4"] = txt관리내역4.Text;
                        _flex[idx, "TP_IV"] = "002";
                    ...
                    else
                    {
                        ...
                        _flex["SEQ_SO"] = 최대차수 + 1;
                        _flex["GI_PARTNER"] = ctx납품처.CodeValue;
                        _flex["LN_PARTNER"] = ctx납품처.CodeName; if (disCount_YN == "Y")
                        {
                        ...
                        _flex["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["QT_SO"]) * D.GetDecimal(_flex["UM_SO"]));
                        _flex["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_SO"]) * cur환율.DecimalValue);
                        _flex["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) * (cur부가세율.DecimalValue / 100m));
                        _flex["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) + D.GetDecimal(_flex["AM_VAT"]));
                        _flex["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) / D.GetDecimal(_flex["QT_SO"]));
                        ...
                        _flex["CD_EXCH"] = D.GetString(cbo화폐단위.SelectedValue);
                        _flex["CD_MNGD1"] = bp관리내역1.CodeValue;
                        _flex["NM_MNGD1"] = bp관리내역1.CodeName;
                        _flex["CD_MNGD2"] = bp관리내역2.CodeValue;
                        _flex["NM_MNGD2"] = bp관리내역2.CodeName;
                        _flex["CD_MNGD3"] = bp관리내역3.CodeValue;
                        _flex["NM_MNGD3"] = bp관리내역3.CodeName;
                        _flex["CD_MNGD4"] = txt관리내역4.Text;
                        _flex["TP_IV"] = "002";
        ...
        bool Chk매출일자 { get { return Checker.IsValid(dtp매출일자, true, lbl매출일자.Text); } }
        bool Chk수금예정일자 { get { return Checker.IsValid(dtp수금예정일자, true, lbl수금예정일자.Text); } }
        #endregion
