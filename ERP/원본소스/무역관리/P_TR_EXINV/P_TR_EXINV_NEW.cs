...
namespace trade
{
    ...
    // **************************************
    public partial class P_TR_EXINV_NEW : Duzon.Common.Forms.PageBase
    {
        #region ★ 멤버필드
        ...
        protected override void InitPaint()
        {
            ...
            bpPanelControl1.IsNecessaryCondition = bpPanelControl3.IsNecessaryCondition = bpPanelControl4.IsNecessaryCondition = 
            bpPanelControl9.IsNecessaryCondition = bpPanelControl12.IsNecessaryCondition = bpPanelControl5.IsNecessaryCondition =
            bpPanelControl21.IsNecessaryCondition = bpPanelControl19 .IsNecessaryCondition = true;
            ...
            // 원산지
            DataTable _국가 = Global.MainFrame.FillDataTable("SELECT CD_SYSDEF AS CODE, NM_SYSDEF AS NAME FROM MA_CODEDTL WHERE CD_COMPANY = '" + CompanyCode + "' AND CD_FIELD = 'MA_B000020' AND USE_YN = 'Y' ORDER BY NAME ");
        ...
        public override bool OnToolBarExitButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IsExistPage("P_TR_EXINV_NEWL", false))
                {
                    if (MessageBoxEx.Show("내역등록창을 먼저 닫아야 합니다. 창을 닫으시겠습니까 ?", PageName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                        return false;
                    UnLoadPage("P_TR_EXINV_NEWL", false);
                }
        ...
        private void OnCloseLoadPageFrom_출고적용(object sender, System.EventArgs e)
        {
            ...
            if (row.Table.Columns.Contains("CD_BANK_SO"))
            {
                _header.CurrentRow["CD_BANK"] = ctx은행.CodeValue = D.GetString(row["CD_BANK_SO"]);
                ctx은행.CodeName = D.GetString(row["NM_BANK_SO"]);
            }

            if (row.Table.Columns.Contains("COND_PAY"))
            {
                _header.CurrentRow[D.GetString(cbo결제형태.Tag)] = cbo결제형태.SelectedValue = D.GetString(row["COND_PAY"]);
            }

            if (row.Table.Columns.Contains("COND_PRICE"))
            {
                _header.CurrentRow[D.GetString(cbo가격조건.Tag)] = cbo가격조건.SelectedValue = D.GetString(row["COND_PRICE"]);
            }

            if (row.Table.Columns.Contains("DC_RMK_TEXT"))
            {
                _header.CurrentRow[D.GetString(txtREMARK.Tag)] = txtREMARK.Text = D.GetString(row["DC_RMK_TEXT"]);
            }
        }
        ...
        private void btn첨부파일_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    AttachmentManager dlg = new AttachmentManager(Global.MainFrame.CurrentModule, Global.MainFrame.CurrentPageID, cd_file_code);
