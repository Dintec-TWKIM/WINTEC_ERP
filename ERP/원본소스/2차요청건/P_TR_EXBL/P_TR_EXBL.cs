...
namespace trade
{
    ...
    // 서브시스템명 : 수출
    // 페 이 지  명 : 선적등록
    // 프로젝트  명 : P_TR_EXBL_NEW
    // 수 정 내 역  : 2013.07.25 - 윤성우 - ONE GRID 수정(입력 전용)
    // **************************************
    public partial class P_TR_EXBL : PageBase
    {
        #region ★ 멤버필드
        ...
        public P_TR_EXBL(string str선적번호)
        {
            try
            {
                InitializeComponent();
        ...
        protected override void InitPaint()
        {
            ...
            bpPanelControl1.IsNecessaryCondition = bpPanelControl5.IsNecessaryCondition = bpPanelControl4.IsNecessaryCondition =
            bpPanelControl9.IsNecessaryCondition = bpPanelControl8.IsNecessaryCondition = bpPanelControl7.IsNecessaryCondition =
            bpPanelControl13.IsNecessaryCondition = bpPanelControl17.IsNecessaryCondition = bpPanelControl21.IsNecessaryCondition = 
            bpPanelControl24.IsNecessaryCondition = true;
            ...
            lds_Combo.Tables[2].DefaultView.Sort = "NAME ASC";
            ...
            _header.SetBinding(dt, panelExt1);
            ...
            cur외화금액.Mask = this.GetFormatDescription(DataDictionaryTypes.TR, FormatTpType.FOREIGN_MONEY, FormatFgType.INSERT);
        ...
        protected override bool BeforeSave()
        {
            ...
            {
                if (!Global.MainFrame.ServerKey.Contains("SDBIO"))
                {
        ...
        private void cbo통화_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    // 기준일자 : 환율을 적용하는 일자
                    ld기표환율 = MA.기준환율적용(dtp기표일자.Text, D.GetString(cbo통화.SelectedValue));
                }

                if (D.GetString(cbo통화.SelectedValue) == "000")
                {
                    ld기표환율 = 1;
                    cur기표환율.Enabled = false;
                }
                ...
                switch (((Control)sender).Name)
                {
                    ...
                    case "dtp기표일자":
                        if (this.bpc거래처.CodeName.ToString() != "")
                        {
                            ...
                            {
                                if (MA.ServerKey(true, new string[] { "SDBIO" }))
                                {
                                    AddDay = int.Parse(txtDAYS.Text);
                                    this.dtp결제만기일.Value = this.dtp기표일자.Value.AddDays(AddDay);
                                    _header.CurrentRow["DT_PAYABLE"] = this.dtp결제만기일.Text;
                                }
                            }
            ...
            try
            {
                ...
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (_송장내역 != null || _송장내역.Rows.Count > 0)
                    {
                        ...
                        if (MA.ServerKey(true, new string[] { "SDBIO" }))
                        {
                            DataRow rowSoh = _biz.SearchSoh(D.GetString(_송장내역.Rows[0]["NO_INV"]));

                            _header.CurrentRow["COND_PAY"] = D.GetString(rowSoh["COND_PAY"]);
                            cbo결제형태.SelectedValue = D.GetString(rowSoh["COND_PAY"]);
                            _header.CurrentRow["COND_DAYS"] = D.GetDecimal(rowSoh["COND_DAYS"]);
                            txtDAYS.DecimalValue = D.GetDecimal(rowSoh["COND_DAYS"]);
                        }
                        ...
                        if (Global.MainFrame.DatabaseType == EnumDbType.MSSQL)
                        {
                            dt = Global.MainFrame.FillDataTable("SELECT TOP 1 DT_RCP_PREARRANGED FROM MA_PARTNER WHERE CD_COMPANY = '" + CompanyCode + "' AND CD_PARTNER = '" + _거래처코드 + "' ");
                        }
                        else
                        {
                            dt = Global.MainFrame.FillDataTable("SELECT DT_RCP_PREARRANGED FROM MA_PARTNER WHERE CD_COMPANY = '" + CompanyCode + "' AND CD_PARTNER = '" + _거래처코드 + "' AND ROWNUM = 1");
                        }
        ...
        #region -> dtp기표일자_DateChanged
        private void dtp기표일자_DateChanged(object sender, EventArgs e)
        {
            try
            {
                //DateChanged 이벤트를 쓰면 이미 저장된 데이타 불러올때도 타므로 _header_ControlValueChanged으로 변경(D20140324015)
                //cbo통화_SelectionChangeCommitted(null, null);
            }
            catch (Exception ex)
            {
        ...
        {
            decimal RT_EXCH = 0m;
            RT_EXCH = Unit.환율(DataDictionaryTypes.TR, MA.기준환율적용(dtp기표일자.Text, D.GetString(cbo통화.SelectedValue)));
            cur기표환율.DecimalValue = RT_EXCH == 0m ? 1 : RT_EXCH;
            _header.CurrentRow["RT_EXCH"] = cur기표환율.DecimalValue;
        } 
        #endregion
