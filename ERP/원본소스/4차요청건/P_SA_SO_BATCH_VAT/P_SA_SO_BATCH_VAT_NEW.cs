...
namespace sale
{
    ...
    // 프로젝트  명 : P_SA_PROJECT
    // **************************************
    public partial class P_SA_SO_BATCH_VAT_NEW : Duzon.Common.Forms.PageBase
    {
        #region ♣ 생성자 & 변수 선언 

        private enum ExcelType { NONE, 부가세포함단가, 부가세포함금액, 부가세포함단가금액없음 };
        ...
        { 
            ...
            _flex.SetCodeHelpCol("CD_CC", Duzon.Common.Forms.Help.HelpID.P_MA_CC_SUB, ShowHelpEnum.Always,
                                  ...
                                  new string[] { "CD_CC", "NM_CC" });
            _flex.SetCodeHelpCol("CD_SALEGRP", Duzon.Common.Forms.Help.HelpID.P_MA_SALEGRP_SUB, ShowHelpEnum.Always,
                                  new string[] { "CD_SALEGRP", "NM_SALEGRP", "TP_SALEPRICE" },
                                  new string[] { "CD_SALEGRP", "NM_SALEGRP", "TP_SALEPRICE" });
            ...
            _flex.SetExceptEditCol("NO_SO", "SEQ_SO", "LN_PARTNER", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "UM_SO", "AM_SO", "AM_WONAMT", "NM_SL", "UNIT_SO_FACT", "GN_PARTNER", "RT_VAT", "NM_CC", "NM_SALEGRP", "NM_PROJECT");

            //그리드 필수입력 해야 하는부분 셋팅
            if (Global.MainFrame.ServerKeyCommon.ToUpper().Contains("HDS")) // 현대에이치디에스
            {
                _flex.VerifyNotNull = new string[] { "CD_PARTNER", "CD_PLANT", "CD_ITEM", "DT_DUEDATE", "DT_REQGI", "TP_VAT", "CD_CC", "CD_SALEGRP", "NO_PROJECT" };
            }
            else
            {
                _flex.VerifyNotNull = new string[] { "CD_PARTNER", "CD_PLANT", "CD_ITEM", "DT_DUEDATE", "DT_REQGI", "TP_VAT", "CD_CC", "CD_SALEGRP" };
            }
            //그리드에 값을 비교하는 부분
        ...
        {
            m_pnHead.UseCustomLayout = true;
            m_pnHead.IsSearchControl = false;   //입력 전용
            bpPanelControl1.IsNecessaryCondition = bpPanelControl2.IsNecessaryCondition = bpPanelControl3.IsNecessaryCondition = bpPanelControl8.IsNecessaryCondition = true;
            bpPanelControl5.IsNecessaryCondition = bpPanelControl6.IsNecessaryCondition = bpPanelControl7.IsNecessaryCondition = true;
            if (App.SystemEnv.PROJECT사용)
                bpPanelControl4.IsNecessaryCondition = true;

            m_pnHead.InitCustomLayout();
            ...
            _flex.SetDataMap("FG_USE2", MA.GetCode("SA_B000063", true), "CODE", "NAME");
            ...
            cbo_TpVat_SelectedIndexChanged(null, null);
        ...
        private void InitEvent()
        {
            ...
            btnApplySaleGrp.Click += new EventHandler(btnApplySaleGrp_Click);
        }
        ...
        private void btn_AddRow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _flex["CD_SALEGRP"] = bp_SalesGroup.CodeValue;
                _flex["NM_SALEGRP"] = bp_SalesGroup.CodeName;
                ...
                //_flex["CD_CC"] = cd_CC;
                //_flex["NM_CC"] = nm_CC;
                if (D.GetString(_flex["CD_SALEGRP"]) != string.Empty)
                {
                    string cdCc = string.Empty;
                    string nmCc = string.Empty;

                    CC조회(D.GetString(_flex["CD_SALEGRP"]), out cdCc, out nmCc);
                    _flex["CD_CC"] = cdCc;
                    _flex["NM_CC"] = nmCc;

                    DataRow rowSaleGrp = BASIC.GetSaleGrp(D.GetString(_flex["CD_SALEGRP"]));
                    if (rowSaleGrp == null) return;

                    _flex["TP_SALEPRICE"] = D.GetString(rowSaleGrp["TP_SALEPRICE"]);
                    _flex["SO_PRICE"] = D.GetString(rowSaleGrp["SO_PRICE"]);
                }
        ...
        protected override bool SaveData()
        {
            ...
            #region 셀바이오텍전용
            DataTable dt거래처정보 = null;
            if (MA.ServerKey(false, "CELLBIO")) //쎌바이오텍
            {
                string MULTI_CD_PARTNER = Common.MultiString(_flex.DataTable.DefaultView.ToTable(true, "CD_PARTNER"), "CD_PARTNER", "|");
                dt거래처정보 = _biz.GetPartnerInfoSearch(MULTI_CD_PARTNER);
                dt거래처정보.PrimaryKey = new DataColumn[] { dt거래처정보.Columns["CD_PARTNER"] };
                foreach (DataRow row in dt거래처정보.Rows)
                {
                    if (D.GetString(row["CD_EMP_SALE"]) == string.Empty)
                    {
                        ShowMessage(" 거래처[" + D.GetString(row["CD_PARTNER"]) + "]에 영업담당자가 등록되어 있지 않습니다.");
                        return false;
                    }
                }
            } 
            ...
            try
            {
                ...
                {
                    no_So = D.GetString(dr["NO_SO"]);

                    if (string.IsNullOrEmpty(no_So))
                    {
                        string filter = "CD_PARTNER = '" + D.GetString(dr["CD_PARTNER"]) + "' AND "
                                      + "CD_SALEGRP = '" + D.GetString(dr["CD_SALEGRP"]) + "' AND "
                                      + "ISNULL(DC_RMK, '') = '" + D.GetString(dr["DC_RMK"]) + "' AND "
                                      + "ISNULL(RMA_REASON, '') = '" + D.GetString(dr["RMA_REASON"]) + "'";
                        ...
                        if (drh.Length == 0)
                        {
                            ...
                            dr_SoH["CD_PARTNER"] = D.GetString(dr["CD_PARTNER"]);
                            dr_SoH["CD_SALEGRP"] = D.GetString(dr["CD_SALEGRP"]);    //bp_SalesGroup.CodeValue;
                            if (MA.ServerKey(false, "CELLBIO")) //쎌바이오텍
                            {
                                DataRow row거래처정보 = dt거래처정보.Rows.Find(D.GetString(dr["CD_PARTNER"]));
                                if(row거래처정보 != null)
                                    dr_SoH["NO_EMP"] = row거래처정보["CD_EMP_SALE"];
                            }
                            else
                            ...
                            dr_SoH["NO_PROJECT"] = dr["NO_PROJECT"];
                            ...
                            if (MA.ServerKey(false, new string[] { "NUGA" })) // 누가의료기기
                                dr_SoH["FG_TAXP"] = D.GetString(dr["FG_TAXP"]) == string.Empty ? "001" : D.GetString(dr["FG_TAXP"]);
                            else
                                dr_SoH["FG_TAXP"] = rdo계산서처리일괄.Checked ? "001" : "002";  //계산서처리 : 001 일괄 , 002 건별
                    ...
                    {
                        string filter = "NO_SO = '" + D.GetString(dr["NO_SO"]) + "'";
                        ...
                        if (drh.Length == 0)
                        {
                            ...
                            dr_SoH["CD_SALEGRP"] = dr["CD_SALEGRP"];    //bp_SalesGroup.CodeValue;
                            dr_SoH["NO_EMP"] = bp_Emp.CodeValue;
                            dr_SoH["TP_SO"] = bp_SoType.CodeValue;
                            dr_SoH["CD_EXCH"] = cbo_CdExch.SelectedValue.ToString();
                            dr_SoH["RT_EXCH"] = txt_RtExch.DecimalValue;
                ...
                {
                    string filterNo_SO = "NO_SO = '" + D.GetString(dr["NO_SO"]) + "'";
                    DataRow[] drNO_SO = dt.Select(filterNo_SO);

                    if (D.GetString(dr["NO_SO"]) == string.Empty || drNO_SO.Length == 0)
                    {
                        decimal seq_No = 1;
                        ...
                        foreach (DataRow dl in drl)
                        {
                            ...
                            dr_SoL["NO_PROJECT"] = dl["NO_PROJECT"];
                            ...
                            dr_SoL["RT_DSCNT"] = 0;
                            dr_SoL["UM_BASE"] = 0;
                            dr_SoL["CD_ZIP"] = dl["CD_ZIP"];
                            dr_SoL["ADDR1"] = dl["ADDR1"];
                            dr_SoL["ADDR2"] = dl["ADDR2"];
                            dr_SoL["NM_CUST_DLV"] = dl["NM_CUST_DLV"];
                            dr_SoL["NO_TEL_D1"] = dl["NO_TEL_D1"];
                            dr_SoL["NO_TEL_D2"] = dl["NO_TEL_D2"];
                            dr_SoL["NUM_USERDEF1"] = dl["NUM_USERDEF1"];
                            dr_SoL["NUM_USERDEF2"] = dl["NUM_USERDEF2"];
                    ...
                    {
                        foreach (DataRow dl in drNO_SO)
                        {
                            DataRow dr_SoL = dt_SoL.NewRow();

                            dr_SoL["NO_SO"] = dr["NO_SO"];
                            dr_SoL["NO_HST"] = no_Hst;
                            dr_SoL["SEQ_SO"] = dl["SEQ_SO"];
                            dr_SoL["CD_PLANT"] = dl["CD_PLANT"];
                            dr_SoL["CD_ITEM"] = dl["CD_ITEM"];
                            dr_SoL["UNIT_SO"] = dl["UNIT_SO"];
                            dr_SoL["DT_DUEDATE"] = dl["DT_DUEDATE"];
				...
				if (MA.ServerKey(true, new string[] { "LINEP", "CNP", "DINTEC", "MCIRCLE" }))
                {
        ...
        {
            if (!ErrorCheck()) return;  //필수입력 체크
            if (bp_SalesGroup.IsEmpty() && BASIC.GetMAEXC("거래처선택-영업그룹적용") != "Y")
            {
                ShowMessage(공통메세지._은는필수입력항목입니다, lbl_영업그룹.Text);
                return;
            }
            ...
            try
            {
                ...
                if (!CheckTable.Columns.Contains("UMVAT_SO") && !CheckTable.Columns.Contains("AMVAT_SO"))
                {
                    ...
                    excelTypeEnum = ExcelType.부가세포함단가금액없음;
                }
                ...
                if (!CheckTable.Columns.Contains("CD_SALEGRP"))
                {
                    CheckTable.Columns.Add("CD_SALEGRP", typeof(string));                    
                }

                if (!CheckTable.Columns.Contains("NM_SALEGRP"))
                {
                    CheckTable.Columns.Add("NM_SALEGRP", typeof(string));
                }
                ...
                if (Global.MainFrame.ServerKeyCommon == "NEOPH")
                {
                    if (!ResultTable.Columns.Contains("FG_USE2")) ResultTable.Columns.Add("FG_USE2");

                    foreach (DataRow row in ResultTable.Rows)
                    {
                        string PITEM_CD_USERDEF1 = D.GetString(row["PITEM_CD_USERDEF1"]);

                        decimal UM_SO = D.GetDecimal(row["UM_SO"]);

                        if (PITEM_CD_USERDEF1 == "300")
                        {
                            row["FG_USE2"] = "300"; //샘플
                        }
                        else if (PITEM_CD_USERDEF1 == "101")
                        {
                            row["FG_USE2"] = "101";
                        }
                        else if (PITEM_CD_USERDEF1 == "100")
                        {
                            if (D.GetDecimal(UM_SO) == 0m )
                                row["FG_USE2"] = "101";  //할증
                            else
                                row["FG_USE2"] = "100";  //본품
                        }
        ...
        private DataTable SO_PK_Check(DataTable dt_Excel, out bool b)
        {
            ...
            try
            {
                ...
                foreach (DataRow excelDr in dt_Excel.Rows)
                {
					...
					CheckDr = CheckSet.Tables[0].Select("CD_PLANT = '" + excelDr["CD_PLANT"].ToString() + "'" + "AND CD_ITEM = '" + excelDr["CD_ITEM"].ToString() + "'");
                    ...
                    foreach (DataRow drs in CheckDr)
                    {
                        ...
                        excelDr["PITEM_CD_USERDEF1"] = D.GetString(drs["CD_USERDEF1"]);
                    }
                    #endregion
                    ...
                    if (D.GetString(excelDr["NO_PROJECT"]) == string.Empty)
                    {
                        if(bp_PJT.CodeValue != string.Empty)
                            excelDr["NM_PROJECT"] = bp_PJT.CodeName;
                    }
                    else
                    {
                        DataRow rowProject = Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(MasterSearch.SA_PROJECTH, new string[] { MA.Login.회사코드, D.GetString(excelDr["NO_PROJECT"]) });
                        excelDr["NM_PROJECT"] = rowProject["NM_PROJECT"];
                    }
                } 
            }
            catch (Exception ex)
        ...
        private DataTable GetCalculater(DataTable CheckTable, ExcelType excelTypeEnum)
        {
            ...
            string cdSaleGrp = string.Empty;
            string nmSaleGrp = string.Empty;
            string cdCc = string.Empty;
            string nmCc = string.Empty;
            string tpSalePrice = string.Empty;
            string soPrice = string.Empty;

            foreach (DataRow dr in CheckTable.Rows)
            {
                if (!MA.ServerKey(false, "NUGA", "AMICOGEN"))
                {
                ...
                if (excelTypeEnum != ExcelType.NONE)
                {
                    if (영업그룹설정여부 == "Y")
                    {
                        DataRow row거래처 = BASIC.GetPartner(D.GetString(dr["CD_PARTNER"]));
                        cdSaleGrp = D.GetString(row거래처["CD_SALEGRP"]);
                        nmSaleGrp = D.GetString(row거래처["NM_SALEGRP"]);
                        _flex["CD_SALEGRP"] = cdSaleGrp;
                        _flex["NM_SALEGRP"] = nmSaleGrp;

                        if (cdSaleGrp != string.Empty)
                        {
                            DataRow rowSaleGrp = BASIC.GetSaleGrp(cdSaleGrp);

                            CC조회(cdSaleGrp, out cdCc, out nmCc);
                    ...
                    {
                        dr["CD_SALEGRP"] = bp_SalesGroup.CodeValue;
                        dr["NM_SALEGRP"] = bp_SalesGroup.CodeName;

                        if (bp_SalesGroup.CodeValue != string.Empty)
                        {
                            DataRow rowSaleGrp = BASIC.GetSaleGrp(bp_SalesGroup.CodeValue);

                            CC조회(bp_SalesGroup.CodeValue, out cdCc, out nmCc);
                            tpSalePrice = D.GetString(rowSaleGrp["TP_SALEPRICE"]);
                            soPrice = D.GetString(rowSaleGrp["SO_PRICE"]);

                            dr["CD_CC"] = cdCc;
                            dr["NM_CC"] = nmCc;
                            dr["TP_SALEPRICE"] = tpSalePrice;
                            dr["SO_PRICE"] = soPrice;
                        }
                    }
                }
                ...
                dr["NO_PROJECT"] = D.GetString(dr["NO_PROJECT"]) == string.Empty ? bp_PJT.CodeValue : D.GetString(dr["NO_PROJECT"]);
                ...
                else if (excelTypeEnum == ExcelType.부가세포함단가금액없음)
                {
                    if (D.GetString(dr["CD_SALEGRP"]) != string.Empty)
                    {
                        decimal umVatSo = BASIC.GetUM(D.GetString(dr["CD_PLANT"]), D.GetString(dr["CD_ITEM"]), D.GetString(dr["CD_PARTNER"]), D.GetString(dr["CD_SALEGRP"]),
                                                      dat_Sodt.Text, D.GetString(cbo_TpPrice.SelectedValue), D.GetString(cbo_CdExch.SelectedValue), "002");
                        dr["UMVAT_SO"] = umVatSo;
                        dr["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["UMVAT_SO"]) * D.GetDecimal(dr["QT_SO"]));
                    }
                }

                dr["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(dr["RT_VAT"]))));
        ...
        private DataTable getSchema(string param)
        {
            ...
            try 
            {
                ...
                { 
                    ...
                    dt.Columns.Add("FG_USE2", typeof(string));       //
                ...
                else if (param == "DEFAULT_SCHEMA") //헤더라인 상관없이 전체스키마를 가져온다.
                {
                    ...
                    dt.Columns.Add("CD_PARTNER", typeof(string));
                    dt.Columns.Add("CD_SALEGRP", typeof(string));
                    dt.Columns.Add("NM_SALEGRP", typeof(string));
                    dt.Columns.Add("NO_EMP", typeof(string));
                    dt.Columns.Add("TP_SO", typeof(string));
                    dt.Columns.Add("CD_EXCH", typeof(string));
                    dt.Columns.Add("RT_EXCH", typeof(decimal));
                    ...
                    dt.Columns.Add("TP_SALEPRICE", typeof(string)); //단가적용형태
                    dt.Columns.Add("SO_PRICE", typeof(string));     //판매단가통제유무
                    dt.Columns.Add("NUM_USERDEF1", typeof(decimal));
                    dt.Columns.Add("NUM_USERDEF2", typeof(decimal));
        ...
        private DataTable getExcelSchema(DataTable dt)
        {
            try
            {
                ...
                if (!dt.Columns.Contains("NO_PROJECT"))
                    dt.Columns.Add("NO_PROJECT", typeof(string));
                if (!dt.Columns.Contains("NM_PROJECT"))
                    dt.Columns.Add("NM_PROJECT", typeof(string));
                ...
                if (!dt.Columns.Contains("FG_USE"))
                    dt.Columns.Add("FG_USE", typeof(string));   //
                if (!dt.Columns.Contains("FG_USE2"))
                    dt.Columns.Add("FG_USE2", typeof(string));   //
                ...
                if (!dt.Columns.Contains("PITEM_CD_USERDEF1"))
                    dt.Columns.Add("PITEM_CD_USERDEF1", typeof(string));

                if (!dt.Columns.Contains("FG_TAXP"))
                    dt.Columns.Add("FG_TAXP", typeof(string));
        ...
        private void _flex_StartEdit(object sender, RowColEventArgs e)
        {
            try
            {
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "AM_SO":
                    case "AMVAT_SO":
                        if (D.GetString(_flex["SO_PRICE"]) == "Y")
                        {
                    ...
                    case "CD_ITEM":
                        if (D.GetString(_flex["CD_SALEGRP"]) == string.Empty)
                        {
                            ShowMessage(공통메세지._은는필수입력항목입니다, _flex.Cols["CD_SALEGRP"].Caption);
                            _flex.Select(_flex.Row, "CD_SALEGRP");
                            e.Cancel = true;
                            return;
                        }
                        break;
            ...
            { 
                    ...
                    (_flex["STA_SO"].ToString() == "" || _flex["STA_SO"].ToString() == "O"))
                {
                    ...
                    if (dlg.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        _flex["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(_flex["RT_VAT"]))));
            ...
            try
            {
                ...
                switch (ColName)
                {
                    ...
                    case "QT_SO":
                        ...
                        _flex[e.Row, "AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AMVAT_SO"]) * (100 / (100 + D.GetDecimal(_flex[e.Row, "RT_VAT"]))));
                ...
                if (Global.MainFrame.ServerKeyCommon == "NEOPH" || Global.MainFrame.ServerKeyCommon == "DZSQL")
                {
                    decimal AM_SO = D.GetDecimal(_flex[e.Row, "AMVAT_SO"]);
                    if (D.GetDecimal(AM_SO) > 0m && D.GetString(_flex[e.Row, "FG_USE2"]) == "101")
                        _flex[e.Row, "FG_USE2"] = "100";  //본품
                    if (D.GetDecimal(AM_SO) == 0m && D.GetString(_flex[e.Row, "FG_USE2"]) == "100")
                        _flex[e.Row, "FG_USE2"] = "101";  //할증
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "CD_SL":
                        ...
                        {
                            ShowMessage(공통메세지._은는필수입력항목입니다, _flex.Cols["CD_PLANT"].Caption);
                            e.Cancel = true;
                            return;
                        }
                        //공장품목셋팅
            ...
            try
            {
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    case "CD_ITEM":
                        ...
                        cdSaleGrp = D.GetString(_flex["CD_SALEGRP"]);
                        nmSaleGrp = D.GetString(_flex["NM_SALEGRP"]);
                        cdCc = D.GetString(_flex["CD_CC"]);
                        nmCc = D.GetString(_flex["NM_CC"]);
                        tpSalePrice = D.GetString(_flex["TP_SALEPRICE"]);
                        soPrice = D.GetString(_flex["SO_PRICE"]);
                        ...
                        foreach (DataRow row in helpReturn.Rows)
                        {
                            ...
                            if (ItemHelpName == "P_MA_PITEM_SUB1" && roopCnt > 0)
                            {
                                ...
                                _flex.Row = cnt;
                                _flex[_flex.Row, "CD_SALEGRP"] = cdSaleGrp;
                                _flex[_flex.Row, "NM_SALEGRP"] = nmSaleGrp;
                                _flex[_flex.Row, "CD_CC"] = cdCc;
                                _flex[_flex.Row, "NM_CC"] = nmCc;
                                _flex[_flex.Row, "TP_SALEPRICE"] = tpSalePrice;
                                _flex[_flex.Row, "SO_PRICE"] = soPrice;
                            }
                            ...
                            if (D.GetString(_flex[_flex.Row, "TP_SALEPRICE"]) == "002" || D.GetString(_flex[_flex.Row, "TP_SALEPRICE"]) == "003")
                            {
                                obj[1] = row["CD_ITEM"];
                            ...
                            { 
                                ...
                                _flex["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) * (100 / (100 + D.GetDecimal(_flex[e.Row, "RT_VAT"]))));
                                _flex["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) - D.GetDecimal(_flex["AM_WONAMT"]));
                                _flex["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"])); //무조건 환종이 국내 이므로 자국내 거래가 환률이 발생할 일이 있것냐~ㅋ : 그래서 환율 안 곱했음
                                _flex["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_SO"]) / (D.GetDecimal(_flex["QT_SO"]) == 0 ? 1 : D.GetDecimal(_flex["QT_SO"])));
                    ...
                    case "CD_PARTNER":
                        ...
                        if (영업그룹설정여부 == "Y")
                        {
                            DataRow row거래처 = BASIC.GetPartner(D.GetString(e.Result.Rows[0]["CD_PARTNER"]));
                            cdSaleGrp = D.GetString(row거래처["CD_SALEGRP"]);
                            nmSaleGrp = D.GetString(row거래처["NM_SALEGRP"]);
                            _flex["CD_SALEGRP"] = cdSaleGrp;
                            _flex["NM_SALEGRP"] = nmSaleGrp;
                        ...
                        break;
                    case "CD_SALEGRP":
                        CC조회(D.GetString(e.Result.Rows[0]["CD_SALEGRP"]), out cdCc, out nmCc);
                        _flex["CD_CC"] = cdCc;
                        _flex["NM_CC"] = nmCc;
                        _flex["TP_SALEPRICE"] = D.GetString(e.Result.Rows[0]["TP_SALEPRICE"]);
                        soPrice = _biz.GetSaleOrgUmCheck(new object[] { MA.Login.회사코드, D.GetString(e.Result.Rows[0]["CD_SALEGRP"]) });
                        _flex["SO_PRICE"] = soPrice;
                        break;
                    default:
                        break;
                }
        ...
        void btnApplySaleGrp_Click(object sender, EventArgs e)
        {
            try
            {
                if (!_flex.HasNormalRow) return;
                if (bp_SalesGroup.IsEmpty()) return;
                ...
                string nmCc = string.Empty;
                CC조회(bp_SalesGroup.CodeValue, out cdCc, out nmCc);
                string tpSalePrice = D.GetString(rowSaleGrp["TP_SALEPRICE"]);
                string soPrice = D.GetString(rowSaleGrp["SO_PRICE"]);

                for (int i = _flex.Rows.Fixed; i < _flex.Rows.Count; i++)
                {
                    _flex[i, "CD_SALEGRP"] = bp_SalesGroup.CodeValue;
                    _flex[i, "NM_SALEGRP"] = bp_SalesGroup.CodeName;
                    _flex[i, "CD_CC"] = cdCc;
                    _flex[i, "NM_CC"] = nmCc;
                    _flex[i, "TP_SALEPRICE"] = tpSalePrice;
                    _flex[i, "SO_PRICE"] = soPrice;
                }

                ShowMessage(공통메세지._작업을완료하였습니다, DD("적용"));
            }
            catch (Exception ex)
            {
                MsgEnd(ex);
        ...
        void CC조회(string cdSaleGrp, out string CD_CC, out string NM_CC)
        {
            수주관리.Config 수주Config = new 수주관리.Config();

            string 수주유형 = bp_SoType.CodeValue;
            string 영업그룹 = cdSaleGrp;
            CD_CC = string.Empty;
            NM_CC = string.Empty;
