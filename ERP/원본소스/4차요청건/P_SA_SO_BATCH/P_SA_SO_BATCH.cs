...
namespace sale
{
    ...
    // 프로젝트  명 : P_SA_PROJECT
    // **************************************
    public partial class P_SA_SO_BATCH : Duzon.Common.Forms.PageBase
    {
        #region ♣ 생성자 & 변수 선언 
        CommonFunction _CommFun = new CommonFunction();
        private P_SA_SO_BATCH_BIZ _biz = new P_SA_SO_BATCH_BIZ();
        ...
        protected override void InitLoad()
        {
            ...
            str동기화 = BASIC.GetMAEXC("프로젝트 거래처/영업그룹 동기화");

            //영업환경설정 여부에 따라 그리드 셋팅이 달라 질 수 있음으로 그리드 셋팅은 꼭 영업 환경설정 이후로 호출해주어야 한다.
            InitGrid();
            InitEvent();
        }
        ...
        { 
            ...
            if (Global.MainFrame.ServerKeyCommon == "DONGAH")
            {
                _flex.SetCol("S", "선택", 45, true, CheckTypeEnum.Y_N);
            }
            else _flex.SetCol("S", "선택", false);
			...
			if (Global.MainFrame.ServerKeyCommon == "DNCOMPANY")
			{
				_flex.SetCol("UM_SO", "단가", 100, 15, true, typeof(decimal), FormatTpType.MONEY);
			}
			else
			{
            ...
            _flex.SetCol("AMVAT_SO", "합계금액", 120, 17, false, typeof(decimal), FormatTpType.MONEY);
            ...
            _flex.SetCol("DC_RMK", "헤더비고", 150, true);
            _flex.SetCol("DC_RMK1", "헤더비고1", 150, true);
            _flex.SetCol("DC1", "라인비고", 150, true);
            
            if (MA.ServerKey(false, "DNCOMPANY"))
                _flex.SetCol("DC2", "수화주전화번호", 150, true);
            else
                _flex.SetCol("DC2", "라인비고1", 150, true);
            ...
            if (str동기화 == "N")
            {
                _flex.SetCol("NO_PROJECT", "프로젝트", 100, true);
                _flex.SetCol("NM_PROJECT", "프로젝트명", 120, false);
            }

            _flex.SetCol("CD_ITEM_PARTNER", "거래처품번", 120, true);
            _flex.SetCol("NM_ITEM_PARTNER", "발주품번", 150, false);
            ...
            _flex.SetCol("SOL_TXT_USERDEF1", "TEXT사용자정의1", 150, true); _flex.Cols["SOL_TXT_USERDEF1"].Visible = false;
            _flex.SetCol("SOL_TXT_USERDEF2", "TEXT사용자정의2", 150, true); _flex.Cols["SOL_TXT_USERDEF2"].Visible = false;

            if (MA.ServerKey(false, "DNCOMPANY"))
            {
                _flex.SetCol("CD_MNGD1", "주문번호", 150, true);
                _flex.SetCol("CD_MNGD2", "수화주명", 150, true);
                _flex.SetCol("CD_MNGD3", "주문자", 150, true);
                _flex.SetCol("CD_MNGD4", "소속", 150, true);
            }

            //그리드에 도움창 띄워주는 부분
            _flex.SetCodeHelpCol("CD_ITEM", Duzon.Common.Forms.Help.HelpID.P_MA_PITEM_SUB1, ShowHelpEnum.Always, 
                                  new string[] { "CD_ITEM", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "TP_ITEM", "CD_SL", "NM_SL", "UNIT_SO_FACT", "LT_GI" }, 
                                  new string[] { "CD_ITEM", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "TP_ITEM", "CD_GISL", "NM_GISL", "UNIT_SO_FACT", "LT_GI" }, 
            ...
            _flex.SetCodeHelpCol("CD_CC", Duzon.Common.Forms.Help.HelpID.P_MA_CC_SUB, ShowHelpEnum.Always,
                                  ...
                                  new string[] { "CD_CC", "NM_CC" });
            _flex.SetCodeHelpCol("NO_PROJECT", "H_SA_PRJ_SUB", ShowHelpEnum.Always,
                                  new string[] { "NO_PROJECT", "NM_PROJECT" },
                                  new string[] { "NO_PROJECT", "NM_PROJECT" });
            _flex.SetCodeHelpCol("CD_ITEM_PARTNER", Duzon.Common.Forms.Help.HelpID.P_MA_CPITEM_SUB1, ShowHelpEnum.Always,
                                  new string[] { "CD_ITEM_PARTNER", "NM_ITEM_PARTNER", "CD_ITEM", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "TP_ITEM", "CD_SL", "NM_SL", "UNIT_SO_FACT", "LT_GI" },
                                  new string[] { "NM_ITEM_PARTNER", "CD_ITEM_PARTNER", "CD_ITEM", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "TP_ITEM", "CD_GISL", "NM_GISL", "UNIT_SO_FACT", "LT_GI" }, Duzon.Common.Forms.Help.ResultMode.SlowMode);
            //그리드에 Edit 모드가 안되게 막는 부분
            _flex.SetExceptEditCol("NO_SO", "SEQ_SO", "LN_PARTNER", "NM_ITEM", "STND_ITEM", "UNIT_SO", "UNIT_IM", "AM_WONAMT", "AMVAT_SO", "NM_SL", "UNIT_SO_FACT", "GN_PARTNER", "RT_VAT", "NM_CC");

            //그리드 필수입력 해야 하는부분 셋팅
            if (Global.MainFrame.ServerKeyCommon.ToUpper().Contains("HDS")) //현대에이치디에스
            {
                _flex.VerifyNotNull = new string[] { "CD_PARTNER", "CD_PLANT", "CD_ITEM", "DT_DUEDATE", "DT_REQGI", "TP_VAT", "CD_CC", "NO_PROJECT" };
            }
            else
            {
            ...
            _flex.SettingVersion = "201502040002";
            _flex.EndSetting(GridStyleEnum.Green, AllowSortingEnum.MultiColumn, SumPositionEnum.Top);

            //헤더에 필요없는 SUM 제거 && 반드시 EndSetting 다음에 코딩해줘야한다.
            if (disCount_YN == "Y") //영업환경설정의 할인율 적용여부 추가 2009.07.16 NJin (Default Value = "N" 으로 셋팅)
            {
                _flex.SetExceptSumCol("UM_SO", "UM_BASE", "RT_DSCNT", "RT_VAT");
            }
            else
                _flex.SetExceptSumCol("UM_SO", "RT_VAT");
        ...
        protected override void InitPaint()
        {
            ...
            oneGrid1.IsSearchControl = false;   //입력 전용
            ...
            if (Global.MainFrame.ServerKeyCommon == "SINJINSM" || Global.MainFrame.ServerKeyCommon == "DZSQL")
            {
                btn업체전용.Text = "본사발주적용";
                btn업체전용.Visible = true;

                //btn업체전용2.Text = "품목자동생성";
                //btn업체전용2.Visible = true;

                btn업체전용3.Text = "수주가계산";
                btn업체전용3.Visible = true;

                btn업체전용4.Text = "견적적용(신진SM)";
                btn업체전용4.Width = 130;
                btn업체전용4.Visible = true;


                btn업체전용.Enabled = true;
                btn업체전용2.Enabled = true;
                btn업체전용3.Enabled = true;
                btn업체전용4.Enabled = true;

                btn_Excel.Enabled = true;
                btn_AddRow.Enabled = true;
            }

            DataSet ds = this.GetComboData("N;MA_B000005", "S;SA_B000021", "N;MA_CODEDTL_005;MA_B000040", "N;MA_B000004", "N;MA_PLANT"); 
            ...
            str.SetCombobox(cbo단가적용, MA.GetCode("SA_B000021"));
            ...
            // 단위
            _flex.SetDataMap("UNIT_SO", ds.Tables[3], "CODE", "NAME");
            _flex.SetDataMap("UNIT_IM", ds.Tables[3], "CODE", "NAME");

            // 공장
            _flex.SetDataMap("CD_PLANT", ds.Tables[4], "CODE", "NAME");
            ...
            if (_biz.Get설정(DefaultSettings.회사코드) != MA.Login.회사코드)
            {
                ...
                _biz.Set설정(DefaultSettings.단가유형, string.Empty);
            }
            else
            {
                bp_SoType.CodeValue = _biz.Get설정(DefaultSettings.수주형태코드);
                bp_SoType.CodeName = _biz.Get설정(DefaultSettings.수주형태명);
                bp_SalesGroup.CodeValue = _biz.Get설정(DefaultSettings.영업그룹코드);
                bp_SalesGroup.CodeName = _biz.Get설정(DefaultSettings.영업그룹명);
                cbo_TpPrice.SelectedValue = _biz.Get설정(DefaultSettings.단가유형);
                DataRow row수주형태 = BASIC.GetTPSO(bp_SoType.CodeValue);
                _매출자동여부 = D.GetString(row수주형태["IV_AUTO"]);
                tp_Gi = D.GetString(row수주형태["TP_GI"]);     //출하형태
                tp_Busi = D.GetString(row수주형태["TP_BUSI"]); //거래구분
                tp_Iv = D.GetString(row수주형태["TP_IV"]);     //매출형태
                _의뢰여부 = D.GetString(row수주형태["GIR"]);
                _출하여부 = D.GetString(row수주형태["GI"]);
                _매출여부 = D.GetString(row수주형태["IV"]);
                trade = D.GetString(row수주형태["TRADE"]);     //수출여부

                if (Sa_Global.IVL_CdCc == "001")
                {
                    object[] obj_Tpso = new object[2];
                    obj_Tpso[0] = LoginInfo.CompanyCode;                        //회사
                    obj_Tpso[1] = bp_SoType.CodeValue;     //수주형태
                    DataRow dr = Sa_ComFunc.GetTPSO(obj_Tpso);

                    cd_CC = D.GetString(dr["CD_CC"]);    //시스템 통제설정이 수주유형의 C/C를 가져올때 셋팅  
                    nm_CC = D.GetString(dr["NM_CC"]);
                }
                else if (Sa_Global.IVL_CdCc == "000")
                {
                    object[] obj = new object[2];
                    obj[0] = Global.MainFrame.LoginInfo.CompanyCode;
                    obj[1] = bp_SalesGroup.CodeValue;
            ...
            if (MA.ServerKey(false, "DNCOMPANY"))
            {
                ColsSetting("SOL_TXT_USERDEF", "SA_B000112", 1, 2);
            }
        }
        ...
        private void InitEvent()
        {
            if (Global.MainFrame.ServerKeyCommon == "SINJINSM" || Global.MainFrame.ServerKeyCommon == "DZSQL")
            {
                btn업체전용.Click += new EventHandler(btn업체전용_Click);
                btn업체전용2.Click += new EventHandler(btn품목자동생성_Click);
                btn업체전용3.Click += new EventHandler(btn수주가계산_Click);
                btn업체전용4.Click += new EventHandler(btn견적적용신진_Click);
            }

            btn단가적용.Click += new EventHandler(btn단가적용_Click);
        }
        ...
        protected override bool SaveData()
        {
            ...
            try
            {
                //로그인사업장으로 넣게 되면 실제 저장되는 공장의 사업장과 틀려지는 문제가 발생(2014.05.15)
                //string cd_bizarea = Global.MainFrame.LoginInfo.BizAreaCode;
                string cd_bizarea = D.GetString(Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(Duzon.ERPU.MF.MasterSearch.MA_PLANT, new object[] { MA.Login.회사코드, D.GetString(_flex.DataView.ToTable().Rows[0]["CD_PLANT"]) })["CD_BIZAREA"]);
                string no_So = string.Empty;
                decimal no_Hst = 0; // 0 차수부터 시작 
                string tp_vat = cbo_TpVat.SelectedValue.ToString();

                dt = GetCalculater(_flex.DataView.ToTable(), "Y", string.Empty);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    dr["CD_BIZAREA"] = D.GetString(Duzon.ERPU.MF.Common.CodeSearch.GetCodeInfo(Duzon.ERPU.MF.MasterSearch.MA_PLANT, new object[] { MA.Login.회사코드, D.GetString(dr["CD_PLANT"]) })["CD_BIZAREA"]);

                    if (MA.ServerKey(false, new string[] { "TELCON" })) //텔콘
                        drh = dt_SoH.Select("CD_PARTNER = '" + D.GetString(dr["CD_PARTNER"]) + "' AND ISNULL(DC_RMK, '') = '" + D.GetString(dr["DC_RMK"]) + "' AND ISNULL(DC_RMK1, '') = '" + D.GetString(dr["DC_RMK1"]) + "' AND CD_BIZAREA = '" + D.GetString(dr["CD_BIZAREA"]) + "' AND ISNULL(NO_PO_PARTNER, '') = '" + D.GetString(dr["NO_PO_PARTNER"]) + "'");
                    else
                        drh = dt_SoH.Select("CD_PARTNER = '" + D.GetString(dr["CD_PARTNER"]) + "' AND ISNULL(DC_RMK, '') = '" + D.GetString(dr["DC_RMK"]) + "' AND ISNULL(DC_RMK1, '') = '" + D.GetString(dr["DC_RMK1"]) + "' AND CD_BIZAREA = '" + D.GetString(dr["CD_BIZAREA"]) + "'");
                    ...
                    if (drh.Length == 0)
                    {
                        ...
                        dr_SoH["CD_BIZAREA"] = dr["CD_BIZAREA"];
                ...
                foreach (DataRow dr in dt_SoH.Rows)
                {
                    ...
                    if (MA.ServerKey(false, new string[] { "TELCON" })) //텔콘
                        drl = dt.Select("CD_PARTNER = '" + D.GetString(dr["CD_PARTNER"]) + "' AND ISNULL(DC_RMK, '') = '" + D.GetString(dr["DC_RMK"]) + "' AND ISNULL(DC_RMK1, '') = '" + D.GetString(dr["DC_RMK1"]) + "' AND CD_BIZAREA = '" + D.GetString(dr["CD_BIZAREA"]) + "' AND ISNULL(NO_PO_PARTNER, '') = '" + D.GetString(dr["NO_PO_PARTNER"]) + "'");
                    else
                        drl = dt.Select("CD_PARTNER = '" + D.GetString(dr["CD_PARTNER"]) + "' AND ISNULL(DC_RMK, '') = '" + D.GetString(dr["DC_RMK"]) + "' AND ISNULL(DC_RMK1, '') = '" + D.GetString(dr["DC_RMK1"]) + "' AND CD_BIZAREA = '" + D.GetString(dr["CD_BIZAREA"]) + "'");
                    ...
                    foreach (DataRow dl in drl)
                    {
                        ...
                        dr_SoL["NO_PROJECT"] = Get_Project(D.GetString(dl["NO_PROJECT"]));
                        ...
                        dr_SoL["DC1"] = dl["DC1"];
                        dr_SoL["DC2"] = dl["DC2"];
                        dr_SoL["NO_PO_PARTNER"] = dl["NO_PO_PARTNER"];
                        dr_SoL["NO_POLINE_PARTNER"] = dl["NO_POLINE_PARTNER"];
                        dr_SoL["NO_RELATION"] = dl["NO_RELATION"];
                        dr_SoL["SEQ_RELATION"] = dl["SEQ_RELATION"];
                        dr_SoL["NUM_USERDEF1"] = dl["NUM_USERDEF1"];
                        ...
                        if (disCount_YN == "Y") //영업환경설정의 할인율 적용여부 추가 2009.07.16 NJin (Default Value = "N" 으로 셋팅)
                        {
                            dr_SoL["RT_DSCNT"] = dl["RT_DSCNT"];
                            dr_SoL["UM_BASE"] = dl["UM_BASE"];
                        }

                        dr_SoL["FG_USE"] = dl["FG_USE"];
                        
                        if (MA.ServerKey(false, "DNCOMPANY"))
                        {
                            dr_SoL["CD_MNGD1"] = dl["CD_MNGD1"];
                            dr_SoL["CD_MNGD2"] = dl["CD_MNGD2"];
                            dr_SoL["CD_MNGD3"] = dl["CD_MNGD3"];
                            dr_SoL["CD_MNGD4"] = dl["CD_MNGD4"];
                            dr_SoL["SOL_TXT_USERDEF1"] = dl["SOL_TXT_USERDEF1"];
                            dr_SoL["SOL_TXT_USERDEF2"] = dl["SOL_TXT_USERDEF2"];
                        }
                ...
                if (MA.ServerKey(false, new string[] { "MCIRCLE", "LINEP", "DZSQL", "CNP", "DINTEC" }))
                {
                    if (_biz.Get과세변경유무 == "N" && _매출자동여부 == "Y")
                    {
                        foreach (DataRow row in dt_SoH.Rows)
                        {
                            ...
                            row["AM_IV"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dt_SoL.Compute("SUM(AM_WONAMT)", "NO_SO = '" + D.GetString(row["NO_SO"]) + "'")));
                            row["AM_IV_EX"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(dt_SoL.Compute("SUM(AM_SO)", "NO_SO = '" + D.GetString(row["NO_SO"]) + "'")));
                            row["AM_IV_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dt_SoL.Compute("SUM(AM_VAT)", "NO_SO = '" + D.GetString(row["NO_SO"]) + "'")));
                        }
                    }
                }
                ...
                _biz.Set설정(DefaultSettings.수주형태코드, bp_SoType.CodeValue);
                _biz.Set설정(DefaultSettings.수주형태명, bp_SoType.CodeName);
                _biz.Set설정(DefaultSettings.영업그룹코드, bp_SalesGroup.CodeValue);
                _biz.Set설정(DefaultSettings.영업그룹명, bp_SalesGroup.CodeName);
                _biz.Set설정(DefaultSettings.단가유형, D.GetString(cbo_TpPrice.SelectedValue));
                ...
                if (Global.MainFrame.ServerKeyCommon == "SINJINSM" || Global.MainFrame.ServerKeyCommon == "DZSQL")
                {
                    ...
                    btn업체전용.Enabled = false;
                    btn업체전용2.Enabled = false;
                    btn업체전용3.Enabled = false;
                    btn업체전용4.Enabled = false;
        ...
        private void btn_Excel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt_Excel.Columns.Contains("AM_SO")) 엑셀구분 = "금액포함";
                else if (!dt_Excel.Columns.Contains("AM_SO") && !dt_Excel.Columns.Contains("UM_SO")) 엑셀구분 = "단가금액미포함";
                ...
                if (CheckTable != null && CheckTable.Rows.Count > 0)
                    ResultTable = GetCalculater(CheckTable, "N", 엑셀구분);//계산로직을 구하고 DEFAULT 공장 및 관리수량을 셋팅 한다.
        ...
        private DataTable SO_PK_Check(DataTable dt_Excel, out bool b)
        {
            ...
            try
            {
                ...
                DataTable dtCpItemMerge = null;

                if (str동기화 == "N")
                {
                    dt프로젝트 = _biz.Get_dt프로젝트();
                ...
                if (dt_Excel.Columns.Contains("CD_ITEM_PARTNER"))
                {
                    ...
                    검증리스트.AppendLine(DD("거래처") + "\t " + DD("거래처품목"));
                    검증리스트.AppendLine("-".PadRight(30, '-'));

                    DataTable dtPartner = dt_Excel.DefaultView.ToTable(true, new string[] { "CD_PARTNER" });
                    bool verifyCpItem = true;

                    foreach (DataRow row in dtPartner.Rows)
                    {
                        DataRow[] drs = dt_Excel.Select("CD_PARTNER = '" + D.GetString(row["CD_PARTNER"]) + "'");
                        string multiCpItem = Common.MultiString(drs, "CD_ITEM_PARTNER", "|");
                        DataTable dtCpItem = _biz.SearchCpItem(D.GetString(row["CD_PARTNER"]), multiCpItem);
                        if (dtCpItemMerge == null) dtCpItemMerge = dtCpItem;
                        else dtCpItemMerge.Merge(dtCpItem);
                    }

                    dtCpItemMerge.PrimaryKey = new DataColumn[] { dtCpItemMerge.Columns["CD_PARTNER"], dtCpItemMerge.Columns["CD_ITEM_PARTNER"] };

                    foreach (DataRow excelDr in dt_Excel.Rows)
                    {
                        if (D.GetString(excelDr["CD_ITEM_PARTNER"]) == string.Empty)
                        {
                            ShowMessage(공통메세지._은는필수입력항목입니다, DD("거래처품번"));
                            b = false;
                            return NewDataTable;
                        }

                        DataRow rowCpItem = dtCpItemMerge.Rows.Find(new object[] { D.GetString(excelDr["CD_PARTNER"]), D.GetString(excelDr["CD_ITEM_PARTNER"]) });

                        if (rowCpItem == null)
                        {
                            검증리스트.AppendLine(D.GetString(excelDr["CD_PARTNER"]) + "\t" + D.GetString(excelDr["CD_ITEM_PARTNER"]));
                            verifyCpItem = false;
                        }
                        else
                        {
                            excelDr["CD_ITEM"] = D.GetString(rowCpItem["CD_ITEM"]);
                            excelDr["NM_ITEM_PARTNER"] = D.GetString(rowCpItem["NM_ITEM_PARTNER"]);
                        }
                    }
                    if (!verifyCpItem)
                    {
                        ShowDetailMessage("해당 거래처에 존재하지 않는 거래처 품목이 존재합니다.", D.GetString(검증리스트));
                        b = false;
                        return NewDataTable;
                    }
                }
                ...
                foreach (DataRow row in dt_Excel.Rows)
                    Multi_Item += D.GetString(row["CD_ITEM"]) + "|";

                
                DataTable dt_품목 = _biz.품목정보(Multi_Item);

                foreach (DataRow excelDr in dt_Excel.Rows)
                {
                    #region -> 품목코드의 존재여부 체크

                    if (D.GetString(excelDr["CD_ITEM"]) == string.Empty)
                    {
                        ShowMessage(공통메세지._은는필수입력항목입니다, DD("품목코드"));
                        b = false;
                        return NewDataTable;
                    }

                    //row품목 = dt_품목.Rows.Find(new object[] { excelDr["CD_PLANT"].ToString(), excelDr["CD_ITEM"].ToString() });
                    CheckDr = dt_품목.Select("CD_PLANT = '" + excelDr["CD_PLANT"].ToString() + "' AND CD_ITEM = '" + excelDr["CD_ITEM"].ToString() + "'");
                    if (CheckDr.Length == 0)
                    {
                    ...
                    excelDr["NM_ITEM"] = CheckDr[0]["NM_ITEM"].ToString();
                    excelDr["STND_ITEM"] = CheckDr[0]["STND_ITEM"].ToString();
                    excelDr["UNIT_SO"] = CheckDr[0]["UNIT_SO"].ToString();
                    excelDr["UNIT_IM"] = CheckDr[0]["UNIT_IM"].ToString();//관리단위
                    excelDr["UNIT_SO_FACT"] = D.GetDecimal(CheckDr[0]["UNIT_SO_FACT"]) == 0 ? 1m : D.GetDecimal(CheckDr[0]["UNIT_SO_FACT"]);

                    #endregion
                      
                    #region -> 거래처코드/납품처코드의 존재여부 체크

                    if (D.GetString(excelDr["CD_PARTNER"]) == string.Empty)
                    {
                        ShowMessage(공통메세지._은는필수입력항목입니다, DD("거래처코드"));
                        b = false;
                        return NewDataTable;
                    }

                    //거래처코드의 존재여부 체크
                    CheckDr = CheckSet.Tables[1].Select("CD_PARTNER = '" + D.GetString(excelDr["CD_PARTNER"]) + "'");

                    if (CheckDr.Length == 0)
                    {
                        ShowMessage(DD("거래처(" + D.GetString(excelDr["CD_PARTNER"]) + ")은 존재하지 않는 거래처입니다."));
                        b = false;
                        return NewDataTable;
                    }

                    excelDr["LN_PARTNER"] = D.GetString(CheckDr[0]["LN_PARTNER"]);

                    //납품처코드의 존재여부 체크
                    if (excelDr["GI_PARTNER"].ToString() != String.Empty)
                    {
                        CheckDr = CheckSet.Tables[1].Select("CD_PARTNER = '" + excelDr["GI_PARTNER"].ToString() + "'");

                        if (CheckDr.Length == 0)
                        {
                            ShowMessage(DD("납품처(" + D.GetString(excelDr["GI_PARTNER"]) + ")은 존재하지 않는 납품처입니다."));
                            b = false;
                            return NewDataTable;
                        }

                        excelDr["GN_PARTNER"] = D.GetString(CheckDr[0]["LN_PARTNER"]);
                    }
                    ...
                    if (excelDr["CD_SL"].ToString() != String.Empty)
                    {
                        CheckDr = CheckSet.Tables[2].Select("CD_SL = '" + excelDr["CD_SL"].ToString() + "'");

                        if (CheckDr.Length == 0 && excelDr["CD_SL"].ToString() != String.Empty)
                        {
                            ShowMessage(DD("창고(" + D.GetString(excelDr["CD_SL"]) + ")은 존재하지 않는 창고입니다."));
                            b = false;
                            return NewDataTable;
                        }

                        excelDr["NM_SL"] = D.GetString(CheckDr[0]["NM_SL"]);
                    }
                    ...
                    if (str동기화 == "N")
                    {
                        if (D.GetString(excelDr["NO_PROJECT"]) == string.Empty) continue;

                        DataRow row프로젝트 = dt프로젝트.Rows.Find(excelDr["NO_PROJECT"]);

                        if (row프로젝트 == null)
                        {
                            ShowMessage("프로젝트(" + D.GetString(excelDr["NO_PROJECT"]) + ")은 존재하지 않는 프로젝트입니다.");
                            b = false;
                            return NewDataTable;
                        }

                        excelDr["NM_PROJECT"] = D.GetString(row프로젝트["NM_PROJECT"]);
                    }
        ...
        private DataTable GetCalculater(DataTable CheckTable, string SaveYN, string 엑셀구분)
        {
            try
            {
                ...
                foreach (DataRow dr in CheckTable.Select("","", DataViewRowState.CurrentRows))
                {
                    if (str동기화 == "Y")
                    {
                        if (bp_PJT.CodeValue != string.Empty && dr["CD_PARTNER"].ToString() != Partner)
                        {
                            dr.Delete();
                            idx++;
                            continue;
                    ...
                    if (dr["CD_PLANT"] == null || dr["CD_PLANT"].ToString() == string.Empty)
                    ...
                    dr["TP_VAT"] = cbo_TpVat.SelectedValue.ToString();
                    dr["RT_VAT"] = txt_TpVat.DecimalValue;

                    dr["SEQ_PROJECT"] = 0;      //프로젝트 항번은 0으로 때려준다. : 프로젝트 적용 받을때 빼곤 항번은 0 으로 가야 트리거에서 집계작업을 안한다.

                    if(SaveYN == "N") //저장할땐 재계산하지 않고 처음 엑셀로 업로드 할때만 재 계산해준다.
                    {
                        if (엑셀구분 == "단가포함")
                        {
                            if (disCount_YN == "Y") //영업환경설정의 할인율 적용여부 추가 2009.07.16 NJin (Default Value = "N" 으로 셋팅)
                                dr["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(dr["UM_BASE"]) - (D.GetDecimal(dr["UM_BASE"]) * D.GetDecimal(dr["RT_DSCNT"])) / 100);

                            /*계산로직*/
                            dr["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["QT_SO"]) * D.GetDecimal(dr["UM_SO"]));
                        }
                        else if (엑셀구분 == "금액포함")
                        {
                            dr["UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(dr["QT_SO"]) > 0M ? Decimal.Round(D.GetDecimal(dr["AM_SO"]) / D.GetDecimal(dr["QT_SO"]), 4, MidpointRounding.AwayFromZero) : 0M);
                        }
                        else if (엑셀구분 == "단가금액미포함")
                        {
                            dr["UM_SO"] = BASIC.GetUM(D.GetString(dr["CD_PLANT"]), D.GetString(dr["CD_ITEM"]), D.GetString(dr["CD_PARTNER"]), bp_SalesGroup.CodeValue, dat_Sodt.Text, D.GetString(cbo_TpPrice.SelectedValue), D.GetString(cbo_CdExch.SelectedValue), "SA");
                            dr["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["QT_SO"]) * D.GetDecimal(dr["UM_SO"]));
                        }
                        dr["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["AM_SO"]) * txt_RtExch.DecimalValue);
                        dr["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["AM_WONAMT"]) * (D.GetDecimal(dr["RT_VAT"]) / 100));
                    }

                    //부가세포함여부가 FG_VAT = "N" 일때 부가세포함 금액계산
                    dr["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(dr["AM_WONAMT"]) + D.GetDecimal(dr["AM_VAT"]));

                    //부가세포함여부가 FG_VAT = "N" 일때 부가세포함 단가계산
                    if (D.GetDecimal(dr["QT_SO"]) != 0)
                        dr["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(dr["AMVAT_SO"]) / D.GetDecimal(dr["QT_SO"]));
                    else
                        dr["UMVAT_SO"] = D.GetDecimal(dr["AMVAT_SO"]);

                    //관리수량
                    if (dr["UNIT_SO_FACT"] == null || dr["UNIT_SO_FACT"].ToString() == string.Empty)
                        dr["QT_IM"] = D.GetDecimal(dr["QT_SO"]);
                    else
                        dr["QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(dr["QT_SO"]) * D.GetDecimal(dr["UNIT_SO_FACT"])); 
                }
                ...
                if (idx != 0)
                {
                    ...
                    ShowMessage("프로젝트에 해당된 거래처만 업로드 됩니다.");
                }
            }
            catch (Exception ex)
            {
        ...
        private DataTable getSchema(string param)
        {
            ...
            try 
            {
                ...
                { 
                    ...
                    dt.Columns.Add("DC_RMK1", typeof(string));
                    ...
                    dt.Columns.Add("RMA_REASON", typeof(string));
                ...
                else if (param == "SOL")
                {
                    ...
                    dt.Columns.Add("FG_TRACK", typeof(string));
                    dt.Columns.Add("NO_ORDER", typeof(string));
                    dt.Columns.Add("NM_CUST", typeof(string));
                    dt.Columns.Add("NO_TEL1", typeof(string));
                    dt.Columns.Add("NO_TEL2", typeof(string));
                    dt.Columns.Add("TXT_USERDEF1", typeof(string));

                    dt.Columns.Add("NO_PO_PARTNER", typeof(string));
                    dt.Columns.Add("NO_POLINE_PARTNER", typeof(decimal));
                    dt.Columns.Add("NO_RELATION", typeof(string));
                    dt.Columns.Add("SEQ_RELATION", typeof(decimal));
                    dt.Columns.Add("GRP_ITEM", typeof(string));
                    dt.Columns.Add("CLS_L", typeof(string));
                    dt.Columns.Add("CLS_S", typeof(string));
                    dt.Columns.Add("NUM_STND_ITEM_1", typeof(decimal));
                    dt.Columns.Add("NUM_STND_ITEM_2", typeof(decimal));
                    dt.Columns.Add("NUM_STND_ITEM_3", typeof(decimal));
                    dt.Columns.Add("NUM_STND_ITEM_4", typeof(decimal));
                    dt.Columns.Add("NUM_STND_ITEM_5", typeof(decimal));
                    dt.Columns.Add("NUM_USERDEF1", typeof(decimal));
                    dt.Columns.Add("NUM_USERDEF2", typeof(decimal));
                    dt.Columns.Add("CD_MNGD1", typeof(string));
                    dt.Columns.Add("CD_MNGD2", typeof(string));
                    dt.Columns.Add("CD_MNGD3", typeof(string));
                    dt.Columns.Add("CD_MNGD4", typeof(string));
        ...
        private DataTable getExcelSchema(DataTable dt)
        {
            try
            {
                ...
                dt.Columns.Add("NO_HST", typeof(decimal));
                dt.Columns.Add("LN_PARTNER", typeof(string));
                if (!dt.Columns.Contains("CD_ITEM"))
                    dt.Columns.Add("CD_ITEM", typeof(string));
                dt.Columns.Add("NM_ITEM", typeof(string));
                dt.Columns.Add("STND_ITEM", typeof(string));
                dt.Columns.Add("UNIT_SO", typeof(string));
                ...
                if (!dt.Columns.Contains("NO_PO_PARTNER"))
                    dt.Columns.Add("NO_PO_PARTNER", typeof(string));
                if (!dt.Columns.Contains("NO_POLINE_PARTNER"))
                    dt.Columns.Add("NO_POLINE_PARTNER", typeof(string));
                if (!dt.Columns.Contains("NO_RELATION"))
                    dt.Columns.Add("NO_RELATION", typeof(string));
                if (!dt.Columns.Contains("SEQ_RELATION"))
                    dt.Columns.Add("SEQ_RELATION", typeof(string));
                if (!dt.Columns.Contains("NUM_USERDEF1"))
                    dt.Columns.Add("NUM_USERDEF1", typeof(string));
                if (!dt.Columns.Contains("SOL_TXT_USERDEF1"))
                    dt.Columns.Add("SOL_TXT_USERDEF1", typeof(string));
                if (!dt.Columns.Contains("SOL_TXT_USERDEF2"))
                    dt.Columns.Add("SOL_TXT_USERDEF2", typeof(string));
            }
            catch (Exception ex)
            {
        ...
        private void SetExchageApply()
        {
            ...
            //obj[0] = Global.MainFrame.LoginInfo.CompanyCode;
            //obj[1] = dat_Sodt.Text;
            //obj[2] = cbo_CdExch.SelectedValue.ToString();

            if (MA.기준환율.Option != MA.기준환율옵션.적용안함)
                rt_exch = MA.기준환율적용(dat_Sodt.Text,cbo_CdExch.SelectedValue.ToString());

            txt_RtExch.DecimalValue = rt_exch;
        }
        #endregion

        #region -> 수주일변경시 발생 이벤트
        private void dat_Sodt_DateChanged(object sender, EventArgs e)
        {
            //SetExchageApply();
        } 
        #endregion
        ...
        private void _flex_StartEdit(object sender, RowColEventArgs e)
        {
            try
            {
                ...
                {
                    case "CD_ITEM":
                        if (D.GetString(_flex["CD_ITEM_PARTNER"]) != string.Empty)
                            e.Cancel = true;
                        break;
                    case "UM_SO":
                    case "AM_SO":
        ...
        private void _flex_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_flex.Cols[_flex.Col].Name == "UM_SO" && (_flex["STA_SO"].ToString() == "" || _flex["STA_SO"].ToString() == "O"))
                {
                    ...
                    P_SA_UM_HISTORY_SUB dlg = new P_SA_UM_HISTORY_SUB(_flex["CD_PARTNER"].ToString(), 
                                                                      _flex["LN_PARTNER"].ToString(), 
                                                                       bp_SoType.CodeValue, 
                                                                       bp_SoType.CodeName,
                                                                      _flex["CD_PLANT"].ToString(),
                                                                      _flex["CD_ITEM"].ToString(),
                                                                      _flex["NM_ITEM"].ToString(),
                                                                      cbo_CdExch.SelectedValue == null ? string.Empty : cbo_CdExch.SelectedValue.ToString());

                    if (dlg.ShowDialog() == DialogResult.OK)
                    {
                        if (disCount_YN == "N") //영업환경설정의 할인율 적용여부 추가 2009.07.16 NJin (Default Value = "N" 으로 셋팅)
                        ...
                        _flex["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_SO"]) * txt_RtExch.DecimalValue);
                        _flex["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) * (D.GetDecimal(_flex["RT_VAT"]) / 100));

                        //부가세포함여부가 FG_VAT = "N" 일때 부가세포함 금액계산
                        _flex["AMVAT_SO"] = D.GetDecimal(_flex["AM_WONAMT"]) + D.GetDecimal(_flex["AM_VAT"]);
                        ...
                        if (D.GetDecimal(_flex["QT_SO"]) != 0)
                            ...
                            _flex["UMVAT_SO"] = D.GetDecimal(_flex["AMVAT_SO"]);
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "DT_DUEDATE":
                        ...
                        if (_flex[e.Row, "LT_GI"] != System.DBNull.Value || _flex[e.Row, "LT_GI"] != null)
                            _flex[e.Row, "DT_REQGI"] = _CommFun.DateAdd(newValue, "D", D.GetInt(_flex[e.Row, "LT_GI"]) * -1);
                    ...
                    case "TP_VAT":
                        ...
                        _flex[e.Row, "AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) + D.GetDecimal(_flex[e.Row, "AM_VAT"]));
                        ...
                        if (D.GetDecimal(newValue) != 0)
                            ...
                            _flex[e.Row, "UMVAT_SO"] = D.GetDecimal(_flex[e.Row, "AMVAT_SO"]);
                    ...
                    case "QT_SO":
                        ...
                        _flex[e.Row, "AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_SO"]) * txt_RtExch.DecimalValue);
                        _flex[e.Row, "AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) * (D.GetDecimal(_flex[e.Row, "RT_VAT"]) / 100));
                        _flex[e.Row, "QT_IM"] = Unit.수량(DataDictionaryTypes.SA, D.GetDecimal(newValue) * D.GetDecimal(_flex[e.Row, "UNIT_SO_FACT"]));
                        _flex[e.Row, "AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) + D.GetDecimal(_flex[e.Row, "AM_VAT"]));
                        if (D.GetDecimal(newValue) != 0) //수량이 0 이 아닐때
                            _flex[e.Row, "UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "UMVAT_SO"]) / D.GetDecimal(_flex[e.Row, "QT_SO"]));
                        else
                            _flex[e.Row, "UMVAT_SO"] = D.GetDecimal(_flex[e.Row, "AMVAT_SO"]);
                    ...
                    case "UM_SO":
                        _flex[e.Row, "AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "QT_SO"]) * D.GetDecimal(newValue));
                        _flex[e.Row, "AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_SO"]) * txt_RtExch.DecimalValue);
                        _flex[e.Row, "AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) * (D.GetDecimal(_flex[e.Row, "RT_VAT"]) / 100));
                        _flex[e.Row, "AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) + D.GetDecimal(_flex[e.Row, "AM_VAT"]));
                        if (D.GetDecimal(_flex[e.Row, "QT_SO"]) != 0) //수량이 0 이 아닐때
                            _flex[e.Row, "UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AMVAT_SO"]) / D.GetDecimal(_flex[e.Row, "QT_SO"]));
                        else
                            _flex[e.Row, "UMVAT_SO"] = D.GetDecimal(_flex[e.Row, "AMVAT_SO"]);
                    ...
                    case "AM_SO":
                        if (D.GetDecimal(_flex[e.Row, "QT_SO"]) != 0)
                            _flex[e.Row, "UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(newValue) / D.GetDecimal(_flex[e.Row, "QT_SO"]));
                        else
                            _flex[e.Row, "UM_SO"] = 0m;

                        _flex[e.Row, "AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(newValue) * txt_RtExch.DecimalValue);
                        _flex[e.Row, "AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) * (D.GetDecimal(_flex[e.Row, "RT_VAT"]) / 100));
                        _flex[e.Row, "AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) + D.GetDecimal(_flex[e.Row, "AM_VAT"]));
                        if (D.GetDecimal(_flex[e.Row, "QT_SO"]) != 0) //수량이 0 이 아닐때
                            _flex[e.Row, "UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AMVAT_SO"]) / D.GetDecimal(_flex[e.Row, "QT_SO"]));
                    ...
                    case "RT_DSCNT": //할인율
                        if (D.GetDecimal(_flex[e.Row, "UM_BASE"]) != 0)
                            _flex[e.Row, "UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "UM_BASE"]) - (D.GetDecimal(_flex[e.Row, "UM_BASE"]) * D.GetDecimal(newValue)) / 100);
                        else
                            _flex[e.Row, "UM_SO"] = 0m;

                        _flex[e.Row, "AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "QT_SO"]) * D.GetDecimal(_flex[e.Row, "UM_SO"]));
                        _flex[e.Row, "AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_SO"]) * txt_RtExch.DecimalValue);
                        _flex[e.Row, "AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) * (D.GetDecimal(_flex[e.Row, "RT_VAT"]) / 100));
                        _flex[e.Row, "AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "AM_WONAMT"]) + D.GetDecimal(_flex[e.Row, "AM_VAT"]));
                        if (D.GetDecimal(_flex[e.Row, "QT_SO"]) != 0) //수량이 0 이 아닐때
                        ...
                        break;
                    case "UM_BASE": //기준단가
                        if (D.GetDecimal(_flex[e.Row, "RT_DSCNT"]) != 0)
                            _flex[e.Row, "UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(newValue) - (D.GetDecimal(newValue) * D.GetDecimal(_flex[e.Row, "RT_DSCNT"])) / 100);
                        else
                            _flex[e.Row, "UM_SO"] = 0m;
        ...
        private void _flex_BeforeCodeHelp(object sender, BeforeCodeHelpEventArgs e)
        {
            try
            {
                if (_flex["STA_SO"] != null && _flex["STA_SO"].ToString() != "" && _flex["STA_SO"].ToString() != "O")
                {
                    e.Cancel = true;
                    return;
                }
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    ...
                    case "CD_ITEM_PARTNER":
                        string cdPartner = D.GetString(_flex["CD_PARTNER"]);
                        if (cdPartner == string.Empty)
                        {
                            ShowMessage(공통메세지._은는필수입력항목입니다, DD("거래처"));
                            e.Cancel = true;
                        }

                        e.Parameter.P14_CD_PARTNER = cdPartner;
                        e.Parameter.P09_CD_PLANT = D.GetString(_flex["CD_PLANT"]);
                        //e.Parameter.P65_CODE5 = D.GetString(_flex["CD_PLANT"]);
                        break;
                }
            }
            catch (Exception ex)
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    case "CD_ITEM":
                        ...
                        foreach (DataRow row in helpReturn.Rows)
                        {
                            ...
                            _flex[_flex.Row, "GRP_ITEM"] = row["GRP_ITEM"];
                            _flex[_flex.Row, "CLS_L"] = row["CLS_L"];
                            _flex[_flex.Row, "CLS_S"] = row["CLS_S"];
                            _flex[_flex.Row, "NUM_STND_ITEM_1"] = row["NUM_STND_ITEM_1"];
                            _flex[_flex.Row, "NUM_STND_ITEM_2"] = row["NUM_STND_ITEM_2"];
                            _flex[_flex.Row, "NUM_STND_ITEM_3"] = row["NUM_STND_ITEM_3"];
                            _flex[_flex.Row, "NUM_STND_ITEM_4"] = row["NUM_STND_ITEM_4"];
                            _flex[_flex.Row, "NUM_STND_ITEM_5"] = row["NUM_STND_ITEM_5"];
                            ...
                            if (tp_SalePrice == "002" || tp_SalePrice == "003")
                            {
                                ...
                                if (disCount_YN == "N")
                                    _flex[_flex.Row, "UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, _biz.UmSearch(obj));
                                else if (disCount_YN == "Y")
                                {
                                    ...
                                    _flex[_flex.Row, "UM_BASE"] = Unit.외화단가(DataDictionaryTypes.SA, _biz.UmSearch(obj));
                                    _flex[e.Row, "UM_SO"] = Unit.외화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex[e.Row, "UM_BASE"]) - (D.GetDecimal(_flex[e.Row, "UM_BASE"]) * D.GetDecimal(_flex[e.Row, "RT_DSCNT"])) / 100);
                                }
                            }
                            ...
                            {
                                _flex["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["UM_SO"]) * D.GetDecimal(_flex["QT_SO"]));
                                _flex["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_SO"]) * txt_RtExch.DecimalValue);
                                _flex["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) * (D.GetDecimal(_flex[e.Row, "RT_VAT"]) / 100));
                                _flex["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(_flex["AM_WONAMT"]) + D.GetDecimal(_flex["AM_VAT"]));
                                if (D.GetDecimal(_flex["QT_SO"]) != 0) //수량이 0 이 아닐때
                                    _flex["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(_flex["AMVAT_SO"]) / D.GetDecimal(_flex["QT_SO"]));
                                else
                                    _flex["UMVAT_SO"] = D.GetDecimal(_flex["AMVAT_SO"]);
                    ...
                    case "CD_ITEM_PARTNER":
                        ...
                        foreach (DataRow row in e.Result.Rows)
                        {
                            ...
                            {
                                btn_AddRow_Click(sender, e);
                                _flex[_flex.Row, "CD_PARTNER"] = cdPartner;
                                _flex[_flex.Row, "LN_PARTNER"] = nmPartner;
                            }
                            else
                            {
                                cdPartner = D.GetString(_flex[_flex.Row, "CD_PARTNER"]);
                                nmPartner = D.GetString(_flex[_flex.Row, "LN_PARTNER"]);
                            }

                            _flex[_flex.Row, "CD_ITEM_PARTNER"] = row["CD_ITEM_PARTNER"];
                            _flex[_flex.Row, "NM_ITEM_PARTNER"] = row["NM_ITEM_PARTNER"];
                            _flex[_flex.Row, "CD_ITEM"] = row["CD_ITEM"];
                            _flex[_flex.Row, "NM_ITEM"] = row["NM_ITEM"];
                            _flex[_flex.Row, "STND_ITEM"] = row["STND_ITEM"];
                            _flex[_flex.Row, "UNIT_SO"] = row["UNIT_SO"];
        ...
        private object Get_Project(string NO_PROJECT)
        {
            string 프로젝트번호 = string.Empty;

            if (str동기화 == "Y")
                프로젝트번호 = bp_PJT.CodeValue;
            else if (str동기화 == "N")
                프로젝트번호 = NO_PROJECT == string.Empty ? bp_PJT.CodeValue : NO_PROJECT;
        ...
        #region ♣ 업체전용버튼
        void btn업체전용_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ErrorCheck()) return;

                H_SA_Z_SINJINSM_PM_ORDER_SUB dlg = new H_SA_Z_SINJINSM_PM_ORDER_SUB(string.Empty, string.Empty);
                if (dlg.ShowDialog() != DialogResult.OK) return;

                DataRow[] returnRows = dlg.ReturnDataRows;
                cbo_CdExch.SelectedValue = D.GetString(returnRows[0]["CD_EXCH"]);
                txt_RtExch.DecimalValue = D.GetDecimal(returnRows[0]["RT_EXCH"]);
                cbo_TpVat.SelectedValue = D.GetString(returnRows[0]["TP_VAT"]);
                txt_TpVat.DecimalValue = D.GetDecimal(returnRows[0]["RT_VAT"]);
                ...
                foreach (DataRow row in returnRows)
                {
                    ...
                    newRow["CD_PARTNER"] = row["BRANCH"];
                    newRow["LN_PARTNER"] = row["LN_PARTNER"];
                    ...
                    newRow["CD_CC"] = cd_CC;
                    newRow["NM_CC"] = nm_CC;
                    newRow["TP_VAT"] = cbo_TpVat.SelectedValue.ToString();
                    newRow["RT_VAT"] = txt_TpVat.DecimalValue;
                    ...
                    newRow["DC_RMK"] = D.GetString(row["REMARK_H"]);
        ...
        void btn품목자동생성_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataRow row in returnRows)
                {
                    ...
                    _flex.DataTable.Rows.Add(newRow.ItemArray);
                }
            ...
            try
            {
                ...
                foreach (DataRow row in _flex.DataTable.Rows)
                {
                    ...
                    if (D.GetString(row["CLS_L"]) == "100")
                    {
                        ...
                        list.Add(cbo_TpVat.SelectedValue);
                        list.Add(cbo_CdExch.SelectedValue);
                        list.Add(txt_RtExch.DecimalValue);
        ...
        void btn견적적용신진_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                H_SA_Z_SINJINSM_EM_ORDER_SUB dlg = new H_SA_Z_SINJINSM_EM_ORDER_SUB(string.Empty, string.Empty);
                if (dlg.ShowDialog() != DialogResult.OK) return;
        ...
        {
            수주관리.Config 수주Config = new 수주관리.Config();

            string 수주유형 = bp_SoType.CodeValue;
            string 영업그룹 = bp_SalesGroup.CodeValue;
            CD_CC = string.Empty;
            NM_CC = string.Empty;
        ...
        #region 단가적용
        void btn단가적용_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataRow row in dr)
                {
                    ...
                    decimal um = BASIC.GetUM(D.GetString(row["CD_PLANT"]), D.GetString(row["CD_ITEM"]), D.GetString(row["CD_PARTNER"]), bp_SalesGroup.CodeValue, dat_Sodt.Text, D.GetString(cbo단가적용.SelectedValue), D.GetString(cbo_CdExch.SelectedValue), "SA");

                    row["UM_SO"] = um;

                    row["AM_SO"] = Unit.외화금액(DataDictionaryTypes.SA, D.GetDecimal(row["QT_SO"]) * D.GetDecimal(um));
                    row["AM_WONAMT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_SO"]) * txt_RtExch.DecimalValue);
                    row["AM_VAT"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]) * (D.GetDecimal(row["RT_VAT"]) / 100));
                    row["AMVAT_SO"] = Unit.원화금액(DataDictionaryTypes.SA, D.GetDecimal(row["AM_WONAMT"]) + D.GetDecimal(row["AM_VAT"]));
                    if (D.GetDecimal(row["QT_SO"]) != 0) //수량이 0 이 아닐때
                        row["UMVAT_SO"] = Unit.원화단가(DataDictionaryTypes.SA, D.GetDecimal(row["AMVAT_SO"]) / D.GetDecimal(row["QT_SO"]));
                    else
                        row["UMVAT_SO"] = D.GetDecimal(row["AMVAT_SO"]);
                }
