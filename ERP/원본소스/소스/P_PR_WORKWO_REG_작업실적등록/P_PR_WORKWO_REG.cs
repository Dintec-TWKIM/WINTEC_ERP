...
namespace prd
{
    ...
    *************************************/
    ...
    public partial class P_PR_WORKWO_REG : Duzon.Common.Forms.PageBase
    {
        ...
        public P_PR_WORKWO_REG(string pg_cd_plant, string pg_no_wo, string pg_tp_rout, string pg_cd_item, string pg_nm_item, string pg_stnd_item, string pg_unit_im, string pg_dt_rel, string pg_dt_due, string pg_fg_gubun)
        {
            try
            {
                InitializeComponent();
        ...
        protected override void InitLoad()
        {
            ...
            LOT관리항목DT = MA.GetCode("PU_C000079");
        ...
        private void InitGridM()
        {
            ...
            _flexM.SetCol("NO_LOT", "LOT번호", 100, 100, false);
            ...
            _flexM.SetCol("SEQ_PROJECT", Config.MA_ENV.YN_UNIT == "Y" ? "UNIT 항번" : "프로젝트항번", 120, false, typeof(decimal));
            //시스템통제로 처리할 예정임.
            ...
            _flexM.SetCol("CD_PACKUNIT", "포장단위", 100);
            _flexM.SetCol("NM_PACKUNIT", "포장명", 100);


            _flexM.SetCol("CD_PARTNER", "수주거래처", 100, false);
            _flexM.SetCol("LN_PARTNER", "수주거래처명", 100, false);

            if(!App.SystemEnv.PMS사용)
                _flexM.SetCol("NO_WBS", "WBS번호", 100, false);

            if (Config.MA_ENV.PJT형여부 == "Y")
            {
                _flexM.SetCol("CD_PJT_ITEM", Config.MA_ENV.YN_UNIT == "Y" ? "UNIT 코드" : "프로젝트 품목코드", 140, false, typeof(string));
                _flexM.SetCol("NM_PJT_ITEM", Config.MA_ENV.YN_UNIT == "Y" ? "UNIT 명" : "프로젝트 품목명", 140, false, typeof(string));
                _flexM.SetCol("PJT_ITEM_STND", Config.MA_ENV.YN_UNIT == "Y" ? "UNIT 규격" : "프로젝트 품목규격", 140, false, typeof(string));
            }
        ...
        private void InitGridD()
        {
            ...
            _flexD.SetCol("NO_LOT", "LOT번호", 100, 100, true, typeof(string));
            ...
            //
            // 구매자재의 코드관리 PU_C000079의 LOT관리항목 추가
            //
            if (LOT관리항목DT.Rows.Count != 0)
            {
                foreach (DataRow row in LOT관리항목DT.Rows)
                {
                    ...
                    if (cnt == 1 && (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                                    Global.MainFrame.ServerKeyCommon.ToUpper() == "SDBIO" ||
                                    Global.MainFrame.ServerKeyCommon.ToUpper() == "SKPLASMA"))
                    {
                        _flexD.SetCol(column, D.GetString(row["NAME"]), 80, true, typeof(string), FormatTpType.YEAR_MONTH_DAY);
                    }

                    else if (cnt == 6 && (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                                           Global.MainFrame.ServerKeyCommon.ToUpper() == "WINPLUS"))
                    {
                        _flexD.SetCol(column, D.GetString(row["NAME"]), 80, true, typeof(decimal), FormatTpType.QUANTITY);
                    }

                    else if (cnt == 7 && (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                                           Global.MainFrame.ServerKeyCommon.ToUpper() == "WINPLUS"))
                    {
                        _flexD.SetCol(column, D.GetString(row["NAME"]), 80, true, typeof(decimal), FormatTpType.QUANTITY);
                    }

                    else if (cnt == 11 && (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                                           Global.MainFrame.ServerKeyCommon.ToUpper() == "WINPLUS"))
                    {
                        _flexD.SetCol(column, D.GetString(row["NAME"]), 80, true, typeof(string), FormatTpType.YEAR_MONTH_DAY);
                    }
                    ...
                    {
                        _flexD.SetCol(column, D.GetString(row["NAME"]), 100, 200, true, typeof(string));
                    }
                }
            }

            _flexD.SetCol("QT_GOOD", "양품실적수량", 100, false, typeof(decimal), FormatTpType.QUANTITY);

            _flexD.SetCol("TXT_USERDEF1", "TXT_USERDEF1", 100, true, typeof(string));
            _flexD.SetCol("TXT_USERDEF2", "TXT_USERDEF2", 100, true, typeof(string));
            _flexD.SetCol("TXT_USERDEF3", "TXT_USERDEF3", 100, true, typeof(string));
            _flexD.SetCol("CD_USERDEF1", "CD_USERDEF1", 100, true);
            _flexD.SetCol("CD_USERDEF2", "CD_USERDEF2", 100, true);
            _flexD.SetCol("CD_USERDEF3", "CD_USERDEF3", 100, true);
            _flexD.SetCol("NUM_USERDEF1", "NUM_USERDEF1", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF2", "NUM_USERDEF2", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF3", "NUM_USERDEF3", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF4", "NUM_USERDEF4", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF5", "NUM_USERDEF5", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF6", "NUM_USERDEF6", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF7", "NUM_USERDEF7", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF8", "NUM_USERDEF8", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF9", "NUM_USERDEF9", 100, true, typeof(decimal));
            _flexD.SetCol("NUM_USERDEF10", "NUM_USERDEF10", 100, true, typeof(decimal));
            _flexD.SetCol("ROUT_NUM_USERDEF1", "ROUT_NUM_USERDEF1", 100, false, typeof(decimal), FormatTpType.QUANTITY);
            _flexD.SetCol("ROUT_NUM_USERDEF2", "ROUT_NUM_USERDEF2", 100, false, typeof(decimal), FormatTpType.QUANTITY);
            _flexD.SetCol("ROUT_NUM_USERDEF3", "ROUT_NUM_USERDEF3", 100, false, typeof(decimal), FormatTpType.QUANTITY);
            _flexD.SetCol("ROUT_TXT_USERDEF1", "ROUT_TXT_USERDEF1", 100, true, typeof(string));
            _flexD.SetCol("ROUT_TXT_USERDEF2", "ROUT_TXT_USERDEF2", 100, true, typeof(string));
            _flexD.SetCol("ROUT_TXT_USERDEF3", "ROUT_TXT_USERDEF3", 100, true, typeof(string));
            ...
            if (LoginInfo.MngLot != "Y")
            {
                ...
                if (LOT관리항목DT.Rows.Count != 0)
                {
                    foreach (DataRow row in LOT관리항목DT.Rows)
                    {
                        int cnt = D.GetInt(row["CODE"]);
                        string column = "CD_MNG" + cnt;
                        _flexD.Cols[column].Visible = false;
                    }
                }
            }
        ...
        protected override void InitPaint()
        {
            ...
            bpP_Plant.IsNecessaryCondition = true;
            bpP_Dt.IsNecessaryCondition = true;
            ...
            if (str_fg_gubun == "작업실적현황" || str_fg_gubun == "작업실적리스트" || str_fg_gubun == "자재투입리스트")
            {
                ...
                dat_Wo.StartDateToString = str_dt_rel;
                dat_Wo.EndDateToString = str_dt_due;
        ...
        private void InitControl()
        {
            ...
            //공장, 상태,  작업지시구분, 작업지시상태, 사용자정의1, 사용자정의2, 사용자정의3
            DataSet ds = GetComboData("NC;MA_PLANT", "N;PR_0000009", "N;PR_0000007", "N;PR_0000006",
                                      "S;PR_0000094", "S;PR_0000095", "S;PR_0000096");
            ...
            if (!Global.MainFrame.ServerKeyCommon.ToUpper().Contains("ANJUN"))
            {
                _flexD.SetDataMap("CD_USERDEF1", ds.Tables[4], "CODE", "NAME");
            }

            _flexD.SetDataMap("CD_USERDEF2", ds.Tables[5], "CODE", "NAME");
            _flexD.SetDataMap("CD_USERDEF3", ds.Tables[6], "CODE", "NAME");
            ...
            //dat_Wo.StartDateToString = DateTime.Now.AddMonths(-1).ToShortDateString().Replace("-","");
            //dat_Wo.EndDateToString = DateTime.Now.AddMonths(1).ToShortDateString().Replace("-","");

            dat_Wo.StartDate = DateTime.Today.AddMonths(-1);
            dat_Wo.EndDate = DateTime.Today.AddMonths(1);
            ...
            if (Global.MainFrame.ServerKeyCommon == "WJIS" ||
                Global.MainFrame.ServerKeyCommon == "DZSQL")
                btn_일일작업공수.Visible = true;

            //윈플러스전용 관리항목 컬럼 속성 변경
            if (LOT관리항목DT.Rows.Count != 0 && (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                                                  Global.MainFrame.ServerKeyCommon.ToUpper() == "WINPLUS"))
            {
                //CD_MNG2 : 등급
                DataTable dt_WINPLUS = ds.Tables[1].Clone();
                DataRow dr_WINPLUS = dt_WINPLUS.NewRow();
                dr_WINPLUS["CODE"] = "";
                dr_WINPLUS["NAME"] = "";
                dt_WINPLUS.Rows.Add(dr_WINPLUS.ItemArray);
                dr_WINPLUS = dt_WINPLUS.NewRow();
                dr_WINPLUS["CODE"] = "S";
                dr_WINPLUS["NAME"] = "S";
                dt_WINPLUS.Rows.Add(dr_WINPLUS.ItemArray);
                dr_WINPLUS = dt_WINPLUS.NewRow();
                ...
                _flexD.SetDataMap("CD_MNG2", dt_WINPLUS, "CODE", "NAME");
            }

            //신진에스엠전용 메뉴점프 버튼 활성화
            if (Global.MainFrame.ServerKeyCommon.ToUpper().Contains("SINJINSM") ||
                Global.MainFrame.ServerKeyCommon.ToUpper().Contains("DZSQL") ||
                Global.MainFrame.ServerKeyCommon.ToUpper().Contains("SQL_") )
                btn_z_sinjin_소품목등록.Visible = true;
        }
        ...
        void 사용자정의셋팅()
        {
            DataTable dt명칭 = MA.GetCode("MA_B000147");
            for (int i = 1; i <= dt명칭.Rows.Count; i++)
            {
                string Name = D.GetString(dt명칭.Rows[i - 1]["NAME"]);

                _flexD.Cols["CD_USERDEF" + D.GetString(i)].Caption = Name;
                _flexD.Cols["CD_USERDEF" + D.GetString(i)].Visible = true;
            }

            DataTable dt명칭TXT = MA.GetCode("MA_B000149");
            for (int i = 1; i <= dt명칭TXT.Rows.Count; i++)
            {
                string Name = D.GetString(dt명칭TXT.Rows[i - 1]["NAME"]);

                _flexD.Cols["TXT_USERDEF" + D.GetString(i)].Caption = Name;
                _flexD.Cols["TXT_USERDEF" + D.GetString(i)].Visible = true;
            }

            DataTable dt명칭NUM = MA.GetCode("MA_B000150");
            for (int i = 1; i <= 10; i++)
            {
                string colName = "NUM_USERDEF" + D.GetString(i);

                if (dt명칭NUM.Rows.Count < i)
                {
                    _flexD.Cols[colName].Visible = false;
                    continue;
                }

                string colCaption = D.GetString(dt명칭NUM.Rows[i - 1]["NAME"]);

                _flexD.Cols[colName].Caption = colCaption;
                _flexD.Cols[colName].Visible = true;
            }

            DataTable dt공정경로NUM = MA.GetCode("PR_0000107");
            for (int i = 1; i <= 3; i++)
            {
                string colName = "ROUT_NUM_USERDEF" + D.GetString(i);

                if (dt공정경로NUM.Rows.Count < i)
                {
                    _flexD.Cols[colName].Visible = false;
                    continue;
                }

                string colCaption = D.GetString(dt공정경로NUM.Rows[i - 1]["NAME"]);

                _flexD.Cols[colName].Caption = colCaption;
                _flexD.Cols[colName].Visible = true;
            }

            DataTable dt공정경로TXT = MA.GetCode("PR_0000108");
            for (int i = 1; i <= 3; i++)
            {
                string colName = "ROUT_TXT_USERDEF" + D.GetString(i);

                if (dt공정경로TXT.Rows.Count < i)
                {
                    _flexD.Cols[colName].Visible = false;
                    continue;
                }

                string colCaption = D.GetString(dt공정경로TXT.Rows[i - 1]["NAME"]);
        ...
        protected override bool BeforeSearch()
        {
            ...
            if (!Checker.IsValid(dat_Wo, false, m_lblperiodwork.Text))
                return false;
        ...
        public override void OnToolBarSearchButtonClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                object[] obj = new object[] {
                    ...
                    m_txtWoTo.CodeValue, 
                    dat_Wo.StartDateToString, 
                    dat_Wo.EndDateToString, 
                    마감구분, 
                    bp프로젝트.CodeValue,
        ...
        protected override bool Verify()
        {
            ...
            //
            // 2014.03.14 실적수량입력하고, 불량수량입력하고
            // 포커스가 떠나지 않은 상태에서 저장버튼을 바로 누르게되면
            // GetChanges에 수정되지 않은 값이 들어오게 되어 문제가 되었음.
            // 그래서 강제로 포커스를 헤더그리드에 갔다가 오도록 수정하였음.
            //
            _flexM.Focus();
            ...
            if (LoginInfo.MngLot == "Y" && _flexM["YN_LOT"].ToString() == "Y")
            {
                ...
                if (Global.MainFrame.ServerKeyCommon.ToUpper() == "SEEGENE")
                {
            ...
            //
            // 20140310 이명철 김현우- D20140310034 : 제이씨코리아주식회사(JC KOREA CORP.)
            // 불량내역을 등록하고 불량입력량을 0으로 수정했을시, 불량내역테이블 초기화
            //
            else if ((reject == 0) && (_dtReject != null && _dtReject.Rows.Count > 0))
            {
                ShowMessage("실적불량수량이 '0'으로 수정되었습니다.\n 입력된 불량내역은 모두 삭제됩니다.");
                _dtReject.Clear();
                return false;
            }
                ...
                else
                {
                            ...
                            if (dlgMatl.DialogResult == DialogResult.OK)
                            {
                                if (dlgMatl is IHelpWindow)
                                {
                                    ...
                                    //if (LoginInfo.MngLot == "Y" && _flexM["YN_LOT"].ToString() == "Y" && dt != null)
                                    {
                                        ...
                                        dtCopy.Columns.Remove("DT_IO");

                                        DataColumn dc = new DataColumn("DT_IO", typeof(string));
                    ...
                    if (dr_Plant != null && dr_Plant.Tables[1].Rows.Count != 0)
                    {
                        if (D.GetString(dr_Plant.Tables[1].Rows[0]["YN_LINK_HR"]) == "Y")
                        {
                            ...
                            dlg_Manday.Set설비코드 = D.GetString(dtWork.Rows[0]["CD_EQUIP"]);
                            dlg_Manday.Set설비명 = D.GetString(dtWork.Rows[0]["NM_EQUIP"]);
                    ...
                    if (_dtReject != null && _dtReject.Rows.Count > 0)
                    {
                        ...
                        if (D.GetString(ds.Tables[1].Rows[0]["YN_AUTOBAD"]) == "Y" && (D.GetString(ds.Tables[1].Rows[0]["FG_AUTOBAD"]) == "000" || D.GetString(ds.Tables[1].Rows[0]["FG_AUTOBAD"]) == "001"))
                        {
                            ...
                            if (dr_Reject_Select != null && dr_Reject_Select.Length > 0)
                            {
                                ...
                                {
                                    //
                                    // LOT관리항목 CD_MNG1~20을 추가하여준다.
                                    //
                                    for (int i = 1; i <= 20; i++)
                                    {
                                        string column = "CD_MNG" + D.GetInt(i);

                                        dr[column] = D.GetString(dtWork.Rows[0][column]);
                                    }
                                ...
                                if (D.GetString(ds.Tables[1].Rows[0]["YN_AUTOBAD_REQ"]) == "Y")
                                {
                                    ...
                                    //
                                    // LOT관리항목 CD_MNG1~20 추가
                                    //
                                    for (int i = 1; i <= 20; i++)
                                    {
                                        string column = "CD_MNG" + D.GetInt(i);

                                        dt_AutoBad_ReqL.Columns.Add(column, typeof(string));
                                    }
                                    ...
                                    foreach (DataRow dr in dt_Auto_Bad.Rows)
                                    {
                                        ...
                                        dr_Ins["NO_LOT"] = dr["NO_LOT"];
                                        dr_Ins["NO_IO_203"] = dt_Auto_Bad_Work.Rows[0]["NO_IO_203"];
                                        dr_Ins["NO_LINE_203"] = dt_Auto_Bad_Work.Rows[0]["NO_LINE_203"];
                                        dr_Ins["DT_LIMIT"] = dtWork.Rows[0]["DT_LIMIT"];


                                        //
                                        // LOT관리항목 CD_MNG1~20 넣어줌
                                        //
                                        for (int i = 1; i <= 20; i++)
                                        {
                                            string column = "CD_MNG" + D.GetInt(i);

                                            dr_Ins[column] = D.GetString(dtWork.Rows[0][column]);
                                        }
            ...
            try
            {
                ...
                switch (_flex.Cols[e.Col].Name)
                {
                    case "DT_WORK":
                        ...
                        //
                        // 2014.04.04 윈플러스에서 실적일자 입력시 관리항목11에 넣어달라고 하였음.
                        //
                        else if (Global.MainFrame.ServerKeyCommon.ToUpper().Contains("WINPLUS") ||
                                 Global.MainFrame.ServerKeyCommon.ToUpper().Contains("DZSQL"))
                        {
                            if (_flexD.Cols.Contains("CD_MNG11"))
                            {
                                _flexD["CD_MNG11"] = _flexD.EditData;
                            }
                        }
                    ...
                    case "QT_WORK":
                        ...
                        //
                        // 윈플러스 전용로직
                        // 실적수량 입력시 관리항목10에 헤더의 수주번호를 넣어달라는 요청
                        // 실적수량 입력시 관리항목6에 입력한 실적수량을 넣어달라는 요청
                        //
                        if (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                            Global.MainFrame.ServerKeyCommon.ToUpper() == "WINPLUS")
                        {
                            if (_flexD.Cols.Contains("CD_MNG10"))
                            {
                                _flexD[e.Row, "CD_MNG10"] = D.GetString(_flexM[_flexM.RowSel, "NO_SO"]);
                            }

                            if (_flexD.Cols.Contains("CD_MNG6"))
                            {
                                _flexD[e.Row, "CD_MNG6"] = D.GetDecimal(_flexD.EditData);
                            }
                        }
                        ...
                        //
                        // 한수 전용로직
                        // 실적수량 입력시 관리항목2에 헤더의 포장단위를 넣어달라는 요청
                        // 실적수량 입력시 관리항목3에 헤더의 비고를 넣어달라는 요청
                        //
                        if (Global.MainFrame.ServerKeyCommon.ToUpper().Contains("DZSQL") ||
                            Global.MainFrame.ServerKeyCommon.ToUpper().Contains("SQL_") ||
                            Global.MainFrame.ServerKeyCommon.ToUpper().Contains("HANSU"))
                        {
                            if (_flexD.Cols.Contains("CD_MNG2"))
                            {
                                //
                                // 포장단위 컬럼이 있고, LOT품목일 때만 넣어준다.
                                //
                                if (_flexM.Cols.Contains("CD_PACKUNIT") && D.GetString(_flexM[_flexM.RowSel, "YN_LOT"]) == "Y")
                                {
                                    _flexD[e.Row, "CD_MNG2"] = D.GetString(_flexM[_flexM.RowSel, "CD_PACKUNIT"]);
                                } 
                            }

                            if (_flexD.Cols.Contains("CD_MNG3"))
                            {
                                _flexD[e.Row, "CD_MNG3"] = D.GetString(_flexM[_flexM.RowSel, "DC_RMK"]);
                            }
                        }


                        //
                        // 실적수량 입력시 양품실적수량(QT_GOOD) 넣어줌.
                        // 양품실적수량(QT_GOOD) = 실적수량(QT_WORK) - 불량입력량(QT_REJECT)
                        //
                        _flexD[e.Row, "QT_GOOD"] = Unit.수량(DataDictionaryTypes.PR, dQT_WORK - D.GetDecimal(_flex[e.Row, "QT_REJECT"]));
                    ...
                    case "QT_REJECT":
                        ...
                        //
                        // 불량입력량 입력시 양품실적수량(QT_GOOD) 넣어줌.
                        // 양품실적수량(QT_GOOD) = 실적수량(QT_WORK) - 불량입력량(QT_REJECT)
                        //
                        _flexD[e.Row, "QT_GOOD"] = Unit.수량(DataDictionaryTypes.PR, D.GetDecimal(_flex[e.Row, "QT_WORK"]) - iQT_REJECT);

                        break;
                    case "CD_MNG1":

                        //
                        // 윈플러스에서 LOT관리항목 CD_MNG1을 입력시 LOT번호를 자동채번하는 규칙을 생성해 달라고 하였음.
                        // NO_LOT = CD_ITEM + DT_WORK + CD_MNG1
                        //
                        if (Global.MainFrame.ServerKeyCommon.ToUpper() == "DZSQL" ||
                            Global.MainFrame.ServerKeyCommon.ToUpper() == "WINPLUS")
                        {
                            //
                            // 입력된 Data가 있고
                            // 작업지시 품목의 LOT관리여부가 YES일 경우에만
                            // 자동채번 하도록 한다.
                            //
                            if ((D.GetString(_flexD.EditData) != string.Empty) &&
                                (D.GetString(_flexM[_flexM.RowSel, "YN_LOT"]) == "Y")
                                )
                            {
                                _flexD[e.Row, "NO_LOT"] = D.GetString(_flexD[e.Row, "CD_ITEM"])
                                                        + D.GetString(_flexD[e.Row, "DT_WORK"])
                                                        + D.GetString(_flexD.EditData);
                            }
                            else
                                _flexD[e.Row, "NO_LOT"] = string.Empty;
                        }
        ...
        private void _flexM_AfterRowChange(object sender, RangeEventArgs e)
        {
            try
            {
                ...
                //
                // LOT사용여부에 따른 LOT관리항목 컬럼의 Visible을 처리한다.
                //
                if (LoginInfo.MngLot == "N" || _flexM["YN_LOT"].ToString() == "N")
                {
                    //
                    // LOT관리항목의 컬럼 Visible도 false로 바꾼다.
                    //
                    if (LOT관리항목DT.Rows.Count != 0)
                    {
                        foreach (DataRow row in LOT관리항목DT.Rows)
                ...
                {
                    //
                    // LOT관리항목의 컬럼 Visible도 true로 바꾼다.
                    //
                    if (LOT관리항목DT.Rows.Count != 0)
                    {
                        foreach (DataRow row in LOT관리항목DT.Rows)
                        {
                            int cnt = D.GetInt(row["CODE"]);
                            string column = "CD_MNG" + cnt;
                            _flexD.Cols[column].Visible = true;
                        }
                    }
                }
        ...
        private bool ShowRejectHelp()
        {
            ...
            DataSet ds = Pr_ComFunc.Get_Plant_Cfg(new object[] { Global.MainFrame.LoginInfo.CompanyCode, D.GetString(dtWork.Rows[0]["CD_PLANT"]) });

            P_PR_WORK_SUB02 dlg = null;

            // LOT사용 업체이면서
            // LOT사용 품목일 경우
            // MM_QTIOLOT에 넣기 위해 LOT번호도 넘겨줘야 한다.
            //if (D.GetString(ds.Tables[1].Rows[0]["YN_AUTOBAD"]) == "Y"
            //    && (D.GetString(ds.Tables[1].Rows[0]["FG_AUTOBAD"]) == "000" || D.GetString(ds.Tables[1].Rows[0]["FG_AUTOBAD"]) == "001")
            //    && D.GetString(ds.Tables[1].Rows[0]["MNG_LOT"]) == "Y"
            //    )
            if (LoginInfo.MngLot == "Y" && _flexM["YN_LOT"].ToString() == "Y")
            {
                if (D.GetString(ds.Tables[1].Rows[0]["YN_AUTOBAD"]) == "Y"
                && (D.GetString(ds.Tables[1].Rows[0]["FG_AUTOBAD"]) == "000" || D.GetString(ds.Tables[1].Rows[0]["FG_AUTOBAD"]) == "001")
                && D.GetString(_flexD[_flexD.RowSel, "NO_LOT"]) == string.Empty)
                {
                    ShowMessage("LOT관리 업체에서 공정불량자동처리일 경우\nLOT번호는 필수입력항목 입니다.", "EK1");
                    return false;
                }
                else
                {
                    dlg = new P_PR_WORK_SUB02(cur_row, MainFrameInterface, m_cboCdPlant.Text, dQT_REJECT,
                        _flexM["NM_ITEM"].ToString(), _flexM["STND_ITEM"].ToString(), _flexM["UNIT_IM"].ToString(), strNO_SFT, D.GetString(_flexD[_flexD.RowSel, "NO_LOT"]));
                }
            }
        ...
        #region -> 신진에스엠전용 버튼 클릭 이벤트(btnbtn_z_sinjin_소품목등록_Click)

        private void btn_z_sinjin_소품목등록_Click(object sender, EventArgs e)
        {
            try
            {
                if (Duzon.Common.Forms.BasicInfo.ActiveDialog == true) return;
                ...
                string[] args = new string[9]{  D.GetString(m_cboCdPlant.SelectedValue),
                                                D.GetString(_flexM["NO_WO"]),		    //작업지시번호
                                                D.GetString(_flexD["DT_WORK"]),		    //실적일
                                                D.GetString(_flexD["CD_WC"]),		    //작업장코드
                                                D.GetString(_flexD["NM_WC"]),		    //작업장명
                                                D.GetString(_flexD["CD_OP"]), 		    //공정코드
                                                D.GetString(_flexD["NM_OP"]),		    //공정명
                                                D.GetString(_flexD["CD_EQUIP"]),		//설비코드
                                                D.GetString(_flexD["NM_EQUIP"])         //설비명
											 };

                object[] obj_args = new object[2] { args, MainFrameInterface };

                if (IsExistPage("P_PR_Z_SHINJIN_SUPPLIES_REG", true) == true)
                    UnLoadPage("P_PR_Z_SHINJIN_SUPPLIES_REG", false);     //페이지를 먼저 닫는다.

                CallOtherPageMethod("P_PR_Z_SHINJIN_SUPPLIES_REG", "소모품등록(신진에스엠)", Grant, obj_args);
            }
            catch (Exception ex)
            {
                MsgEnd(ex);
            ...
            try
            {
                ...
                switch (sCtrName)
                {
                    ...
                    case "bp_Wc_Op":
                        ...
                        {
                            Global.MainFrame.ShowMessage(공통메세지._은는필수입력항목입니다, bp_Cd_WC.Text);
                            e.QueryCancel = true;
                            return;
                        }
